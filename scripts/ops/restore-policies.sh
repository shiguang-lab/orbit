#!/usr/bin/env bash
# scripts/ops/restore-policies.sh — restore ONLY the API-key policy tables from a
...
usage() {
  cat <<'EOF'
Usage: scripts/ops/restore-policies.sh <snapshot-id> [--data-dir <path>] [--yes|-y] [-h|--help]
...
Stop every split service that mounts the data directory before running. For Docker
Compose, --data-dir must name a host path where the stopped data volume is mounted.
EOF
}
