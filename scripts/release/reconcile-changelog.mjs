...
export function repoSlug(cwd = ROOT) {
  ...
  return m ? m[1] : "shiguang-lab/orbit";
}
...
export function main(argv = process.argv.slice(2)) {
  ...
  if (o.help) {
    console.log("Usage: pnpm release:reconcile -- --version <version> [--base <ref>] [--head <ref>] [--dry-run]");
    return;
  }
  const rootVersion = JSON.parse(fs.readFileSync(path.join(ROOT, "package.json"), "utf8")).version;
  const version = o.version || rootVersion;
  if (!version) {
    throw new Error("release version is required: pass --version <version> (the workspace root package has no version)");
  }
