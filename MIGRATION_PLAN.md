...
>
> API 路由跟随菜单一起迁移。每迁移一个**菜单单元** = 「前端页面 + 对应 control-api 路由 + 领域服务」一次做完，
...
| # | 菜单入口 | 页面(路由) | control-api/edge-gateway 路由组 | 说明 |
|---|---|---|---|---|
...
3. 页面全部业务数据来自 **control-api/edge-gateway**（通过 `admin→vite proxy→本地服务→领域包` 链路）
