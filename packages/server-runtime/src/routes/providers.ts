...
/** Public inference utility endpoints mounted on the edge gateway only. */
export function publicProviderRoutes(
  app: FastifyInstance,
  ...
  const run = async (
    ...
    key: "webFetch" | "webSearch" | "embeddings" | "imageGenerations" | "audioSpeech",
    unavailable: string,
    failed: string,
  ) => {
    try {
      const handler = engine?.[key] as ((body: Record<string, unknown>) => Promise<{ status: number; payload: unknown }>) | undefined;
      if (!handler) return reply.status(501).send({ error: unavailable });
      const result = await handler((request.body ?? {}) as Record<string, unknown>);
  ...
  app.post("/v1/web/fetch", (request, reply) => run(request, reply, "webFetch", "Web fetch is unavailable", "Failed to execute web fetch"));
  app.post("/v1/search", (request, reply) => run(request, reply, "webSearch", "Web search is unavailable", "Failed to execute search"));
  app.post("/v1/embeddings", (request, reply) => run(request, reply, "embeddings", "Embeddings unavailable", "Failed to generate embeddings"));
  app.post("/v1/images/generations", (request, reply) => run(request, reply, "imageGenerations", "Image generation unavailable", "Failed to generate image"));
  app.post("/v1/audio/speech", (request, reply) => run(request, reply, "audioSpeech", "Speech generation unavailable", "Failed to generate speech"));
}
...
export function providerRoutes(
  ...
  /** Official provider playground endpoint; the engine delegates to Shiguang Gateway's
  ...
  /** GET /api/providers/catalog —— 静态 provider catalog 由 Gateway 统一投影 */
  app.get("/providers/catalog", async (_request, reply) => {
    try {
      // Catalog metadata is the local static registry and is available before
      // any provider connection is configured.
  ...
  /** Local static model registry used during provider setup. */
  ...
  /** GET /api/providers/expiration —— Shiguang Gateway provider expiration projection */
  ...
  /** GET /api/providers/openrouter-stats —— cached Shiguang Gateway catalog enrichment */
  ...
  /** Provider-level model catalog. Shiguang Gateway uses this for the detail page before a connection exists. */
  app.get("/provider-models", async (request, reply) => {
    try {
      const providerId = new URL(request.url, "http://gateway").searchParams.get("provider")?.trim() ?? "";
  ...
  app.delete("/providers/:id/models", async (request, reply) => {
    try {
      const { id } = request.params as { id: string };
      const modelId = new URL(request.url, "http://gateway").searchParams.get("modelId")?.trim() ?? "";
      if (!modelId) return reply.status(400).send({ error: { type: "invalid_request", message: "modelId is required" } });
  ...
  app.post("/providers/test-batch", async (request, reply) => {
    try {
      ...
      // 选中的连接 ID 或按 mode 取连接。除 selected 外与 Shiguang Gateway 一致只测试启用连接。
