...
export interface KeyEngine {
  ...
  getApiKeyDevices(id: string): Promise<{ fingerprint: string; ip: string; userAgent: string; lastSeen: number }[]>;
  getApiKeyUsageLimitStatus(key: ApiKeyView): Promise<Record<string, unknown>>;
  createApiKey(
    name: string,
    machineId: string,
    scopes?: string[],
...
export function keyRoutes(app: FastifyInstance, opts: { engine?: KeyEngine } = {}): void {
  ...
  app.get("/keys/:id/devices", async (request, reply) => {
    ...
    const key = await engine.getApiKeyById(id);
    if (!key) return reply.status(404).send({ error: "Key not found" });
    const devices = await engine.getApiKeyDevices(id);
    return reply.status(200).send({ keyId: id, name: key.name, count: devices.length, devices });
  });
  ...
  app.get("/keys/:id/usage-limits", async (request, reply) => {
    ...
    const key = await engine.getApiKeyById(id);
    if (!key) return reply.status(404).send({ error: "Key not found" });
    const status = await engine.getApiKeyUsageLimitStatus(key);
