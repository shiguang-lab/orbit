...
export const freeTierRoutes: FastifyPluginAsync = async (app: FastifyInstance) => {
  app.get("/free-tier/summary", async (request, reply) => {
    try {
      ...
      const { computeFreeModelTotals } = await import("@shiguang-gateway/open-sse/config/freeModelCatalog");
      const { FREE_CATALOG_CURATED_AT } = await import("@shiguang-gateway/open-sse/config/freeModelCatalog.data");
      ...
      const noCredentialProviders = listNoCredentialProviders();
