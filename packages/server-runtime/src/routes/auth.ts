 ...
 * 密码校验/暴力破解守卫通过本地 runtime 引擎适配器注入。
 * 会话默认使用 Shiguang Gateway 原生 auth_token；仅在显式 SSO 模式下由 shiguang 身份终结。
 */
...
export function authRoutes(
  ...
  const localPasswordAuth = opts.localPasswordAuth ?? false;

  /** GET /api/auth/session —— native Shiguang Gateway session or opt-in shiguang session */
  app.get("/auth/session", async (request, reply) => {
    return handleSession(request, reply, devBypass, broker, localPasswordAuth);
  });

  /** POST /api/auth/login —— Shiguang Gateway's official password login */
