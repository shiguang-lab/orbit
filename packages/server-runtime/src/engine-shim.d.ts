/**
 * 本地 runtime 类型声明：让 tsc 在编译期把 vendor 模块视为 any。
 * 运行时路径只解析到仓库内 `packages/gateway-runtime`，不依赖同级官方 checkout。
 * 这样 Gateway 的 typecheck 只严格检查自身源码，vendor runtime 保持独立快照。
 */
declare module "@/*" {
  const value: any;
...
declare module "@shiguang-gateway/open-sse/*" {
  const value: any;
  export default value;
  export = value;
}
