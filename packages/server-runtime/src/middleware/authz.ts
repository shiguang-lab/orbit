...
export interface AuthzOptions {
  ...
  /** Edge has no management routes; let its dispatcher return a clean 404. */
...
export function authzPlugin(app: FastifyInstance, opts: AuthzOptions = {}): void {
  ...
  app.addHook("preHandler", async (request, reply) => {
    ...
    const apiClientPath = pathname === "/api/v1" || pathname.startsWith("/api/v1/") ||
      pathname === "/api/v1beta" || pathname.startsWith("/api/v1beta/") ||
      pathname === "/api/a2a" || pathname.startsWith("/api/a2a/");
    const rootClientPath = pathname === "/v1" || pathname.startsWith("/v1/") ||
      pathname === "/v1beta" || pathname.startsWith("/v1beta/") ||
      pathname === "/a2a" || pathname.startsWith("/a2a/");

    // Enforce the split before route matching. This protects against a dynamic
    // catch-all or a future adapter accidentally mounting the other surface.
    if (opts.surface === "edge-gateway" && pathname.startsWith("/api/") && !apiClientPath) {
      return reply.status(404).send({ error: { type: "not_found", message: "Route not found" }, requestId: request.id });
    }
    if (opts.surface === "control-api" && (apiClientPath || rootClientPath)) {
      return reply.status(404).send({ error: { type: "not_found", message: "Route not found" }, requestId: request.id });
    }
