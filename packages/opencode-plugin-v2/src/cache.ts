...
import type {
  OrbitEnrichmentEntry,
  OrbitEnrichmentMap,
  OrbitProviderConnection,
  OrbitRawAutoCombo,
  OrbitRawCombo,
  OrbitRawModelEntry,
...
export interface CatalogSnapshot {
  ...
  autoCombos: OrbitRawAutoCombo[];
  providers?: OrbitProviderConnection[];
...
interface DiskSnapshotV2 {
   ...
   */
  enrichment?: [string, OrbitEnrichmentEntry][];
  writtenAt: number;
}
...
export function diskSnapshotPath(providerId: string): string {
  ...
  return join(dir, "plugins", `orbit-${providerId}.json`);
}
...
export async function readDiskSnapshot(
  ...
  try {
    ...
    const models = (parsed.models as OrbitRawModelEntry[]).filter(
      (entry) => !isStaleSnapshotModel(entry)
    );
    if (stale > 0) {
      logger?.warn(`[orbit-v2] dropping ${stale} stale snapshot entries without api block`);
    }
    if (models.length === 0) return undefined;
    return {
      models,
      combos: parsed.combos as OrbitRawCombo[],
      autoCombos: Array.isArray(parsed.autoCombos)
        ? (parsed.autoCombos as OrbitRawAutoCombo[])
        : [],
      providers: Array.isArray(parsed.providers)
        ? (parsed.providers as OrbitProviderConnection[])
        : [],
      ...
      enrichment: Array.isArray(parsed.enrichment)
        ? new Map(parsed.enrichment as [string, OrbitEnrichmentEntry][])
        : undefined,
