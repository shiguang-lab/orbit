...
import {
  ...
  createLogger,
  defaultOrbitAutoCombosFetcher,
  defaultOrbitCombosFetcher,
  defaultOrbitEnrichmentFetcher,
  defaultOrbitModelsFetcher,
  defaultOrbitProvidersFetcher,
  type OrbitEnrichmentMap,
  type OrbitProviderConnection,
...
function toResolvedOptions(parsed: PluginOptions): ResolvedOptions {
  return {
    ...
    apiKey: parsed.apiKey ?? process.env.ORBIT_API_KEY ?? "",
...
export default define({
  ...
  setup: async (ctx: PluginContext) => {
    ...
    log.info(`[orbit-v2] init providerId=${X}`);
    ...
    const ensureCredential = async (): Promise<void> => {
      ...
      else if (moved) log.info(`[orbit-v2] API key picked up from the ${next.origin} source`);
      credentialChecked = true;
    };

    const fetchModelsSafe = async (): Promise<OrbitRawModelEntry[]> => {
      try {
        return await defaultOrbitModelsFetcher(
          resolved.baseURL,
          resolved.apiKey,
          timeouts.models
        );
        ...
        log.warn(
          `[orbit-v2] models fetch failed, publishing empty catalog: ${err instanceof Error ? err.message : String(err)}`
        );
        return [];
      }
    };
    ...
    const fetchCombosSafe = async (): Promise<SourceResult<OrbitRawCombo[]>> => {
      try {
        return {
          ok: true,
          value: await defaultOrbitCombosFetcher(
            resolved.baseURL,
            resolved.managementReadToken ?? resolved.apiKey,
            timeouts.combos
          ),
        ...
        log.warn(`[orbit-v2] combos fetch failed, keeping the last known combos: ${reason}`);
        return { ok: false };
      }
    };
    ...
    const fetchProvidersSafe = async (): Promise<SourceResult<OrbitProviderConnection[]>> => {
      if (!resolved.usableOnly) return { ok: true, value: [] };
      try {
        return {
          ok: true,
          value: await defaultOrbitProvidersFetcher(
            resolved.baseURL,
            resolved.managementReadToken ?? resolved.apiKey,
            timeouts.models,
            reportSourceError
        ...
        log.warn(
          `[orbit-v2] providers fetch failed, keeping the last known provider list: ${err instanceof Error ? err.message : String(err)}`
        );
        return { ok: false };
      }
    };
    ...
    const fetchEnrichmentSafe = async (): Promise<SourceResult<OrbitEnrichmentMap>> => {
      if (resolved.enrichment === false) return { ok: true, value: new Map() };
      try {
        return {
          ok: true,
          value: await defaultOrbitEnrichmentFetcher(
            resolved.baseURL,
            resolved.managementReadToken ?? resolved.apiKey,
            timeouts.enrichment,
            reportSourceError
        ...
        log.warn(
          `[orbit-v2] enrichment fetch failed, keeping the last known names/pricing: ${err instanceof Error ? err.message : String(err)}`
        );
        return { ok: false };
      }
    };
    const fetchAutoCombosSafe = async (): Promise<SourceResult<OrbitRawAutoCombo[]>> => {
      try {
        return {
          ok: true,
          value: await defaultOrbitAutoCombosFetcher(
            resolved.baseURL,
            resolved.managementReadToken ?? resolved.apiKey,
            timeouts.autoCombos,
            log,
        ...
        const reason = err instanceof Error ? err.message : String(err);
        log.warn(`[orbit-v2] auto combos fetch failed, keeping the last known ones: ${reason}`);
        return { ok: false };
      }
    };
    ...
    async function refreshSnapshot(): Promise<CatalogSnapshot> {
      ...
      void optional.then(
        ...
        (err) => {
          ...
          log.warn(
            `[orbit-v2] optional catalog sources failed unexpectedly: ${err instanceof Error ? err.message : String(err)}`
          );
        }
      );
      return snapshot;
    ...
    async function upgradeWithOptional(
      ...
      [combos, autoCombos, providers, enrichment]: [
        SourceResult<OrbitRawCombo[]>,
        SourceResult<OrbitRawAutoCombo[]>,
        SourceResult<OrbitProviderConnection[]>,
        SourceResult<OrbitEnrichmentMap>,
      ]
    ): Promise<void> {
      ...
      if (optionalChanged && typeof ctx.catalog.reload === "function") {
        try {
          ...
          log.warn(
            `[orbit-v2] catalog reload after late sources failed, keeping current catalog: ${err instanceof Error ? err.message : String(err)}`
          );
        }
      }
    }
    ...
    const catalogRegistration = ctx.catalog.transform(async (draft) => {
      ...
      if (snapshot.models.length === 0) {
        ...
        if (stale !== undefined && stale.models.length > 0) {
          log.warn(
            `[orbit-v2] models fetch returned empty, keeping last-known catalog (${stale.models.length} models, ${stale.combos.length} combos)`
          );
          effective = stale;
        }
      }
      const counts = await (async (): Promise<{
        ...
        try {
          ...
          log.warn(
            `[orbit-v2] catalog publish failed, keeping current catalog: ${err instanceof Error ? err.message : String(err)}`
          );
      ...
      if (changed && typeof ctx.catalog.reload === "function") {
        ...
        try {
          ...
          log.warn(
            `[orbit-v2] catalog reload failed, keeping current catalog: ${err instanceof Error ? err.message : String(err)}`
          );
        }
      }
    });
      ...
      try {
        integrationRegistration = integrationHook((draft) => {
          draft.update(X, (integration) => {
            integration.name = parsed.displayName ?? "Orbit";
          });
          draft.method.update({ integrationID: X, method: { type: "key", label: "API key" } });
          draft.method.update({
            integrationID: X,
            method: { type: "env", names: ["ORBIT_API_KEY"] },
          });
        });
      } catch (err) {
        log.warn(
          `[orbit-v2] host refused the integration hook, the connect action will be missing: ${err instanceof Error ? err.message : String(err)}`
        );
    ...
    if (parsed.geminiSanitization !== false && typeof languageHook === "function") {
      try {
        ...
        log.warn(
          `[orbit-v2] host refused the language-model hook, Gemini tool schemas will not be cleaned: ${err instanceof Error ? err.message : String(err)}`
        );
      }
    }
    ...
    if (languageRegistration !== undefined) {
      try {
        ...
        log.warn(
          `[orbit-v2] language-model hook registration failed, Gemini tool schemas will not be cleaned: ${err instanceof Error ? err.message : String(err)}`
        );
      }
    }
  },
