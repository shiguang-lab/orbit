...
async function keyFromConnection(
  ...
  try {
    ...
    if (credential.type !== "key") {
      log.warn(
        `[orbit-v2] ignoring the stored ${credential.type} credential: this plugin authenticates with an API key`
      );
      return undefined;
    }
    ...
    log.warn(
      `[orbit-v2] could not read the stored credential: ${err instanceof Error ? err.message : String(err)}`
    );
    return undefined;
  }
}
...
export function warnIfMissing(resolved: ResolvedApiKey, integrationID: string, log: Logger): void {
  ...
  log.warn(
    `[orbit-v2] no API key for "${integrationID}": the catalog will be empty. ` +
