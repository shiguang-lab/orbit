...
export function assertContext(ctx: unknown): void {
  if (!isObject(ctx)) {
    throw new Error("[orbit-v2] contract breach: ctx must be an object");
  }
  if (!isTransformHolder(ctx.catalog) || typeof ctx.catalog.transform !== "function") {
    throw new Error("[orbit-v2] contract breach: ctx.catalog.transform must be a function");
  }
  if (!isObject(ctx.options)) {
    throw new Error("[orbit-v2] contract breach: ctx.options must be an object");
  }
}
