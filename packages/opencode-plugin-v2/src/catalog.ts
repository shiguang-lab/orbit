...
import {
  ...
  type LogLevel,
  type Logger,
  type OrbitAutoCombosFetcher,
  type OrbitCombosFetcher,
  type OrbitEnrichmentFetcher,
  type OrbitEnrichmentMap,
  type OrbitModelsFetcher,
  type OrbitProviderConnection,
  type OrbitProvidersFetcher,
  type OrbitRawAutoCombo,
  type OrbitRawCombo,
  type OrbitRawModelEntry,
  applyEnrichment,
...
export type ModelsFetcher = OrbitModelsFetcher;
export type CombosFetcher = OrbitCombosFetcher;
export type AutoCombosFetcher = OrbitAutoCombosFetcher;
export type ProvidersFetcher = OrbitProvidersFetcher;
export type EnrichmentFetcher = OrbitEnrichmentFetcher;
...
export interface ResolvedOptions {
  ...
  hiddenModels?: string[];
  usableOnly: boolean;
  enrichment?: OrbitEnrichmentMap | boolean;
  /**
...
export function legacyApiToInfoApi(api: LegacyModelV2["api"]): ModelV2Info["api"] {
  if (!api || typeof api.npm !== "string" || api.npm.length === 0) {
    throw new Error(
      "[orbit-v2] refusing to publish a model without an api block (missing api.npm)"
    );
  }
...
export function passesComboAllowlist(combo: OrbitRawCombo, visible?: ModelListFilter): boolean {
...
async function resolveEnrichmentOverlay(
  ...
  try {
    ...
    log.warn(
      `[orbit-v2] enrichment fetch failed, continuing without names/pricing: ${err instanceof Error ? err.message : String(err)}`
    );
    return new Map();
  }
}
...
async function resolveUsableAliases(
  opts: ResolvedOptions,
  providersFetcher: OrbitProvidersFetcher | undefined,
  onSourceError: ((endpoint: string, reason: string) => void) | undefined,
  enrichment: OrbitEnrichmentMap,
  timeoutMs: number,
  log: Logger
): Promise<ReturnType<typeof usableProviderAliasSet> | undefined> {
  if (!opts.usableOnly) return undefined;
  let rawConnections: OrbitProviderConnection[];
  try {
    const fetchProviders = providersFetcher ?? defaultOrbitProvidersFetcher;
    ...
    log.warn(
      `[orbit-v2] providers fetch failed, usableOnly filter disabled for this refresh: ${err instanceof Error ? err.message : String(err)}`
    );
    rawConnections = [];
  }
...
interface PublishContext {
  ...
  enrichment: OrbitEnrichmentMap;
  rawModelById: Map<string, OrbitRawModelEntry>;
...
async function publishCombos(ctx: PublishContext): Promise<number | undefined> {
  ...
  try {
    ...
    log.warn(
      `[orbit-v2] combos fetch failed, falling back to models-only catalog: ${err instanceof Error ? err.message : String(err)}`
    );
    return undefined;
  }
  ...
  for (let pass = 0; pass < MAX_COMBO_PASSES && pending.length > 0; pass++) {
    ...
    for (const combo of pending) {
      ...
      const memberEntries: OrbitRawModelEntry[] = [];
      let deferred = false;
      ...
      if (publishedKeys.has(key)) {
        ...
        if (!isIntentionalDedup) {
          ...
          if (!warnedCombos.has(dedupeKey)) {
            ...
            log.warn(`[orbit-v2] combo key "${key}" collides with a model id; combo wins.`);
          }
        }
      }
  ...
  if (unresolved.length > 0) {
    log.warn(
      `[orbit-v2] ${unresolved.length} combo(s) could not resolve all nested combo-refs after ${MAX_COMBO_PASSES} passes; dropped to avoid over-claiming.`
    );
  }
  return comboCount;
}
 ...
 */
function synthesizeNestedMember(name: string, nested: LegacyModelV2): OrbitRawModelEntry {
...
export async function publishCatalog(
  ...
  log.debug(`[orbit-v2] host catalog contract detected: ${hostContract}`);
  ...
  let rawModels: OrbitRawModelEntry[];
  try {
  ...
  const rawModelById = new Map<string, OrbitRawModelEntry>();
  ...
  let rawAutoCombos: OrbitRawAutoCombo[];
  try {
    ...
    log.warn(
      `[orbit-v2] auto combos fetch failed, falling back to models+combos catalog: ${err instanceof Error ? err.message : String(err)}`
    );
  ...
  for (const autoCombo of rawAutoCombos) {
    ...
    if (publishedKeys.has(key)) {
      ...
      if (!warnedCombos.has(dedupeKey)) {
        warnedCombos.add(dedupeKey);
        log.warn(
          `[orbit-v2] auto combo key "${key}" collides with a model id; auto combo wins.`
        );
      }
    }
