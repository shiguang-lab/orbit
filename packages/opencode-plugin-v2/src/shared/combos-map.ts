...
import { type ApiFormatV2, type OrbitRawModelEntry, resolveApiBlockV2 } from "./models-map.js";

export interface OrbitRawComboMemberRef {
...
export interface OrbitRawCombo {
  id: string;
  name?: string;
  ...
  models?: OrbitRawComboMemberRef[];
  /** Hidden combos are excluded from the OC model picker. */
  isHidden?: boolean;
  /** When Orbit attaches a lifecycle hint we forward it; today it doesn't. */
  release_date?: string;
  /**
 ...
 * `OrbitModelsFetcher` so unit tests can inject a stub instead of
 * monkey-patching global `fetch`.
 */
export type OrbitCombosFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<OrbitRawCombo[]>;
 ...
 * plugin working if a future Orbit build trims the wrapper).
 *
 * Differences from `defaultOrbitModelsFetcher`:
 ...
 */
export const defaultOrbitCombosFetcher: OrbitCombosFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  if (!apiKey) throw new Error("[orbit-v2] apiKey required to fetch /api/combos");
  if (!baseURL) throw new Error("[orbit-v2] baseURL required to fetch /api/combos");
