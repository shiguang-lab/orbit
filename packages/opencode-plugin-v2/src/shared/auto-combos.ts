...
/**
 * Raw shape of an auto combo entry as returned by Orbit's
 ...
 */
export interface OrbitRawAutoCombo {
...
function fallbackWarn(message: string, ...args: unknown[]): void {
  console.warn(`[orbit-plugin] [WARN] ${message}`, ...args);
}
 ...
 */
export const defaultOrbitAutoCombosFetcher: OrbitAutoCombosFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 5_000,
  logger?: AutoCombosWarnSink,
  ...
  try {
    ...
    const rawList: unknown[] = Array.isArray(body)
      ...
      : body && typeof body === "object" && Array.isArray((body as { combos?: unknown }).combos)
        ...
        : [];
    const out: OrbitRawAutoCombo[] = [];
    for (const r of rawList) {
      if (r && typeof r === "object" && typeof (r as { id?: unknown }).id === "string") {
        out.push(r as OrbitRawAutoCombo);
      }
    }
    return out;
  } catch (err) {
