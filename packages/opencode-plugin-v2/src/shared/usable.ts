import type { OrbitEnrichmentMap } from "./enrich.js";
import type { OrbitRawCombo } from "./combos-map.js";

/** Subset of `/api/providers` connections read by the usableOnly filter. */
export interface OrbitProviderConnection {
  /** Connection UUID. */
  id: string;
...
export type OrbitProvidersFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number,
  onSourceError?: (endpoint: string, reason: string) => void
) => Promise<OrbitProviderConnection[]>;
 ...
 */
export const defaultOrbitProvidersFetcher: OrbitProvidersFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000,
  onSourceError
) => {
  const empty: OrbitProviderConnection[] = [];
    ...
    const list = Array.isArray(body)
      ...
      : Array.isArray((body as { connections?: unknown[] })?.connections)
        ...
        : Array.isArray((body as { data?: unknown[] })?.data)
          ...
          : [];
    const out: OrbitProviderConnection[] = [];
...
export function usableProviderAliasSet(
  connections: OrbitProviderConnection[],
 ...
 */
export function isUsableCombo(combo: OrbitRawCombo, usable: UsableProviderSet): boolean {
