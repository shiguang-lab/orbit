...
export interface OrbitRawModelEntry {
  id: string;
  object?: string;
  owned_by?: string;
  root?: string | null;
 ...
 * Orbit instance. Surfaced as a dependency so unit tests can inject a
 * stub without monkey-patching global `fetch`.
 *
 * Why we inline this instead of using `@Orbit/opencode-provider`'s
 ...
 */
export type OrbitModelsFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<OrbitRawModelEntry[]>;

/**
 * Default fetcher: `GET <baseURL>/v1/models` with bearer auth + AbortController
 * timeout. Accepts both the `{object:"list", data:[…]}` envelope Orbit
 * emits today and a bare-array envelope (defensive — keeps the plugin
 * working if a future Orbit build trims the wrapper). Anything that
  ...
  try {
    ...
    const rawList: unknown[] = Array.isArray(body)
      ...
      : body && typeof body === "object" && Array.isArray((body as { data?: unknown }).data)
        ...
        : [];
    const out: OrbitRawModelEntry[] = [];
    for (const r of rawList) {
      if (r && typeof r === "object" && typeof (r as { id?: unknown }).id === "string") {
        out.push(r as OrbitRawModelEntry);
      }
    }
    return out;
  } finally {
...
function warnDeprecatedPrefixesOnce(prefixes: string[]): void {
  ...
  console.warn(
    "[orbit-plugin] [WARN] apiFormat.anthropicPrefixes is deprecated; convert to anthropicModels (full IDs)"
  );
}
 ...
 *   2. `cost: undefined` is illegal (cost is required). Orbit doesn't
 ...
 *      is Orbit's responsibility at routing time.
 *   3. `tool_call` → `toolcall` (ModelV2 field name; one word).
 *   4. `attachment` maps from `capabilities.vision` per Orbit
 ...
 *      that wins.
 *   5. `thinking` from Orbit has no 1:1 ModelV2 slot. We OR it into
 *      `reasoning` so thinking-only models still surface a non-false
 *      reasoning flag.
 *   6. `last_updated` from Orbit has no ModelV2 slot — dropped.
 *      `release_date` lands in ModelV2.release_date with `""` fallback
 *      (the field is required as `string`).
 *   7. `temperature: true` per Orbit convention (OpenAI-compat mode
 ...
 *      Unknown strings (future Orbit additions) are ignored — when the
 *      server adds new modalities we can map them here without breaking
 *      existing entries.
 *   9. `status: "active"` — Orbit doesn't tier models alpha/beta on
...
export function mapRawModelToModelV2(
  ...
  // UI offers exactly the tiers Orbit vouches for (instead of opencode's
  ...
  return {
    ...
    // `providerId` so credentials resolve as `(Orbit, model)`.
