...
export interface OrbitEnrichmentEntry {
   ...
   * Populated by `defaultOrbitEnrichmentFetcher` from
...
export type OrbitEnrichmentMap = Map<string, OrbitEnrichmentEntry>;

/**
 * Reverse-index the enrichment map from `providerCanonical → providerAlias`.
 *
 * Orbit's `/api/pricing/models` is keyed by short ALIAS (`cc`, `cx`,
...
export function buildCanonicalToAliasMap(
  enrichment: OrbitEnrichmentMap | undefined
): Map<string, string> {
...
): OrbitEnrichmentEntry | undefined {
 ...
 *
 * Real example: Orbit's `pricing['cohere']` slot lists 10 curated
 ...
 * a minimal `OrbitEnrichmentEntry` whenever the direct lookup
...
export function buildAliasIndex(
  enrichment: OrbitEnrichmentMap | undefined
): Map<string, OrbitEnrichmentEntry> {
  const out = new Map<string, OrbitEnrichmentEntry>();
  if (!enrichment) return out;
  for (const entry of enrichment.values()) {
    const alias = typeof entry.providerAlias === "string" ? entry.providerAlias.trim() : "";
...
export function resolveProviderTagEntry(
  rawId: string,
  direct: OrbitEnrichmentEntry | undefined,
  aliasIndex: Map<string, OrbitEnrichmentEntry>,
  canonicalToAlias?: Map<string, string>
): OrbitEnrichmentEntry | undefined {
  if (direct) {
 ...
 * pricing + free-tier budgets) from a running Orbit instance.
 */
/**
 ...
 */
export type OrbitEnrichmentSourceError = (endpoint: string, reason: string) => void;

export type OrbitEnrichmentFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number,
  onSourceError?: OrbitEnrichmentSourceError
 ...
 */
export const defaultOrbitEnrichmentFetcher: OrbitEnrichmentFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000,
  onSourceError
  ...
  const out: OrbitEnrichmentMap = new Map();
      ...
      if (providers && typeof providers === "object") {
        for (const [providerAlias, slot] of Object.entries(providers)) {
          ...
          for (const m of models) {
            ...
            const entry: OrbitEnrichmentEntry = {
              providerAlias,
              providerCanonical,
            };
      ...
      if (body && typeof body === "object" && !Array.isArray(body)) {
        for (const [providerAlias, slot] of Object.entries(body as Record<string, unknown>)) {
          ...
          for (const [modelId, raw] of Object.entries(slot as Record<string, unknown>)) {
            ...
            const parsed: NonNullable<OrbitEnrichmentEntry["pricing"]> = {};
...
export function applyEnrichment(
  model: ModelV2,
  enrichment: OrbitEnrichmentEntry | undefined,
