...
describe("catalog auto combos (v1 parity)", () => {
  it("publishes auto/* ids from the auto-combos fetcher", async () => {
    ...
    try {
      ...
      assert.ok(models.has("Orbit/auto"), "auto entry must be published");
      assert.ok(models.has("Orbit/auto/coding"), "auto/coding entry must be published");
      const coding = models.get("Orbit/auto/coding");
  ...
  it("hidden auto combos are skipped", async () => {
    ...
    try {
      ...
      assert.ok(!models.has("Orbit/auto/offline"));
      assert.ok(models.has("Orbit/auto/fast"));
    } finally {
      restore();
    }
  });
