...
describe("plugin-v2 entrypoint", () => {
  it("boot line is silent by default, visible with startupDebug", async () => {
    ...
    try {
      await (plugin as unknown as { setup: (ctx: FakeCtx) => Promise<void> }).setup(
        fakeCtx({ baseURL: "https://gw.example.com", providerId: "Orbit" }, seen)
      );
    } finally {
    ...
    try {
      await (plugin as unknown as { setup: (ctx: FakeCtx) => Promise<void> }).setup(
        fakeCtx(
          { baseURL: "https://gw.example.com", providerId: "Orbit", startupDebug: true },
          seen2
        )
      );
    } finally {
    ...
    assert.ok(
      warns2.some((line) => line.includes("init providerId=Orbit")),
  ...
  it("declares key plus env methods and no oauth in the integration transform", async () => {
    ...
    const ctx: FakeCtx = {
      options: { baseURL: "https://gw.example.com", providerId: "Orbit" },
      catalog: {
        transform: (cb: (draft: unknown) => unknown) => {
          seen.push({ kind: "catalog" });
    ...
    assert.deepEqual(draft.updates, [{ id: "Orbit", name: "Orbit" }]);
    const keyMethod = draft.methods.find((m) => m.method.type === "key");
    const envMethod = draft.methods.find((m) => m.method.type === "env");
    assert.ok(keyMethod);
    assert.equal(keyMethod?.integrationID, "Orbit");
    assert.ok(envMethod);
    assert.deepEqual(envMethod?.method.names, ["ORBIT_API_KEY"]);
  ...
  it("lazy refresh: [m1] then [m1,m2] reloads once; identical runs never reload", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "Orbit-lazy-"));
