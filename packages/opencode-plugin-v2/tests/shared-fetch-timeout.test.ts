...
import { defaultOrbitCombosFetcher } from "../src/shared/combos-map.js";
...
describe("shared default fetchers abort on timeout", () => {
  it("models fetcher aborts a hanging request after timeoutMs", async () => {
    ...
    try {
      await assert.rejects(
        defaultOrbitModelsFetcher("https://gw.example.com", "k", 20),
        /aborted/i
      );
    } finally {
  ...
  it("combos fetcher aborts a hanging request after timeoutMs", async () => {
    ...
    try {
      await assert.rejects(
        defaultOrbitCombosFetcher("https://gw.example.com", "k", 20),
        /aborted/i
      );
    } finally {
      globalThis.fetch = origFetch;
