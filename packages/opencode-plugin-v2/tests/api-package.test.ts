...
describe("catalog api package (models + combos + auto-combos)", () => {
  it("every published entry carries a non-empty supported api.package", async () => {
    ...
    for (const key of ["Orbit/gpt-x", "Orbit/combo-a", "Orbit/auto"]) {
  ...
  it("auto-combos follow the same anthropic apiFormat rule as models", async () => {
    ...
    assert.equal(apiPackageOf(models.get("Orbit/anthropic/claude-x")), "@ai-sdk/anthropic");
    assert.equal(apiPackageOf(models.get("Orbit/auto/coding")), "@ai-sdk/anthropic");
    assert.equal(apiPackageOf(models.get("Orbit/auto/fast")), "@ai-sdk/openai-compatible");
  });
});
