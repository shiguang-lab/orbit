...
interface Fixture {
  models: OrbitRawModelEntry[];
  combos: OrbitRawCombo[];
}
...
describe("v1-vs-v2 catalog parity", () => {
  it("same fixture models publish the same key set modulo documented exclusions", async () => {
    ...
    const fetcher: OrbitModelsFetcher = async () => fixture.models;
    const combosFetcher: OrbitCombosFetcher = async () => fixture.combos;

    const recorded = await loadV1Parity();
    assert.equal(recorded.hookId, "opencode-Orbit", "v1 published under its own provider id");

    const stripX = (k: string): string =>
      k.startsWith("Orbit/") ? k.slice("Orbit/".length) : k;
    ...
    const mAlpha = draft.models.get("Orbit/m-alpha");
    ...
    // (`Combo Fast` -> `Orbit/combo-fast`) and keys the colliding combo
    // by its raw id (`good-combo` -> `Orbit/good-combo`, combo wins);
    // v2 publishes combo ids verbatim (`Orbit/combo-fast`,
    // `Orbit/good-combo` overwriting the raw model). v1 and v2 therefore
    ...
    // `startsWith("Orbit/")` filter and is dropped here).
    ...
    try {
      ...
      assert.ok(draft2.models.has("Orbit/good-combo"), "colliding combo key wins in v2");
    } finally {
    ...
    for (const combo of fixture.combos) {
      const members = (combo.models ?? [])
        ...
        .filter((m): m is OrbitRawModelEntry => m !== undefined);
      assert.deepEqual(
        JSON.parse(JSON.stringify(sharedMapCombo(combo, members, "Orbit", TEST_OPTS.baseURL))),
