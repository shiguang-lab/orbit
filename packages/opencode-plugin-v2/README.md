# @orbit/opencode-plugin-v2

OpenCode v2 plugin (`define({ id, setup })`, Promise API) that publishes the live Orbit catalog — models from `/v1/models`, combos from `/api/combos` (least-common-denominator join), auto-combos from `/api/combos/auto`, enrichment (names + pricing), and usable-provider filtering — into the v2 `catalog.transform`, with `key` + `env` auth via `integration.transform`.

Companion to `@Orbit/opencode-plugin` (OpenCode v1, same repo). The two packages are independent: this one carries its own catalog-mapping logic and the v1 plugin is left untouched.

## Install

```sh
npm install @orbit/opencode-plugin-v2
```
...
```json
{
  "plugins": [
    {
      "package": "@orbit/opencode-plugin-v2",
      "options": {
        "providerId": "Orbit",
...
3. **`ORBIT_API_KEY` in the environment.**
...
| Key                              | Default                                        | Notes                                                                                                                 |
| -------------------------------- | ---------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| `providerId`                     | `"Orbit"`                                  | Provider id and integration id; models publish under `<providerId>/…`                                                 |
| `baseURL`                        | required                                       | Orbit gateway root (no `/v1` suffix needed)                                                                       |
| `apiKey`                         | connected credential, then `ORBIT_API_KEY` | Chat key for `/v1/*` — see [Credentials](#credentials)                                                                |
| `managementReadToken`            | falls back to `apiKey`                         | Management key for `/api/*` (combos, providers, enrichment) — usually **not** the same key                            |
| `displayName`                    | `"Orbit"`                                  | Provider display name                                                                                                 |
