/**
 * Build the ShiguangGateway `customModels` entries for Factory Droid's `settings.json`.
 *
 * Ported from upstream PR decolua/9router#618 (author Anurag Saxena) — multi-model
 * support for the Factory Droid CLI tool. Adapted to ShiguangGateway branding
 * (`custom:ShiguangGateway-<i>`) and extracted as a pure helper so the route-handler
...
/** True when a `customModels` entry was written by ShiguangGateway (any index). */
export function isShiguangGatewayCustomModel(entry: { id?: unknown } | null | undefined): boolean {
