...
export const DEFAULT_DISPLAY_BASE_URL = "http://127.0.0.1:8787";
...
/**
 * Resolve the deploy basePath for display URLs (e.g. `/shiguangGateway`).
 *
 * Priority:
 * 1. Explicit `envBasePath` argument / `NEXT_PUBLIC_SHIGUANG_GATEWAY_BASE_PATH`
  ...
  const fromEnv = normalizeBasePath(
    envBasePath ??
      (typeof process !== "undefined"
        ? process.env.NEXT_PUBLIC_SHIGUANG_GATEWAY_BASE_PATH || process.env.SHIGUANG_GATEWAY_BASE_PATH
        : undefined)
  );
 ...
 * @param envBasePath       `NEXT_PUBLIC_SHIGUANG_GATEWAY_BASE_PATH` / `SHIGUANG_GATEWAY_BASE_PATH`
 */
export function resolveDisplayBaseUrl(
  ...
  // Example: NEXT_PUBLIC_BASE_URL=https://host/shiguangGateway must not be collapsed to
  ...
  // reverse-proxy subpaths (SHIGUANG_GATEWAY_BASE_PATH).
 ...
 *   1. Public browser origin (+ SHIGUANG_GATEWAY_BASE_PATH when set) — proves the
 ...
 * `src/shared/utils/resolveGatewayBaseUrl.ts` instead.
 */
