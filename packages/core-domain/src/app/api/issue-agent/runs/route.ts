...
import { z } from "zod";
import { appendIssueAgentAuditRecord } from "../../../../lib/issueAgent/audit.ts";
import {
...
function isIssueAgentEnabled(): boolean {
  return ENABLED_VALUES.has((process.env.SHIGUANG_GATEWAY_ISSUE_AGENT_ENABLED ?? "").toLowerCase());
}
...
export async function POST(request: Request) {
    ...
    return NextResponse.json(
      {
        ...
        enabled: false,
        requiredEnv: "SHIGUANG_GATEWAY_ISSUE_AGENT_ENABLED=true",
      },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      {
        ...run,
        runner: "shiguangGateway-chat-completions",
