...
import { KIRO_MITM_PROFILE } from "../../../../mitm/targets/kiro.ts";
import { ANTIGRAVITY_MITM_PROFILE } from "../../../../mitm/targets/antigravity.ts";
...
async function buildMitmResponse() {
  const { getMitmStatus, getCachedPassword } = await import("../../../../mitm/manager.runtime.ts");
    ...
    if (searchParams.get("download") === "cert") {
      ...
      return new NextResponse(fs.readFileSync(certPath), {
        headers: {
          ...
          "Content-Disposition": 'attachment; filename="shiguangGateway-mitm-ca.crt"',
        },
      });
    }
  ...
  try {
    ...
    if (typeof parsed.data.enabled === "boolean") {
      const { getCachedPassword, setCachedPassword, startMitm, stopMitm } =
        await import("../../../../mitm/manager.runtime.ts");
      const { isRoot } = await import("../../../../mitm/systemCommands.ts");
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { getMitmStatus } = await import("../../../../mitm/manager.runtime.ts");
    const status = await getMitmStatus();
    if (status.running) {
      return NextResponse.json(
    ...
    const { generateCert } = await import("../../../../mitm/cert/generate.ts");
    await generateCert();
