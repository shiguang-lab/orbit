 ...
 * Exports a legacy ShiguangGateway-compatible JSON backup.
 */
  ...
  try {
    ...
    return new NextResponse(JSON.stringify(exportData, null, 2), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="shiguangGateway-legacy-backup-${new Date().toISOString().replace(/[:.]/g, "-")}.json"`,
      },
    });
  } catch (error) {
