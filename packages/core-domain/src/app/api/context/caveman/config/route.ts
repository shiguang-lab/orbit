...
/** Legacy Caveman alias retained for route parity; canonical ownership is control-api. */
export async function GET(request: NextRequest) {
  if (!(await isAuthenticated(request))) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  try {
    return NextResponse.json(await getCompressionSettings());
  } catch (error) {
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    if (isValidationFailure(validation)) return NextResponse.json({ error: validation.error }, { status: 400 });
    return NextResponse.json(await updateCompressionSettings(validation.data as Parameters<typeof updateCompressionSettings>[0]));
  } catch (error) {
    return NextResponse.json({ error: sanitizeErrorMessage(error) }, { status: 500 });
  }
}
