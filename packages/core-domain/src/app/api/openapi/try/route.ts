...
const tryRequestSchema = z.object({
  ...
  path: z
    ...
    .refine(
      ...
      "Path must target an ShiguangGateway API endpoint"
    ),
...
function getRequestOrigin(request: NextRequest) {
  // The management route runs on control-api, but protocol requests belong to
  // edge-gateway in the split deployment. Keep the browser origin only for
  // legacy all-in-one/local mode; production config supplies the edge base.
  const configured = (process.env.SHIGUANG_GATEWAY_BASE_URL || process.env.EDGE_GATEWAY_INTERNAL_URL || "").trim();
