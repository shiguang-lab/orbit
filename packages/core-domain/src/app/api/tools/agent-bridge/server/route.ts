 ...
 */
import { AgentBridgeServerActionSchema } from "../../../../../shared/schemas/agentBridge.ts";
import { getCachedPassword, setCachedPassword } from "../../../../../mitm/manager.ts";
import { installCertResult, checkCertInstalled } from "../../../../../mitm/cert/install.ts";
import { generateCert } from "../../../../../mitm/cert/generate.ts";
import { resolveMitmDataDir } from "../../../../../mitm/dataDir.ts";
import {
...
/**
 * Resolve the ShiguangGateway API key the spawned MITM child (`server.cjs`) uses to
 ...
 * "ROUTER_API_KEY required" even though ShiguangGateway already had a usable key in
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    if (action === "start") {
      ...
      const { startMitm } = await import("../../../../../mitm/manager.runtime.ts");
    ...
    if (action === "stop") {
      ...
      const { stopMitm } = await import("../../../../../mitm/manager.runtime.ts");
    ...
    if (action === "restart") {
      const pwd = sudoPassword || getCachedPassword() || "";
      const { startMitm, stopMitm, getMitmStatus } = await import("../../../../../mitm/manager.runtime.ts");
