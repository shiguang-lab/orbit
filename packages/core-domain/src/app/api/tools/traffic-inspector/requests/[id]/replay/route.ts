 ...
 *
 * Re-issues the captured request through the local ShiguangGateway instance and
...
const SHIGUANG_GATEWAY_BASE = process.env.SHIGUANG_GATEWAY_BASE_URL ?? process.env.INTERNAL_BASE_URL ?? "http://127.0.0.1:8787";
...
export async function POST(_request: Request, { params }: Params): Promise<Response> {
  ...
  const url = `${SHIGUANG_GATEWAY_BASE}${entry.path}`;

  const replayHeaders: Record<string, string> = {
    "content-type": "application/json",
    "x-shiguangGateway-source": "inspector-replay",
  };
