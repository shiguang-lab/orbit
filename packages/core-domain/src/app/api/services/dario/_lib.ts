...
import { getSupervisor, registerSupervisor } from "../../../../lib/services/registry.ts";
import { ServiceSupervisor } from "../../../../lib/services/ServiceSupervisor.ts";
import { resolveSpawnArgs, DARIO_DEFAULT_PORT } from "../../../../lib/services/installers/dario.ts";
...
export async function getOrInitSupervisor(): Promise<ServiceSupervisor> {
  ...
  try {
    ...
    // Do not create a supervisor with a fabricated token. The API route will
    // surface the real storage/decryption failure to the operator.
    throw error;
  }
