...
async function testComboTarget(target, baseInternalUrl, internalApiKey: string | null, prompt?: string) {
  ...
  try {
    ...
    const testBody = buildComboTestRequestBody(modelStr, isEmbedding, { prompt });
    ...
    try {
      res = await fetch(internalUrl, {
        ...
        headers: {
          ...
          // ShiguangGateway's semantic cache or other request reuse layers.
          "X-ShiguangGateway-No-Cache": "true",
          ...(target.connectionId ? { "X-ShiguangGateway-Connection": target.connectionId } : {}),
...
export async function POST(request) {
  ...
  try {
    ...
    const { comboName, prompt } = validation.data;
    ...
    const results = await Promise.all(
      targets.map((target) => testComboTarget(target, baseInternalUrl, internalApiKey, prompt))
    );
