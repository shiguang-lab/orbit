...
import { getAuthzBypassSnapshot } from "../../lib/config/runtimeSettings.ts";
import {
...
export const LOCAL_ONLY_API_PREFIXES: ReadonlyArray<string> = [
  ...
  "/api/local/", // T-12: 1-click local service launchers (Redis today; spawns podman/docker) — loopback-enforced by the control app's local-redis/local-endpoints.ts guard (Hard Rules #15 + #17)
  ...
  "/api/acp/agents", // ACP custom-agent registry: POST registers a client-chosen `binary`; GET / POST {action:"refresh"} runs detectInstalledAgents() -> execFileSync(probe.command, probe.args, { shell }) transitively (apps/control-api/src/acp/runtime/agent-registry.ts) — RCE-via-tunnel surface (Hard Rules #15 + #17, #7948)
