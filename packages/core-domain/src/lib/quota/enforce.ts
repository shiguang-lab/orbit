...
export async function enforceQuotaShare(input: EnforceInput): Promise<EnforceDecision> {
  ...
  let allocations: Array<{
    poolId: string;
    allocation: import("../db/quotaPools.ts").PoolAllocation;
  }>;
  try {
  ...
  let pool: import("../db/quotaPools.ts").QuotaPool | null = null;
  let poolAllocation: import("../db/quotaPools.ts").PoolAllocation | null = null;
  for (const { poolId, allocation } of allocations) {
    let p: import("../db/quotaPools.ts").QuotaPool | null = null;
    try {
      p = getPool(poolId);
    } catch {
      continue;
  ...
  if (input.model) {
    let modelCap: import("../db/quotaModelCaps.ts").ModelCap | null = null;
    try {
    ...
    if (modelCap && modelCap.capValue > Number.EPSILON) {
      ...
      if (modelConsumed >= modelCap.capValue) {
        try {
          const { notifyWebhookEvent } = await import("../webhookDispatcher.ts");
