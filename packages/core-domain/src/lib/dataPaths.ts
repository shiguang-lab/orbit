...
export function resolveWritableDataDir({ isCloud = false }: { isCloud?: boolean } = {}): string {
  ...
  // OPERATOR'S REAL database (~/.shiguangGateway/storage.sqlite — live provider credentials).
  ...
  // `SHIGUANG_GATEWAY_ALLOW_DEFAULT_DATA_DIR=1` opts back in, so the intent is recorded.
  if (
