 ...
 *      SHIGUANG_GATEWAY_LOCAL_ENDPOINTS_ENABLED=1 is set. The flag is opt-in so
 *      accidental dev deployments do not expose the API.
 *   3. Trust-list the ShiguangGateway desktop app via a shared bearer token
 *      (SHIGUANG_GATEWAY_LOCAL_ENDPOINTS_TOKEN). The desktop app injects the header
...
export function isLocalRequestAllowed(): { allowed: true } | { allowed: false; reason: string } {
  ...
  if (headers) {
    ...
    // (`[::1]:8787`) alongside bare `::1`, `localhost`, and `127.0.0.1`.
  ...
  // Production opt-in
  if (process.env.NODE_ENV === "production" && process.env.SHIGUANG_GATEWAY_LOCAL_ENDPOINTS_ENABLED !== "1") {
