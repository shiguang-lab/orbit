...
import { matchesCron } from "./cronMatch.ts";
// Reuses the CLI's own backup implementation rather than duplicating it — the
// `shiguangGateway backup auto enable` schedule this job executes is written by
...
function getIntervalMs() {
  const raw = process.env.SHIGUANG_GATEWAY_BACKUP_SCHEDULE_JOB_INTERVAL_MS;
