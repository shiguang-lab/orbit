...
async function recognizeVerifiedSupporter(identity: string): Promise<void> {
  const { emitGamificationEvent } = await import("../gamification/events.ts");
...
export async function syncRadarIntel(deps: IntelSyncDeps = {}): Promise<IntelSyncStatus> {
  ...
  try {
    ...
    const settings = getSettingsFn
      ? getSettingsFn()
      : (await import("../db/radar.ts")).getRadarSettings();
    ...
    const existing = getCacheFn
      ? getCacheFn()
      : (await import("../db/radar.ts")).getRadarIntelCache();
    ...
    else (await import("../db/radar.ts")).setRadarIntelCache(cacheEntry);
