...
import { buildComboScoringInspectorResponse } from "./comboScoringInspector.ts";
import { inspectTargetResilience } from "./resilienceExplain.ts";
import type {
  ComboScoringInspectorFactor,
  ComboScoringInspectorTarget,
  ResilienceExplanation,
} from "../../shared/types/utilization.ts";
...
function buildFactors(log: ExplainLog, targetStats: TargetStats): ExplanationFactor[] {
  ...
  factors.push({
    ...
    details: log.comboName
      ...
      : "No combo metadata was persisted, so ShiguangGateway treated this as direct routing.",
  });
  ...
  factors.push({
    ...
    details: "Shows whether ShiguangGateway cache or compression affected cost/latency for this route.",
  });
