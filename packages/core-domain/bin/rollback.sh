#!/usr/bin/env bash
# bin/rollback.sh — roll ShiguangGateway back to a previous release to mitigate a bad
# deploy. Part of the deploy-rollback incident-recovery flow.
#
# Methods (auto-detected; override with --method):
#   • npm    — `npm install -g shiguang-gateway@<version>` and, if PM2 manages it,
#              `pm2 restart shiguang-gateway`. This is how the VPS deploy runs.
#   • docker — re-tag the local image shiguang-gateway:<version> to shiguang-gateway:prod and
...
Rolls ShiguangGateway back to <version> (e.g. 3.8.35 or v3.8.35). With no version,
...
if [ -z "$VERSION" ]; then
  ...
  VERSION="$(npm view shiguang-gateway versions --json 2>/dev/null | node -e '
    let s = "";
...
ops_log "target: shiguang-gateway@$VERSION via $METHOD"
ops_confirm "Roll ShiguangGateway back to $VERSION via $METHOD?" || ops_die "aborted"

case "$METHOD" in
  npm)
    ops_require_cmd npm
    npm install -g "shiguang-gateway@$VERSION"
    if command -v pm2 >/dev/null 2>&1 && pm2 jlist 2>/dev/null | grep -q '"name":"shiguang-gateway"'; then
      pm2 restart shiguang-gateway --update-env
      ops_log "pm2 restarted shiguang-gateway"
    else
      ops_log "installed shiguang-gateway@$VERSION — restart the service to apply (no PM2 'shiguang-gateway' process found)"
    fi
    ;;
  docker)
    ops_require_cmd docker
    if ! docker image inspect "shiguang-gateway:$VERSION" >/dev/null 2>&1; then
      ops_die "local image shiguang-gateway:$VERSION not found — build it from the $VERSION checkout first (this compose builds the 'prod' tag, it does not pull a registry tag)"
    fi
    docker tag "shiguang-gateway:$VERSION" shiguang-gateway:prod
    docker compose -f "$REPO_ROOT/docker-compose.prod.yml" up -d --no-build
    ops_log "recreated prod service from shiguang-gateway:$VERSION"
    ;;
