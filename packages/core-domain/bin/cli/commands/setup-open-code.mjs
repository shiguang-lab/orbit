/**
 * shiguangGateway setup opencode — Wire the bundled @shiguang-gateway/opencode-plugin
 * into a local OpenCode install.
 *
 * Closes the gap where `npm install -g shiguangGateway` ships the plugin
 * inside the shiguangGateway package (`@shiguang-gateway/opencode-plugin/dist/`) but
 ...
 *   3. Copies the built plugin into `<opencode>/plugins/shiguangGateway/`.
 *   4. Creates or updates `opencode.json` with a single `plugin` entry
 *      pointing at the local copy (so OC ≥1.15 picks it up).
 *   5. Optionally runs `opencode auth login --provider shiguangGateway`
...
// We walk up from this file to find the shiguangGateway package root. The script
// lives at `<shiguangGateway>/bin/cli/commands/setup-open-code.mjs`, so the
...
// The bundled plugin ships at PACKAGE_ROOT/@shiguang-gateway/opencode-plugin/
// (see root package.json `files`: ["@shiguang-gateway/", ...]). The env override
// exists so tests can point at a fixture without building the real plugin.
const BUNDLED_PLUGIN_DIR =
  process.env.SHIGUANG_GATEWAY_OPENCODE_PLUGIN_DIR || join(PACKAGE_ROOT, "@shiguangGateway", "opencode-plugin");
...
/**
 * Locate the bundled @shiguang-gateway/opencode-plugin dist. The plugin may be
 * present in two states:
 *
 *   - Built (`dist/index.cjs` + `dist/index.js` exist) — preferred,
 *     ships from a published shiguangGateway tarball after Step 8.8 of
...
function resolveBundledPlugin() {
  if (!existsSync(BUNDLED_PLUGIN_DIR)) {
    throw new Error(
      `Bundled @shiguang-gateway/opencode-plugin not found at ${BUNDLED_PLUGIN_DIR}.\n` +
        `This usually means shiguangGateway was installed from a source tree that does not ` +
        `include the workspace package. Try reinstalling shiguangGateway (npm install -g shiguangGateway) ` +
        `or run \`cd @shiguang-gateway/opencode-plugin && npm install && npm run build\` from the source repo.`
    );
  }
  ...
  if (!existsSync(esmEntry)) {
    throw new Error(
      `@shiguang-gateway/opencode-plugin dist/ not built (looked for ${esmEntry}).\n` +
...
/**
 * Copy the plugin package into `<opencodeConfig>/plugins/shiguangGateway/`. We
 ...
 * resolve the peer deps from the shiguangGateway package's tree, which is
 * unreliable.
 */
function installPluginToOpenCode(pluginInfo, opencodeConfigDir) {
  const targetDir = join(opencodeConfigDir, "plugins", "shiguangGateway");
...
function registerPluginInOpenCodeConfig({
    ...
    try {
      ...
      throw new Error(
        `Failed to parse existing ${configPath}: ${err.message}\n` +
          `Fix or remove the file manually, then re-run \`shiguangGateway setup opencode\`.`
      );
    }
  }
  ...
  const entry = [
    `./plugins/shiguangGateway/dist/index.js`,
    {
      providerId,
      baseURL,
  ...
  // drop a legacy `opencode-shiguangGateway-auth` entry if present — that
  // package is the obsolete predecessor of @shiguang-gateway/opencode-plugin
  // and was the root cause of issue #3711.
  const filtered = plugins.filter((p) => {
    if (typeof p === "string") {
      return !p.includes("opencode-shiguangGateway-auth");
    }
    if (Array.isArray(p) && p[1] && typeof p[1] === "object") {
      ...
      if (typeof p[0] === "string" && p[0].includes("opencode-shiguangGateway-auth")) {
        return false;
      }
    }
    return true;
 ...
 *
 * The bundled @shiguang-gateway/opencode-plugin registers its provider under
 ...
 * `--provider opencode-shiguangGateway`.
 *
 ...
 *
 * @param {object} opts
 * @param {string} [opts.providerId="shiguangGateway"]
 * @param {string} [opts.baseURL="http://localhost:8787"]  (Commander camelCases
...
export async function runSetupOpenCodeCommand(opts = {}) {
  const providerId = opts.providerId || "shiguangGateway";
...
/**
 * Register the `shiguangGateway setup opencode` subcommand on the parent
 * `setup` command. Commander builds the doc/help from the chain, so
 * `shiguangGateway setup --help` automatically shows the new subcommand.
 *
...
export function registerSetupOpenCode(setupCommand) {
  setupCommand
    ...
    .description(
      t("setup.opencode") ||
        "Install and register the bundled @shiguang-gateway/opencode-plugin with a local OpenCode install"
    )
    .option(
      "--provider-id <id>",
      "OpenCode provider id to register (default: shiguangGateway)",
      "shiguangGateway"
    )
    .option(
      "--base-url <url>",
      "ShiguangGateway base URL the plugin should talk to (default: active context or http://localhost:8787)"
    )
    .option(
      "--remote <url>",
      "Remote ShiguangGateway URL, e.g. http://192.168.0.15:8787 (overrides --base-url and the context)"
    )
