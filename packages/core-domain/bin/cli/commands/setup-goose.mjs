/**
 * shiguangGateway setup-goose — configure Goose (block/goose) for ShiguangGateway.
 *
...
export function resolveGooseTarget(opts = {}) {
  ...
  if (opts.remote) root = stripToRoot(opts.remote);
    ...
    if (!root) root = `http://localhost:${Number(opts.port ?? process.env.PORT ?? 8787) || 8787}`;
  }
...
export async function runSetupGooseCommand(opts = {}) {
  ...
  const guard = await guardHostConfigTarget(configPath, {
    toolLabel: "Goose",
    hostCommand: "shiguangGateway setup-goose",
    allowContainerWrite: Boolean(opts.allowContainerWrite ?? opts["allow-container-write"]),
    dryRun,
  });
  if (guard !== 0) return guard;

  printHeading("ShiguangGateway → Goose (openai-compatible)");
...
export function registerSetupGoose(program) {
  program
    .command("setup-goose")
    .description(
      "Configure Goose for ShiguangGateway: write ~/.config/goose/config.yaml + print the env recipe"
    )
