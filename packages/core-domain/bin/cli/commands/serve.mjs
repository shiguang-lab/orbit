...
// URL scheme for the "ShiguangGateway is running" banner — flipped to https when
...
export function registerServe(program) {
  const command = program
    ...
    .option(
      "--tls-cert <path>",
      t("serve.tls_cert") ||
        "Path to a TLS certificate (PEM) to serve HTTPS (also SHIGUANG_GATEWAY_TLS_CERT)"
    )
    .option(
      "--tls-key <path>",
      t("serve.tls_key") ||
        "Path to the TLS private key (PEM) to serve HTTPS (also SHIGUANG_GATEWAY_TLS_KEY)"
    )
    .action(async (opts) => {
      await runServe(opts);
    });
...
export async function runServe(opts = {}) {
  ...
  if (opts.tray === true && opts.trayWorker !== true) {
    const port = parsePort(opts.port ?? process.env.PORT ?? "8787", 8787);
    const tlsCert = opts.tlsCert ?? process.env.SHIGUANG_GATEWAY_TLS_CERT;
    const tlsKey = opts.tlsKey ?? process.env.SHIGUANG_GATEWAY_TLS_KEY;
    urlScheme = resolveTlsOptions({
      ...process.env,
      ...(tlsCert ? { SHIGUANG_GATEWAY_TLS_CERT: tlsCert } : {}),
      ...(tlsKey ? { SHIGUANG_GATEWAY_TLS_KEY: tlsKey } : {}),
    })
      ? "https"
      : "http";
    const result = await startDetachedTray({
      cliPath: join(ROOT, "bin", "shiguang-gateway.mjs"),
      port,
      maxRestarts: opts.maxRestarts ?? 2,
      tlsCert,
      tlsKey,
    });
    console.log(`\x1b[32m✔ ShiguangGateway tray started in background\x1b[0m`);
    console.log(`  \x1b[1mDashboard:\x1b[0m  ${urlScheme}://localhost:${port}`);
    return result;
  }

  // Same prep as bin/shiguang-gateway.mjs — keep it here so a direct `runServe()` call
...
\x1b[0m`);
  ...
  if (!nodeSupport.nodeCompatible) {
    ...
    console.warn(`\x1b[33m  ⚠  Warning: You are running Node.js ${process.versions.node}.
     ...
     Or run:      shiguangGateway runtime repair  (rebuilds into a user-writable runtime; works without a C++ toolchain)\x1b[0m
`);
  }
  ...
  if (!existsSync(serverJs)) {
    ...
    if (isMise) {
      console.error(
        "  \x1b[33m⚠ mise detected:\x1b[0m If you installed via `npm install -g shiguangGateway`,"
      );
      console.error("    try: \x1b[36mnpx shiguangGateway@latest\x1b[0m  (downloads a fresh copy)");
      console.error("    or:  \x1b[36mmise exec -- npx shiguangGateway\x1b[0m");
    } else if (isNvm) {
      console.error(
        "  \x1b[33m⚠ nvm detected:\x1b[0m Try reinstalling after loading the correct Node version:"
      );
      console.error("    \x1b[36mnvm use --lts && npm install -g shiguangGateway\x1b[0m");
    } else {
      console.error("  Try: \x1b[36mnpm install -g shiguangGateway\x1b[0m  (reinstall)");
      console.error("  Or:  \x1b[36mnpx shiguangGateway@latest\x1b[0m");
    }
    process.exit(1);
  }
  ...
  if (
    ...
    console.error(
      "  Or run: \x1b[36mshiguangGateway runtime repair\x1b[0m" +
  ...
  // of RAM under load. An explicit SHIGUANG_GATEWAY_MEMORY_MB still wins.
  const memoryLimit = resolveMaxOldSpaceMb(
    process.env.SHIGUANG_GATEWAY_MEMORY_MB,
  ...
  const env = {
    ...process.env,
    SHIGUANG_GATEWAY_PORT: String(port),
    PORT: String(dashboardPort),
    DASHBOARD_PORT: String(dashboardPort),
    API_PORT: String(apiPort),
    // #10492: HOSTNAME is standard shell state on Unix-like systems, not an
    // ShiguangGateway bind setting. The resolver only keeps its legacy meaning on
    // Windows; SHIGUANG_GATEWAY_SERVER_HOST is the cross-platform explicit setting.
...
function runDaemon(serverJs, env, memoryLimit, dashboardPort, apiPort) {
  ...
  const server = spawn(
    ...
    [
      ...(process.versions.bun
        ? ["--preload", join(APP_DIR, "../src/runtime/setupPolyfill.ts")]
  ...
  server.unref();
  console.log(`\x1b[32m✔ ShiguangGateway started in background (PID: ${server.pid})\x1b[0m`);
...
function runWithoutRecovery(serverJs, env, memoryLimit, dashboardPort, apiPort, noOpen, startedAt) {
  ...
  const shutdown = () => {
    console.log("\n\x1b[33m⏹ Shutting down ShiguangGateway...\x1b[0m");
...
async function runWithSupervisor(
  ...
  if (showLog) process.env.SHIGUANG_GATEWAY_SHOW_LOG = "1";
...
async function maybeStartTray(port, apiPort, supervisor) {
  try {
    ...
    process.stderr.write(`[shiguangGateway][tray] failed to start: ${err?.message ?? String(err)}\n`);
    return false;
  }
}
...
async function onReady(dashboardPort, apiPort, noOpen, startedAt) {
  ...
  console.log(`
  \x1b[32m✔ ShiguangGateway is running!\x1b[0m \x1b[2m(started in ${elapsed}s)\x1b[0m
