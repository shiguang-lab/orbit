...
export class ServerSupervisor {
  ...
  start() {
    ...
    const showLog = process.env.SHIGUANG_GATEWAY_SHOW_LOG === "1";
    // #6321: stdout used to be discarded (`"ignore"`) whenever `--log`/SHIGUANG_GATEWAY_SHOW_LOG
  ...
  handleExit(code, err) {
    ...
    setTimeout(async () => {
      await waitUntilPortFree(process.env.PORT || 8787);
      this.start();
    }, delay);
  }
