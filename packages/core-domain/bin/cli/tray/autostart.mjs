...
function resolveCliPath() {
  ...
  try {
    const which = execSync("command -v shiguang-gateway 2>/dev/null", { encoding: "utf8" }).trim();
    if (which) candidates.push(which);
  } catch {
    // command -v unavailable
  }
  candidates.push(join(dirname(fileURLToPath(import.meta.url)), "..", "..", "shiguang-gateway.mjs"));

  for (const candidate of candidates) {
    if (!candidate) continue;
    try {
      const resolved = realpathSync(candidate);
      if (resolved.endsWith("shiguang-gateway.mjs") && existsSync(resolved)) return resolved;
    } catch {
      // try next candidate
    }
  }
