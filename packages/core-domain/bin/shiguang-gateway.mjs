...
await import("../src/runtime/setupPolyfill.ts");
...
process.on("exit", () => {
  ...
  if (process.env.SHIGUANG_GATEWAY_NO_UPDATE_NOTIFIER) return;
  ...
  if (_notifier.update) {
    _notifier.notify({
      ...
      message:
        ...
        "Run `npm install -g shiguangGateway` or `shiguangGateway update --apply`",
    });
  }
});
