 ...
 *
 * Problem (#7791): when ShiguangGateway is installed via `npm i -g shiguangGateway`, the
 * package files live under `node_modules/shiguangGateway/`. tsx's tsconfig-path
 * resolution does not apply there, so specifiers like `@/shared/utils/featureFlags`
 * (declared in tsconfig.json `paths` as `@/* → ./src/*`) or
 * `@shiguang-gateway/open-sse/services/usage` fail with `ERR_MODULE_NOT_FOUND`.
 ...
 *
 * Each entry:
 *   prefix  — specifier prefix to match (e.g. `"@/"`, `"@shiguang-gateway/open-sse/"`)
 ...
 *             prefix *without* a trailing slash (e.g. `@shiguang-gateway/open-sse` →
 ...
 *   "@/..."                     → <root>/src/...
 *   "@shiguang-gateway/open-sse/..."   → <root>/open-sse/...
 *   "@shiguang-gateway/open-sse"       → <root>/open-sse/index.*
 *
...
export function resolveAlias(specifier, root) {
  ...
  // Exact match (e.g. `@shiguang-gateway/open-sse` with no trailing path) →
