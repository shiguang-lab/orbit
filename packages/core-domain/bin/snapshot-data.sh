#!/usr/bin/env bash
# bin/snapshot-data.sh — consistent point-in-time snapshot of the ShiguangGateway data
...
Env: DATA_DIR (default ~/.shiguang-gateway), DB_BACKUPS_DIR (default $DATA_DIR/db_backups).
EOF
}
