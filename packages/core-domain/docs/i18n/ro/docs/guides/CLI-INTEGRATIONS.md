...
title: "Integrări CLI — direcționează orice CLI de codare către ShiguangGateway"
version: 3.8.50
...
ShiguangGateway oferă o familie de comenzi `setup-*` care configurează un CLI de codare
(Codex, Claude Code, OpenCode, Cline, …) pentru a folosi ShiguangGateway ca backend — astfel
încât instrumentul comunică cu **un** endpoint și ShiguangGateway redirecționează către furnizorul corect cu
fallback automat. Fiecare comandă citește catalogul de modele **live** de la un ShiguangGateway
...
Există, de asemenea, un launcher generic — `shiguang-gateway run <target>` — care lansează
...
`open-code`, `qwen-code`, `gemini-cli`), iar `shiguang-gateway completion` oferă
aceleași cuvinte țintă derivate din manifest. Launcherele legate de fiecare instrument —
`shiguang-gateway launch` (Claude Code) și `shiguang-gateway launch-codex` (Codex) — rămân
disponibile.
...
- [Modul Remote](./REMOTE-MODE.md) — controlează un ShiguangGateway la distanță (VPS / Tailnet) de pe laptopul tău
...
Fiecare comandă respectă **contextul activ** (setat cu `shiguang-gateway connect`, vezi
...
| -------------------------- | ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | --------------- |
| `shiguang-gateway setup-codex`    | OpenAI Codex CLI             | `~/.codex/<name>.config.toml` — un profil pentru fiecare model de text compatibil (`codex --profile <name>`)                                                   | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | Ambele          |
| `shiguang-gateway setup-claude`   | Claude Code                  | `~/.claude/profiles/<name>/settings.json` — un profil pentru fiecare model potrivit (`CLAUDE_CONFIG_DIR`)                                                      | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | Ambele          |
| `shiguang-gateway setup-opencode` | OpenCode (compatibil openai) | `~/.config/opencode/opencode.json` — furnizor `shiguang-gateway` cu fiecare model din catalog (`opencode -m shiguang-gateway/<model>`)                                       | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | Ambele          |
| `shiguang-gateway setup-cline`    | Cline                        | `~/.cline/data/{globalState,secrets}.json` (mod CLI) + imprimă setările extensiei VS Code                                                                      | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | Ambele          |
| `shiguang-gateway setup-kilo`     | Kilo Code                    | `~/.local/share/kilo/auth.json` (CLI) + îmbină `kilocode.*` în `settings.json` al VS Code, dacă este prezent                                                   | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | Ambele          |
| `shiguang-gateway setup-continue` | Continue / `cn` CLI          | `~/.continue/config.yaml` — modele `provider: openai`, cheie prin `${{ secrets.SHIGUANG_GATEWAY_API_KEY }}`                                                           | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Ambele          |
| `shiguang-gateway setup-cursor`   | Cursor                       | Nimic — imprimă pașii în aplicație (configurația Cursor este opacă SQLite)                                                                                     | `--remote` `--api-key` `--only` `--port`                                                                                                   | Ambele          |
| `shiguang-gateway setup-roo`      | Roo Code                     | `~/.shiguang-gateway/roo-settings.json` (document de import) + setează `roo-cline.autoImportSettingsPath` dacă există un `settings.json` al VS Code                   | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | Ambele          |
| `shiguang-gateway setup-crush`    | Crush                        | `~/.config/crush/crush.json` — furnizor `openai-compat`, cheie prin `$SHIGUANG_GATEWAY_API_KEY`                                                                       | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Ambele          |
| `shiguang-gateway setup-goose`    | Goose                        | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + imprimă rețeta de mediu                                                         | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Ambele          |
| `shiguang-gateway setup-aider`    | Aider                        | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + imprimă rețeta de mediu                                                                       | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Ambele          |
| `shiguang-gateway setup-qwen`     | Qwen Code                    | `~/.qwen/settings.json` — array `V4 modelProviders.openai` + `SHIGUANG_GATEWAY_API_KEY` în `~/.qwen/.env`                                                             | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | Ambele          |
| `shiguang-gateway run <target>`   | Lansare runtime (generic)    | Nimic — lansează `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` cu mediul și argumentele corecte; Qwen și Gemini folosesc un home izolat temporar | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | Ambele          |
| `shiguang-gateway launch`         | Claude Code                  | Nimic — lansează `claude` cu `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` injectat                                                                              | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | Ambele          |
| `shiguang-gateway launch-codex`   | OpenAI Codex CLI             | Nimic — lansează `codex` cu furnizorul `shiguang-gateway` injectat prin flag-uri `-c`                                                                                 | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | Ambele          |

Note despre flag-uri (verificate în sursa comenzii):

- `--remote <url>` — preia catalogul de la un ShiguangGateway la distanță (suprascrie `--port`
  și contextul activ). `--api-key <key>` furnizează acreditivul pentru acel
  server (se default-ează la variabila de mediu `SHIGUANG_GATEWAY_API_KEY`, sau token-ul contextului activ).
...
- `--model <id>` pe `shiguang-gateway run` urmează conectarea per-țintă din manifest
  (`bin/cli/cli-manifest.mjs`): **aider** primește `--model openai/<id>` și
  **opencode** `--model shiguang-gateway/<id>` (prefixul este adăugat doar când id-ul
  nu îl poartă deja); **qwen** și **gemini** primesc id-ul exact; **claude** îl primește prin `ANTHROPIC_MODEL`, **goose** prin `GOOSE_MODEL`, și
  **codex** prin argumente `-c model_providers.shiguang-gateway.*`. **Qwen este singura țintă de execuție
  care necesită în mod strict `--model`** — `shiguang-gateway run qwen` fără el iese
  `2` cu o eroare explicită.
- `--port <port>` — portul local ShiguangGateway (default `20128`, ignorat când `--remote`
  este setat). Prezent pe toate comenzile `setup-*` și pe ambele launchere.
- Codurile de ieșire ale `shiguang-gateway run`: codul de ieșire al CLI-ului copil este propagat
...
> Există de asemenea o integrare mai bogată a plugin-ului — `shiguang-gateway setup opencode` — care
> instalează `@shiguang-gateway/opencode-plugin`. Acestea sunt comenzi diferite; tabelul
...
Cu ShiguangGateway rulând pe `localhost:20128`, pur și simplu rulează comanda de configurare pentru instrumentul tău. Catalogul este obținut de la serverul local.
...
export SHIGUANG_GATEWAY_API_KEY=sk-...  # referit prin {env:SHIGUANG_GATEWAY_API_KEY}, niciodată pe disc
...
shiguang-gateway run claude -- --print-system-prompt "revizuiește acest diff"
```
...
Indică orice comandă de configurare către un ShiguangGateway la distanță cu `--remote` + `--api-key`. Catalogul este obținut de la distanță; configurația este scrisă pe mașina ta locală.
...
shiguang-gateway setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m shiguang-gateway/glm/glm-5.2 "..."   # exportă mai întâi SHIGUANG_GATEWAY_API_KEY
...
```bash
shiguang-gateway connect 192.168.0.15        # generează un token scoperit, stochează contextul
shiguang-gateway setup-codex                 # ← acum folosește catalogul la distanță
shiguang-gateway setup-opencode              # ← același lucru
shiguang-gateway launch                      # ← Claude Code împotriva distanței
```
...
ShiguangGateway expune suprafața OpenAI la `/v1`, suprafața Anthropic la rădăcină, și o suprafață nativă Gemini la `/v1beta`. Fiecare integrare este conectată la forma pe care instrumentul său o așteaptă (verificat în sursa comenzii):
...
| `setup-codex`, `launch-codex` (`model_providers.shiguang-gateway.base_url`)       | cu `/v1`          | Da                                         |
...
Când actualizezi cu `shiguang-gateway update` (după confirmare sau cu `--apply`),
ShiguangGateway rulează instalarea cu `--include=optional` inclus:

```bash
npm install -g shiguang-gateway@latest --include=optional
```

Aceasta **nu** este o opțiune pe care o transmiți la `shiguang-gateway update` — este întotdeauna aplicată de
...
# [DRY RUN] Ar rula: npm install -g shiguang-gateway@latest --include=optional
```

Alte opțiuni `shiguang-gateway update` (verificate în sursă): `--check` (iese cu 1 dacă
este depășit), `--apply` (instalează fără a solicita), `--changelog`, `--no-backup`,
`--yes`.

---

## Google Gemini CLI prin `shiguang-gateway run gemini`
...
Gemini a ShiguangGateway (`/v1beta`). `shiguang-gateway run gemini` conectează asta automat:

- `GOOGLE_GEMINI_BASE_URL` → URL-ul de bază activ ShiguangGateway (rădăcină, fără `/v1`);
- `GEMINI_API_KEY` → acreditivul rezolvat ShiguangGateway (opțiune/env/context);
- un **`GEMINI_CLI_HOME`** temporar izolat al cărui `.gemini/settings.json`
  ...
  să nu suprascrie lansarea dirijată de ShiguangGateway — eliminată după ieșire;
...
ShiguangGateway, există un cadru opțional la
...
Opțional: `SHIGUANG_GATEWAY_SMOKE_TARGETS="codex,opencode,qwen"` restricționează verificarea;
`SHIGUANG_GATEWAY_SMOKE_TIMEOUT_MS` suprascrie timeout-ul de 120s pe țintă.
...
- [Configurarea Codex CLI](./CODEX-CLI-CONFIGURATION.md) — configurarea de bază `[model_providers.shiguang-gateway]` unică
