# ShiguangGateway Architecture (Português (Portugal))
...
- facade: `src/lib/usage/*` (decomposed modules in `src/lib/usage/*`)
...
- `src/lib/usage/*`: usage history/call logs facade on top of SQLite tables
