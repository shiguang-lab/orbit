...
5. O diretório `open-sse/` é publicado como o pacote de **workspace npm** `@shiguang-gateway/open-sse`. O código-fonte o importa via `@shiguang-gateway/open-sse/...` (resolvido pelo Next.js `transpilePackages`). Os caminhos de arquivo neste documento ainda usam o nome do diretório `open-sse/` para consistência.
