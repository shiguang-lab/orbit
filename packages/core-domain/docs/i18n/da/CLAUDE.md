...
**ShiguangGateway** — samlet AI proxy/router. Én endpoint, 329 LLM udbydere, auto-fallback.
...
ShiguangGateway har tre relaterede, men distinkte mekanismer til midlertidige fejl. Hold deres
...
- **Imports**: ekstern → intern (`@/`, `@shiguang-gateway/open-sse`) → relativ
...
- **Sti aliaser**: `@/*` → `src/`, `@shiguang-gateway/open-sse` → `open-sse/`, `@shiguang-gateway/open-sse/*` → `open-sse/*`
- **Standardport**: 20128 (API + dashboard på samme port)
- **Data katalog**: `DATA_DIR` miljøvariabel, standard til `~/.shiguang-gateway/`
