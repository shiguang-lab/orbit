# ShiguangGateway — Panduan Mencopot Pemasangan (Bahasa Indonesia)
...
Panduan ini menjelaskan cara mencopot pemasangan ShiguangGateway dari sistem Anda secara bersih.

---

## Mencopot Pemasangan dengan Cepat (v3.6.2+)

ShiguangGateway menyediakan dua skrip bawaan untuk penghapusan yang bersih:
...
Perintah ini menghapus aplikasi ShiguangGateway tetapi **mempertahankan** basis data, konfigurasi, kunci API, dan pengaturan penyedia Anda di `~/.shiguang-gateway/`. Gunakan ini jika Anda berencana memasang ulang nanti dan ingin menyimpan pengaturan yang ada.
...
- Berkas cadangan
- Berkas log
- Semua berkas di direktori `~/.shiguang-gateway/`
...
- Buka `Settings → Apps → ShiguangGateway → Uninstall`
- Atau jalankan uninstaller NSIS dari direktori instalasi

**macOS:**

- Seret `ShiguangGateway.app` dari `/Applications` ke Trash
- Hapus data: `rm -rf ~/Library/Application Support/shiguang-gateway`

**Linux:**

- Hapus berkas AppImage
- Hapus data: `rm -rf ~/.shiguang-gateway`
...
ShiguangGateway menyimpan data di lokasi-lokasi berikut secara default:
