...
Panduan ini mendokumentasikan standar infrastruktur jaringan terbaik untuk mengamankan **ShiguangGateway** dan mengekspos aplikasi Anda ke internet secara aman, **tanpa membuka satu pun port (Zero Inbound)**.

## Apa yang Telah Dilakukan pada VM Anda?

Kami mengaktifkan ShiguangGateway dalam mode **Split-Port** melalui PM2:
...
3. Klik **Add a Tunnel**, pilih **Cloudflared**, dan beri nama `ShiguangGateway-VM`.
...
- **Subdomain:** `shiguang-gateway` atau `panel`
...
2. Pilih **Self-hosted**.
3. Di **Application name**, masukkan `Panel ShiguangGateway`.
4. Di **Application domain**, masukkan `shiguang-gateway.domainanda.com` (sama dengan yang Anda buat di "Rute 2").
5. Klik **Next**.
...
3. Klik **Create rule**.
4. **Name:** `Anti-Penyalahgunaan ShiguangGateway API`
...
2. ShiguangGateway hanya berkomunikasi melalui HTTPS keluar (`cloudflared`) dan tidak menerima koneksi TCP langsung dari internet.
