...
5. `open-sse/` 目錄以 `@shiguang-gateway/open-sse` **npm workspace 套件**形式發布。原始碼透過 `@shiguang-gateway/open-sse/...` 匯入（由 Next.js `transpilePackages` 解析）。本文件中的檔案路徑為求一致仍使用目錄名稱 `open-sse/`。
