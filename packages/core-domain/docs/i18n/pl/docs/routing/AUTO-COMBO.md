---
title: "Silnik ShiguangGateway Auto-Combo"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Silnik ShiguangGateway Auto-Combo
...
1. ShiguangGateway wykrywa prefiks `auto/` w `src/sse/handlers/chat.ts`
...
per-API-key (tabela `auto_candidate_overrides`, migracja `128`) — ShiguangGateway jest
...
[#6940](https://github.com/diegosouzapw/ShiguangGateway/issues/6940). Ponieważ
...
[#3227/#3233](https://github.com/diegosouzapw/ShiguangGateway/issues/3227) i jest
...
| `X-ShiguangGateway-Budget`          | a positive number (max USD per request)                                                                                                                                                 | Twardy sufit kosztu: kandydaci, których szacowany koszt go przekracza, są filtrowani przed selekcją. Co się dzieje, gdy **każdy** kandydat przekracza limit, kontroluje `X-ShiguangGateway-Budget-Fallback` poniżej.                        |
| `X-ShiguangGateway-Budget-Fallback` | `cheapest` (default, aliases: `cheapest-viable`, `soft`) or `strict` (aliases: `block`, `hard`)                                                                                         | `cheapest`: fallback do globalnie najtańszego kandydata nawet jeśli nadal przekracza cap (zachowanie legacy). `strict`: odmawia selekcji — żądanie fail-fast z `HTTP 402` zamiast cichego overspend. Nieznane wartości są ignorowane. |
...
Silnik combo ShiguangGateway obsługuje **19 strategii routingu** (zadeklarowanych w `src/shared/constants/routingStrategies.ts` → `ROUTING_STRATEGY_VALUES`). Sam silnik Auto Combo jest wystawiony pod strategią `auto`; pozostałe są dostępne dla persystowanych combo.
...
import {
  registerStrategy,
  type RouterStrategy,
} from "@shiguang-gateway/open-sse/services/autoCombo/routerStrategy";
...
| Komenda                                | Co robi                                                                         |
| :------------------------------------- | :------------------------------------------------------------------------------ |
| `npm run test:combo:live`              | In-process real routing z `RUN_COMBO_LIVE=1`; snapshot żywej DB ShiguangGateway       |
| `npm run test:combo:live:vps`          | Wywołania HTTP przeciw żywemu serwerowi ShiguangGateway (ustaw `COMBO_LIVE_BASE_URL`) |
