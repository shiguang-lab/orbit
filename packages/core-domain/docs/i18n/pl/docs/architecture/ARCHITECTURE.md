...
5. Katalog `open-sse/` jest publikowany jako **pakiet npm workspace** `@shiguang-gateway/open-sse`. Kod źródłowy importuje go przez `@shiguang-gateway/open-sse/...` (rozwiązywane przez Next.js `transpilePackages`). Ścieżki plików w tym dokumencie nadal używają nazwy katalogu `open-sse/` dla spójności.
