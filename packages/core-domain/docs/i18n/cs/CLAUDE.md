...
**ShiguangGateway** — jednotný AI proxy/router. Jeden koncový bod, 329 poskytovatelů LLM, automatické zálohování.
...
ShiguangGateway má tři související, ale odlišné mechanismy dočasného selhání. Udržujte jejich rozsah oddělený při ladění chování routování. Viz
...
- **Importy**: externí → interní (`@/`, `@shiguang-gateway/open-sse`) → relativní
...
- **Cestovní aliasy**: `@/*` → `src/`, `@shiguang-gateway/open-sse` → `open-sse/`, `@shiguang-gateway/open-sse/*` → `open-sse/*`
- **Výchozí port**: 20128 (API + dashboard na stejném portu)
- **Adresář dat**: `DATA_DIR` env var, výchozí hodnota `~/.shiguang-gateway/`
