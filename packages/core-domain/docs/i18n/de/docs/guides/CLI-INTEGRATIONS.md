...
> Es gibt auch eine umfangreichere Plugin-Integration — `shiguang-gateway setup opencode` — die `@shiguang-gateway/opencode-plugin` installiert. Es sind verschiedene Befehle; die obige Tabelle dokumentiert `setup-opencode`.
