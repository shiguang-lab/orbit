...
- **Nhập khẩu**: bên ngoài → bên trong (`@/`, `@shiguang-gateway/open-sse`) → tương đối
...
- **Biểu thức đường dẫn**: `@/*` → `src/`, `@shiguang-gateway/open-sse` → `open-sse/`, `@shiguang-gateway/open-sse/*` → `open-sse/*`
