...
| `apps/edge-gateway/src/cloud-agents/domain/` | Cloud agent registry and provider adapters; Nest controllers live under `apps/edge-gateway/src/cloud-agents/`. |
...
- `translatorEvents.ts`, `usage/*`, `usageAnalytics.ts`, `webhookDispatcher.ts`
...
Separate npm workspace published as `@shiguang-gateway/open-sse`. Owns request
...
- **`scripts/dev/`** — `run-next.mjs`, `run-next-playwright.mjs`,
  ...
  `run-protocol-clients-tests.mjs`, `healthcheck.mjs`,
  `system-info.mjs`.
