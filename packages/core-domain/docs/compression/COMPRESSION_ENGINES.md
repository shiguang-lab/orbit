...
ShiguangGateway compression is built around engine contracts. A mode can run one engine directly
...
ShiguangGateway never delegates the decision: the adapter pins the model gate to the package's
most restrictive scope, so host environment settings can only narrow the allowlist, never
widen it past ShiguangGateway's measured receipts.
...
actually reach `shiguang-gateway_ccr_retrieve`** (`callerSupportsCcrRetrieve()` in
...
with a `22-87%` range, and a `~46%` input-compression tool. ShiguangGateway uses the Caveman input-side
...
- **Global npm (`npm i -g shiguang-gateway`) / standalone** — run the install command above inside
...
| Tool                                | Scope               | Purpose                          |
| ----------------------------------- | ------------------- | -------------------------------- |
| `shiguang-gateway_compression_status`      | `read:compression`  | Settings, analytics, cache stats |
| `shiguang-gateway_compression_configure`   | `write:compression` | Update global settings           |
| `shiguang-gateway_set_compression_engine`  | `write:compression` | Set mode and optional pipeline   |
| `shiguang-gateway_list_compression_combos` | `read:compression`  | List compression combos          |
| `shiguang-gateway_compression_combo_stats` | `read:compression`  | Read combo/engine analytics      |
