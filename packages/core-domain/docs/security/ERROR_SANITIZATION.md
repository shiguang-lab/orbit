...
```ts
import { buildErrorBody } from "@shiguang-gateway/open-sse/utils/error.ts";
...
} from "@shiguang-gateway/open-sse/utils/error.ts";
```
...
```ts
import { sanitizeErrorMessage } from "@shiguang-gateway/open-sse/utils/error.ts";
...
- The `pino` redaction config (`packages/runtime-logging/src/redaction.ts`) handles structured log redaction separately. This doc covers only the response-message surface.
