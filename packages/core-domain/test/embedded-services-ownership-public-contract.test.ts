...
test("supervisor and installer contracts are control-api-only", () => {
  const forbidden = /core-domain\/control\/embedded-services-(?:lifecycle|install)/;
  for (const root of ["apps/edge-gateway", "apps/worker", "apps/realtime", "packages/open-sse"]) {
    for (const file of sourceFiles(path.join(repoRoot, root))) {
      assert.doesNotMatch(fs.readFileSync(file, "utf8"), forbidden, file);
    }
  }
  const streamingExecutor = fs.readFileSync(path.join(repoRoot, "packages/open-sse/executors/ninerouter.ts"), "utf8");
