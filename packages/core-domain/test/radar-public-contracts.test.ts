...
test("broad Radar aliases and declaration stay retired", () => {
  ...
  const retiredImport = /core-domain\/(?:control\/radar(?:-db|-sync|-referrals-sync|-offers-sync|-intel-sync)?|worker\/radar-scheduler)(?=["'])/;
...
test("Radar scheduler lifecycle remains worker-owned", () => {
  const lifecycleImport = "@shiguang-gateway/core-domain/radar/sync-lifecycle";
  ...
  for (const app of ["control-api", "edge-gateway", "realtime"]) {
    for (const file of sourceFiles(path.join(repoRoot, "apps", app, "src"))) {
      assert.doesNotMatch(fs.readFileSync(file, "utf8"), /core-domain\/radar\/sync-lifecycle/, file);
    }
  }
});
