...
test("mutable guardrail registry aliases stay retired", () => {
  ...
  const evaluationSpecifier = "@shiguang-gateway/core-domain/guardrails/evaluation";
  const managementSpecifier = "@shiguang-gateway/core-domain/control/guardrails";
  ...
  assert.deepEqual(evaluationConsumers.sort(), [
    "packages/open-sse/handlers/chat.ts",
    "packages/open-sse/handlers/chatCore.ts",
    "packages/open-sse/handlers/chatCore/postCallGuardrailContext.ts",
  ]);
