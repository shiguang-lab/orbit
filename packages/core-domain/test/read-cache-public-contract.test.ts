...
test("db/read-cache is the single narrow read-cache contract", async () => {
  const entry = manifest.exports["./db/read-cache"];
  assert.equal(typeof entry, "object");
  assert.deepEqual(entry, {
    types: "./src/public/readCache.d.ts",
    import: "./src/db/readCache.ts",
  });
...
test("scenario-specific read-cache aliases stay retired", () => {
