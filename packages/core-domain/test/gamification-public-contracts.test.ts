...
test("gamification DB, rules, and request lifecycle ownership stay separated", () => {
  ...
  const forbiddenControl = /@shiguang-gateway\/core-domain\/control\/gamification(?:-db|-notifications)?(?=["'])/;
  ...
  const notificationConsumers = sourceFiles(path.join(repoRoot, "apps"))
    .filter((file) => fs.readFileSync(file, "utf8").includes("core-domain/control/gamification-notifications"))
