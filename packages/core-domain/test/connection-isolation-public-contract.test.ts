...
test("connection isolation uses one narrow shared runtime and type contract", async () => {
  ...
  assert.deepEqual(Object.keys(runtime), ["isConnectionUnavailableToAuxiliaryActivity"]);
  assert.equal(
    fs.existsSync(path.join(packageRoot, "src/public/connectionIsolation.d.ts")),
    false,
  );
});

test("scenario-specific connection isolation aliases stay retired", () => {
