...
test("scenario-specific database core aliases stay retired", () => {
  ...
  for (const root of ["apps", "packages/open-sse"]) {
    for (const file of sourceFiles(path.join(repoRoot, root))) {
      ...
      assert.doesNotMatch(
        source,
        /core-domain\/(?:runtime\/db-core|usage\/reporting-support\/database)/,
        file,
      );
      if (source.includes("core-domain/db/ping")) {
        assert.doesNotMatch(source, /import\s*{[^}]*getDbInstance[^}]*}\s*from\s*["']@shiguang-gateway\/core-domain\/db\/ping["']/s, file);
      }
    }
  }
});
