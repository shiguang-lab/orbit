...
description: "Detect installed CLI coding tools (Claude Code, Codex, Cursor, Copilot, Cline and more), search GitHub for matching agent skills, and install them to the detected tools via ShiguangGateway's built-in APIs."
---
...
Discover and install agent skills for your coding CLI tools — all through ShiguangGateway's built-in APIs.
...
No separate Skill Collector app needed — ShiguangGateway's own CLI detection + GitHub search handles everything.

---

## Step 1 — Detect installed coding tools

Query ShiguangGateway's CLI tool detection to find which coding agents are installed:

```bash
curl -H "Authorization: Bearer $SHIGUANG_GATEWAY_API_KEY" http://localhost:20128/api/skills/collect/detect
```

This returns:

- Every CLI tool in ShiguangGateway's catalog (`CLI_TOOL_IDS`: claude, codex, cursor, copilot, opencode, cline, kilocode, hermes, hermes-agent, openclaw, droid, continue, qwen, windsurf, devin, antigravity, etc.)
...
```bash
curl -H "Authorization: Bearer $SHIGUANG_GATEWAY_API_KEY" \
...
```bash
AUTH_HEADER="Authorization: Bearer $SHIGUANG_GATEWAY_API_KEY"
...
- ShiguangGateway must be running locally on port 20128 (default) — see `docs/frameworks/SKILLS.md` for custom-port setups.
...
- This replaces the standalone Skill Collector Python app — all logic is now inside ShiguangGateway.
<!-- skill:custom-end -->
