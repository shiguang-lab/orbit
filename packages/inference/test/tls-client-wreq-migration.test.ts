...
import { createTlsClientModule } from "../src/services/tlsClientBase.ts";
import {
  TlsClient,
  createWreqTransportClient,
  type WreqTransportResponseLike,
} from "../src/utils/tlsClient.ts";
...
type TlsUtilsWithLifecycleTestSeam = typeof import("../src/utils/tlsClient.ts") & {
...
test("a stale request lease cannot invalidate a healthy replacement transport", async () => {
  ...
  assert.equal(
    await new Response(replacementResponse.body as ReadableStream<Uint8Array> | null).text(),
    "healthy"
  );
...
test("one lifecycle cleanup closes persistent sessions and ephemeral transports", async () => {
  ...
  const tlsUtils =
    (await import("../src/utils/tlsClient.ts")) as TlsUtilsWithLifecycleTestSeam;
