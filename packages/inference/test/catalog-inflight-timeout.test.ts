/**
 * #12627 — a hung coalesced catalog rebuild must not pin later GET /v1/models
 * clients: the cold path waits at most CATALOG_BUILD_TIMEOUT_MS, then serves the
 * last-good 200 when one exists.
 */
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "orbit-12627-"));
process.env.DATA_DIR = TEST_DATA_DIR;

const catalogCache = await import("../src/catalog/catalogCache.ts");
import type { CatalogPayload } from "../src/catalog/catalogCache.ts";
...
test("#12627 timeout serves last-good 200 when a prior build succeeded", async () => {
  ...
  assert.equal(second.headers.get("X-Model-Catalog-Source"), "last-good");
});
