...
} from "../src/utils/chatgptWebBrowserSession.ts";
...
describe("ChatGPT Web clean-room browser-owned session", () => {
  ...
  test("aborts the browser-owned request when the turn timeout expires", async () => {
    // The production timeout is intentionally unref'ed so an idle process can exit.
    // Keep this isolated test process alive long enough to observe that timeout.
