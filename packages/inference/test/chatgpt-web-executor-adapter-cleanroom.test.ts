...
import {
  ...
  type ChatGptWebSessionFactoryInput,
} from "../src/utils/chatgptWebExecutorAdapter.ts";
import { resolveChatGptWebAttachments } from "../src/utils/chatgptWebAttachments.ts";
import type { ChatGptWebBrowserSession } from "../src/utils/chatgptWebBrowserSession.ts";
...
describe("ChatGPT Web clean-room executor request adapter", () => {
  ...
  test("maps reasoning effort monotonically and preserves multi-message roles", () => {
    ...
    for (const [effort, effortIndex] of expected) {
      ...
      assert.equal(selection.kind, "picker");
      assert.equal(
        selection.kind === "picker" ? selection.effortIndex : undefined,
        effortIndex
      );
    }
    ...
    assert.equal(prepared.selection.kind, "picker");
    assert.equal(prepared.selection.kind === "picker" ? prepared.selection.modelLabel : undefined, "GPT-5.5");
...
describe("ChatGPT Web clean-room executor response adapter", () => {
  ...
  test("executes through an injected browser session factory", async () => {
    ...
    let observed: ChatGptWebSessionFactoryInput | null = null;
