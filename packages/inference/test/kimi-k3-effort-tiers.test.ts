...
import { buildSyncedCapabilities, mergeSyncedCapabilities } from "../src/catalog/syncedCapabilities.ts";
import {
  shouldExposeSyncedEffortVariants,
  appendSyncedEffortVariants,
} from "../src/utils/syncedEffortVariants.ts";
...
test("Kimi K3 merge path also publishes effort_tiers (#12299)", () => {
  ...
  assert.deepEqual(merged.effort_tiers, KIMI_K3_TIERS, "merge path must publish kimi K3 effort_tiers");
  assert.equal(merged.tool_calling, true, "existing tool_calling must be preserved");
});

test("Non-K3 kimi models keep the exclusion; codex/glm unaffected", () => {
  assert.equal(
    buildSyncedCapabilities(
      { id: "kimi-latest", supportsThinking: true, supportedThinkingEfforts: KIMI_K3_TIERS },
      "kimi-coding-apikey"
    ),
    undefined,
    "non-K3 kimi model must stay excluded"
  );
  assert.equal(
    buildSyncedCapabilities(
      { id: "k3", supportsThinking: true, supportedThinkingEfforts: KIMI_K3_TIERS },
    ...
    undefined,
    "codex-owned entries must stay excluded even with a K3-like id"
  );
});
