...
export class CloudflareAIExecutor extends BaseExecutor {
  ...
  transformRequest(
    ...
    // #6390 refused any non-text part and #12002 went further: passing the array
    // through is better than refusing, since an image is only meaningful to a
    // multimodal model, and those accept the array shape. When the target model is
    // text-only, Cloudflare answers with its own 400, which is more useful than a
    // gateway refusal that pre-empts every model alike.
