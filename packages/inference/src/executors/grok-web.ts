...
import { buildGrokCookieHeader } from "@orbit/core/edge/web-cookie-auth";
import {
  tlsFetchGrok,
...
// ─── Model mappings ─────────────────────────────────────────────────────────
// Grok Web exposes UI modes, not stable public model IDs. Keep Orbit model
...
function grokStreamErrorChunk(): string {
  return sseChunk(buildErrorBody(502, GROK_STREAM_FAILURE_MESSAGE, undefined, {
    type: "upstream_error",
            ...
            if (chunk.fullMessage) {
              ...
              if (!buffered) {
                ...
                controller.enqueue(encoder.encode(sseChunk({
                  ...
                  model,
                  system_fingerprint: fp || null,
                  choices: [{ index: 0, delta: { content: chunk.fullMessage }, finish_reason: null, logprobs: null }],
                })));
                await handOffFirstOutput();
              }
            }
