...
import { LOCAL_PROVIDERS } from "@orbit/providers/catalog";
import { isForbiddenCustomHeaderName } from "@orbit/contracts/upstream-headers";
import { getClaudeCodeCompatibleRequestDefaults } from "@orbit/core/providers/request-defaults";
import { applyClineAuthHeaders } from "@orbit/core/edge/cline-auth";
import {
...
import { resolveAlibabaProviderBaseUrl } from "@orbit/providers/alibaba-regions";
...
export class DefaultExecutor extends BaseExecutor {
  ...
  transformRequest(model, body, stream, credentials) {
    ...
    // #11861: Nous Research inference gateway (portal.nousresearch.com) requires
    // a top-level `tags` array containing at least a `user=` item on raw API-key
    // requests. Without `tags`, upstream returns 400 "missing tags". Without
    // `user=...`, upstream returns 400 "missing user tag".
    if (
