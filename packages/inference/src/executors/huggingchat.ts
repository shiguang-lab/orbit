  ...
  async execute(input: ExecuteInput): Promise<{
    ...
    if (stream) {
      ...
      try {
        ...
        log?.error?.("HUGGINGCHAT", `Stream failed before content: ${sanitizeErrorMessage(message)}`);
        return {
          response: new Response(
            JSON.stringify(
      ...
      const sseStream = new ReadableStream<Uint8Array>({
        async pull(controller) {
          ...
          try {
            ...
            log?.error?.(
              "HUGGINGCHAT",
              `Stream error: ${sanitizeErrorMessage(err instanceof Error ? err.message : String(err))}`
            );
            controller.error(
    ...
    try {
      ...
      if (!(err instanceof HuggingChatStreamError)) throw err;
      const message = err.message;
      log?.error?.("HUGGINGCHAT", `Generation error: ${sanitizeErrorMessage(message)}`);
      return {
        response: new Response(
          JSON.stringify(
            buildErrorBody(502, message, undefined, {
