...
import { findDeepSeekPowNonce, MAX_DEEPSEEK_POW_DIFFICULTY } from "./pow-hash.js";
...
function solveInWorker(
  ...
  return new Promise<number>((resolve, reject) => {
    const worker = new Worker(new URL("./pow-worker.mjs", import.meta.url), {
      workerData: validated,
      // This plain-JavaScript worker does not need the parent TypeScript loader.
      execArgv: [],
    });
    let settled = false;
