 ...
 * cookie is accepted as input for existing ShiguangGateway connections, but only the
...
import { extractKimiAccessToken } from "@orbit/core/edge/web-cookie-auth";
...
export class KimiWebExecutor extends BaseExecutor {
  ...
  private buildRequestBody(
    ...
    const options: Record<string, unknown> = {
      ...
      thinking: true,
      // ShiguangGateway exposes text chat only. Kimi's built-in audio/ask-user tools
