...
import { getCallLogPipelineCaptureStreamChunks } from "@orbit/config/logEnv";
...
export async function handleEmbedding({
  ...
  try {
    ...
    if (apiKeyId && connectionId && provider) {
      try {
        const { scheduleRecordConsumption } = await import("@orbit/core/quota/consumption-recorder");
