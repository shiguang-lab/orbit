...
import * as chatAdmission from "@orbit/inference/handlers/chatAdmission";
import { buildClientRawRequest, resolveDispatchClientRawRequest } from "@orbit/inference/handlers/chat/clientRawRequest";
export { buildClientRawRequest, resolveDispatchClientRawRequest };
import { normalizeReasoningRequest } from "@orbit/core/runtime/reasoning-effort";
import { isDetailedLoggingEnabled } from "@orbit/core/db/detailed-logs";
import { resolvePreviousResponseState } from "@orbit/core/runtime/responses-continuation";
...
import { applyNoThinkingAlias } from "../utils/noThinkingAlias.ts";
import { resolveCcDiscoveryAliasStrip } from "@orbit/core/runtime/cc-discovery-alias";
import {
  handleComboChat,
  resolveComboTargets,
  shouldSkipConnDisable,
} from "../services/combo.ts";
import type { ComboLike, SingleModelTarget } from "../services/combo/types.ts";
import { mergeAbortSignals } from "../executors/base.ts";
import { resolveRequestAutoControls } from "../services/autoCombo/requestControls.ts";
import { isVerifiedNativeCodexRequest } from "../config/codexIdentity.ts";
import { resolveCompressionSettings } from "../handlers/chatCore/compressionSettings.ts";
import type { CompressionExclusions } from "../services/compression/exclusions.ts";
import { resolveComboConfig } from "../services/comboConfig.ts";
import { injectHandoffIntoBody } from "../services/contextHandoff.ts";
import { HTTP_STATUS } from "@orbit/contracts/http-status";
import { ANTIGRAVITY_PRE_RESPONSE_TIMEOUT_CODE } from "../config/constants.ts";
import {
  getTargetFormat,
...
import { createHookContext, runHooks } from "@orbit/core/middleware/pre-request-hook-execution";
import { rejectPeerRequest } from "@orbit/core/routing/peer-routing";
...
} from "@orbit/core/session-affinity/store";
import { dispatchChatWithAffinityEviction } from "@orbit/inference/handlers/chatDispatch";
import { getCachedSettings, getCombosCacheVersion } from "@orbit/core/db/read-cache";
...
} from "@orbit/core/providers/request-defaults";
import {
...
import { wrapResponseWithOAuthSessionRelease } from "../services/oauthSessionOccupancy.ts";
import {
...
} from "@orbit/inference/handlers/reasoningRouting";
import { createVirtualAutoCombo, resolveAutoRoutingState } from "@orbit/inference/handlers/autoRouting";
import { getComboFailureLogError } from "@orbit/inference/handlers/comboFailureLogging";

// Pipeline integration — wired modules
import { classify429FromError, type FailureKind } from "@orbit/core/resilience/rate-limit-classification";
import { isSubscriptionQuotaText } from "../services/quotaTextCooldowns.ts";
import { resolveUseUpstream429BreakerHints } from "@orbit/core/edge/provider-hints";
...
import { getCircuitBreaker, isLocalStreamLifecycleError } from "@orbit/core/resilience/circuit-breaker";
import { markAccountExhaustedFrom429 } from "@orbit/core/quota/cache";
import { resolveForcedConnectionForCredentialPool } from "@orbit/inference/services/sessionAffinityPin";
import { RequestTelemetry, recordTelemetry } from "@orbit/core/metrics/request-telemetry";
...
import { hasProviderQuotaBypassScope } from "@orbit/core/runtime/api-key-policy-scopes";
...
} from "../services/taskAwareRouter.ts";
import {
  hasNativeWebSearchTool,
  resolveWebSearchRouteOverride,
} from "../services/webSearchRouting.ts";
import {
...
import { startQuotaMonitor } from "../services/quotaMonitor.ts";
import {
  isFallbackDecision,
  shouldUseFallback,
...
} from "../services/codexQuotaFetcher.ts";
import { registerBailianCodingPlanQuotaFetcher } from "../services/bailianQuotaFetcher.ts";
import { registerQwenTokenPlanQuotaFetcher } from "../services/qwenTokenPlanQuotaFetcher.ts";
import { registerCrofUsageFetcher } from "../services/crofUsageFetcher.ts";
import { registerDeepseekQuotaFetcher } from "../services/deepseekQuotaFetcher.ts";
import { registerOpenrouterQuotaFetcher } from "../services/openrouterQuotaFetcher.ts";
import { registerOpencodeQuotaFetcher } from "../services/opencodeQuotaFetcher.ts";
import { registerGrokWebQuotaFetcher } from "../services/grokQuotaFetcher.ts";
import { registerGenericQuotaFetchers } from "../services/genericQuotaFetcher.ts";
import "@orbit/inference/services/quotaTrackersBatch";
import {
  disableCooldownAwareRetry,
  getCooldownAwareRetryDecision,
  resolveCooldownAwareRetrySettings,
  waitForCooldownAwareRetry,
} from "@orbit/inference/services/cooldownAwareRetry";
import {
  shouldRetrySameAccountTransport,
  sameAccountTransportRetryDelayMs,
} from "../services/sameAccountTransportRetry";
import { constrainConnectionsToQuota, resolveQuotaKeyScope } from "@orbit/core/runtime/quota-key";
import { checkConnectionCapacity } from "@orbit/inference/utils/backpressure";
import {
...
export { shouldTripProviderBreakerForResult } from "@orbit/inference/handlers/chatPredicates";
...
async function handleChatImplementation(
  ...
  if (!combo) {
    ...
    if (providerPrefix) {
      try {
        const { getComboByName } = await import("@orbit/core/db/combos");
