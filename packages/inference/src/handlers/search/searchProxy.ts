...
/**
 * #12139 — upstream statuses that mean "this account cannot serve search right now".
 * 432 (plan limit) is included alongside the usual 402/429/5xx family so a
 * plan-metered upstream (Tavily, Context7, …) cools the connection down instead of
 * being retried on every search.
 */
const SEARCH_COOLDOWN_STATUSES = new Set<number>([
...
/** True when a failed search response should put the connection into cooldown. */
...
export async function resolveSearchProxy(
  ...
  try {
    const { resolveProxyForConnection } = await import("@orbit/core/db/settings");
...
export async function emitSearchProxyEvent(
  ...
  try {
    const { logProxyEvent } = await import("@orbit/core/logging/proxy-logs");
    let targetOrigin = "";
    let targetPath = "";
    try {
    ...
    if (!response.ok) {
      ...
      if (connectionId && shouldCoolDownSearchConnection(response.status, errorText)) {
        try {
          ...
          /* non-critical — background cooldown mark must not break the search response */
        }
      }
      logCall({
