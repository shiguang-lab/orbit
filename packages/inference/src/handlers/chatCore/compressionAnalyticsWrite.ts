...
type CalculateCost = typeof import("@orbit/core/pricing/cost-calculator").calculateCost;
...
export function writeCompressionSkip(opts: WriteOpts, skipReason: string): Promise<void> {
  return (async () => {
    try {
      const { insertCompressionAnalyticsRow } = await import("@orbit/core/db/compression-analytics");
      const { stats } = opts;
  ...
  return (async () => {
    try {
      const { insertCompressionAnalyticsRow, insertCompressionEngineBreakdown } =
        await import("@orbit/core/db/compression-analytics");
      const { stats } = opts;
      ...
      try {
        const calculateCost =
          dependencies.calculateCost ?? (await import("@orbit/core/pricing/cost-calculator")).calculateCost;
