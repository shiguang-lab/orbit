 ...
 *
 * validateResponseQuality's streaming peek only sees the START of a stream,
 * so a duplicate that arrives after real content has already been relayed
 * cannot fail the attempt over — this is the first point the fully assembled
 * body is available. Too late to retry; a durable audit row still beats a
 * clean HTTP 200 with no trace (#12446).
 *
