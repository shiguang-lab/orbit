...
} from "@orbit/core/edge/capability-filter";
import {
...
import { normalizeThinkingForModel } from "@orbit/contracts/model-specs";
import {
  buildErrorBody,
  createErrorResult,
  parseUpstreamError,
...
import { updateProviderConnection, getProviderConnectionById } from "@orbit/core/db/provider-connections";
...
import { appendRequestLog } from "@orbit/core/usage/request-logs";
import { finalizePendingScope, updatePendingScope } from "@orbit/core/usage/pending-request-scope";
...
import type { EnforceDecision } from "@orbit/core/quota/enforcement-decision";
...
import { getModelNormalizeToolCallId, getModelPreserveOpenAIDeveloperRole } from "@orbit/core/db/models";
import {
  getProviderCredentials,
  getProviderCredentialsWithQuotaPreflight,
  extractSessionAffinityKey,
} from "../services/auth.ts";
import { assertExclusiveConnectionLeaseFence } from "@orbit/core/db/exclusive-connection-leases";
import { deleteSessionAccountAffinity } from "@orbit/core/session-affinity/store";
import { getCacheControlSettings } from "@orbit/core/edge/cache-control";
import { evaluateGuardrailsPostCall } from "@orbit/core/guardrails/evaluation";
import {
  shouldPreserveCacheControl,
  resolveConnectionCacheOverride,
} from "../utils/cacheControlPolicy.ts";
import { getCachedSettings } from "@orbit/core/db/read-cache";
import { applyCodexGlobalFastServiceTier } from "@orbit/core/edge/codex-fast-tier";
...
import {
  ...
  isCacheableForRead,
  isCacheableForWrite,
} from "@orbit/core/cache/semantic";
import { saveIdempotency } from "@orbit/core/edge/idempotency";
import {
...
import { resolveInterceptSearch, resolveInterceptFetch } from "@orbit/core/db/provider-interception-rules";
import {
...
import { isClaudeExtraUsageBlockEnabled } from "@orbit/core/usage/claude-extra-usage";
import {
...
export async function handleChatCore({
  ...
  // #12189: detect Claude Code CLI so we can auto-enable model echo — this
  // prevents session restore failures when the resolved upstream model (e.g.
  ...
  if (body && Array.isArray(allMessages) && allMessages.length > 0) {
    ...
    try {
        ...
        try {
          ...
          if (routingComboIds.length > 0) {
            const { getCompressionComboForRoutingCombo } =
              await import("@orbit/core/control/compression-combos");
      ...
      try {
        const { listCompressionCombos } = await import("@orbit/core/control/compression-combos");
  ...
  try {
    ...
    // ── Plugin onError hook ──
    try {
      const { runOnError } = await import("@orbit/core/edge/plugins-runtime");
      await runOnError(
  ...
  if (apiKeyInfo?.id && credentials?.connectionId) {
    try {
      ...
      const { enforceQuotaShare } = await import("@orbit/core/quota/services");
              ...
              if (provider === "antigravity" && res.response.status === 422) {
                ...
                if (byopBody.includes("gcp_project_required")) {
                  ...
                  if (byopFailedId) {
                    ...
                    try {
                      const { setConnectionRateLimitUntil } = await import("@orbit/core/db/provider-connections");
