 ...
 *
 * Goal: ensure the final request body ShiguangGateway sends to Anthropic's
 ...
 *
 * Related: ShiguangGateway issue #2260.
 */
...
export interface InjectBillingHeaderOp {
   ...
   *   - shiguangGateway-daystamp: sha256(YYYY-MM-DD + version).slice(0,3)
   */
  versionFormat: "ex-machina" | "shiguangGateway-daystamp";
  /**
 ...
 * proven against the live ShiguangGateway deployment (call log
...
/**
 * Compute the `cc_version` suffix per the ShiguangGateway native-OAuth algorithm:
...
export function buildBillingHeaderValue(
  ...
  const suffix =
    ...
    (options.versionFormat === "shiguangGateway-daystamp"
