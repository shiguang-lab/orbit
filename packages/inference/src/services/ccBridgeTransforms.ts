 ...
 *
 * Goal: ensure the final request body Orbit sends to Anthropic's
...
export interface InjectBillingHeaderOp {
   ...
   *   - orbit-daystamp: sha256(YYYY-MM-DD + version).slice(0,3)
   */
  versionFormat: "ex-machina" | "orbit-daystamp";
  /**
 ...
 * proven against the live Orbit deployment (call log
...
/**
 * Compute the `cc_version` suffix per the Orbit native-OAuth algorithm:
...
export function buildBillingHeaderValue(
  ...
  const suffix =
    ...
    (options.versionFormat === "orbit-daystamp"
