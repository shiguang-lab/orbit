...
export async function initializeRateLimits() {
  ...
  try {
    const [{ getCachedProviderConnections }, { getSettings }] = await Promise.all([
      import("@orbit/core/db/read-cache"),
      import("@orbit/core/db/settings"),
    ]);
...
export async function applyRequestQueueSettings(nextSettings: RequestQueueSettings) {
  ...
  preservedReplacementSettings.clear();
  const { getCachedProviderConnections } = await import("@orbit/core/db/read-cache");
...
export async function withRateLimit(provider, connectionId, model, fn, signal = null) {
  ...
  // the dedicated execution backstop (`requestQueue.executionMaxWaitMs`), never
  // by the queue-wait budget: non-incremental gateways (Console Go / Command
  // Code) legitimately run for minutes before first bytes, and an expiration at
  // the queue budget killed them mid-flight (false 504s).
...
async function persistLearnedLimitsNow() {
  try {
    const { updateSettings } = await import("@orbit/core/db/settings");
