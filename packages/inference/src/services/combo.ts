...
import {
  ...
  getLastSessionModel,
  getHandoff,
} from "@orbit/core/db/context-handoffs";
...
import { notifyWebhookEvent } from "@orbit/core/shared/webhook-dispatcher";
...
import { getCachedProviderConnections } from "@orbit/core/db/read-cache";
...
export function clearStaleLKGP(
  ...
  void (async () => {
    try {
      const { clearLKGP } = await import("@orbit/core/db/lkgp");
...
export async function buildAutoCandidates(
  ...
  const { getPricingForModel } = await import("@orbit/core/pricing/db");
  const quotaPromises = new Map<string, Promise<unknown>>();
  let historicalLatencyStats: Record<string, HistoricalLatencyStatsEntry> = {};
  try {
    const { getModelLatencyStats } = await import("@orbit/core/usage/model-latency-stats");
...
async function handleComboChatInner({
  ...
  // #5923 (Finding #4) — reset-window config for the shared per-target quota-exhaustion cutoff
  // below. Also consumed by the native Codex pin gate, so it is resolved before that block.
  ...
  if (activeNativeTurnPin) {
    ...
    if (pinnedTargets.length === 0) {
      ...
      // #12240: the pinned provider+model is gone from the pool. The turn already emitted output,
      // so it cannot move to another model — terminate it cleanly instead of retrying.
      log.warn(
        "COMBO",
    ...
    // #12240: every candidate may exist but be unusable for a MODEL-SCOPED reason
    // (lockout / per-model quota / exhausted cutoff). Retrying cannot help the pinned
    // model recover, so return a non-retryable 400 and keep the pin for the next turn.
  ...
  const dispatchWithCooldownRetry = async (): Promise<Response> => {
    ...
    // #11804: the loop-safety timer is armed per setTry iteration but must be cleared on
    // EVERY exit path, not just the happy one. Hoisted to this function scope so the
    // `finally` below always reaches it — otherwise every error exit (all_targets_skipped /
    // all_accounts_inactive / aggregated status / final fallback / global timeout) returned
    // to the client leaving a 10-minute timer pending whose closure retains orderedTargets
    // and the exhausted provider/connection sets. Field evidence on the issue: the client got
    // a 502 immediately and "Combo loop safety timeout … force-terminating" logged 600s later.
    ...
    for (let setTry = 0; setTry <= maxSetRetries; setTry++) {
      ...
      const loopSafetyPromise = new Promise<Response>((resolve) => {
        ...
        // #11804: publish the per-iteration handle to the function-scope binding so the
        // `finally` at the end of dispatchWithCooldownRetry can always release it.
        ...
        for (let retry = 0; retry <= maxRetries; retry++) {
          ...
          if (result.ok) {
            ...
            if (provider) {
              ...
              void (async () => {
                try {
                  const { setLKGP } = await import("@orbit/core/db/lkgp");
                  await Promise.all([
      ...
      if (anySuccess) {
        // G1: the safety timer is released by the dispatchWithCooldownRetry `finally`
        // (#11804), which covers the happy path too — no per-exit clear needed here.
    ...
    return errorResponseWithComboDiagnostics(
      ...
      // construction here means a future `return` added to this function cannot silently
