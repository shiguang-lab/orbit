import { handleEmbedding } from "../../handlers/embeddings.js";
import {
  parseEmbeddingModel,
...
import { errorResponse, unavailableResponse } from "@orbit/utils/errors/error-response";
import { HTTP_STATUS } from "@orbit/contracts/http-status";
import * as coreLog from "@orbit/core/sse/logger";
...
import { getDatabaseSettings } from "@orbit/core/db/database-settings";
...
import { findEmbeddingComboDimensionConflict } from "./familyGuard.js";
import {
...
const log = {
  debug: (...args: unknown[]) =>
    coreLog.debug(String(args[0] ?? "EMBED"), String(args[1] ?? ""), args[2]),
  info: (...args: unknown[]) =>
    coreLog.info(String(args[0] ?? "EMBED"), String(args[1] ?? ""), args[2]),
  warn: (...args: unknown[]) =>
    coreLog.warn(String(args[0] ?? "EMBED"), String(args[1] ?? ""), args[2]),
  error: (...args: unknown[]) =>
    coreLog.error(String(args[0] ?? "EMBED"), String(args[1] ?? ""), args[2]),
};
...
export async function createEmbeddingResponse(
  ...
  if (!credentials && providerConfig.authType !== "none") {
    ...
    if ("allExpired" in credentials && credentials.allExpired) {
      const expiredStatus = credentials.expiredStatus;
      const quotaExhausted = expiredStatus === "credits_exhausted";
      return errorResponse(
        quotaExhausted ? HTTP_STATUS.PAYMENT_REQUIRED : HTTP_STATUS.UNAUTHORIZED,
        `[${provider}] All ${credentials.expiredCount || 1} connection(s) ${quotaExhausted ? "credits exhausted" : "authentication expired"} — please reconnect in the dashboard`
      );
    }
