 ...
 * synthetic `orbit_web_fetch` function tool; the actual `/v1/web/fetch` call
...
export { ORBIT_WEB_FETCH_FALLBACK_TOOL_NAME } from "@orbit/contracts/gateway-tool-names";
import { ORBIT_WEB_FETCH_FALLBACK_TOOL_NAME } from "@orbit/contracts/gateway-tool-names";
...
function buildFallbackTool(targetFormat?: string | null): JsonRecord {
  const name = ORBIT_WEB_FETCH_FALLBACK_TOOL_NAME;
  const description = [
...
export function prepareWebFetchFallbackBody<T extends WebFetchFallbackBody>(
  ...
  if (!toolNames.has(ORBIT_WEB_FETCH_FALLBACK_TOOL_NAME)) {
  ...
  if (isBuiltInWebFetchToolChoice(body.tool_choice)) {
    nextBody.tool_choice = (
      isResponsesTarget
        ? { type: "function", name: ORBIT_WEB_FETCH_FALLBACK_TOOL_NAME }
        : { type: "function", function: { name: ORBIT_WEB_FETCH_FALLBACK_TOOL_NAME } }
    ) as T["tool_choice"];
  }
  ...
  return {
    ...
    fallback: {
      enabled: true,
      toolName: ORBIT_WEB_FETCH_FALLBACK_TOOL_NAME,
