...
/** Where a combo request's compression window came from — surfaced in the CONTEXT log. */
 ...
 *
 * Precedence:
 * 1. `comboContextLength` — an operator-set `context_length` on the combo record
 *    (combo editor → Context length). It is an explicit declaration of the window
 *    this combo runs at, so it outranks every inferred value, including a target's
 *    own registry/catalog window. The field was persisted and advertised via
 *    `/v1/models`, but nothing consulted it at request time — so a combo whose
 *    members carry no per-model window fell through to the provider's generic
 *    default and rejected large requests the operator had explicitly sized for.
 * 2. The executing target's own specific window. chatCore always executes with the
 *    CONCRETE target's provider/model (handleSingleModel resolves the target before
 *    delegating), so that window is authoritative. An unconditional
 ...
 *
 * A non-finite or non-positive `comboContextLength` is ignored, so steps 2-3 keep
 * their pre-existing behavior exactly.
 */
