...
import { nodeTypeFromId } from "@orbit/core/runtime/provider-node-select";
import { extractApiKey, isValidGatewayApiKey } from "@orbit/auth";
import { describeUpstreamFailure } from "@orbit/core/shared/upstream-error";
...
import { getRecentEgressIpForConnection, EGRESS_IP_LOOKUP_WINDOW_MS } from "@orbit/core/db/proxy-logs";
import { validateApiKey } from "@orbit/core/db/api-keys";
import {
  getActiveExclusiveConnectionLease,
  hashLeaseOwnerId,
  type ExclusiveConnectionLease,
} from "@orbit/core/db/exclusive-connection-leases";
import { getSettings } from "@orbit/core/db/settings";
import {
  describePeakHourWindow,
  evaluatePeakHourProtection,
} from "@orbit/core/runtime/peak-hour-protection";
import { buildJinaEnvCredentials } from "@orbit/core/runtime/jina-credentials";
import { buildGeminiEnvCredentials } from "@orbit/core/runtime/gemini-credentials";
...
import { isMicrosoftDesignerWebRetiredProviderId } from "@orbit/contracts/designer-web-retirement";
import { isRuntimeRetiredProviderId } from "@orbit/contracts/provider-retirement";
import {
  createLazyConnectionView,
  toProviderConnection,
  type ProviderConnectionView,
} from "@orbit/core/db/provider-connection-view";
import {
...
} from "../services/antigravityFamilyCooldown.ts";
import { markQuotaPreflightAccountUnavailable } from "../services/quotaPreflightUnavailable.ts";
...
import { compareLeastUsedConnections } from "./leastUsedOrdering.ts";
import { isLocalProvider } from "../config/providerRegistry.ts";
import { COOLDOWN_MS, RateLimitReason } from "../config/constants.ts";
...
import { syncHealthFromDB, type KeyHealth } from "../services/apiKeyRotator.ts";
import {
  classifyProviderError,
  PROVIDER_ERROR_TYPES,
} from "@orbit/core/domain/provider-error-classifier";
import {
...
} from "../services/alibabaFreeTier.ts";
...
} from "../services/codexAccount/index.ts";
import {
  getProviderById,
  getProviderAlias,
  resolveProviderId,
...
} from "@orbit/inference/services/sessionAffinityPin";
import {
...
} from "@orbit/core/selection/shuffle-deck";
...
} from "@orbit/inference/services/exclusiveConnectionLeasePolicy";
import {
  getOAuthSessionAvailability,
  reserveOAuthSession,
} from "../services/oauthSessionOccupancy.ts";
...
export { readHeaderValue, type AuthRequestHeaders } from "@orbit/inference/services/headerReader";
export { extractApiKey } from "@orbit/auth";
export { extractSessionAffinityKey } from "@orbit/inference/services/sessionAffinityPin";
...
export async function getProviderCredentials(
  ...
  try {
    ...
    if (
      ...
      // An explicit combo/header connection that is absent from the current
      // active/allowed pool must stay scoped to that connection. Emptying the
      // pool routes through the existing ineligible-credential result instead
      // of silently selecting a sibling account from the same provider.
      connections = [];
    } else if (!strictManagedCredential) {
    ...
    if (connection) {
      ...
      const sorted = [...orderedConnections].sort(compareLeastUsedConnections);
      connection = sorted[0];
  ...
  try {
    ...
    if (provider) {
      const { invalidateFreeAccessState } =
        await import("../services/autoCombo/freeAccessQuota.ts");
...
export async function isValidApiKey(apiKey: string) {
  return isValidGatewayApiKey(apiKey, validateApiKey);
}
