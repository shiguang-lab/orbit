/**
 * Round-robin combo dispatcher. Lift-as-is from combo.ts so the rotation loop
 * (semaphore queue + rate-limit aware fallback) is reviewable on its own.
 * @internal
 */
...
import { makeConnectionConcurrencyResolver, lookupPositiveCap } from "./concurrencyCaps.ts";
