...
import { getReasoningTokens } from "@orbit/contracts/usage/tokenAccounting";
 ...
 * SSE wire level — an upstream bug, not a reconstruction artifact).
 * Used two ways: to fail a non-streaming response over to a sibling combo
 * target (see validateResponseQuality below), and, post-stream, to flag an
 * already-relayed streaming response as an on-spec violation despite its
 * clean HTTP 200 (see attemptLogging.ts) — a streaming response can't be
 * retried once real content has started reaching the client (the
 * quality-gate peek only ever validates the START of a stream, by design,
 * to avoid buffering the whole response and defeating streaming's latency
 * purpose), so flagging it after the fact is what's actually achievable
 * for that path.
 */
