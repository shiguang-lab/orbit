...
class ScoreStrategyImpl implements RouterStrategy {
  ...
  select(pool: ProviderCandidate[], context: RoutingContext): RoutingDecision {
    ...
    const exploring = Math.random() < explorationRate && ranked.length > 1;
    const selected = exploring ? ranked[Math.floor(Math.random() * ranked.length)]! : ranked[0]!;
    return {
      provider: selected.provider,
      model: selected.model,
      strategy: this.name,
      reason: `ScoreStrategy: score=${selected.score.toFixed(3)}${exploring ? " (exploration)" : ""}`,
