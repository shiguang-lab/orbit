...
import { isClaudeExtraUsageAllowed } from "@orbit/core/usage/claude-extra-usage";
...
function windowsForScope(
  ...
  if (!scope?.requestedModel || !isAntigravityQuotaProvider(scope.provider)) return windows;
...
export function evaluateQuotaCutoff(
  ...
  if (windows && Object.keys(windows).length > 0) {
    ...
    if (isAntigravityQuotaProvider(scope?.provider) && scope?.requestedModel) {
      return { proceed: true, quotaPercent: quota.percentUsed };
    }
