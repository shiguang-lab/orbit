...
import { SafeOutboundFetchError } from "@orbit/core/network/safe-outbound-fetch";
import { normalizeSessionCookieHeader } from "@orbit/core/edge/web-cookie-auth";
import { normalizeGeminiCookieInput } from "../../utils/geminiCookies.ts";
import { buildJulesApiUrl } from "@orbit/contracts/cloud-agent";
import {
  META_AI_ASBD_ID,
...
export async function validateClaudeWebProvider({ apiKey, providerSpecificData = {} }: any) {
  try {
    ...
    const { tlsFetchClaude, TlsClientUnavailableError } =
      await import("../../services/claudeTlsClient.ts");
...
export async function validateCopilotWebProvider({ apiKey, providerSpecificData = {} }: any) {
  try {
    ...
    const { extractAccessToken } = await import("../../executors/copilot-web.ts");
