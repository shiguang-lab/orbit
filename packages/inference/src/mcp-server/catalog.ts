import { getCodexRequestDefaults } from "@orbit/core/providers/request-defaults";
...
export async function getMcpModelsCatalog(
  ...
  const fetchJson = deps.fetchJson ?? ((path: string) => import("./server.ts").then((m) => m.gatewayRouteFetch(path)));
