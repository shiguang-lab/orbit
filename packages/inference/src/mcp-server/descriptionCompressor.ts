...
export function isMcpDescriptionCompressionEnabled(
  ...
  if (isDisabledEnvValue(process.env.SHIGUANG_GATEWAY_MCP_COMPRESS_DESCRIPTIONS)) return false;
  if (isDisabledEnvValue(process.env.SHIGUANG_GATEWAY_MCP_DESCRIPTION_COMPRESSION)) return false;
