 ...
 */
export const CHATGPT_CONNECTOR_NAME = "Orbit Codex v2";
...
export function getConfigDir(): string {
  ...
  const dataDir = process.env.DATA_DIR?.trim() || process.env.ORBIT_DATA_DIR?.trim();
  return resolve(expandUserPath(dataDir || join(homedir(), ".orbit")), "chatgpt-web-codex");
}
