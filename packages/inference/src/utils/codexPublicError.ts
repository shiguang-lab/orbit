...
const CODEX_PUBLIC_ERROR_RULES = new Map<string, CodexPublicErrorRule>([
  ...
  ["compaction_control_unavailable", { type: "invalid_request_error", allowsStatus: exactStatuses(409) }],
  ["compaction_handoff_failed", { type: "invalid_request_error", allowsStatus: exactStatuses(409) }],
  ["compaction_source_unavailable", { type: "invalid_request_error", allowsStatus: exactStatuses(409) }],
  ["connector_not_found", { type: "connector_error", allowsStatus: exactStatuses(424) }],
  ["context_length_exceeded", { type: "invalid_request_error", allowsStatus: exactStatuses(400, 413) }],
  ...
  ["structured_output_validation_failed", { type: "server_error", allowsStatus: exactStatuses(502) }],
  ["subscription_required", { type: "permission_error", allowsStatus: exactStatuses(403) }],
  ["upstream_server_error", { type: "server_error", allowsStatus: (status) => status >= 500 && status <= 599 && status !== 503 }],
  ["upstream_websocket_connect_failed", { type: "provider_error", allowsStatus: exactStatuses(502) }],
...
export function projectCodexPublicError(input: CodexPublicErrorInput): CodexPublicError {
  ...
  const rule = typeof input.code === "string" ? CODEX_PUBLIC_ERROR_RULES.get(input.code) : undefined;
