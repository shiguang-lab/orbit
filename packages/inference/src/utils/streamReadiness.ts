...
export async function ensureStreamReadiness(
  ...
  options: {
    timeoutMs: number;
    /** Hard ceiling for liveness-extended deadlines. When omitted, no hard
     *  ceiling is applied beyond `timeoutMs`. */
    maxTimeoutMs?: number;
  ...
  // #12189: hard ceiling — the deadline may extend on liveness signals (bytes
  // arriving), but never past this absolute maximum. When maxTimeoutMs is
  // omitted the initial timeoutMs itself acts as the ceiling (no extension).
  const maxDeadline =
  ...
  try {
    while (true) {
      ...
      // #12189 liveness extension: bytes arrived → connection is alive, not
      // dead. Reset the deadline so slow-but-alive upstreams (reasoning
      // warm-ups, keepalive-only phases) are not aborted. The hard ceiling
      // (maxDeadline) prevents unbounded waits and preserves the operator's
      // fast-fail intent for truly dead connections.
      const now = Date.now();
