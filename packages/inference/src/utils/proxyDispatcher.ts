import "@orbit/core/runtime/setup-polyfill";
...
export function getProxyDispatcherConnectionLimit(
  ...
  const raw = env.SHIGUANG_GATEWAY_PROXY_DISPATCHER_CONNECTIONS;
  if (raw == null || raw.trim() === "") return DEFAULT_PROXY_DISPATCHER_CONNECTIONS;

  const parsed = Number(raw);
  if (!Number.isFinite(parsed) || parsed < 1) {
    console.warn(
      `[ProxyDispatcher] Invalid SHIGUANG_GATEWAY_PROXY_DISPATCHER_CONNECTIONS="${raw}". Using default ${DEFAULT_PROXY_DISPATCHER_CONNECTIONS}.`
    );
...
export function getDefaultDispatcherConnectionLimit(
  ...
  const raw = env.SHIGUANG_GATEWAY_DIRECT_DISPATCHER_CONNECTIONS;
  if (raw == null || raw.trim() === "") return DEFAULT_PROXY_DISPATCHER_CONNECTIONS;

  const parsed = Number(raw);
  if (!Number.isFinite(parsed) || parsed < 1) {
    console.warn(
      `[ProxyDispatcher] Invalid SHIGUANG_GATEWAY_DIRECT_DISPATCHER_CONNECTIONS="${raw}". Using default ${DEFAULT_PROXY_DISPATCHER_CONNECTIONS}.`
    );
    return DEFAULT_PROXY_DISPATCHER_CONNECTIONS;
  }
...
function buildProxyDispatcher(
  ...
  return new ProxyAgent({
    ...
    // that only speak CONNECT then reject it (501). ShiguangGateway tunnels ALL proxied
