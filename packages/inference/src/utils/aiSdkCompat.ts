 ...
 * `stream`. Without this hint, Orbit previously routed those requests with
...
export function resolveStreamFlag(
  ...
  // Orbit accumulates the provider stream and converts it to JSON for the client
  ...
  // Orbit fallback would force SSE upstream and fail JSON-only providers as
  ...
  // No explicit stream param — preserve Orbit's streaming default unless
