 ...
 *
 * When Orbit translates a Claude-native streamed response to OpenAI Chat
 ...
 *   - Explicit opt-in: `x-orbit-thinking-marker: on` force-keeps it for the
 *     shrinking set of content-scanning clients (#4633).
 *   - Explicit opt-out: `x-orbit-thinking-marker: off` (same as default).
...
export const THINKING_MARKER_HEADER = "x-orbit-thinking-marker";
...
/**
 * Interpret the explicit `x-orbit-thinking-marker` request header.
 ...
 *   2. Explicit `x-orbit-thinking-marker` header → honor on/off (#5312)
...
export function resolveSuppressThinkClose(opts: {
  ...
  // Legacy content-scanning clients opt in with x-orbit-thinking-marker: on.
  return true;
}
