...
function isSchemaNode(record: JsonRecord): boolean {
  ...
  if (record.not !== undefined || record.$ref !== undefined || record.enum !== undefined) return true;
...
function promoteBooleanRequired(record: JsonRecord): void {
  ...
  for (const [name, schema] of Object.entries(properties)) {
    ...
    if (child.required === true && !required.includes(name)) required.push(name);
    if ("required" in child && !Array.isArray(child.required)) delete child.required;
  }
  if (required.length > 0) record.required = required;
  else if (!Array.isArray(record.required)) delete record.required;
}

function normalizeMalformedSchemaObjects(obj: unknown, parentKey?: string): void {
  if (!obj || typeof obj !== "object") return;
  if (Array.isArray(obj)) {
    for (const item of obj) normalizeMalformedSchemaObjects(item, parentKey);
    return;
  }
  ...
  for (const [key, value] of Object.entries(record)) {
    if (value && typeof value === "object") normalizeMalformedSchemaObjects(value, key);
  }
}
...
export function cleanJSONSchemaForAntigravity(schema: unknown): unknown {
  ...
  // Normalize malformed skill schemas before the regular Gemini cleanup phases.
