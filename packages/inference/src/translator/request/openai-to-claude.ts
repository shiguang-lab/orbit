...
} from "@orbit/contracts/model-specs";
    ...
    if (Array.isArray(msg.content)) {
      for (const part of msg.content) {
        if (part.type === "text" && part.text) {
          ...
          //
          // #6953/#12105 — strip thinking blocks that carry no signature at all.
          // `signature: ""` is the shape codex/gpt-5.x emit; a MISSING field is
          // what the response translator produces from cross-provider
          // `reasoning_content` (#12105). Neither can be replayed to Anthropic,
          // and fabricating DEFAULT_THINKING_CLAUDE_SIGNATURE is worse than
          // dropping: prepareClaudeRequest treats any non-empty signature on the
          // latest assistant turn as genuine and forwards the block verbatim, so
          // the fake signature 400s upstream. This mirrors the stricter
          // "non-empty string" check already used in claudeHelper.ts.
