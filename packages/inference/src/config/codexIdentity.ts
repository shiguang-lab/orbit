...
/**
 * #12189: detect the Claude Code CLI as the request *client* from request
 * headers. Used to auto-enable model echo so session restores work when the
 * resolved upstream model (e.g. `oc/nemotron-3-ultra-free`) is not recognized
 * by the Claude Code client on `--resume`.
 */
