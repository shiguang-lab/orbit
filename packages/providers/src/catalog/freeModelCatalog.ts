export { FREE_CATALOG_CURATED_AT, FREE_MODEL_BUDGETS } from "./freeModelCatalog.data.ts";
...
function resolveCatalogProviderId(provider: string): string {
  if (REGISTRY[provider]) return provider;
  return Object.entries(REGISTRY).find(([, definition]) => definition.alias === provider)?.[0] ?? provider;
}

/** Classify a concrete provider/model target against the curated free-model catalog. */
export function classifyPaidModelTarget(value: string): PaidModelTargetVerdict {
  ...
  const canonicalProvider = resolveCatalogProviderId(provider);
  const entries = FREE_MODEL_BUDGETS.filter(
    (entry) => entry.provider === provider || entry.provider === canonicalProvider,
  );
  if (!entries.some((entry) => grantsFreeAccess(entry.freeType))) return "unknown";
  return entries.some(
    (entry) => entry.modelId === modelId && grantsFreeAccess(entry.freeType),
  )
    ? "free"
    : "paid";
}
