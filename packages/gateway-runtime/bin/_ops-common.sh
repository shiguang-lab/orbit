# bin/_ops-common.sh — shared helpers for the ShiguangGateway ops runbook scripts.
#
...
ops_set_data_dir "${DATA_DIR:-$HOME/.shiguang-gateway}"
...
ops_find_snapshot() {
  ...
  ops_die "no snapshot matching '$id' under $SHIGUANG_GATEWAY_BACKUPS_DIR (run bin/snapshot-data.sh first)"
}
