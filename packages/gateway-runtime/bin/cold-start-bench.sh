#!/usr/bin/env bash
# bin/cold-start-bench.sh — measure ShiguangGateway cold-start against the target
...
Boots ShiguangGateway, times cold-start to the first /api/health/ping 200, measures
...
if [ -n "$BASE_URL" ]; then
  ...
  PORT="$PORT" $START_CMD >/tmp/shiguang-gateway-coldstart.log 2>&1 &
  SERVER_PID="$!"
