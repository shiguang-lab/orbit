 ...
 *
 * Purely informational invocations must NOT create `~/.shiguangGateway/.env` or write
 * a key — they never touch encrypted storage. Generating a 32-byte key and a
 * `.env` file just to print `shiguangGateway --version` (or `--help`) is a surprising
 ...
 *
 * Returns TRUE for everything else, INCLUDING a bare `shiguangGateway` (no args) — the
...
export function shouldProvisionStorageKey(argv) {
  ...
  // Bare `shiguangGateway` runs the default `serve` command → must provision.
