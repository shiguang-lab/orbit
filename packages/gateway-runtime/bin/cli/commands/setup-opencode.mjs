 ...
 * (openai-compatible). Distinct from `shiguangGateway setup opencode` (which wires the
 * @shiguang-gateway/opencode-plugin). This writes the `shiguangGateway` provider into
...
export async function runSetupOpencodeCommand(opts = {}) {
  ...
  // bin/shiguang-gateway.mjs before any command runs, so importing here is safe.
  let raw;
  let configPath;
  try {
