...
export async function getLatestVersion(execFn = execFileAsync) {
  try {
    ...
    const { stdout } = await execFn(
      npmBin(),
      ["view", "shiguangGateway", "version", "--prefer-online"],
...
export async function createBackup() {
  ...
  const backupDir = path.join(homedir(), ".shiguangGateway", "backups", `shiguangGateway-${Date.now()}`);
  ...
  try {
    ...
    const files = ["shiguang-gateway.mjs", "cli", "nodeRuntimeSupport.mjs", "mcp-server.mjs"];
...
// `shiguangGateway update` then correctly reports "already up to date" (the files
...
// `shiguangGateway --version` to verify.") implied the update was already live.
//
...
export async function printPostApplyGuidance(latest, deps = { readPidFile, isPidRunning }) {
  ...
  if (running) {
    ...
    printInfo("  Run `shiguangGateway restart` now to apply this update.");
  } else {
    printInfo(`No running ShiguangGateway server was detected via the CLI's PID file.`);
    printInfo(`  Start it with \`shiguangGateway serve\` (or restart your existing process) to run ${latest}.`);
  }
  printInfo("`shiguangGateway --version` will keep reporting the old version until the process restarts.");
}
...
export async function runUpdateCommand(opts = {}) {
  ...
  if (showChangelog) {
    try {
      const { stdout } = await execFileAsync(
        npmBin(),
        ["view", "shiguangGateway", "changelog"],
        npmExecOptions(process.platform, { timeoutMs: 15000 })
      );
      if (stdout.trim()) {
        console.log(stdout.trim());
      } else {
        console.log(`Changelog: https://github.com/your-org/shiguangGateway/releases/tag/v${latest}`);
      }
    } catch {
      console.log(`Changelog: https://github.com/your-org/shiguangGateway/releases/tag/v${latest}`);
    }
  ...
  if (checkOnly) {
    console.log("\n  Run `shiguangGateway update --apply` to install automatically.");
    return 1; // exit 1 = outdated (useful for scripts)
  }

  if (dryRun) {
    console.log("\n  [DRY RUN] Would run: npm install -g shiguangGateway@latest --include=optional");
    if (!skipBackup) console.log("  [DRY RUN] Would create backup in ~/.shiguangGateway/backups/");
    return 0;
  }
  ...
  printInfo("Updating ShiguangGateway...");
  try {
    ...
    execSync("npm install -g shiguangGateway@latest --include=optional", { stdio: "inherit" });
    // Trust-but-verify: `npm install -g` exits 0 even when a shadowing local install
    // (e.g. ~/node_modules/shiguang-gateway ahead of the global prefix on PATH) means the
    ...
    if (afterVersion && compareVersions(afterVersion, latest) < 0) {
      ...
      console.log(
        "  A local `node_modules/shiguang-gateway` is likely shadowing the global install on PATH."
      );
      console.log("  Diagnose with:");
      console.log("    which -a shiguang-gateway");
      console.log("    command -v shiguang-gateway");
      console.log("    npm prefix -g");
      console.log(
        "  Then remove the shadowing local copy (e.g. `npm uninstall shiguang-gateway` from its directory)"
      );
      console.log("  or reorder PATH so the global bin comes first.");
      return 1;
    }
    printSuccess(`Installed shiguangGateway@${latest} to disk.`);
    await printPostApplyGuidance(latest);
    return 0;
  } catch (err) {
    printError(`Update failed: ${err.message}`);
    printInfo("Restore from backup:");
    const backupDir = path.join(homedir(), ".shiguangGateway", "backups");
