 ...
 * below matches the documented ShiguangGateway block
 ...
 *   var named by `env_key` (`SHIGUANG_GATEWAY_API_KEY`), so the generated content is
 ...
 *   `[model_providers.shiguangGateway]` are set. An existing file that fails TOML
 *   parsing aborts generation instead of clobbering the operator's config.
 */

export const CODEX_MODEL_PROVIDER_ID = "shiguangGateway";
...
export async function generateCodexConfig(options: {
  ...
  const header =
    "# Generated by ShiguangGateway. The API key is read from the SHIGUANG_GATEWAY_API_KEY\n" +
