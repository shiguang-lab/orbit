...
import { POST as postAudioTranscription } from "../../app/api/v1/audio/transcriptions/route.ts";
import { handleValidatedEmbeddingRequestBody } from "../../app/api/v1/embeddings/route.ts";
import { POST as postRerank } from "../../app/api/v1/rerank/route.ts";
import {
...
function extractUpstreamDetailMessage(value: unknown): string | null {
  ...
  const status = record.status || errorRecord.status;
  if (typeof status === "string" && status.trim() && status !== "403") {
    return status.trim();
  }
...
function isGenericHttpProviderError(message: string): boolean {
  return (
    /\b(?:returned|provider returned)\s+HTTP\s+\d{3}\b/i.test(message) ||
    /\bupstream error\s*\(\d{3}\)/i.test(message) ||
...
export function extractProviderErrorMessage(body: unknown, fallback: string) {
  ...
  if (
    ...
    (isGenericHttpProviderError(baseMessage) || baseMessage === fallback || baseMessage.toLowerCase().includes("upstream error"))
  ) {
