...
function getLHCState() {
  if (!globalThis.__shiguangGatewayLocalHC) {
    globalThis.__shiguangGatewayLocalHC = {
      initialized: false,
      sweepTimer: null,
      healthCache: new Map(),
  ...
  return globalThis.__shiguangGatewayLocalHC;
}
...
function isLocalHealthCheckDisabled(): boolean {
  return (
    isEnvFlagEnabled("SHIGUANG_GATEWAY_DISABLE_LOCAL_HEALTHCHECK") ||
    isBuildProcess() ||
