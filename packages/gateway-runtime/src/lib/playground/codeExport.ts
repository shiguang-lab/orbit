...
/**
 * Endpoint suportado pelo Playground Studio. Reflete os 13 endpoints da API ShiguangGateway
...
export interface PlaygroundState {
  ...
  baseUrl: string; // ex.: "http://localhost:8787"
...
export const API_KEY_PLACEHOLDER = "$SHIGUANG_GATEWAY_API_KEY";
...
function buildCurlSnippet(state: PlaygroundState): string {
  ...
  const lines: string[] = [
    `# Set your API key: export SHIGUANG_GATEWAY_API_KEY="your-key-here"`,
    `curl -s -X POST \\`,
    `  "${url}" \\`,
...
function buildPythonSnippet(state: PlaygroundState): string {
  ...
  const lines: string[] = [
    ...
    `import requests`,
    ``,
    `api_key = os.environ["SHIGUANG_GATEWAY_API_KEY"]`,
    ``,
    `url = "${url}"`,
    `headers = {`,
