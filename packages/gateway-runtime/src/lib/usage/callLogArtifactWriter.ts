...
function resolveTsxLoader(
  ...
  // Production images keep pnpm's workspace links instead of hoisting tsx to
  // /app/node_modules.  A bare `--import tsx/esm` therefore works in a
  // workspace package but fails when the parent process is launched from
  // /app.  Resolve the package-local loader explicitly and retain the bare
  // specifier as the development fallback.
  const candidates = [
    path.resolve(moduleDir, "../../../node_modules/tsx/dist/loader.mjs"),
    path.resolve(cwd, "packages/gateway-runtime/node_modules/tsx/dist/loader.mjs"),
    path.resolve(cwd, "node_modules/tsx/dist/loader.mjs"),
  ];
  return candidates.find((candidate) => exists(candidate)) ?? "tsx/esm";
}
...
export function resolveCallLogArtifactWorker(context: WorkerResolutionContext = {}): {
  ...
  if (exists(moduleTs)) {
    return {
      workerFile: moduleTs,
      execArgv: ["--import", resolveTsxLoader(moduleDir, cwd, exists)],
    };
  }
