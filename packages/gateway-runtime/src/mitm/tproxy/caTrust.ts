 ...
 * trust store under a DEDICATED slot (`shiguangGateway-tproxy-ca.crt`), separate from
 * the static MITM cert (`shiguangGateway-mitm.crt` in `cert/install.ts`), so the two
...
export const TPROXY_CA_CERT_NAME = "shiguangGateway-tproxy-ca.crt";
...
export async function installTproxyCa(
  ...
  if (process.env.SHIGUANG_GATEWAY_SKIP_SYSTEM_TRUST === "1" && deps.run === undefined) {
    console.log("[tproxy-ca] SHIGUANG_GATEWAY_SKIP_SYSTEM_TRUST=1 — skipping OS trust-store mutation");
    return;
  }
