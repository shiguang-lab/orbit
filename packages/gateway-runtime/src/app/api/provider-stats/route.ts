...
import { getProviderCallStats, getModelCallStats } from "../../../lib/db/providerStats.ts";
...
export async function GET() {
  try {
    ...
    try {
      const { getTelemetrySummary } = await import("../../../shared/utils/requestTelemetry.ts");
    ...
    try {
      const { getToolLatencyByProvider } = await import(
        "../../../../open-sse/services/toolLatencyTracker.ts"
      );
