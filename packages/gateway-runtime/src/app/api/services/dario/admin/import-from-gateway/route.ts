/**
 * GET/POST /api/services/dario/admin/import-from-shiguangGateway
 *
 * Imports an existing ShiguangGateway `claude` provider connection's OAuth tokens
 * directly into Dario's account store, skipping the interactive browser OAuth
 * flow entirely. This works because ShiguangGateway's native `claude` provider and
 ...
 * PKCE/token-exchange ourselves, since ShiguangGateway's own decrypt() already
 ...
 * the file we stop+start the ShiguangGateway-managed supervisor to force a clean
...
function safeAliasFromSource(email: string | null | undefined, connectionId: string): string {
  const base = (email || connectionId || "shiguangGateway").toLowerCase();
  const cleaned = base.replace(/[^a-z0-9_.-]/g, "-").replace(/^[^a-z0-9]+/, "");
  const alias = cleaned || "shiguangGateway";
  return `shiguangGateway-${alias}`.slice(0, 64);
}
