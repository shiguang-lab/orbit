...
import fs from "fs";
import os from "os";
import { getDbInstance, resetDbInstance, SQLITE_FILE } from "../../../../lib/db/core.ts";
import { openDatabaseAsync } from "../../../../lib/db/adapters/driverFactory.ts";
 ...
 * via `SHIGUANG_GATEWAY_DB_IMPORT_MAX_MB`. Invalid / out-of-range values fall back to the 100 MB
...
// Required tables that must exist in a valid ShiguangGateway database
...
export async function POST(request: Request) {
  ...
  try {
    ...
    tmpPath = path.join(os.tmpdir(), `shiguangGateway-import-${Date.now()}.sqlite`);
    ...
    try {
      ...
      if (missingTables.length > 0) {
        return NextResponse.json(
          {
            error: `Invalid ShiguangGateway database. Missing tables: ${missingTables.join(", ")}`,
          },
          { status: 400 }
        );
      }
