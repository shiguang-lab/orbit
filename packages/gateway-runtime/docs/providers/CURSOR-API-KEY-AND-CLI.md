...
Two ways to put Cursor behind ShiguangGateway without an IDE session:

1. **`cursor-api` provider** (card "Cursor API", alias `cua`): an API-key
   provider that holds a Cursor user API key (`crsr_…`, generated at
   `https://cursor.com/dashboard/api`). Any ShiguangGateway client then reaches
...
2. **Cursor CLI passthrough**: point the Cursor CLI (`agent`) at ShiguangGateway so
   every RPC the CLI makes is authenticated with an ShiguangGateway API key, forwarded
...
curl -sS http://localhost:20128/v1/chat/completions \
  -H "Authorization: Bearer <shiguang-gateway-api-key>" \
  -H "Content-Type: application/json" \
...
- Model listing for `cursor-api` comes from the static Cursor registry (the
  ...
  needed on the ShiguangGateway host.
...
| Path                                                                                                                         | Auth expected from the CLI   | What ShiguangGateway does                                                                                                                                                |
| ---------------------------------------------------------------------------------------------------------------------------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `POST /auth/exchange_user_api_key`                                                                                           | `Bearer <ShiguangGateway API key>` | Validates the key, mints a 1h HS256 JWT (signed with `JWT_SECRET`) and returns it                                                                                  |
...
that. A 401 from ShiguangGateway makes the CLI exchange again.

### Setup

1. Create an ShiguangGateway API key (Dashboard → API keys) and a `cursor-api`
   connection.
...
| ------------------------------------------------ | --------------------------------------------- |
| Unknown ShiguangGateway key and `REQUIRE_API_KEY=true` | 401 `unauthenticated` on exchange             |
...
| ShiguangGateway API key revoked after exchange         | 401 on the next RPC                           |
