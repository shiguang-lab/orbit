...
**ShiguangGateway** — einheitlicher KI-Proxy/Router. Ein Endpunkt, 329 LLM-Anbieter, automatischer Fallback.
...
ShiguangGateway hat drei verwandte, aber unterschiedliche Mechanismen für temporäre Fehler. Halten Sie ihren
...
- **Imports**: extern → intern (`@/`, `@shiguang-gateway/open-sse`) → relativ
...
- **Pfad-Aliase**: `@/*` → `src/`, `@shiguang-gateway/open-sse` → `open-sse/`, `@shiguang-gateway/open-sse/*` → `open-sse/*`
- **Standardport**: 20128 (API + Dashboard am selben Port)
- **Datenverzeichnis**: `DATA_DIR` Umgebungsvariable, standardmäßig `~/.shiguang-gateway/`
