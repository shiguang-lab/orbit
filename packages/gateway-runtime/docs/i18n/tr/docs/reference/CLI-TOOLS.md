---
title: "CLI Araçları — ShiguangGateway"
version: 3.8.50
lastUpdated: 2026-08-23
---

# CLI Araçları — ShiguangGateway (Türkçe)
...
ShiguangGateway, üç özel pano sayfasına yayılmış üç CLI araçları kategorisiyle entegre olur:

| Sayfa          | Rota                    | Konsept                                                                    | Sayı         |
| -------------- | ----------------------- | -------------------------------------------------------------------------- | ------------ |
| **CLI Code's** | `/dashboard/cli-code`   | ShiguangGateway'a yönlendirdiğiniz kodlama araçları (İstemci → CLI → ShiguangGateway → Sağlayıcı) | 26           |
| **CLI Ajanları**| `/dashboard/cli-agents` | ShiguangGateway'a yönlendirdiğiniz özerk ajanlar (aynı akış, daha geniş kapsam)  | 8            |
| **ACP Ajanları**| `/dashboard/acp-agents` | ShiguangGateway'un stdio/ACP ile başlattığı CLI'lar (ters başlatma akışı)       | bkz. kayıt   |
...
Claude / Codex / OpenCode / Cline / KiloCode / Continue / Hermes / Goose / ...
           │
           ▼  (hepsi ShiguangGateway'a yönlendirilir)
    http://SUNUCUNUZ:20128/v1
           │
           ▼  (ShiguangGateway doğru sağlayıcıya yönlendirir)
