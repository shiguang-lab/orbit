...
5. `open-sse/` 目录作为 `@shiguang-gateway/open-sse` **npm workspace 包**发布。源码通过 `@shiguang-gateway/open-sse/...` 导入（由 Next.js 的 `transpilePackages` 解析）。本文档中的文件路径为了一致性仍使用目录名 `open-sse/`。
