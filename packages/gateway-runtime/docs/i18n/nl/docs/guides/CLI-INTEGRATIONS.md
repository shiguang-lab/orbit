...
title: "CLI Integraties — wijs elke coderende CLI aan ShiguangGateway"
version: 3.8.50
...
ShiguangGateway levert een familie van `setup-*` commando's die een coderende
CLI (Codex, Claude Code, OpenCode, Cline, …) configureert om ShiguangGateway als backend te gebruiken — zodat
de tool met **één** endpoint communiceert en ShiguangGateway naar de juiste provider doorverwijst met
automatische fallback. Elk commando leest de **live** modelcatalogus van een draaiende
ShiguangGateway (lokaal of op afstand) en schrijft het configuratiebestand van de tool op **jouw**
...
Er is ook een generieke launcher — `shiguang-gateway run <target>` — die
...
`open-code`, `qwen-code`, `gemini-cli`), en `shiguang-gateway completion` biedt de
zelfde manifest-afgeleide doelwoorden. De legacy per-tool launchers —
`shiguang-gateway launch` (Claude Code) en `shiguang-gateway launch-codex` (Codex) — blijven
beschikbaar.
...
- [Remote Mode](./REMOTE-MODE.md) — bestuur een remote ShiguangGateway (VPS / Tailnet) vanaf je laptop
...
Elk commando respecteert de **actieve context** (gesteld met `shiguang-gateway connect`, zie
...
| -------------------------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ | ---------------- |
| `shiguang-gateway setup-codex`    | OpenAI Codex CLI             | `~/.codex/<name>.config.toml` — één profiel per compatibel tekstmodel (`codex --profile <name>`)                                                                         | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | Beide            |
| `shiguang-gateway setup-claude`   | Claude Code                  | `~/.claude/profiles/<name>/settings.json` — één profiel per gematcht model (`CLAUDE_CONFIG_DIR`)                                                                         | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | Beide            |
| `shiguang-gateway setup-opencode` | OpenCode (openai-compatibel) | `~/.config/opencode/opencode.json` — `shiguang-gateway` provider met elk catalogusmodel (`opencode -m shiguang-gateway/<model>`)                                                       | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | Beide            |
| `shiguang-gateway setup-cline`    | Cline                        | `~/.cline/data/{globalState,secrets}.json` (CLI-modus) + print VS Code extensie-instellingen                                                                             | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | Beide            |
| `shiguang-gateway setup-kilo`     | Kilo Code                    | `~/.local/share/kilo/auth.json` (CLI) + voegt `kilocode.*` samen in VS Code `settings.json` indien aanwezig                                                              | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | Beide            |
| `shiguang-gateway setup-continue` | Continue / `cn` CLI          | `~/.continue/config.yaml` — `provider: openai` modellen, sleutel via `${{ secrets.SHIGUANG_GATEWAY_API_KEY }}`                                                                  | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Beide            |
| `shiguang-gateway setup-cursor`   | Cursor                       | Niets — print de stappen in de app (Cursor-configuratie is ondoorzichtig SQLite)                                                                                         | `--remote` `--api-key` `--only` `--port`                                                                                                   | Beide            |
| `shiguang-gateway setup-roo`      | Roo Code                     | `~/.shiguang-gateway/roo-settings.json` (import doc) + stelt `roo-cline.autoImportSettingsPath` in als er een VS Code `settings.json` bestaat                                   | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | Beide            |
| `shiguang-gateway setup-crush`    | Crush                        | `~/.config/crush/crush.json` — `openai-compat` provider, sleutel via `$SHIGUANG_GATEWAY_API_KEY`                                                                                | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Beide            |
| `shiguang-gateway setup-goose`    | Goose                        | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + print omgevingsrecept                                                                     | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Beide            |
| `shiguang-gateway setup-aider`    | Aider                        | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + print omgevingsrecept                                                                                   | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Beide            |
...
| `shiguang-gateway run <target>`   | Runtime launch (generiek)    | Niets — start `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` met de juiste omgeving en argumenten; Qwen en Gemini gebruiken een tijdelijke geïsoleerde home | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | Beide            |
| `shiguang-gateway launch`         | Claude Code                  | Niets — start `claude` met `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` geïnjecteerd                                                                                      | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | Beide            |
| `shiguang-gateway launch-codex`   | OpenAI Codex CLI             | Niets — start `codex` met de `shiguang-gateway` provider geïnjecteerd via `-c` vlaggen                                                                                          | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | Beide            |

Opmerkingen over vlaggen (gecontroleerd in de commandobron):

- `--remote <url>` — haal de catalogus op van een remote ShiguangGateway (overschrijft `--port`
  en de actieve context). `--api-key <key>` levert de referentie voor die
  server (standaard ingesteld op de `SHIGUANG_GATEWAY_API_KEY` omgevingsvariabele, of de token van de actieve context).
...
- `--model <id>` op `shiguang-gateway run` volgt de manifest's per-target bedrading
  (`bin/cli/cli-manifest.mjs`): **aider** ontvangt `--model openai/<id>` en
  **opencode** `--model shiguang-gateway/<id>` (de prefix wordt alleen toegevoegd wanneer de id
  ...
  **codex** via `-c model_providers.shiguang-gateway.*` args. **Qwen is het enige run
  doel dat hard `--model` vereist** — `shiguang-gateway run qwen` zonder het verlaat
  `2` met een expliciete fout.
- `--port <port>` — lokale ShiguangGateway poort (standaard `20128`, genegeerd wanneer `--remote`
  is ingesteld). Aanwezig op alle `setup-*` en beide launchers.
- `shiguang-gateway run` exitcodes: de eigen exitcode van de child CLI wordt
...
> Er is ook een rijkere pluginintegratie — `shiguang-gateway setup opencode` — die
> `@shiguang-gateway/opencode-plugin` installeert. Het zijn verschillende commando's; de tabel
...
Met ShiguangGateway draaiend op `localhost:20128`, voer gewoon de setup-opdracht uit voor jouw
...
export SHIGUANG_GATEWAY_API_KEY=sk-...  # verwezen via {env:SHIGUANG_GATEWAY_API_KEY}, nooit op schijf
...
```bash
shiguang-gateway launch                 # Claude Code → lokale ShiguangGateway
shiguang-gateway launch-codex           # Codex CLI → lokale ShiguangGateway
...
Wijs elke setup-opdracht aan een externe ShiguangGateway met `--remote` + `--api-key`. De
...
shiguang-gateway setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m shiguang-gateway/glm/glm-5.2 "..."   # exporteer eerst SHIGUANG_GATEWAY_API_KEY
...
```bash
shiguang-gateway connect 192.168.0.15        # mint een scoped token, slaat de context op
shiguang-gateway setup-codex                 # ← gebruikt nu de externe catalogus
shiguang-gateway setup-opencode              # ← hetzelfde
shiguang-gateway launch                      # ← Claude Code tegen de externe
```
...
ShiguangGateway exposeert het OpenAI oppervlak op `/v1`, het Anthropic oppervlak op de root,
...
| `setup-codex`, `launch-codex` (`model_providers.shiguang-gateway.base_url`)       | met `/v1`            | Ja                                             |
...
Wanneer je update met `shiguang-gateway update` (na bevestiging, of met `--apply`),
voert ShiguangGateway de installatie uit met `--include=optional` ingebakken:

```bash
npm install -g shiguang-gateway@latest --include=optional
```

Dit is **geen** vlag die je doorgeeft aan `shiguang-gateway update` — het wordt altijd toegepast door de
...
# [DRY RUN] Zou uitvoeren: npm install -g shiguang-gateway@latest --include=optional
```

Andere `shiguang-gateway update` vlaggen (geverifieerd in de bron): `--check` (verlaat met 1 als
...
Gemini-interface van ShiguangGateway (`/v1beta`). `shiguang-gateway run gemini` verbindt dat automatisch:

- `GOOGLE_GEMINI_BASE_URL` → de actieve ShiguangGateway basis-URL (root, geen `/v1`);
- `GEMINI_API_KEY` → de opgeloste ShiguangGateway referentie (optie/env/context);
- een **tijdelijk geïsoleerde `GEMINI_CLI_HOME`** waarvan de `.gemini/settings.json`
  ...
  nooit de ShiguangGateway-gestuurde lancering overschrijft — verwijderd na exit;
...
ShiguangGateway-server te valideren, bestaat er een opt-in harnas op
...
Optioneel: `SHIGUANG_GATEWAY_SMOKE_TARGETS="codex,opencode,qwen"` beperkt de sweep;
`SHIGUANG_GATEWAY_SMOKE_TIMEOUT_MS` overschrijft de timeout van 120s per doel.

## Zie ook

- [Claude Code configuratie](./CLAUDE-CODE-CONFIGURATION.md) — de diepere Claude Code gids
- [Codex CLI configuratie](./CODEX-CLI-CONFIGURATION.md) — de eenmalige `[model_providers.shiguang-gateway]` basisinstelling
