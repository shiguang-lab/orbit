...
title: "CLI Mchanganyiko — elekeza CLI ya uandishi kwenye ShiguangGateway"
version: 3.8.50
...
ShiguangGateway inatoa familia ya amri `setup-*` ambazo zinaweka mchanganyiko wa uandishi
CLI (Codex, Claude Code, OpenCode, Cline, …) kutumia ShiguangGateway kama backend yake — hivyo
chombo kinawasiliana na **nukta** moja na ShiguangGateway inaelekeza kwa mtoa huduma sahihi kwa
kuanguka kiotomatiki. Kila amri inasoma **katalogi** ya mfano wa moja kwa moja kutoka kwa
ShiguangGateway inayofanya kazi (ya ndani au ya mbali) na kuandika faili la usanidi la chombo
...
Pia kuna mchezaji wa jumla — `shiguang-gateway run <target>` — ambaye anazalisha
...
`open-code`, `qwen-code`, `gemini-cli`), na `shiguang-gateway completion` inatoa
maneno ya malengo yanayotokana na orodha hiyo. Mchezaji wa zamani wa kila chombo —
`shiguang-gateway launch` (Claude Code) na `shiguang-gateway launch-codex` (Codex) — bado
zinapatikana.
...
- [Hali ya Mbali](./REMOTE-MODE.md) — endesha ShiguangGateway ya mbali (VPS / Tailnet) kutoka kwa kompyuta yako
...
Kila amri heshimu **muktadha wa sasa** (iliyowekwa na `shiguang-gateway connect`, ona
...
| -------------------------- | ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------- |
| `shiguang-gateway setup-codex`    | OpenAI Codex CLI              | `~/.codex/<name>.config.toml` — wasifu mmoja kwa kila mfano wa maandiko unaofaa (`codex --profile <name>`)                                               | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | Zote                    |
| `shiguang-gateway setup-claude`   | Claude Code                   | `~/.claude/profiles/<name>/settings.json` — wasifu mmoja kwa kila mfano uliofanana (`CLAUDE_CONFIG_DIR`)                                                 | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | Zote                    |
| `shiguang-gateway setup-opencode` | OpenCode (inayofaa na openai) | `~/.config/opencode/opencode.json` — mtoa huduma wa `shiguang-gateway` na kila mfano wa katalogi (`opencode -m shiguang-gateway/<model>`)                              | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | Zote                    |
| `shiguang-gateway setup-cline`    | Cline                         | `~/.cline/data/{globalState,secrets}.json` (hali ya CLI) + inachapisha mipangilio ya nyongeza ya VS Code                                                 | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | Zote                    |
| `shiguang-gateway setup-kilo`     | Kilo Code                     | `~/.local/share/kilo/auth.json` (CLI) + inachanganya `kilocode.*` katika `settings.json` ya VS Code ikiwa inapatikana                                    | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | Zote                    |
| `shiguang-gateway setup-continue` | Continue / `cn` CLI           | `~/.continue/config.yaml` — `provider: openai` mifano, funguo kupitia `${{ secrets.SHIGUANG_GATEWAY_API_KEY }}`                                                 | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Zote                    |
| `shiguang-gateway setup-cursor`   | Cursor                        | Hakuna — inachapisha hatua za ndani ya programu (mipangilio ya Cursor ni SQLite isiyoonekana)                                                            | `--remote` `--api-key` `--only` `--port`                                                                                                   | Zote                    |
| `shiguang-gateway setup-roo`      | Roo Code                      | `~/.shiguang-gateway/roo-settings.json` (nyaraka ya kuagiza) + inaweka `roo-cline.autoImportSettingsPath` ikiwa `settings.json` ya VS Code inapatikana          | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | Zote                    |
| `shiguang-gateway setup-crush`    | Crush                         | `~/.config/crush/crush.json` — mtoa huduma wa `openai-compat`, funguo kupitia `$SHIGUANG_GATEWAY_API_KEY`                                                       | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Zote                    |
| `shiguang-gateway setup-goose`    | Goose                         | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + inachapisha mapishi ya mazingira                                          | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Zote                    |
| `shiguang-gateway setup-aider`    | Aider                         | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + inachapisha mapishi ya mazingira                                                        | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Zote                    |
| `shiguang-gateway setup-qwen`     | Qwen Code                     | `~/.qwen/settings.json` — V4 `modelProviders.openai` orodha + `SHIGUANG_GATEWAY_API_KEY` katika `~/.qwen/.env`                                                  | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | Zote                    |
| `shiguang-gateway run <target>`   | Uzinduzi wa wakati (jumla)    | Hakuna — anazalisha `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` na mazingira na hoja sahihi; Qwen na Gemini hutumia nyumbani iliyotengwa | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | Zote                    |
| `shiguang-gateway launch`         | Claude Code                   | Hakuna — anazalisha `claude` na `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` ikingizwa                                                                    | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | Zote                    |
| `shiguang-gateway launch-codex`   | OpenAI Codex CLI              | Hakuna — anazalisha `codex` na mtoa huduma wa `shiguang-gateway` ikingizwa kupitia bendera `-c`                                                                 | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | Zote                    |

Maelezo kuhusu bendera (yamehakikishwa katika chanzo cha amri):

- `--remote <url>` — pata katalogi kutoka kwa ShiguangGateway ya mbali (inabatilisha `--port`
  na muktadha wa sasa). `--api-key <key>` inatoa akidi kwa ajili ya
  seva hiyo (inatumika kama chaguo la `SHIGUANG_GATEWAY_API_KEY` env var, au token ya muktadha wa sasa).
...
- `--model <id>` kwenye `shiguang-gateway run` inafuata uunganisho wa orodha ya malengo
  (`bin/cli/cli-manifest.mjs`): **aider** inapata `--model openai/<id>` na
  **opencode** `--model shiguang-gateway/<id>` (kiambatisho kinajumuishwa tu wakati id
  ...
  **codex** kupitia `-c model_providers.shiguang-gateway.*` hoja. **Qwen ndiyo lengo pekee la kuendesha
  ambalo linahitaji kwa nguvu `--model`** — `shiguang-gateway run qwen` bila hiyo inatoka
  `2` na makosa wazi.
- `--port <port>` — bandari ya ndani ya ShiguangGateway (chaguo la msingi `20128`, ignored when `--remote`
  is set). Ipo kwenye kila `setup-*` na mchezaji wote wawili.
- Nambari za kutoka za `shiguang-gateway run`: nambari ya kutoka ya CLI ya mtoto inasambazwa
...
> Pia kuna mchanganyiko wa nyongeza wenye utajiri zaidi — `shiguang-gateway setup opencode` — ambayo
> inasakinisha `@shiguang-gateway/opencode-plugin`. Hizi ni amri tofauti; jedwali
...
Ikiwa ShiguangGateway inafanya kazi kwenye `localhost:20128`, endesha tu amri ya usanidi kwa zana yako. Katalogi inapatikana kutoka kwa seva ya ndani.
...
export SHIGUANG_GATEWAY_API_KEY=sk-...  # inarejelea kupitia {env:SHIGUANG_GATEWAY_API_KEY}, kamwe sio kwenye diski
...
```bash
shiguang-gateway launch                 # Claude Code → ShiguangGateway ya ndani
shiguang-gateway launch-codex           # Codex CLI → ShiguangGateway ya ndani
...
Elekeza amri yoyote ya usanidi kwenye ShiguangGateway ya mbali kwa `--remote` + `--api-key`. Katalogi inapatikana kutoka kwa mbali; usanidi unandikwa kwenye mashine yako ya ndani.
...
shiguang-gateway setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m shiguang-gateway/glm/glm-5.2 "..."   # export SHIGUANG_GATEWAY_API_KEY kwanza
...
```bash
shiguang-gateway connect 192.168.0.15        # inaunda token iliyo na upeo, inahifadhi muktadha
shiguang-gateway setup-codex                 # ← sasa inatumia katalogi ya mbali
shiguang-gateway setup-opencode              # ← sawa
shiguang-gateway launch                      # ← Claude Code dhidi ya mbali
```
...
ShiguangGateway inatoa uso wa OpenAI kwenye `/v1`, uso wa Anthropic kwenye mzizi,
...
| `setup-codex`, `launch-codex` (`model_providers.shiguang-gateway.base_url`)       | pamoja na `/v1`            | Ndio                                              |
...
Unaposasisha kwa kutumia `shiguang-gateway update` (baada ya kuthibitisha, au kwa `--apply`),
ShiguangGateway inatekeleza usakinishaji kwa kutumia `--include=optional` iliyojumuishwa:

```bash
npm install -g shiguang-gateway@latest --include=optional
```

Hii **si** bendera unayoipatia `shiguang-gateway update` — inatumika kila wakati na
...
# [DRY RUN] Ingefanya: npm install -g shiguang-gateway@latest --include=optional
```

Bendera nyingine za `shiguang-gateway update` (zilizothibitishwa kwenye chanzo): `--check` (ondoka 1 ikiwa
imepitwa na wakati), `--apply` (sakinisha bila kuomba), `--changelog`, `--no-backup`,
`--yes`.

---

## Google Gemini CLI kupitia `shiguang-gateway run gemini`
...
Gemini wa ShiguangGateway (`/v1beta`). `shiguang-gateway run gemini` inafanya hivyo kiotomatiki:

- `GOOGLE_GEMINI_BASE_URL` → URL ya msingi ya ShiguangGateway inayotumika (mizizi, hakuna `/v1`);
- `GEMINI_API_KEY` → akidi ya ShiguangGateway iliyotatuliwa (chaguo/env/muktadha);
- **nyumba ya muda ya `GEMINI_CLI_HOME`** ambayo `.gemini/settings.json`
  ...
  hakitabadilisha uzinduzi unaoelekezwa na ShiguangGateway — inatolewa baada ya kutoka;
...
server HALISI ya ShiguangGateway, kuna vifaa vya kujiunga katika
...
Hiari: `SHIGUANG_GATEWAY_SMOKE_TARGETS="codex,opencode,qwen"` inapunguza safu;
`SHIGUANG_GATEWAY_SMOKE_TIMEOUT_MS` inabadilisha muda wa sekunde 120 kwa kila lengo.
...
- [Usanidi wa Codex CLI](./CODEX-CLI-CONFIGURATION.md) — usanidi wa msingi wa mara moja `[model_providers.shiguang-gateway]`
