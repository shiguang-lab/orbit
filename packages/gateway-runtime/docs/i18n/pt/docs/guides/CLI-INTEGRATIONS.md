...
O ShiguangGateway fornece uma família de comandos `setup-*` que configuram uma CLI de codificação (Codex, Claude Code, OpenCode, Cline, …) para usar o ShiguangGateway como seu backend — assim, a ferramenta comunica-se com **um** endpoint e o ShiguangGateway direciona para o provedor certo com fallback automático. Cada comando lê o catálogo de modelos **ao vivo** de um ShiguangGateway em execução (local ou remoto) e escreve o próprio arquivo de configuração da ferramenta na **sua** máquina. A chave da API é referenciada por uma variável de ambiente sempre que a ferramenta a suporta. Comandos que persistem um arquivo de ambiente local da ferramenta estão anotados abaixo.
...
| `shiguang-gateway setup-qwen`     | Qwen Code                                  | `~/.qwen/settings.json` — array `modelProviders.openai` V4 + `SHIGUANG_GATEWAY_API_KEY` em `~/.qwen/.env`                                                                 | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | Ambos           |
...
- `--model <id>` em `shiguang-gateway run` segue a fiação por alvo do manifesto
  ...
  **codex** via argumentos `-c model_providers.shiguang-gateway.*`. **Qwen é o único alvo de execução
...
- Códigos de saída de `shiguang-gateway run`: o próprio código de saída da CLI filha é propagado
...
Com o ShiguangGateway a correr em `localhost:20128`, basta executar o comando de configuração para a sua ferramenta. O catálogo é buscado no servidor local.
...
shiguang-gateway run claude -- --print-system-prompt "revise este diff"
```
...
Aponte qualquer comando de configuração para um ShiguangGateway remoto com `--remote` + `--api-key`. O catálogo é buscado remotamente; a configuração é escrita na sua máquina local.
...
```bash
shiguang-gateway connect 192.168.0.15        # gera um token com escopo, armazena o contexto
...
O ShiguangGateway expõe a superfície OpenAI em `/v1`, a superfície Anthropic na raiz, e uma superfície nativa Gemini em `/v1beta`. Cada integração está ligada à forma que a sua ferramenta espera (verificado na fonte do comando):
...
Quando você atualiza com `shiguang-gateway update` (após confirmar, ou com `--apply`),
o ShiguangGateway executa a instalação com `--include=optional` incorporado:

```bash
npm install -g shiguang-gateway@latest --include=optional
```

Este **não** é um parâmetro que você passa para `shiguang-gateway update` — ele é sempre aplicado pelo
...
Outros parâmetros de `shiguang-gateway update` (verificados no código-fonte): `--check` (sai com 1 se
...
Gemini do ShiguangGateway (`/v1beta`). `shiguang-gateway run gemini` conecta isso automaticamente:
...
- um **`GEMINI_CLI_HOME` isolado temporário** cujo `.gemini/settings.json`
  ...
  nunca sobrescreva o lançamento direcionado pelo ShiguangGateway — removido após a saída;
...
do ShiguangGateway, existe um suporte opcional em
...
Opcional: `SHIGUANG_GATEWAY_SMOKE_TARGETS="codex,opencode,qwen"` restringe a varredura;
`SHIGUANG_GATEWAY_SMOKE_TIMEOUT_MS` substitui o tempo limite de 120s por alvo.
...
- [Configuração do Codex CLI](./CODEX-CLI-CONFIGURATION.md) — a configuração base única `[model_providers.shiguang-gateway]`
