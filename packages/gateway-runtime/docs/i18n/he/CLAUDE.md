...
**ShiguangGateway** — פרוקסי/נתב AI מאוחד. נקודת קצה אחת, 329 ספקי LLM, חזרה אוטומטית.
...
ShiguangGateway יש שלושה מנגנוני כישלון זמניים הקשורים אך שונים. שמור על התחום שלהם נפרד כאשר אתה מדבג התנהגות נתיב. ראה את
...
- **ייבוא**: חיצוני → פנימי (`@/`, `@shiguang-gateway/open-sse`) → יחסי
...
- **Alias נתיב**: `@/*` → `src/`, `@shiguang-gateway/open-sse` → `open-sse/`, `@shiguang-gateway/open-sse/*` → `open-sse/*`
- **פורט ברירת מחדל**: 20128 (API + לוח מחוונים באותו פורט)
- **ספריית נתונים**: משתנה סביבה `DATA_DIR`, ברירת מחדל ל-`~/.shiguang-gateway/`
