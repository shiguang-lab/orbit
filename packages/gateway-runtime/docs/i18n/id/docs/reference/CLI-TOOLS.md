...
(Codex) tetap tersedia. CLI Gemini hanya untuk peluncuran: itu adalah target `shiguang-gateway run`
...
kontainer. ShiguangGateway mendeteksi itu dan keluar dengan kode `2` dengan instruksi daripada
...
`--allow-container-write` ketika mengonfigurasi CLI kontainer itu sendiri adalah apa yang sebenarnya Anda maksud; `SHIGUANG_GATEWAY_ALLOW_CONTAINER_CONFIG_WRITE=true` melakukan hal yang sama untuk
server. Lihat
...
Halaman ini (yang diubah namanya dari `/dashboard/agents`) menunjukkan CLI yang dapat **dibuat** oleh ShiguangGateway sebagai mesin eksekusi backend melalui protokol stdio/ACP. Katalog ini dikelola secara terpisah di `src/lib/acp/registry.ts` dan **tidak** sama dengan `CLI_TOOLS`.
...
```bash
shiguang-gateway doctor                       # Periksa konfigurasi, DB, port, runtime, memori, kelangsungan
shiguang-gateway doctor --json                # JSON yang dapat dibaca mesin
shiguang-gateway doctor --no-liveness         # Lewati probe kesehatan HTTP
shiguang-gateway doctor --host 0.0.0.0        # Ganti host kelangsungan
...
shiguang-gateway providers test-all                        # Uji setiap koneksi aktif
shiguang-gateway providers validate                        # Validasi struktural hanya lokal
...
shiguang-gateway auth export --force                          # Ekspor SEMUA kredensial DECRYPTED koneksi ke stdout sebagai JSON
