...
**ShiguangGateway** — unified AI proxy/router. One endpoint, 329 provider catalog entries, auto-fallback when an upstream route is available.
...
ShiguangGateway has three related but distinct temporary-failure mechanisms. Keep their
...
- **Imports**: external → internal (`@/`, `@shiguang-gateway/open-sse`) → relative
...
repo** instead of polluting the main ShiguangGateway tree.
...
- **Path aliases**: `@/*` → `src/`, `@shiguang-gateway/open-sse` → `open-sse/`, `@shiguang-gateway/open-sse/*` → `open-sse/*`
- **Default port**: 20128 (API + dashboard on same port)
- **Data directory**: `DATA_DIR` env var, defaults to `~/.shiguang-gateway/`
...
ShiguangGateway has **~48 quality-gate scripts** (`scripts/check/` + `scripts/quality/`) wired
