...
ShiguangGateway dostarcza system wtyczek w stylu WordPressa. Wtyczki to samodzielne
...
Wtyczki mieszkają w katalogu danych ShiguangGateway:

```
~/.shiguang-gateway/plugins/<plugin-name>/
  ├─ plugin.json
  └─ index.js          # (or whatever manifest.main points to)
```

`getDefaultPluginDir()` (`src/lib/plugins/scanner.ts`) rozwiązuje to do
`<home>/.shiguang-gateway/plugins`, gdzie `<home>` pochodzi ze zmiennych środowiskowych `HOME` /
...
| `requires`         | object?   | `{ shiguang-gateway?, permissions[] }`                             |
...
install (POST /api/plugins, path)
  ...
  → atomic rename into ~/.shiguang-gateway/plugins/<name> → insert DB row
