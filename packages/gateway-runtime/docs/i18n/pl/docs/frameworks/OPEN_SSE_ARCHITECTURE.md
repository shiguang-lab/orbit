...
1. **Reużywalność** — `open-sse` jest publikowany jako `@shiguang-gateway/open-sse` na npm, więc inne projekty mogą go używać niezależnie
...
├── package.json          # @shiguang-gateway/open-sse
...
```ts
import { getExecutor } from "@shiguang-gateway/open-sse/executors";
...
```ts
import { needsTranslation, translateRequest } from "@shiguang-gateway/open-sse/translator";
