...
ShiguangGateway has three distinct but related resilience mechanisms. Each has a different scope and purpose. Keep them separate when debugging routing behavior.
...
**Scope:** one client session (`X-Session-Id` / `x-codex-session-id` / `x-shiguang-gateway-session` header) pinned to one connection, for **any** provider.
...
**Scope:** one active managed HTTP client/session owns one eligible ShiguangGateway connection.
...
Managed inference requests present the opaque `X-ShiguangGateway-Lease-Owner` value and exact
`X-ShiguangGateway-Lease-Generation`. The owner uses `vlo_` followed by 43 base64url characters; only
...
foreign active lease, ShiguangGateway returns HTTP `429`, lease-capacity-unavailable code, a
...
connectionId)` before the `exhaustedConnections` lookup) — a plain
  ...
  `X-ShiguangGateway-Selected-Connection-Id` header, never hits that key match. For
...
- **Emergency fallback** — controlled by `SHIGUANG_GATEWAY_EMERGENCY_FALLBACK`; operators can override it from the Feature Flags page without a restart.
