...
> (typed registry — code lands with [#5868](https://github.com/diegosouzapw/ShiguangGateway/pull/5868))
...
1. **Lifecycle** (`RouterBackendLifecycle`) — _how the engine runs_:
   - `in-process` — runs inside the ShiguangGateway Node process (the native TS pipeline).
   - `supervised` — a local child process ShiguangGateway installs/starts/stops/health-checks
     via `ServiceSupervisor`, then consumes as a provider connection.
   - `external` — an HTTP endpoint ShiguangGateway dispatches to but does **not** manage
...
[#5868](https://github.com/diegosouzapw/ShiguangGateway/pull/5868)) declares every engine once, with its
...
[#5817](https://github.com/diegosouzapw/ShiguangGateway/pull/5817); until it merges,
