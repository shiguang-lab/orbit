...
**Problem it solves:** ShiguangGateway proxies many API keys against the same upstream
...
- All persistence is in the existing ShiguangGateway SQLite DB (`DATA_DIR/storage.sqlite`).
...
1. `localStorage.getItem("shiguang-gateway:quota-share:pools")` is non-empty.
