...
ShiguangGateway connects to an **Obsidian** vault as a **context source** — a local Markdown
...
vault sync (driven by `src/lib/obsidianSync.ts`). Enabling it points ShiguangGateway at a
...
per call (per-API-key first, then global). Tools that hit the ShiguangGateway **sync server**
(the four `obsidian_sync_*` tools) additionally require the sync auth token configured
in ShiguangGateway settings.
...
| `obsidian_sync_status`       | ShiguangGateway sync server status: running, vault name, port, uptime, last sync.          |
...
`open-sse/mcp-server/server.ts`, gated on `SHIGUANG_GATEWAY_MCP_ENFORCE_SCOPES=true`, with
allowed scopes sourced from `SHIGUANG_GATEWAY_MCP_SCOPES` or the API key's scope context. See
