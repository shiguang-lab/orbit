...
ShiguangGateway ships a WordPress-style plugin system. Plugins are self-contained
...
Plugins live under the ShiguangGateway data directory:
...
`<home>/.shiguang-gateway/plugins`, where `<home>` is taken from the `HOME` /
