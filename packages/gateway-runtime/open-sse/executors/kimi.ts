...
function resolveKimiProtocol(
  ...
  const targetFormat = credentials?.providerSpecificData?._shiguangGatewayKimiTargetFormat;
...
function getThinkingPolicy(credentials: ProviderCredentials): KimiThinkingPolicy {
  return (asRecord(credentials.providerSpecificData?._shiguangGatewayKimiThinking) ||
