 ...
 *
 * When ShiguangGateway translates a Claude-native streamed response to OpenAI Chat
 ...
 *   - Explicit opt-in: `x-shiguangGateway-thinking-marker: on` force-keeps it for the
 *     shrinking set of content-scanning clients (#4633).
 *   - Explicit opt-out: `x-shiguangGateway-thinking-marker: off` (same as default).
...
export const THINKING_MARKER_HEADER = "x-shiguangGateway-thinking-marker";
...
/**
 * Interpret the explicit `x-shiguangGateway-thinking-marker` request header.
 ...
 *   2. Explicit `x-shiguangGateway-thinking-marker` header → honor on/off (#5312)
...
export function resolveSuppressThinkClose(opts: {
  ...
  // Legacy content-scanning clients opt in with x-shiguangGateway-thinking-marker: on.
  return true;
}
