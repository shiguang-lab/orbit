 ...
 * `stream`. Without this hint, ShiguangGateway previously routed those requests with
...
export function resolveStreamFlag(
  ...
  // ShiguangGateway accumulates the provider stream and converts it to JSON for the client
  ...
  // ShiguangGateway fallback would force SSE upstream and fail JSON-only providers as
  ...
  // No explicit stream param — preserve ShiguangGateway's streaming default unless
