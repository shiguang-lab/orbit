...
const TEST_DATA_DIR = fs.mkdtempSync(
  path.join(os.tmpdir(), "orbit-chatgpt-web-cleanroom-restore-")
);
...
test("migration 175 restores chatgpt-web writes while cgpt-web remains fail-closed", () => {
  const db = core.getDbInstance();
  const applied = db
    .prepare("SELECT version, name FROM _orbit_migrations WHERE version = 175")
    .get() as { version: string; name: string } | undefined;
  assert.deepEqual(applied, { version: "175", name: "restore_chatgpt_web_cleanroom" });
