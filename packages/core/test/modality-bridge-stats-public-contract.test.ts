...
const expectedRuntimeExports = ["buildModalityBridgeHeader", "getBridgeStats"] as const;
...
test("guardrails/modality-bridge-stats exposes only shared stats and header capabilities", async () => {
  const entry = manifest.exports["./guardrails/modality-bridge-stats"];
  assert.deepEqual(entry, {
    types: "./src/public/modalityBridgeStats.d.ts",
    import: "./src/guardrails/modalityBridgeStats.ts",
  });
...
test("scenario-specific modality bridge stats aliases stay retired", () => {
