...
const retiredAliases = [
  "./shared/validation/schemas/cli",
  "./control/cli-tools-validation-cli",
] as const;
...
test("CLI model settings use one focused validation contract", async () => {
  ...
  assert.deepEqual(Object.keys(runtime).sort(), ["cliAuthOnlyConfigSchema", "cliModelConfigSchema"]);
});

test("broad CLI validation aliases stay retired", () => {
