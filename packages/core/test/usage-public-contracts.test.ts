...
test("mixed usage database facade and aliases stay retired", () => {
  ...
  const consumers = [
    ...
    ...sourceFiles(path.join(repoRoot, "packages/inference")),
  ];
  for (const file of consumers) {
    const source = fs.readFileSync(file, "utf8");
    assert.doesNotMatch(
      source,
      /core\/(?:edge\/usage-db|runtime\/usage-db|control\/provider-discovery-support\/callLogs|usage\/reporting-support\/call-logs)/,
      file,
    );
  }
});
...
test("call-log artifact reader stays physical, read-only, and accurately typed", () => {
  ...
  const consumers = [
    ...
    fs.readFileSync(file, "utf8").includes("core/usage/call-log-artifacts"),
  );
  assert.deepEqual(
    consumers.map((file) => path.relative(repoRoot, file).split(path.sep).join("/")),
    ["packages/inference/src/services/conversationTurnContent.ts"],
  );
  assert.match(
    fs.readFileSync(consumers[0], "utf8"),
    /import\s*\{\s*readCallArtifact\s*\}\s*from\s*["']@orbit\/core\/usage\/call-log-artifacts["']/,
  );
});
