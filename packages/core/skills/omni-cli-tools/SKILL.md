---
name: omni-cli-tools
description: "Manage CLI tool integrations exposed via the API. List, configure, and invoke CLI tool plugins that extend Orbit's automation surface."
---
...
Read Crush CLI Orbit config

Local-only. Reads the Orbit provider block in Crush's config.
...
Write Crush CLI Orbit config

Local-only. Registers Orbit as an `openai-compat` provider in Crush's config.
...
Remove Orbit from Crush CLI config

Local-only. Removes the Orbit provider block from Crush's config.
...
Read CodeWhale CLI Orbit config

Local-only. Reads the Orbit config block from `~/.codewhale/config.toml` (with `~/.deepseek/config.toml` legacy fallback).
...
Write CodeWhale CLI Orbit config

Local-only. Writes the Orbit config block in CodeWhale TOML format.
...
Remove Orbit from CodeWhale CLI config

Local-only. Removes the Orbit config block from CodeWhale's config.
