...
description: "Interact with ShiguangGateway via JSON-RPC 2.0 agent-to-agent protocol. 6 built-in A2A skills: smart-routing, quota-management, provider-discovery, cost-analysis, health-report, list-capabilities."
---
...
<!-- skill:custom-start -->
<!-- Migrated from skills/shiguang-gateway-a2a/SKILL.md (preserved curated content) -->

# ShiguangGateway — A2A Protocol
...
ShiguangGateway publishes an Agent Card at `/.well-known/agent.json` and accepts
JSON-RPC 2.0 calls at `/a2a`.

## Discovery

```bash
curl $SHIGUANG_GATEWAY_URL/.well-known/agent.json
```
...
```bash
curl -X POST $SHIGUANG_GATEWAY_URL/a2a \
...
- `401` → missing/invalid `SHIGUANG_GATEWAY_KEY`

## Reference

Full docs: https://github.com/diegosouzapw/ShiguangGateway/blob/main/docs/frameworks/A2A-SERVER.md
<!-- skill:custom-end -->
