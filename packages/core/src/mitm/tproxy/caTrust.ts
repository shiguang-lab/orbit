 ...
 * trust store under a DEDICATED slot (`orbit-tproxy-ca.crt`), separate from
 * the static MITM cert (`orbit-mitm.crt` in `cert/install.ts`), so the two
...
export const TPROXY_CA_CERT_NAME = "orbit-tproxy-ca.crt";
...
export async function installTproxyCa(
  ...
  if (process.env.ORBIT_SKIP_SYSTEM_TRUST === "1" && deps.run === undefined) {
    console.log("[tproxy-ca] ORBIT_SKIP_SYSTEM_TRUST=1 — skipping OS trust-store mutation");
    return;
  }
