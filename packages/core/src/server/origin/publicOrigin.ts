import { classifyHostLocality } from "../authz/routeGuard.ts";
import { PEER_IP_HEADER } from "../authz/headers.ts";
import { resolveStampedPeer } from "../authz/peerStamp.ts";
 ...
 * over a private-network host — e.g. `http://192.168.0.15:8787` instead of the
...
export function resolvePublicOrigin(request: Request): PublicOriginCandidate {
  ...
  return { origin: process.env.PUBLIC_BASE_URL ?? "http://127.0.0.1:8787", source: "request-url" };
}
