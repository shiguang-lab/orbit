...
function isConfigured(content: string, baseUrl: string): boolean {
  ...
  try {
    ...
    if (url.hostname === "127.0.0.1") aliases.add(normalized.replace("127.0.0.1", "localhost"));
    if (url.hostname === "localhost") aliases.add(normalized.replace("localhost", "127.0.0.1"));
  } catch {
...
export async function detectTool(id: string): Promise<DetectedTool | null> {
  ...
  const configured = !!configContents && isConfigured(configContents, resolveGatewayBaseUrl());
