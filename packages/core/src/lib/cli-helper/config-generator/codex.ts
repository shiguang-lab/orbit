 ...
 * below matches the documented Orbit block
 ...
 *   var named by `env_key` (`ORBIT_API_KEY`), so the generated content is
 ...
 *   `[model_providers.orbit]` are set. An existing file that fails TOML
 *   parsing aborts generation instead of clobbering the operator's config.
 */

export const CODEX_MODEL_PROVIDER_ID = "orbit";
...
export async function generateCodexConfig(options: {
  ...
  const header =
    "# Generated by Orbit. The API key is read from the ORBIT_API_KEY\n" +
