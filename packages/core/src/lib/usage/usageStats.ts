 ...
 * Token Plan), where the upstream exposes no API-key usage endpoint — ShiguangGateway
 * counts the tokens it routed and compares against the known monthly limit.
 * Only reflects traffic that went THROUGH ShiguangGateway (not the provider's panel).
 */
...
/**
 * Total USD spend ShiguangGateway has recorded for a single provider connection, across all time
 ...
 * total. Only reflects traffic that went THROUGH ShiguangGateway, not the provider's own dashboard. Used
...
export async function getUsageStats() {
  ...
  const { getProviderConnections } = await import("../localDb.ts");
