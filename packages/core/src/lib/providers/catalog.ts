import { APIKEY_PROVIDERS_GATEWAYS } from "@orbit/providers/catalog/gateways";
import { APIKEY_PROVIDERS_ENTERPRISE } from "@orbit/providers/catalog/enterprise";
import {
  APIKEY_PROVIDERS,
  IDE_PROVIDER_IDS,
...
/** Management UI metadata projected from the runtime provider registry. */
export function getDashboardProviderCatalog() {
  return {
    categories: Object.values(STATIC_PROVIDER_CATALOG_GROUPS).map((group) => ({
      key: group.category,
      displayAuthType: group.displayAuthType,
      toggleAuthType: group.toggleAuthType,
      providers: Object.values(group.providers).map((provider) => {
        const serviceKinds = provider.serviceKinds as string[] | undefined;
        const dashboardSection = Object.hasOwn(APIKEY_PROVIDERS_GATEWAYS, provider.id)
          ? "aggregator"
          : Object.hasOwn(APIKEY_PROVIDERS_ENTERPRISE, provider.id)
            ? "enterprise"
            : serviceKinds?.includes("embedding") && !serviceKinds.includes("llm")
              ? "embedding"
              : serviceKinds?.includes("video") && !serviceKinds.includes("llm")
                ? "video"
                : serviceKinds?.includes("image") && !serviceKinds.includes("llm")
                  ? "image"
                  : "llm";
        return {
          ...provider,
          isIde: IDE_PROVIDER_IDS.has(provider.id),
          ...(group.category === "apikey" ? { dashboardSection } : {}),
        };
      }),
    })),
  };
