...
import { getProviderDisplayName } from "../display/names";
...
const lease = (row: LeaseRow) => {
  const value = rowToCamel(row) as Record<string, unknown>;
  return {
    id: value.id,
    leaseOwnerHash: value.leaseOwnerHash,
...
export function getExclusiveConnectionLeaseStatus(input: {
  ...
  return immediate(() => {
    ...
    return row
      ? {
          lease: lease(row),
          provider: getProviderDisplayName(row.connection_provider),
