...
import { revokeNativeCodexTurnPinsForConnection } from "../runtimeHooks.ts";
...
export async function deleteProviderConnection(id: string) {
  ...
  void revokeNativeCodexTurnPinsForConnection(id).catch(() => {});
...
export async function deleteProviderConnectionsByProvider(providerId: string) {
  ...
  for (const connectionId of connectionIds) {
    ...
    void revokeNativeCodexTurnPinsForConnection(connectionId).catch(() => {});
  }
  backupDbFile("pre-write");
  invalidateDbCache("connections");
  invalidateReasoningRoutingRuleCache();
