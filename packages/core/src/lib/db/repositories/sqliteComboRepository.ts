...
import { normalizeComboRecord } from "../../combos/steps.ts";
import { validateComboInvariant } from "../../combos/invariants.ts";
...
export async function deleteCombo(id: string) {
  ...
  const remove = db.transaction(() => {
    ...
    return {
      deleted: true,
      lkgpKeys: combo?.name ? deleteLKGPRowsByComboName(combo.name) : [],
    };
  });
  const { deleted, lkgpKeys } = remove();
  if (!deleted) return false;
  if (lkgpKeys.length > 0) {
    const { invalidateCachedLKGP } = await import("../readCache");
    for (const key of lkgpKeys) invalidateCachedLKGP(key);
  }
  return true;
}
