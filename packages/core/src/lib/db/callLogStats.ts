 ...
 * /api/provider-metrics, /api/search/stats, and /api/search/analytics routes
...
// ---------------------------------------------------------------------------
// /api/search/analytics — aggregated search analytics
// ---------------------------------------------------------------------------
...
export function getErrorTypeBreakdown(
  ...
  const rows = db
    .prepare(
      `
      SELECT
        -- '2026-08-20' = migration 158 added error_type. Lower bound, not exact:
