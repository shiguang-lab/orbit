...
let _legacyStaticKey: Buffer | null = null;
...
function getLegacyStaticKey(): Buffer | null {
  if (_legacyStaticKey !== null) return _legacyStaticKey;
  const secret = ensureSecretLoaded();
  if (!secret || typeof secret !== "string" || secret.trim().length === 0) return null;
  try {
    _legacyStaticKey = scryptSync(secret, LEGACY_STATIC_SALT, KEY_LENGTH);
  } catch {
    return null;
  }
  return _legacyStaticKey;
}
...
export function decrypt(
  ...
  try {
    ...
    const legacyStaticKey = getLegacyStaticKey();
    if (legacyStaticKey) {
      const legacyStaticDecrypted = tryDecryptWithKey(legacyStaticKey);
      if (legacyStaticDecrypted !== null) return legacyStaticDecrypted;
    }

    const legacyDynamicKey = getLegacyDynamicKey();
    if (legacyDynamicKey) {
      const legacyDynamicDecrypted = tryDecryptWithKey(legacyDynamicKey);
      if (legacyDynamicDecrypted !== null) return legacyDynamicDecrypted;
    }
...
export function migrateLegacyEncryptedString(ciphertext: string | null | undefined): {
  ...
  // 2. If it decrypts with a former static or dynamic key, it needs migration.
  if (legacyStaticKey) {
    const legacyStaticDecrypted = tryDecryptWithKey(legacyStaticKey);
    if (legacyStaticDecrypted !== null) return { updated: true, value: encrypt(legacyStaticDecrypted) };
  }
  if (legacyKey) {
