...
CREATE TABLE IF NOT EXISTS version_manager (
  ...
  tool              TEXT NOT NULL UNIQUE,           -- 'cliproxyapi' | 'shiguang-gateway'
  ...
  auto_start        INTEGER NOT NULL DEFAULT 0,    -- 1 = start with ShiguangGateway
...
-- Determines whether each provider uses ShiguangGateway's native executor,
