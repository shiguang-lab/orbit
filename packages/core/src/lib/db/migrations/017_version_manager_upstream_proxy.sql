...
CREATE TABLE IF NOT EXISTS version_manager (
  ...
  tool              TEXT NOT NULL UNIQUE,           -- 'cliproxyapi' | 'orbit'
  ...
  auto_start        INTEGER NOT NULL DEFAULT 0,    -- 1 = start with Orbit
...
-- Determines whether each provider uses Orbit's native executor,
