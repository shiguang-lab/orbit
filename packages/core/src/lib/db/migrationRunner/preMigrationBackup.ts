/**
 * Content-addressed pre-migration snapshots (#12435 / upstream #327).
 *
 * The SHA-256 content address lives in the first portion of the canonical
 * `db_<snapshot-id>_<reason>.sqlite` shape, preserving reason parsing while
 * making unchanged retries an O(1) lookup even with tens of thousands of old
 * backups. A content hash reuses an identical prior snapshot, so repeated
 * zero-progress startups (crash loop on a failing migration) retain ONE
 * restore point for that database state instead of growing db_backups/
 * without bound.
 */
 ...
 * directly instead. Publication uses an atomic, no-overwrite hard link. If the
 * filesystem cannot provide that primitive, the caller fails closed instead of
 * exposing a partial canonical `.sqlite` file. A content hash reuses an identical
 * prior snapshot, so repeated zero-progress startups retain one restore point for
 * that database state without ever deleting a published backup.
 */
export function createPreMigrationBackup(db: SqliteAdapter): PreMigrationBackupReceipt | null {
  ...
  try {
    ...
    const backupPath = path.join(backupDir, `db_state-${sha256}_pre-migration.sqlite`);
