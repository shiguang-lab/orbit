...
async function syncQuotaCombosGuarded(poolId: string): Promise<void> {
  try {
    const { syncQuotaCombos } = await import("../quota/quotaCombos.ts");
...
async function removeQuotaCombosGuarded(poolId: string): Promise<void> {
  try {
    const { removeQuotaCombosForPool } = await import("../quota/quotaCombos.ts");
...
export interface QuotaPool {
  ...
  name: string;
  /** Optional group this pool belongs to. */
  groupId: string | null;
  createdAt: string;
...
export interface PoolCreate {
  connectionId: string;
  name: string;
  /** Optional group to assign this pool to. */
  groupId?: string;
  allocations?: PoolAllocation[];
  /**
