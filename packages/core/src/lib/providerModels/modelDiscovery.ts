...
// expose (as opposed to the flat `supportedThinkingEfforts` field Orbit's own
...
// Maps common upstream synonyms onto Orbit's canonical effort vocabulary
 ...
 *
 * A flat top-level `defaultThinkingEffort` (Orbit's own import format, and
...
export function detectSupportedThinkingEfforts(record: JsonRecord): string[] | undefined {
  ...
  // Orbit's own catalog surfaces effort tiers inside `capabilities.effort_tiers`,
