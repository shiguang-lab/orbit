...
// expose (as opposed to the flat `supportedThinkingEfforts` field ShiguangGateway's own
...
// Maps common upstream synonyms onto ShiguangGateway's canonical effort vocabulary
 ...
 *
 * A flat top-level `defaultThinkingEffort` (ShiguangGateway's own import format, and
...
export function detectSupportedThinkingEfforts(record: JsonRecord): string[] | undefined {
  ...
  // ShiguangGateway's own catalog surfaces effort tiers inside `capabilities.effort_tiers`,
