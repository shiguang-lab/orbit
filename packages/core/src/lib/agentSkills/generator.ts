 ...
 *
 * Usage (library):
 *   import { generateAgentSkills, buildSkillMarkdown } from "./generator.ts";
 *
 * Usage (CLI):
...
function buildApiBody(skill: AgentSkill, sources: BuildSources): string {
  ...
  const curlBase = resolveGatewayBaseUrl();
  ...
  if (ops.length === 0) {
    ...
    for (const op of ops) {
      ...
      if (usesDashboardSession && op.path === "/api/auth/login" && op.method === "POST") {
        lines.push(`curl -X POST ${curlBase}${op.path} \\`);
        ...
        if (op.method === "GET") {
          ...
          lines.push(
            `CSRF_TOKEN=$(curl -s ${curlBase}/api/auth/csrf -b cookie.jar | jq -r .token)`
          );
          lines.push(`curl ${curlMethod}${curlBase}${op.path} \\`);
          lines.push("  -b cookie.jar \\");
...
export async function generateAgentSkills(opts: GeneratorOptions): Promise<GeneratorReport> {
  const { dryRun = true, prune = false, outputDir = getSkillsDir(), onlyIds } = opts;
