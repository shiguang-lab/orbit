...
export type CodexImportPayload = {
  ...
  // shows "Token Expired" for freshly imported tokens.
...
export function normalizeCodexImportRecord(input: unknown): NormalizeResult {
  ...
  if (chatgptAccountId) {
    ...
    // (it matches rows via provider_specific_data.workspaceId) instead of
    // inserting a duplicate on re-import.
 ...
 * WITHOUT clobbering state the import cannot know about:
 *
 ...
 *   runtime quota state and the operator-set codexFingerprintMode survive —
 *   the same pattern the single-file import uses.
 * - priority: drop the forwarded priority — the upsert path never reorders
 *   siblings, so overwriting the matched row's priority can duplicate another
 *   connection's. The operator's existing ordering wins.
 *
