...
class VectorStoreImpl implements VectorStore {
  ...
  async searchHybrid(
    ...
    const rrfK = RRF_K;
    const q = liveVecQuantization();
    // Sanitize the FTS5 MATCH input: an unsanitized queryText can carry FTS
    // control operators ("AND", "OR", "NEAR", quotes, filters) and crash or
    // skew the MATCH expression. Empty sanitized query → disable FTS side.
