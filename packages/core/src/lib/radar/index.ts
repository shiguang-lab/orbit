 ...
 * serve (#12215). Recomputing from the raw baseline instead would drop that
 * state, and the totals honour it — `computeFreeModelTotals` treats
 * `enabled: false` as absent, and `applyFeed` skips tombstoned entries — so
 * models the operator removed would silently reappear in the published
 * numbers.
 */
