...
import {
  ...
  sanitizeRequest,
} from "../../shared/utils/inputSanitizer.ts";
import { getFeatureFlagOverride } from "../db/featureFlags.ts";
import { parseEnvBoolean } from "../../shared/utils/envBoolean.ts";
import {
  resolveBlockThreshold,
  shouldBlockDetections,
  SEVERITY_SCORES as SHARED_SEVERITY_SCORES,
  type InjectionSeverity,
} from "../../shared/utils/injectionSeverity.ts";
...
function emitGuardrailLog(
  ...
  logger?.[level]?.("GUARDRAIL", message, meta);
}
...
export function evaluatePromptInjection(
  ...
  const sanitizerResult = sanitizeRequest(requestBody, {
    info() {},
    warn() {},
  });
  const contents = extractMessageContents(requestBody);
  ...
  const existingDetections = new Set(
    sanitizerResult.detections.map((d) => `${d.pattern}:${d.match}:${d.severity}`)
  );
