...
import { SQLITE_FILE } from "../db/core.ts";
...
function getPollIntervalMs() {
  const parsed = Number.parseInt(process.env.SHIGUANG_GATEWAY_CONFIG_HOT_RELOAD_MS || "", 10);
...
async function runHotReloadCheck(source: string, skipBackgroundServices: boolean) {
  const settings = await getSettings();
  const changes = await applyRuntimeSettings(settings, { source, skipBackgroundServices });
  logChanges(source, changes);
}

let skipBackgroundServicesForProcess = false;
...
function queueHotReloadCheck(source: string) {
  ...
  activeCheck = runHotReloadCheck(source, skipBackgroundServicesForProcess)
    .catch((error) => {
      console.warn(
...
export function startRuntimeConfigHotReload(
  options: { pollIntervalMs?: number; skipBackgroundServices?: boolean } = {}
) {
  if (pollTimer || sqliteWatcher) return;

  skipBackgroundServicesForProcess = options.skipBackgroundServices === true;
  ...
  if (SQLITE_FILE) {
    try {
      ...
      // fs.watch may emit resource errors asynchronously (for example when
      // the process exceeds the host's watcher limit). Treat that as a
      // degraded polling-only mode instead of crashing the app.
      sqliteWatcher.once("error", (error) => {
        console.warn(
          "[HOT_RELOAD] SQLite file watch stopped, polling only:",
          error instanceof Error ? error.message : error,
        );
...
export function stopRuntimeConfigHotReload() {
  ...
  queuedSource = null;
  activeCheck = null;
  skipBackgroundServicesForProcess = false;
}
