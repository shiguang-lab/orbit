...
import type { AgentSkill, SkillArea, SkillCategory } from "../../lib/agentSkills/types.ts";

// Skills are shipped in this runtime package. They must not be fetched from the
// upstream repository at runtime. Relative URLs keep the catalog portable
// across local, container, and reverse-proxied deployments.
export const AGENT_SKILLS_RAW_BASE = "/api/agent-skills";
export const AGENT_SKILLS_BLOB_BASE = "/api/agent-skills";

export function getAgentSkillRawUrl(id: string): string {
  return `${AGENT_SKILLS_RAW_BASE}/${encodeURIComponent(id)}/raw`;
}

export function getAgentSkillBlobUrl(id: string): string {
  return `${AGENT_SKILLS_BLOB_BASE}/${encodeURIComponent(id)}/raw`;
}
