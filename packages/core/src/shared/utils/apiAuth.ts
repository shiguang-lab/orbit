...
export { isDashboardSessionAuthenticated } from "@orbit/auth/dashboard-session";
...
async function validateBearerApiKey(apiKey: string | null): Promise<boolean> {
  ...
  try {
    const { validateApiKey } = await import("../../lib/db/apiKeys.ts");
 ...
 *
 * Scope set is sourced from `@orbit/auth` so this
