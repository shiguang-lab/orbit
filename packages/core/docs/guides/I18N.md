...
Orbit supports **43 languages** with full dashboard UI translation, translated documentation, and RTL support for Arabic and Hebrew.
...
Orbit uses a hash-based incremental translator for docs, backed by an
OpenAI-compatible LLM endpoint (typically `cx/gpt-5.4-mini` through Orbit
Cloud):
...
| Variable                            | Purpose                                 |
| ----------------------------------- | --------------------------------------- |
| `ORBIT_TRANSLATION_API_URL`     | OpenAI-compatible base URL, e.g. `…/v1` |
| `ORBIT_TRANSLATION_API_KEY`     | bearer token (kept out of logs)         |
| `ORBIT_TRANSLATION_MODEL`       | model id, e.g. `cx/gpt-5.4-mini`        |
| `ORBIT_TRANSLATION_TIMEOUT_MS`  | optional, default `60000`               |
| `ORBIT_TRANSLATION_CONCURRENCY` | optional, default `4`                   |
...
- **Chunked batching**: Joins multiple strings with `__ORBIT_I18N_SEPARATOR__` delimiters to minimize API calls (max 1800 chars per request)
...
**Secondary translator** — uses any OpenAI-compatible LLM API (including Orbit itself) to translate existing `docs/i18n/` markdown files. Best for polishing or re-translating docs with better quality than Google Translate.
...
The `orbit` CLI has its own i18n layer separate from the Next.js dashboard.
...
| -------- | ------------------------ | --------------------------------------- |
| 1        | `--lang` flag            | `orbit --lang de status`            |
| 2        | `ORBIT_LANG` env var | `ORBIT_LANG=ja orbit providers` |
...
```bash
# Set language and save to ~/.orbit/.env (persists across sessions)
orbit config lang set pt-BR

# View current language
orbit config lang get

# List all 42 available languages
orbit config lang list

# JSON output
orbit config lang list --output json
```

The saved preference is written atomically to `~/.orbit/.env` and is loaded by the
...
(`Orbit`, `OAuth`, `MCP`, `A2A`, `DATA_DIR`, …) that must appear verbatim
