...
ShiguangGateway supports **43 languages** with full dashboard UI translation, translated documentation, and RTL support for Arabic and Hebrew.
...
ShiguangGateway uses a hash-based incremental translator for docs, backed by an
OpenAI-compatible LLM endpoint (typically `cx/gpt-5.4-mini` through ShiguangGateway
Cloud):
...
| Variable                            | Purpose                                 |
| ----------------------------------- | --------------------------------------- |
| `SHIGUANG_GATEWAY_TRANSLATION_API_URL`     | OpenAI-compatible base URL, e.g. `…/v1` |
| `SHIGUANG_GATEWAY_TRANSLATION_API_KEY`     | bearer token (kept out of logs)         |
| `SHIGUANG_GATEWAY_TRANSLATION_MODEL`       | model id, e.g. `cx/gpt-5.4-mini`        |
| `SHIGUANG_GATEWAY_TRANSLATION_TIMEOUT_MS`  | optional, default `60000`               |
| `SHIGUANG_GATEWAY_TRANSLATION_CONCURRENCY` | optional, default `4`                   |
...
- **Chunked batching**: Joins multiple strings with `__SHIGUANG_GATEWAY_I18N_SEPARATOR__` delimiters to minimize API calls (max 1800 chars per request)
...
**Secondary translator** — uses any OpenAI-compatible LLM API (including ShiguangGateway itself) to translate existing `docs/i18n/` markdown files. Best for polishing or re-translating docs with better quality than Google Translate.
...
The `shiguang-gateway` CLI has its own i18n layer separate from the Next.js dashboard.

### How it works

- Every user-facing string in CLI commands goes through `t("module.key", vars)` from `apps/cli/src/cli/i18n.mjs`.
- Catalogs are JSON files in `apps/cli/src/cli/locales/` — 43 ship out-of-the-box.
...
| -------- | ------------------------ | --------------------------------------- |
| 1        | `--lang` flag            | `shiguang-gateway --lang de status`            |
| 2        | `SHIGUANG_GATEWAY_LANG` env var | `SHIGUANG_GATEWAY_LANG=ja shiguang-gateway providers` |
...
```bash
# Set language and save to ~/.shiguang-gateway/.env (persists across sessions)
shiguang-gateway config lang set pt-BR

# View current language
shiguang-gateway config lang get

# List all 42 available languages
shiguang-gateway config lang list

# JSON output
shiguang-gateway config lang list --output json
```

The saved preference is written atomically to `~/.shiguang-gateway/.env` and is loaded by the
...
shiguang-gateway --lang de providers list
```
...
43 locale files ship in `apps/cli/src/cli/locales/`. Full translations: `en`, `pt-BR`.
...
2. Run `node apps/cli/src/cli/scripts/generate-locales.mjs` — creates the locale file.
...
`src/i18n/messages/zh-CN.json` and `apps/cli/src/cli/locales/zh-CN.json`.

### scripts/i18n/glossary/protected-terms.json

Flat list of product/provider/model/protocol/CLI/env/identifier names
(`ShiguangGateway`, `OAuth`, `MCP`, `A2A`, `DATA_DIR`, …) that must appear verbatim
