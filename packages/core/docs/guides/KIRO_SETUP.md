...
This guide covers adding Kiro (AWS-hosted AI coding assistant) accounts to ShiguangGateway,
...
## How ShiguangGateway Solves This (v3.8.0+)

Starting with v3.8.0, ShiguangGateway calls `registerClient()` (AWS SSO OIDC) during every
Kiro connection import. This gives each ShiguangGateway connection its own dedicated OIDC
...
- ShiguangGateway v3.8.0 or later.
...
2. Choose one of:
   ...
   - **Auto-Import** — click the button; ShiguangGateway reads credentials from the
...
4. ShiguangGateway validates the key and saves the connection.
...
(`clientSecretExpiresAt`). ShiguangGateway stores this timestamp in `providerSpecificData`
...
- Ensure ShiguangGateway can reach `https://oidc.us-east-1.amazonaws.com` (or the configured
...
- The key must be able to call `ListAvailableProfiles`; otherwise ShiguangGateway cannot
