...
This guide covers adding Kiro (AWS-hosted AI coding assistant) accounts to Orbit,
...
## How Orbit Solves This (v3.8.0+)

Starting with v3.8.0, Orbit calls `registerClient()` (AWS SSO OIDC) during every
Kiro connection import. This gives each Orbit connection its own dedicated OIDC
...
2. Choose one of:
   ...
   - **Auto-Import** — click the button; Orbit reads credentials from the
...
4. Orbit validates the key and saves the connection.
...
(`clientSecretExpiresAt`). Orbit stores this timestamp in `providerSpecificData`
...
- Ensure Orbit can reach `https://oidc.us-east-1.amazonaws.com` (or the configured
...
- The key must be able to call `ListAvailableProfiles`; otherwise Orbit cannot
