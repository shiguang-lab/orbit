...
- Outbound URL guard: `@orbit/utils/network` — validates provider URLs against private/localhost CIDR ranges
...
- SSRF / outbound URL guard: `@orbit/utils/network` — blocks private/loopback/link-local ranges for all provider calls
...
- `@orbit/utils/network` blocks all private/loopback/link-local target URLs before they reach provider executors
...
5. The `open-sse/` directory is published as the `@orbit/inference` **npm workspace package**. Source code imports it via `@orbit/inference/...` (resolved by Next.js `transpilePackages`). File paths in this document still use the directory name `open-sse/` for consistency.
