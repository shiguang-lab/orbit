...
Orbit compression is built around engine contracts. A mode can run one engine directly
...
Orbit never delegates the decision: the adapter pins the model gate to the package's
most restrictive scope, so host environment settings can only narrow the allowlist, never
widen it past Orbit's measured receipts.
...
actually reach `orbit_ccr_retrieve`** (`callerSupportsCcrRetrieve()` in
...
with a `22-87%` range, and a `~46%` input-compression tool. Orbit uses the Caveman input-side
...
- **Global npm (`npm i -g orbit`) / standalone** — run the install command above inside
...
| Tool                                | Scope               | Purpose                          |
| ----------------------------------- | ------------------- | -------------------------------- |
| `orbit_compression_status`      | `read:compression`  | Settings, analytics, cache stats |
| `orbit_compression_configure`   | `write:compression` | Update global settings           |
| `orbit_set_compression_engine`  | `write:compression` | Set mode and optional pipeline   |
| `orbit_list_compression_combos` | `read:compression`  | List compression combos          |
| `orbit_compression_combo_stats` | `read:compression`  | Read combo/engine analytics      |
