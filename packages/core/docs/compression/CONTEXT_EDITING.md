...
Delegated **Context Editing** is a Claude-only context-management feature. Unlike ShiguangGateway's local
...
tool-use / tool-result blocks from its own running context window. ShiguangGateway only attaches a body
parameter (`context_management.edits[]`); Claude does the actual clearing against its own tokenizer.

This is a delegated capability by nature: other providers reject the parameter, so ShiguangGateway scopes
...
ShiguangGateway injects a single edit into the outbound Anthropic Messages body:
...
ShiguangGateway already emits on Claude requests.
...
left as-is. If a `clear_thinking_20251015` edit is also present, ShiguangGateway stable-sorts the
...
- Zod schema: `contextEditingConfigSchema` in `apps/control/src/compression/compression-config-schemas.ts`.
...
After a Claude response, ShiguangGateway records how much context the provider actually cleared. This is
...
| ----------------- | --------------------------------------------------- | ------------------------------------------- |
| Where it runs     | In ShiguangGateway, before the request leaves the proxy   | In the provider (Claude), server-side       |
...
The two are complementary: local engines compress the bytes ShiguangGateway sends; Context Editing lets
