...
Two ways to put Cursor behind Orbit without an IDE session:

1. **`cursor-api` provider** (card "Cursor API", alias `cua`): an API-key
   provider that holds a Cursor user API key (`crsr_…`, generated at
   `https://cursor.com/dashboard/api`). Any Orbit client then reaches
...
2. **Cursor CLI passthrough**: point the Cursor CLI (`agent`) at Orbit so
   every RPC the CLI makes is authenticated with an Orbit API key, forwarded
...
curl -sS http://localhost:20128/v1/chat/completions \
  -H "Authorization: Bearer <orbit-api-key>" \
  -H "Content-Type: application/json" \
...
| Path                                                                                                                         | Auth expected from the CLI   | What Orbit does                                                                                                                                                |
| ---------------------------------------------------------------------------------------------------------------------------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `POST /auth/exchange_user_api_key`                                                                                           | `Bearer <Orbit API key>` | Validates the key, mints a 1h HS256 JWT (signed with `JWT_SECRET`) and returns it                                                                                  |
...
that. A 401 from Orbit makes the CLI exchange again.

### Setup

1. Create an Orbit API key (Dashboard → API keys) and a `cursor-api`
   connection.
...
| ------------------------------------------------ | --------------------------------------------- |
| Unknown Orbit key and `REQUIRE_API_KEY=true` | 401 `unauthenticated` on exchange             |
...
| Orbit API key revoked after exchange         | 401 on the next RPC                           |
