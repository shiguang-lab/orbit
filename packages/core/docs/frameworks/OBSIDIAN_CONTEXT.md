...
Orbit connects to an **Obsidian** vault as a **context source** — a local Markdown
...
vault sync (driven by `src/lib/obsidianSync.ts`). Enabling it points Orbit at a
...
per call (per-API-key first, then global). Tools that hit the Orbit **sync server**
...
| `obsidian_sync_status`       | Orbit sync server status: running, vault name, port, uptime, last sync.          |
...
`open-sse/mcp-server/server.ts`, gated on `ORBIT_MCP_ENFORCE_SCOPES=true`, with
allowed scopes sourced from `ORBIT_MCP_SCOPES` or the API key's scope context. See
