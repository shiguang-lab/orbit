...
Orbit ships a WordPress-style plugin system. Plugins are self-contained
...
Plugins live under the Orbit data directory:
...
`<home>/.orbit/plugins`, where `<home>` is taken from the `HOME` /
