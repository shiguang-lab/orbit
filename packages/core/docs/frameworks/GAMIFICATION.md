...
ShiguangGateway includes a local-first gamification layer that rewards users for
...
| Community Servers | Federate with external ShiguangGateway instances                      |
...
All tables live in the main ShiguangGateway SQLite database, created by migration
...
Follows the standard ShiguangGateway pattern — imports `getDbInstance()` from
...
The raw token is returned to the user exactly once at creation time. ShiguangGateway
...
All routes follow the standard ShiguangGateway pattern:
