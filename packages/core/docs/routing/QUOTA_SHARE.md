...
**Problem it solves:** Orbit proxies many API keys against the same upstream
...
- All persistence is in the existing Orbit SQLite DB (`DATA_DIR/storage.sqlite`).
...
1. `localStorage.getItem("orbit:quota-share:pools")` is non-empty.
