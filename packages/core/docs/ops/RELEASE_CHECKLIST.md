...
- [ ] `npm run check:node-runtime` — supported runtime floor met (`>=24.20.0 <25`, per `SUPPORTED_NODE_RANGE` in `apps/cli/src/nodeRuntimeSupport.mjs`; aligned with root `package.json` `engines`)
