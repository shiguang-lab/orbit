...
All Orbit management API routes are classified into one of three protection
...
the operator put Orbit behind an nginx/Cloudflare/Tailscale tunnel and a JWT
leaked, or auth was misconfigured — the attacker turns "call an API" into "run a
command on the host" (remote code execution). Orbit closes this by enforcing a
...
If you run Orbit behind a reverse proxy or tunnel (nginx, Caddy, Cloudflare
...
- A `403 LOCAL_ONLY` in Orbit's logs for one of these paths is the guard
