...
TPROXY transparent decrypt is ShiguangGateway's **5th capture mode** for the
...
configuration change** — but the process must trust the dynamic CA ShiguangGateway
...
> **#6684 update:** the AgentBridge static server (`src/bin/mitm/server.cjs`) now
...
> and installs under the pre-existing `shiguang-gateway-mitm.crt` trust-store slot
> (superseding the old single leaf there — no dual-trust cleanup needed),
> kept fully separate from TPROXY's own `shiguang-gateway-tproxy-ca.crt` slot below.
...
- Generates a long-lived CA via `generateMitmCa()` (CN `"ShiguangGateway MITM CA"`,
...
`shiguang-gateway-tproxy-ca.crt` (constant `TPROXY_CA_CERT_NAME`) — kept separate from
the static MITM cert's slot (`shiguang-gateway-mitm.crt`) so the two never clobber each
other.
...
host means anything ShiguangGateway intercepts can be decrypted. It is gated behind the
...
- Confirm the process trusts the dynamic CA. The CA is installed under
  `shiguang-gateway-tproxy-ca.crt`; apps with their own trust store (Firefox/Chrome NSS)
...
mode reverts the rules; if ShiguangGateway was killed mid-session, use the AgentBridge
