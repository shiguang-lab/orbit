...
TPROXY transparent decrypt is Orbit's **5th capture mode** for the
...
configuration change** — but the process must trust the dynamic CA Orbit
...
> and installs under the pre-existing `orbit-mitm.crt` trust-store slot
> (superseding the old single leaf there — no dual-trust cleanup needed),
> kept fully separate from TPROXY's own `orbit-tproxy-ca.crt` slot below.
...
- Generates a long-lived CA via `generateMitmCa()` (CN `"Orbit MITM CA"`,
...
`orbit-tproxy-ca.crt` (constant `TPROXY_CA_CERT_NAME`) — kept separate from
the static MITM cert's slot (`orbit-mitm.crt`) so the two never clobber each
other.
...
host means anything Orbit intercepts can be decrypted. It is gated behind the
...
- Confirm the process trusts the dynamic CA. The CA is installed under
  `orbit-tproxy-ca.crt`; apps with their own trust store (Firefox/Chrome NSS)
...
mode reverts the rules; if Orbit was killed mid-session, use the AgentBridge
