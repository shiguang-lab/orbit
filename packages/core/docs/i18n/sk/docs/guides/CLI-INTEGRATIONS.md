...
title: "CLI Integrácie — nasmerujte akýkoľvek kódovací CLI na Orbit"
version: 3.8.50
...
Orbit dodáva rodinu príkazov `setup-*`, ktoré konfigurovanú kódovaciu
CLI (Codex, Claude Code, OpenCode, Cline, …) na používanie Orbit ako svojho backendu — takže
nástroj komunikuje s **jedným** koncovým bodom a Orbit smeruje k správnemu poskytovateľovi s
automatickým zálohovaním. Každý príkaz číta **živý** modelový katalóg z bežiaceho
Orbit (lokálneho alebo vzdialeného) a zapisuje vlastný konfiguračný súbor nástroja na **vašom**
...
Existuje aj generický spúšťač — `orbit run <target>` — ktorý spúšťa
...
`open-code`, `qwen-code`, `gemini-cli`), a `orbit completion` ponúka
rovnaké slová cieľov odvodené z manifestu. Dedičstvo per-nástrojových spúšťačov —
`orbit launch` (Claude Code) a `orbit launch-codex` (Codex) — zostáva
k dispozícii.
...
- [Vzdialený režim](./REMOTE-MODE.md) — ovládajte vzdialený Orbit (VPS / Tailnet) zo svojho laptopu
...
Každý príkaz rešpektuje **aktívny kontext** (nastavený pomocou `orbit connect`, pozri
...
| -------------------------- | ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | -------------------- |
| `orbit setup-codex`    | OpenAI Codex CLI               | `~/.codex/<name>.config.toml` — jeden profil pre každý kompatibilný textový model (`codex --profile <name>`)                                                   | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | Obe                  |
| `orbit setup-claude`   | Claude Code                    | `~/.claude/profiles/<name>/settings.json` — jeden profil pre každý zhodujúci sa model (`CLAUDE_CONFIG_DIR`)                                                    | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | Obe                  |
| `orbit setup-opencode` | OpenCode (openai-kompatibilný) | `~/.config/opencode/opencode.json` — `orbit` poskytovateľ so všetkými modelmi katalógu (`opencode -m orbit/<model>`)                                   | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | Obe                  |
| `orbit setup-cline`    | Cline                          | `~/.cline/data/{globalState,secrets}.json` (CLI režim) + vytlačí nastavenia rozšírenia VS Code                                                                 | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | Obe                  |
| `orbit setup-kilo`     | Kilo Code                      | `~/.local/share/kilo/auth.json` (CLI) + zlúči `kilocode.*` do `settings.json` VS Code, ak je prítomné                                                          | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | Obe                  |
| `orbit setup-continue` | Continue / `cn` CLI            | `~/.continue/config.yaml` — `provider: openai` modely, kľúč cez `${{ secrets.ORBIT_API_KEY }}`                                                             | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Obe                  |
| `orbit setup-cursor`   | Cursor                         | Nič — vytlačí kroky v aplikácii (konfigurácia Cursor je nepriehľadná SQLite)                                                                                   | `--remote` `--api-key` `--only` `--port`                                                                                                   | Obe                  |
| `orbit setup-roo`      | Roo Code                       | `~/.orbit/roo-settings.json` (import dokumentu) + nastaví `roo-cline.autoImportSettingsPath`, ak existuje `settings.json` vo VS Code                       | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | Obe                  |
| `orbit setup-crush`    | Crush                          | `~/.config/crush/crush.json` — `openai-compat` poskytovateľ, kľúč cez `$ORBIT_API_KEY`                                                                     | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Obe                  |
| `orbit setup-goose`    | Goose                          | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + vytlačí recept prostredia                                                       | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Obe                  |
| `orbit setup-aider`    | Aider                          | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + vytlačí recept prostredia                                                                     | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Obe                  |
| `orbit setup-qwen`     | Qwen Code                      | `~/.qwen/settings.json` — V4 `modelProviders.openai` pole + `ORBIT_API_KEY` v `~/.qwen/.env`                                                               | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | Obe                  |
| `orbit run <target>`   | Spúšťanie za behu (generické)  | Nič — spúšťa `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` s príslušným prostredím a argumentmi; Qwen a Gemini používajú dočasný izolovaný domov | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | Obe                  |
| `orbit launch`         | Claude Code                    | Nič — spúšťa `claude` s `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` injikovaným                                                                                | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | Obe                  |
| `orbit launch-codex`   | OpenAI Codex CLI               | Nič — spúšťa `codex` s poskytovateľom `orbit` injikovaným cez `-c` flagy                                                                                   | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | Obe                  |

Poznámky k flagom (overené v zdroji príkazu):

- `--remote <url>` — načíta katalóg z vzdialeného Orbit (prepíše `--port`
  a aktívny kontext). `--api-key <key>` poskytuje poverenie pre ten
  server (predvolene na `ORBIT_API_KEY` env var, alebo token aktívneho kontextu).
...
- `--model <id>` na `orbit run` nasleduje wiring per-target z manifestu
  (`bin/cli/cli-manifest.mjs`): **aider** dostáva `--model openai/<id>` a
  **opencode** `--model orbit/<id>` (prefix sa pridáva iba vtedy, keď id
  ho už nemá); **qwen** a **gemini** dostávajú id verbatim;
  **claude** ho dostáva cez `ANTHROPIC_MODEL`, **goose** cez `GOOSE_MODEL`, a
  **codex** cez `-c model_providers.orbit.*` args. **Qwen je jediným spúšťacím
  cieľom, ktorý tvrdohlavo vyžaduje `--model`** — `orbit run qwen` bez neho skončí
  s chybou `2`.
- `--port <port>` — lokálny port Orbit (predvolene `20128`, ignorovaný pri nastavení `--remote`).
  Prítomné na všetkých `setup-*` a oboch spúšťačoch.
- `orbit run` kódy ukončenia: vlastný kód ukončenia dieťaťa CLI je propagovaný
...
> Existuje aj bohatšia pluginová integrácia — `orbit setup opencode` — ktorá
> inštaluje `@orbit/opencode-plugin`. Sú to rôzne príkazy; tabuľka
...
S Orbit bežiacim na `localhost:20128`, jednoducho spustite príkaz na nastavenie pre váš nástroj. Katalóg sa načíta z lokálneho servera.
...
export ORBIT_API_KEY=sk-...  # odkazované cez {env:ORBIT_API_KEY}, nikdy na disku
...
```bash
orbit launch                 # Claude Code → lokálny Orbit
orbit launch-codex           # Codex CLI → lokálny Orbit
orbit launch-codex --profile glm52
orbit run claude --model openai/gpt-5.4
orbit run codex --model openai/gpt-5.4 --dry-run --json
orbit run aider --model glm/glm-5.2 -- --message "odpoveď OK"
orbit run goose --model glm/glm-5.2
orbit run opencode --model glm/glm-5.2 -- run "odpoveď OK"
orbit run qwen --model glm/glm-5.2 -- -p "odpoveď OK"
orbit run gemini --model glm/glm-5.2 -- --skip-trust -p "odpoveď OK"

# Explicitná cesta príkazu: prejdite cez čokoľvek, čo príde po --
orbit run claude -- --print-system-prompt "skontrolujte tento diff"
```
...
Nasmerujte akýkoľvek príkaz na nastavenie na diaľkový Orbit s `--remote` + `--api-key`. Katalóg sa načíta z diaľky; konfigurácia sa zapisuje na vašom lokálnom počítači.
...
orbit setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m orbit/glm/glm-5.2 "..."   # najprv exportujte ORBIT_API_KEY
...
```bash
orbit connect 192.168.0.15        # vytvorí obmedzený token, uloží kontext
orbit setup-codex                 # ← teraz používa diaľkový katalóg
orbit setup-opencode              # ← rovnaké
orbit launch                      # ← Claude Code proti diaľke
```
...
Orbit vystavuje OpenAI rozhranie na `/v1`, Anthropic rozhranie na root,
...
| `setup-codex`, `launch-codex` (`model_providers.orbit.base_url`)       | so `/v1`              | Áno                                          |
...
Keď aktualizujete pomocou `orbit update` (po potvrdení alebo s `--apply`),
Orbit spúšťa inštaláciu s `--include=optional` zabudovaným:

```bash
npm install -g orbit@latest --include=optional
```

Toto **nie je** flag, ktorý prechádzate do `orbit update` — vždy sa aplikuje
...
# [DRY RUN] Spustil by: npm install -g orbit@latest --include=optional
```

Iné flagy `orbit update` (overené v zdroji): `--check` (výstup 1, ak je
zastaralý), `--apply` (inštalácia bez výzvy), `--changelog`, `--no-backup`,
`--yes`.

---

## Google Gemini CLI cez `orbit run gemini`
...
Gemini povrch Orbit (`/v1beta`). `orbit run gemini` to automaticky
prepojí:

- `GOOGLE_GEMINI_BASE_URL` → aktívna základná URL Orbit (root, bez `/v1`);
- `GEMINI_API_KEY` → vyriešené poverenie Orbit (možnosť/env/kontекст);
- **dočasný izolovaný `GEMINI_CLI_HOME`**, ktorého `.gemini/settings.json`
  ...
  nikdy neprepisuje spustenie riadené Orbit — odstránené po ukončení;
...
proti SKUTOČNEMU serveru Orbit existuje opt-in rámec na
...
Voliteľné: `ORBIT_SMOKE_TARGETS="codex,opencode,qwen"` obmedzuje testovanie;
`ORBIT_SMOKE_TIMEOUT_MS` prepisuje timeout 120s na cieľ.

## Pozri tiež

- [Claude Code konfigurácia](./CLAUDE-CODE-CONFIGURATION.md) — hlbší sprievodca Claude Code
- [Codex CLI konfigurácia](./CODEX-CLI-CONFIGURATION.md) — jednorazové nastavenie `[model_providers.orbit]`
