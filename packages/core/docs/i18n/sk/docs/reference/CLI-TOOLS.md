...
Orbit integruje tri kategórie CLI nástrojov rozložené na troch špecializovaných stránkach dashboardu:

| Stránka        | Trasa                   | Koncept                                                                                       | Počet             |
| -------------- | ----------------------- | --------------------------------------------------------------------------------------------- | ----------------- |
| **CLI Kód**    | `/dashboard/cli-code`   | Nástroje na kódovanie, ktoré smerujete na Orbit (Klient → CLI → Orbit → Poskytovateľ) | 26                |
| **CLI Agenti** | `/dashboard/cli-agents` | Autonómni agenti, ktorých smerujete na Orbit (rovnaký tok, širší rozsah)                  | 8                 |
| **ACP Agenti** | `/dashboard/acp-agents` | CLI, ktoré Orbit spúšťa ako backend cez stdio/ACP (opačný tok)                            | pozri registráciu |
...
Claude / Codex / OpenCode / Cline / KiloCode / Continue / Hermes Agent / Goose / ...
           │
           ▼  (všetky smerujú na Orbit)
    http://YOUR_SERVER:20128/v1
           │
           ▼  (Orbit smeruje k správnemu poskytovateľovi)
    Anthropic / OpenAI / Gemini / DeepSeek / Groq / Mistral / ...

ACP Agenti (opačný tok spúšťania):
    Klientsky požiadavok → Orbit → spúšťa CLI cez stdio/ACP → odpoveď
```
...
Nemusíte písať konfiguráciu každého nástroja ručne. Orbit dodáva príkaz `setup-*`
pre každý podporovaný CLI, ktorý číta **živý** katalóg modelov z bežiaceho
Orbit (lokálne alebo vzdialene) a zapisuje vlastnú konfiguráciu nástroja na vašom počítači:
...
vzdialenému Orbit), `--dry-run` (náhľad bez zápisu) a `--port`. Nástroje
...
spúšťače pre každý nástroj `orbit launch` (Claude Code) a `orbit launch-codex`
(Codex) zostávajú k dispozícii. Gemini CLI je len na spúšťanie: je to cieľ
`orbit run`, ale nemá recept `setup-*`/`configure`.
...
Príkaz `setup-*` vykonaný vo vnútri kontajnera Orbit zapisuje do
vlastného domova kontajnera, ktorý žiadny hostiteľský CLI nečíta a ktorý zmizne s
kontajnerom. Orbit to zistí a ukončí s kódom `2` s pokynmi namiesto
zápisu. Dva podporované spôsoby — nainštalovať CLI na hostiteľovi a
`orbit connect` do kontajnera, alebo pripojiť konfiguračné adresáre a nastaviť
`CLI_CONFIG_HOME` (profil compose `host`). Každý príkaz `setup-*`, plus
`orbit configure` a `orbit config set`, akceptuje
`--allow-container-write`, keď konfigurácia vlastných CLI kontajnera je to, čo ste
naozaj mysleli; `ORBIT_ALLOW_CONTAINER_CONFIG_WRITE=true` robí to isté pre
server. Pozrite sa na
[Docker Príručka → Konfigurácia hostiteľských CLI nástrojov](../guides/DOCKER_GUIDE.md#configuring-host-cli-tools-when-orbit-runs-in-docker).
...
kilo, continue) — príkazom `hostSetupCommand` (napr. `orbit setup-opencode`), ktorý sa má vykonať
...
| **Konfigurovateľné** | Podporované `orbit configure <cli>` (existuje recept na nastavenie)               | `bin/cli/cli-manifest.mjs` (`configure: true`)                    |
| **Spustiteľné**      | Podporované `orbit run <target>` (definovaná injekcia env/args)                   | `bin/cli/cli-manifest.mjs` (`run: true`)                          |
...
Táto stránka (prezvaná z `/dashboard/agents`) zobrazuje CLI, ktoré môže Orbit **vytvoriť** ako backendové vykonávacie motory prostredníctvom protokolu stdio/ACP. Katalóg je spravovaný samostatne v `src/lib/acp/registry.ts` a **nie** je to isté ako `CLI_TOOLS`.
...
### Krok 1 — Získajte API kľúč Orbit
...
# Google Gemini CLI (spustiteľné cez `orbit run gemini` → /v1beta surface)
...
kľúč zostáva v premennej prostredia `ORBIT_API_KEY` (`env_key`), nikdy
v súbore:
...
> Použite `opencode run "your prompt" --model orbit/claude-sonnet-4-5-thinking --variant high`
...
Alebo použite dashboard Orbit → **CLI Tools → Cline → Použiť konfiguráciu**.
...
Alebo použite dashboard Orbit → **CLI Tools → KiloCode → Použiť konfiguráciu**.
...
Použite toto, keď je VS Code Insiders nakonfigurovaný pre vlastné modely koncových bodov a chcete, aby Orbit fungoval bez vlastného poľa hlavičky.
...
**Príklad s tokenizovaným aliasom Orbit:**
...
- Nahraďte `sk-your-orbit-key` API kľúčom vytvoreným v Orbit.
...
# CLI používa svoju vlastnú autentifikáciu — Orbit nie je potrebný ako backend pre Kiro CLI samotný.
# Použite kiro-cli spolu s Orbit pre iné nástroje.
kiro-cli status
```

Pre desktopovú aplikáciu **Kiro IDE** použite MITM koncový bod vystavený Orbit
pod `/dashboard/cli-tools → Kiro`.

---

## 10. Interný Orbit CLI

Binárny súbor `orbit` poskytuje príkazy pre životný cyklus servera, nastavenie, diagnostiku a správu poskytovateľov. Vstupný bod: `bin/orbit.mjs`.

```bash
orbit                              # Spustiť server (predvolený port 20128)
orbit setup                        # Interaktívny sprievodca nastavením
orbit doctor                       # Skontrolovať konfiguráciu, DB, porty, runtime
orbit providers list               # Konfigurované pripojenia poskytovateľov
orbit providers test-all           # Otestovať každé aktívne pripojenie
orbit reset-password               # Obnoviť heslo administrátora
orbit logs                         # Streamovať protokoly požiadaviek
orbit health                       # Podrobné zdravie (prerušovače, cache, pamäť)
orbit --version                    # Vytlačiť verziu
orbit --help                       # Zobraziť všetky príkazy
```
...
```bash
orbit setup                        # Interaktívny sprievodca nastavením
orbit setup --non-interactive      # CI/automatizačný režim (číta env premenné + prapory)
orbit setup --password '<value>'   # Nastaviť heslo administrátora priamo
...
| Var                 | Účel                                                                   |
| ------------------- | ---------------------------------------------------------------------- |
| `ORBIT_API_KEY` | API kľúč poskytovateľa (viazaný na `--api-key` cez Commander `.env()`) |
| `DATA_DIR`          | Prepisuje adresár dát Orbit                                        |

Všetky ostatné neinteraktívne vstupy sú odovzdávané ako prapory, nie environmentálne premenné:
`--password`, `--provider`, `--provider-name`, `--provider-base-url`, `--default-model`
(pozri možnosti `orbit setup` vyššie).

### Diagnostika

```bash
orbit doctor                       # Skontrolovať konfiguráciu, DB, porty, runtime, pamäť, životnosť
orbit doctor --json                # Strojovo čitateľný JSON
orbit doctor --no-liveness         # Preskočiť HTTP health probe
orbit doctor --host 0.0.0.0        # Prepisovať hostiteľov životnosti
orbit doctor --liveness-url <url>  # Úplný URL prepis koncového bodu zdravia
```
...
```bash
orbit providers available                       # Katalóg poskytovateľov Orbit
orbit providers available --search openai       # Filtrovať katalóg podľa id/názvu/aliasu/kategórie
orbit providers available --category api-key    # Filtrovať podľa kategórie (api-key, oauth, free, ...)
orbit providers available --json                # Strojovo čitateľný JSON

orbit providers list                            # Konfigurované pripojenia poskytovateľov
orbit providers list --json

orbit providers test <id|name>                  # Otestovať jedno konfigurované pripojenie
orbit providers test-all                        # Otestovať každé aktívne pripojenie
orbit providers validate                        # Lokálna štrukturálna validácia
orbit providers add <provider> --credential-env PROVIDER_KEY
orbit providers import ./providers.json --dry-run --json
orbit providers auth <provider>                 # Existujúci OAuth tok
...
redigovanú prítomnosť/tvar. `providers available` číta katalóg Orbit;
...
```bash
orbit reset-password                # Obnoviť heslo administrátora (tiež: orbit-reset-password)
orbit reset-encrypted-columns       # Zobraziť varovanie + suchý beh pre reset šifrovaných poverení
orbit reset-encrypted-columns --force  # Skutočne nulovať šifrované poverenia v SQLite
```

### Export poverení (⚠ zaobchádzajte opatrne)

```bash
orbit auth export                                 # Zobraziť varovanie + bránu potvrdenia — žiadny prístup k DB
orbit auth export --force                          # ExportOVAŤ VŠETKY DEŠIFROVANÉ poverenia pripojení do stdout ako JSON
orbit auth export --force --id <id>                 # Exportovať iba zodpovedajúce pripojenie
orbit auth export --force --format env               # Vydávať riadky ORBIT_<PROVIDER>_<FIELD>=<value>
orbit auth export --force --out creds.json           # Zapísať do súboru (vytvoreného s 0600 povoleniami)
```
...
Tieto predpokladajú bežiaci server Orbit, pokiaľ nie je uvedené inak:

```bash
orbit status                       # Komplexný stav runtime
orbit logs                         # Streamovať protokoly požiadaviek (--json, --search, --follow)
orbit config show                  # Zobraziť aktuálnu konfiguráciu

orbit provider list                # Zoznam dostupných poskytovateľov (alias príkazu providers list)
orbit provider add                 # Registrovať Orbit ako poskytovateľa na nástroji
orbit keys add | list | remove     # Spravovať API kľúče
orbit models [provider]            # Zoznam modelov (--json, --search)
orbit combo list | switch | create | delete

orbit backup                       # Snapshot konfigurácie + DB
orbit restore                      # Obnoviť z predchádzajúceho snapshotu

orbit health                       # Podrobné zdravie (prerušovače, cache, pamäť)
orbit quota                        # Využitie kvóty poskytovateľa
orbit cache                        # Stav cache
orbit cache clear                  # Vyčistiť sémantické + podpisové cache

orbit mcp status | restart         # Stav servera MCP / reštart
orbit a2a status | card            # Stav servera A2A / karta agenta

orbit tunnel list | create | stop  # Spravovať tunely (cloudflare/tailscale/ngrok)
orbit env show | get <k> | set <k> <v>  # Skontrolovať / nastaviť env premenné (dočasné)

orbit test                         # Test konektivity poskytovateľa
orbit update                       # Skontrolovať aktualizácie
orbit completion                   # Generovať dokončenie shellu
```
...
Príklady pripravené na vloženie s tokenizovanou Orbit URL:
...
| ------------------------------------------------ | ---------------------------- | --------------------------------------------------------- |
| `Connection refused`                             | Orbit nebeží             | `orbit serve`                                         |
