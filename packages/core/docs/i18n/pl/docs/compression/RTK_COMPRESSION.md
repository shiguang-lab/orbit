...
`rtkEngine` (`@orbit/inference/services/compression/engines/rtk`) jest
`CompressionEngine` i nie ma metody `updateConfig`. Aktualizuj konfigurację silnika
przez helper rejestru:

```ts
import { updateEngineConfig } from "@orbit/inference/services/compression/engines/registry";
...
```ts
import { loadRtkFilters } from "@orbit/inference/services/compression/engines/rtk/filterLoader";
