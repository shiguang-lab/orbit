...
Osobny workspace npm (`@orbit/inference`). Obsługuje przetwarzanie żądań + wykonanie u providerów.
...
| `cli/{args,data-dir,encryption,io,providers,provider-store,provider-test,settings-store,sqlite}.mjs` | Moduły helperów CLI                                                                                                           |
