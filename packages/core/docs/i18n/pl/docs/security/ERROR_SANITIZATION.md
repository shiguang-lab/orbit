...
```ts
import { buildErrorBody } from "@orbit/inference/utils/error.ts";
...
} from "@orbit/inference/utils/error.ts";
```
...
```ts
import { sanitizeErrorMessage } from "@orbit/inference/utils/error.ts";
...
- Konfiguracja redakcji `pino` (`packages/utils/src/logging/redaction.ts`) obsługuje redakcję logów strukturalnych osobno. Ten dokument obejmuje wyłącznie powierzchnię komunikatów w odpowiedziach.
