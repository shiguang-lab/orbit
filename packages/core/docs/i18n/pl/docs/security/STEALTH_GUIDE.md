...
Orbit integruje się z providerami, których edge aktywnie fingerprintuje nieoficjalne klienty (TLS JA3/JA4, kolejność nagłówków, kształt body JSON, tokeny integralności). Ta strona dokumentuje powierzchnie stealth, które Orbit udostępnia, oraz miejsca ich implementacji.

## Uwaga prawna i etyczna

Funkcje stealth istnieją po to, by Orbit mógł działać jako warstwa kompatybilności między oficjalnymi kontami użytkownika (Claude Code CLI, Codex, Antigravity, Cursor itd.) a ujednoliconym API Orbit. **Nie** służą do omijania fraud detection, współdzielenia poświadczeń ani naruszania Terms of Service providera. Maintainerzy oczekują, że operatorzy będą przestrzegać upstream ToS, które zaakceptowali przy tworzeniu kont.
...
Gdy `cliCompatMode` jest włączony, Orbit przekształca wychodzące żądania Claude tak, by były nieodróżnialne od ruchu `claude-cli`. Współpracują trzy moduły:
...
Server-side check integralności, który oficjalne Claude Code CLI liczy przez Bun/Zig. Orbit reimplementuje to z `xxhash-wasm`:
...
Żądania Antigravity zachowują tekst wywołującego bajt po bajcie. Orbit nie wstawia zero-width characters do promptów ani nie zmienia nazw/nie wstrzykuje tooli, by udawać klienta IDE.
...
Egzekucja upstream jest po **stronie Google**, nie ma nic, co Orbit mógłby temu zapobiec. Nazwa zmiennej env i istniejąca dokumentacja sprawiają wrażenie bezpiecznego przełącznika; nim nie jest.
...
- Wielu użytkowników Orbit uderzających w overage credits równolegle z tego samego zewnętrznego IP wzmacnia sygnał.
...
- Oryginalny raport incydentu: Discussion [#1183](https://github.com/diegosouzapw/Orbit/discussions/1183)
...
Dla CLI, których binarów nie da się przekierować przez `OPENAI_BASE_URL`, Orbit uruchamia lokalny proxy z terminacją TLS. Endpointy są pod `src/app/api/cli-tools/antigravity-mitm/`.
...
Nazwa pliku cert: `orbit-mitm.crt`. Dopasowanie fingerprintu przez `getCertFingerprint()` (SHA-1 z DER).

Dodatkowo `updateNssDatabases()` instaluje do per-user NSS DB, gdy dostępne jest `certutil`: `~/.pki/nssdb`, `~/snap/chromium/.../nssdb`, wszystkie profile Firefox (w tym snap), pod nickiem **`Orbit MITM Root CA`**.
...
Orbit czyści inbound nagłówki klienta przed forwardem, by żądanie przychodzące z Cursor nie wyciekało `User-Agent: Cursor/X.Y.Z` do upstream Claude. Zobacz `src/shared/constants/upstreamHeaders.ts` — denylist utrzymywany w lockstep ze schematami Zod i testami jednostkowymi.
