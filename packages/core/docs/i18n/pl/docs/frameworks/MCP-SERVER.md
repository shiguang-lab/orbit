---
title: "Dokumentacja serwera MCP Orbit"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Dokumentacja serwera MCP Orbit
...
Orbit MCP jest wbudowany. Uruchom go poleceniem:
...
| :------------------------------ | :-------------------- | :----------------------------------------------------------------- |
| `orbit_get_health`          | `read:health`         | Uptime, pamięć, circuit breakery, limity rate, statystyki cache    |
| `orbit_list_combos`         | `read:combos`         | Wszystkie skonfigurowane combo ze strategiami (opcjonalne metryki) |
| `orbit_get_combo_metrics`   | `read:combos`         | Metryki wydajności dla konkretnego combo                           |
| `orbit_switch_combo`        | `write:combos`        | Aktywacja lub deaktywacja combo                                    |
| `orbit_check_quota`         | `read:quota`          | Quota used/total, procent pozostały, czas resetu, health tokenów   |
| `orbit_route_request`       | `execute:completions` | Wysłanie chat completion przez routing Orbit                   |
| `orbit_cost_report`         | `read:usage`          | Raport kosztów wg okresu (session/day/week/month)                  |
| `orbit_list_models_catalog` | `read:models`         | Pełny katalog modeli z capabilities, statusem i pricing            |

## Phase 1 — Search

| Tool                   | Scopes           | Description                                                                                                                            |
| :--------------------- | :--------------- | :------------------------------------------------------------------------------------------------------------------------------------- |
| `orbit_web_search` | `execute:search` | Wyszukiwanie w sieci przez bramkę search Orbit (Serper/Brave/Perplexity/Exa/Tavily/Google PSE/Linkup/SearchAPI/SearXNG) z failover |

## Advanced Tools (11) — Phase 2

| Tool                               | Scopes                               | Description                                                                                   |
| :--------------------------------- | :----------------------------------- | :-------------------------------------------------------------------------------------------- |
| `orbit_simulate_route`         | `read:health`, `read:combos`         | Symulacja routingu dry-run z drzewem fallback                                                 |
| `orbit_set_budget_guard`       | `write:budget`                       | Budżet sesji z akcją degrade/block/alert                                                      |
| `orbit_set_routing_strategy`   | `write:combos`                       | Aktualizacja strategii combo w runtime (priority/weighted/auto itd.)                          |
| `orbit_set_resilience_profile` | `write:resilience`                   | Zastosowanie presetu resilience `aggressive` / `balanced` / `conservative`                    |
| `orbit_test_combo`             | `execute:completions`, `read:combos` | Live test każdego providera w combo przez realne wywołanie upstream                           |
| `orbit_get_provider_metrics`   | `read:health`                        | Metryki per provider z latencją p50/p95/p99 i stanem circuit breakera                         |
| `orbit_best_combo_for_task`    | `read:combos`, `read:health`         | Rekomendacja combo wg typu zadania z ograniczeniami budżetu/latencji                          |
| `orbit_explain_route`          | `read:health`, `read:usage`          | Wyjaśnienie, dlaczego request poszedł do providera (czynniki scoringu + fallbacki)            |
| `orbit_get_session_snapshot`   | `read:usage`                         | Pełny snapshot sesji: koszt, tokeny, top modele/providerzy, błędy, budget guard               |
| `orbit_db_health_check`        | `read:health`, `write:resilience`    | Diagnoza (i opcjonalna auto-naprawa) driftu bazy, np. uszkodzone refy combo / wiersze-sieroty |
| `orbit_sync_pricing`           | `pricing:write`                      | Synchronizacja danych cenowych ze źródeł zewnętrznych (LiteLLM); obsługuje `dryRun`           |

## Cache Tools (2)

| Tool                    | Scopes        | Description                                           |
| :---------------------- | :------------ | :---------------------------------------------------- |
| `orbit_cache_stats` | `read:cache`  | Statystyki semantic cache, prompt-cache i idempotency |
| `orbit_cache_flush` | `write:cache` | Flush cache globalnie albo wg signature/model         |

## Compression Tools (13)

| Tool                                | Scopes              | Description                                                                                                                       |
| :---------------------------------- | :------------------ | :-------------------------------------------------------------------------------------------------------------------------------- |
| `orbit_compression_status`      | `read:compression`  | Ustawienia kompresji, podsumowanie analytics oraz statystyki cache-aware (zawiera metadane `analytics.mcpDescriptionCompression`) |
| `orbit_compression_configure`   | `write:compression` | Konfiguracja trybu kompresji, progu, target ratio, zachowania system-prompt oraz przełącznika kompresji opisów MCP                |
| `orbit_set_compression_engine`  | `write:compression` | Wybór aktywnego silnika (off/caveman/rtk/stacked) oraz intensywności Caveman/RTK                                                  |
| `orbit_list_compression_combos` | `read:compression`  | Lista nazwanych compression combo i ich pipeline'ów silników                                                                      |
| `orbit_compression_combo_stats` | `read:compression`  | Analytics pogrupowane wg compression combo i silnika                                                                              |
| `orbit_ccr_store`               | `write:compression` | Zapis izolowanej per-caller treści w ograniczonym in-memory magazynie CCR i zwrot markera plus referencji `ccr://`                |
| `orbit_ccr_retrieve`            | `read:compression`  | Odczyt treści CCR w całości albo w trybach head, tail, lines, grep i stats                                                        |
| `orbit_ccr_inspect`             | `read:compression`  | Podgląd metadanych CCR należących do callera bez zwracania treści                                                                 |
| `orbit_ccr_list`                | `read:compression`  | Stronicowana lista metadanych bloków CCR należących do callera                                                                    |
| `orbit_ccr_delete`              | `write:compression` | Usunięcie bloku CCR należącego do callera                                                                                         |
| `orbit_ccr_stats`               | `read:compression`  | Raport użycia pamięci w zakresie callera, liczników cyklu życia i limitów magazynu                                                |
| `orbit_rtk_discover`            | `read:compression`  | Wykrywanie powtarzającego się szumu w opt-in próbkach wyjścia RTK                                                                 |
| `orbit_rtk_learn`               | `read:compression`  | Generowanie draftu filtra RTK do przeglądu na podstawie opt-in próbek                                                             |
...
`orbit_compression_status` raportuje kompresję opisów MCP osobno pod
...
Osobno od powyższych narzędzi kompresji Orbit zawiera filtr post-execution, który
...
| Tool                        | Scopes         | Description                                                                            |
| :-------------------------- | :------------- | :------------------------------------------------------------------------------------- |
| `orbit_oneproxy_fetch`  | `read:proxies` | Pobranie darmowych proxy z marketplace 1proxy (filtry protocol/country/quality/limit)  |
| `orbit_oneproxy_rotate` | `read:proxies` | Pobranie kolejnego dostępnego proxy wg strategii (`random` / `quality` / `sequential`) |
| `orbit_oneproxy_stats`  | `read:proxies` | Statystyki poola, status sync, rozkład wg protocol i country                           |
...
| Tool                      | Scopes         | Description                                                                                    |
| :------------------------ | :------------- | :--------------------------------------------------------------------------------------------- |
| `orbit_memory_search` | `read:memory`  | Wyszukiwanie pamięci po query / type / API key z egzekwowaniem token-budget                    |
| `orbit_memory_add`    | `write:memory` | Dodanie nowego wpisu pamięci (`factual` / `episodic` / `procedural` / `semantic`)              |
| `orbit_memory_clear`  | `write:memory` | Czyszczenie pamięci dla klucza API, opcjonalnie filtrowane po type lub timestampie `olderThan` |
...
| Tool                          | Scopes           | Description                                                                             |
| :---------------------------- | :--------------- | :-------------------------------------------------------------------------------------- |
| `orbit_skills_list`       | `read:skills`    | Lista zarejestrowanych skills z opcjonalnym filtrem po API key, name lub stanie enabled |
| `orbit_skills_enable`     | `write:skills`   | Włączenie lub wyłączenie konkretnego skilla po ID                                       |
| `orbit_skills_execute`    | `execute:skills` | Wykonanie skilla z podanym inputem i zwrot rekordu wykonania                            |
| `orbit_skills_executions` | `read:skills`    | Lista niedawnej historii wykonań skills                                                 |
...
| Tool                              | Scopes         | Description                                                                                                         |
| :-------------------------------- | :------------- | :------------------------------------------------------------------------------------------------------------------ |
| `orbit_agent_skills_list`     | `read:catalog` | Lista wszystkich 42 agent skills z opcjonalnymi filtrami `category` (api\|cli) i `area`; zwraca metadata + coverage |
| `orbit_agent_skills_get`      | `read:catalog` | Pełne metadata + treść SKILL.md dla pojedynczego skilla po kanonicznym `id`                                         |
| `orbit_agent_skills_coverage` | `read:catalog` | Statystyki coverage: ile z 22 API i 20 CLI skills ma pliki SKILL.md na filesystemie względem sum katalogu           |
...
Orbit przez ten sam model połączeń co providery LLM. Są wystawione przez
...
fallbackami `_meta` i env `ORBIT_MCP_SCOPES` — ta zmiana tylko zapełnia to pierwsze,
...
istniejący łańcuch `meta`/env bez zmian. To NIE zmienia domyślnej wartości `ORBIT_MCP_ENFORCE_SCOPES`
...
| :-------------------------------------- | :--------------------------------- | :------------------------------------------------------------------------------------------------------------ |
| `ORBIT_BASE_URL`                    | `http://localhost:20128`           | Bazowy URL używany przez serwer MCP przy wywołaniach wewnętrznych API Orbit                               |
| `ORBIT_API_KEY`                     | (empty)                            | Klucz API przekazywany jako `Authorization: Bearer` do wewnętrznych wywołań API                               |
| `ORBIT_MCP_ENFORCE_SCOPES`          | `false` (only `"true"` enables it) | Po włączeniu brakujące scope'y odmawiają wywołań tooli i logują `scope_denied:<reason>` w audycie             |
| `ORBIT_MCP_SCOPES`                  | (empty)                            | Lista scope'ów rozdzielona przecinkami uważanych domyślnie za „dostępne” (gdy caller nie podaje własnych)     |
| `ORBIT_MCP_COMPRESS_DESCRIPTIONS`   | (unset = on)                       | Ustawienie na `0/false/off/no` wyłącza kompresję opisów MCP w czasie rejestracji                              |
| `ORBIT_MCP_DESCRIPTION_COMPRESSION` | (unset = on)                       | Alternatywny alias tego samego przełącznika co powyżej                                                        |
...
| `DATA_DIR`                              | `~/.orbit`                     | Plik heartbeat jest zapisywany do `${DATA_DIR}/runtime/mcp-heartbeat.json`                                    |
...
- Przełącznik process-wide przez `ORBIT_MCP_COMPRESS_DESCRIPTIONS=false` albo `ORBIT_MCP_DESCRIPTION_COMPRESSION=false`.
- Statystyki realtime są dostępne przez `orbit_compression_status` pod `analytics.mcpDescriptionCompression` i oznaczone `source: "mcp_metadata_estimate"`, aby odróżnić je od rzeczywistych rachunków użycia providera.
