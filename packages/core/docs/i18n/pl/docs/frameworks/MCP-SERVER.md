---
title: "Dokumentacja serwera MCP ShiguangGateway"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Dokumentacja serwera MCP ShiguangGateway
...
ShiguangGateway MCP jest wbudowany. Uruchom go poleceniem:
...
| :------------------------------ | :-------------------- | :----------------------------------------------------------------- |
| `shiguang-gateway_get_health`          | `read:health`         | Uptime, pamięć, circuit breakery, limity rate, statystyki cache    |
| `shiguang-gateway_list_combos`         | `read:combos`         | Wszystkie skonfigurowane combo ze strategiami (opcjonalne metryki) |
| `shiguang-gateway_get_combo_metrics`   | `read:combos`         | Metryki wydajności dla konkretnego combo                           |
| `shiguang-gateway_switch_combo`        | `write:combos`        | Aktywacja lub deaktywacja combo                                    |
| `shiguang-gateway_check_quota`         | `read:quota`          | Quota used/total, procent pozostały, czas resetu, health tokenów   |
| `shiguang-gateway_route_request`       | `execute:completions` | Wysłanie chat completion przez routing ShiguangGateway                   |
| `shiguang-gateway_cost_report`         | `read:usage`          | Raport kosztów wg okresu (session/day/week/month)                  |
| `shiguang-gateway_list_models_catalog` | `read:models`         | Pełny katalog modeli z capabilities, statusem i pricing            |

## Phase 1 — Search

| Tool                   | Scopes           | Description                                                                                                                            |
| :--------------------- | :--------------- | :------------------------------------------------------------------------------------------------------------------------------------- |
| `shiguang-gateway_web_search` | `execute:search` | Wyszukiwanie w sieci przez bramkę search ShiguangGateway (Serper/Brave/Perplexity/Exa/Tavily/Google PSE/Linkup/SearchAPI/SearXNG) z failover |

## Advanced Tools (11) — Phase 2

| Tool                               | Scopes                               | Description                                                                                   |
| :--------------------------------- | :----------------------------------- | :-------------------------------------------------------------------------------------------- |
| `shiguang-gateway_simulate_route`         | `read:health`, `read:combos`         | Symulacja routingu dry-run z drzewem fallback                                                 |
| `shiguang-gateway_set_budget_guard`       | `write:budget`                       | Budżet sesji z akcją degrade/block/alert                                                      |
| `shiguang-gateway_set_routing_strategy`   | `write:combos`                       | Aktualizacja strategii combo w runtime (priority/weighted/auto itd.)                          |
| `shiguang-gateway_set_resilience_profile` | `write:resilience`                   | Zastosowanie presetu resilience `aggressive` / `balanced` / `conservative`                    |
| `shiguang-gateway_test_combo`             | `execute:completions`, `read:combos` | Live test każdego providera w combo przez realne wywołanie upstream                           |
| `shiguang-gateway_get_provider_metrics`   | `read:health`                        | Metryki per provider z latencją p50/p95/p99 i stanem circuit breakera                         |
| `shiguang-gateway_best_combo_for_task`    | `read:combos`, `read:health`         | Rekomendacja combo wg typu zadania z ograniczeniami budżetu/latencji                          |
| `shiguang-gateway_explain_route`          | `read:health`, `read:usage`          | Wyjaśnienie, dlaczego request poszedł do providera (czynniki scoringu + fallbacki)            |
| `shiguang-gateway_get_session_snapshot`   | `read:usage`                         | Pełny snapshot sesji: koszt, tokeny, top modele/providerzy, błędy, budget guard               |
| `shiguang-gateway_db_health_check`        | `read:health`, `write:resilience`    | Diagnoza (i opcjonalna auto-naprawa) driftu bazy, np. uszkodzone refy combo / wiersze-sieroty |
| `shiguang-gateway_sync_pricing`           | `pricing:write`                      | Synchronizacja danych cenowych ze źródeł zewnętrznych (LiteLLM); obsługuje `dryRun`           |

## Cache Tools (2)

| Tool                    | Scopes        | Description                                           |
| :---------------------- | :------------ | :---------------------------------------------------- |
| `shiguang-gateway_cache_stats` | `read:cache`  | Statystyki semantic cache, prompt-cache i idempotency |
| `shiguang-gateway_cache_flush` | `write:cache` | Flush cache globalnie albo wg signature/model         |

## Compression Tools (13)

| Tool                                | Scopes              | Description                                                                                                                       |
| :---------------------------------- | :------------------ | :-------------------------------------------------------------------------------------------------------------------------------- |
| `shiguang-gateway_compression_status`      | `read:compression`  | Ustawienia kompresji, podsumowanie analytics oraz statystyki cache-aware (zawiera metadane `analytics.mcpDescriptionCompression`) |
| `shiguang-gateway_compression_configure`   | `write:compression` | Konfiguracja trybu kompresji, progu, target ratio, zachowania system-prompt oraz przełącznika kompresji opisów MCP                |
| `shiguang-gateway_set_compression_engine`  | `write:compression` | Wybór aktywnego silnika (off/caveman/rtk/stacked) oraz intensywności Caveman/RTK                                                  |
| `shiguang-gateway_list_compression_combos` | `read:compression`  | Lista nazwanych compression combo i ich pipeline'ów silników                                                                      |
| `shiguang-gateway_compression_combo_stats` | `read:compression`  | Analytics pogrupowane wg compression combo i silnika                                                                              |
| `shiguang-gateway_ccr_store`               | `write:compression` | Zapis izolowanej per-caller treści w ograniczonym in-memory magazynie CCR i zwrot markera plus referencji `ccr://`                |
| `shiguang-gateway_ccr_retrieve`            | `read:compression`  | Odczyt treści CCR w całości albo w trybach head, tail, lines, grep i stats                                                        |
| `shiguang-gateway_ccr_inspect`             | `read:compression`  | Podgląd metadanych CCR należących do callera bez zwracania treści                                                                 |
| `shiguang-gateway_ccr_list`                | `read:compression`  | Stronicowana lista metadanych bloków CCR należących do callera                                                                    |
| `shiguang-gateway_ccr_delete`              | `write:compression` | Usunięcie bloku CCR należącego do callera                                                                                         |
| `shiguang-gateway_ccr_stats`               | `read:compression`  | Raport użycia pamięci w zakresie callera, liczników cyklu życia i limitów magazynu                                                |
| `shiguang-gateway_rtk_discover`            | `read:compression`  | Wykrywanie powtarzającego się szumu w opt-in próbkach wyjścia RTK                                                                 |
| `shiguang-gateway_rtk_learn`               | `read:compression`  | Generowanie draftu filtra RTK do przeglądu na podstawie opt-in próbek                                                             |
...
`shiguang-gateway_compression_status` raportuje kompresję opisów MCP osobno pod
...
Osobno od powyższych narzędzi kompresji ShiguangGateway zawiera filtr post-execution, który
...
| Tool                        | Scopes         | Description                                                                            |
| :-------------------------- | :------------- | :------------------------------------------------------------------------------------- |
| `shiguang-gateway_oneproxy_fetch`  | `read:proxies` | Pobranie darmowych proxy z marketplace 1proxy (filtry protocol/country/quality/limit)  |
| `shiguang-gateway_oneproxy_rotate` | `read:proxies` | Pobranie kolejnego dostępnego proxy wg strategii (`random` / `quality` / `sequential`) |
| `shiguang-gateway_oneproxy_stats`  | `read:proxies` | Statystyki poola, status sync, rozkład wg protocol i country                           |
...
| Tool                      | Scopes         | Description                                                                                    |
| :------------------------ | :------------- | :--------------------------------------------------------------------------------------------- |
| `shiguang-gateway_memory_search` | `read:memory`  | Wyszukiwanie pamięci po query / type / API key z egzekwowaniem token-budget                    |
| `shiguang-gateway_memory_add`    | `write:memory` | Dodanie nowego wpisu pamięci (`factual` / `episodic` / `procedural` / `semantic`)              |
| `shiguang-gateway_memory_clear`  | `write:memory` | Czyszczenie pamięci dla klucza API, opcjonalnie filtrowane po type lub timestampie `olderThan` |
...
| Tool                          | Scopes           | Description                                                                             |
| :---------------------------- | :--------------- | :-------------------------------------------------------------------------------------- |
| `shiguang-gateway_skills_list`       | `read:skills`    | Lista zarejestrowanych skills z opcjonalnym filtrem po API key, name lub stanie enabled |
| `shiguang-gateway_skills_enable`     | `write:skills`   | Włączenie lub wyłączenie konkretnego skilla po ID                                       |
| `shiguang-gateway_skills_execute`    | `execute:skills` | Wykonanie skilla z podanym inputem i zwrot rekordu wykonania                            |
| `shiguang-gateway_skills_executions` | `read:skills`    | Lista niedawnej historii wykonań skills                                                 |
...
| Tool                              | Scopes         | Description                                                                                                         |
| :-------------------------------- | :------------- | :------------------------------------------------------------------------------------------------------------------ |
| `shiguang-gateway_agent_skills_list`     | `read:catalog` | Lista wszystkich 42 agent skills z opcjonalnymi filtrami `category` (api\|cli) i `area`; zwraca metadata + coverage |
| `shiguang-gateway_agent_skills_get`      | `read:catalog` | Pełne metadata + treść SKILL.md dla pojedynczego skilla po kanonicznym `id`                                         |
| `shiguang-gateway_agent_skills_coverage` | `read:catalog` | Statystyki coverage: ile z 22 API i 20 CLI skills ma pliki SKILL.md na filesystemie względem sum katalogu           |
...
ShiguangGateway przez ten sam model połączeń co providery LLM. Są wystawione przez
...
fallbackami `_meta` i env `SHIGUANG_GATEWAY_MCP_SCOPES` — ta zmiana tylko zapełnia to pierwsze,
...
istniejący łańcuch `meta`/env bez zmian. To NIE zmienia domyślnej wartości `SHIGUANG_GATEWAY_MCP_ENFORCE_SCOPES`
...
| :-------------------------------------- | :--------------------------------- | :------------------------------------------------------------------------------------------------------------ |
| `SHIGUANG_GATEWAY_BASE_URL`                    | `http://localhost:20128`           | Bazowy URL używany przez serwer MCP przy wywołaniach wewnętrznych API ShiguangGateway                               |
| `SHIGUANG_GATEWAY_API_KEY`                     | (empty)                            | Klucz API przekazywany jako `Authorization: Bearer` do wewnętrznych wywołań API                               |
| `SHIGUANG_GATEWAY_MCP_ENFORCE_SCOPES`          | `false` (only `"true"` enables it) | Po włączeniu brakujące scope'y odmawiają wywołań tooli i logują `scope_denied:<reason>` w audycie             |
| `SHIGUANG_GATEWAY_MCP_SCOPES`                  | (empty)                            | Lista scope'ów rozdzielona przecinkami uważanych domyślnie za „dostępne” (gdy caller nie podaje własnych)     |
| `SHIGUANG_GATEWAY_MCP_COMPRESS_DESCRIPTIONS`   | (unset = on)                       | Ustawienie na `0/false/off/no` wyłącza kompresję opisów MCP w czasie rejestracji                              |
| `SHIGUANG_GATEWAY_MCP_DESCRIPTION_COMPRESSION` | (unset = on)                       | Alternatywny alias tego samego przełącznika co powyżej                                                        |
...
| `DATA_DIR`                              | `~/.shiguang-gateway`                     | Plik heartbeat jest zapisywany do `${DATA_DIR}/runtime/mcp-heartbeat.json`                                    |
...
- Przełącznik process-wide przez `SHIGUANG_GATEWAY_MCP_COMPRESS_DESCRIPTIONS=false` albo `SHIGUANG_GATEWAY_MCP_DESCRIPTION_COMPRESSION=false`.
- Statystyki realtime są dostępne przez `shiguang-gateway_compression_status` pod `analytics.mcpDescriptionCompression` i oznaczone `source: "mcp_metadata_estimate"`, aby odróżnić je od rzeczywistych rachunków użycia providera.
