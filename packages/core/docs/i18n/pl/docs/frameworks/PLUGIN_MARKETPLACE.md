...
Orbit dostarcza system wtyczek w stylu WordPressa. Wtyczki to samodzielne
...
Wtyczki mieszkają w katalogu danych Orbit:

```
~/.orbit/plugins/<plugin-name>/
  ├─ plugin.json
  └─ index.js          # (or whatever manifest.main points to)
```

`getDefaultPluginDir()` (`src/lib/plugins/scanner.ts`) rozwiązuje to do
`<home>/.orbit/plugins`, gdzie `<home>` pochodzi ze zmiennych środowiskowych `HOME` /
...
| `requires`         | object?   | `{ orbit?, permissions[] }`                             |
...
install (POST /api/plugins, path)
  ...
  → atomic rename into ~/.orbit/plugins/<name> → insert DB row
