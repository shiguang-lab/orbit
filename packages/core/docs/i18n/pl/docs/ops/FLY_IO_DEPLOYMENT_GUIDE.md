---
title: "Przewodnik wdrażania Orbit na Fly.io"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Przewodnik wdrażania Orbit na Fly.io

Ten dokument opisuje rzeczywisty proces wdrażania Orbit na Fly.io i obejmuje dwa scenariusze:
...
Przewodnik opiera się na zweryfikowanej, działającej konfiguracji bieżącego projektu. Nazwa aplikacji to `orbit`.
...
- Adres dostępu: `https://orbit.fly.dev/`
...
- `app = 'orbit'` określa, do której aplikacji Fly kierowane jest wdrożenie
...
- Jeśli wcześniej używałeś innej nazwy, np. `oroute`, nie myl jej z `orbit`
...
Jeśli zmieniłeś nazwę aplikacji, zastąp `orbit` wybraną nazwą.
...
Te parametry były używane w rzeczywistych wdrożeniach bieżącej aplikacji `orbit`:
...
- `ORBIT_WS_BRIDGE_SECRET` (wymagany w produkcji — służy do uwierzytelniania mostu WebSocket)
...
| `ORBIT_WS_BRIDGE_SECRET`     | Wymagana w produkcji | Sekret uwierzytelniania mostu WebSocket                    |
...
1. **Ustaw `NEXT_PUBLIC_BASE_URL` na publiczną domenę HTTPS**
   ...
   Jeśli używasz własnej domeny, zastąp ją odpowiednią domeną (np. `https://orbit.yourdomain.com`).
...
- Przeznaczone dla bieżącego projektu `orbit`
...
- `ORBIT_WS_BRIDGE_SECRET` jest wymagany w produkcji; jego brak zepsuje handshake mostu WebSocket
...
- Czy przeglądasz aplikację `orbit`
...
Jeśli bieżące repozytorium jest forkiem i chcesz synchronizować aktualizacje z upstream `https://github.com/diegosouzapw/Orbit`, postępuj według poniższego przepływu.
...
- Przeglądasz inną aplikację (np. `oroute` zamiast `orbit`)
