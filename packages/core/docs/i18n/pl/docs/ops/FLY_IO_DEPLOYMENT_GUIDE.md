---
title: "Przewodnik wdrażania ShiguangGateway na Fly.io"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Przewodnik wdrażania ShiguangGateway na Fly.io

Ten dokument opisuje rzeczywisty proces wdrażania ShiguangGateway na Fly.io i obejmuje dwa scenariusze:
...
Przewodnik opiera się na zweryfikowanej, działającej konfiguracji bieżącego projektu. Nazwa aplikacji to `shiguang-gateway`.
...
- Adres dostępu: `https://shiguang-gateway.fly.dev/`
...
- `app = 'shiguang-gateway'` określa, do której aplikacji Fly kierowane jest wdrożenie
...
- Jeśli wcześniej używałeś innej nazwy, np. `oroute`, nie myl jej z `shiguang-gateway`
...
Jeśli zmieniłeś nazwę aplikacji, zastąp `shiguang-gateway` wybraną nazwą.
...
Te parametry były używane w rzeczywistych wdrożeniach bieżącej aplikacji `shiguang-gateway`:
...
- `SHIGUANG_GATEWAY_WS_BRIDGE_SECRET` (wymagany w produkcji — służy do uwierzytelniania mostu WebSocket)
...
| `SHIGUANG_GATEWAY_WS_BRIDGE_SECRET`     | Wymagana w produkcji | Sekret uwierzytelniania mostu WebSocket                    |
...
1. **Ustaw `NEXT_PUBLIC_BASE_URL` na publiczną domenę HTTPS**
   ...
   Jeśli używasz własnej domeny, zastąp ją odpowiednią domeną (np. `https://shiguang-gateway.yourdomain.com`).
...
- Przeznaczone dla bieżącego projektu `shiguang-gateway`
...
- `SHIGUANG_GATEWAY_WS_BRIDGE_SECRET` jest wymagany w produkcji; jego brak zepsuje handshake mostu WebSocket
...
- Czy przeglądasz aplikację `shiguang-gateway`
...
Jeśli bieżące repozytorium jest forkiem i chcesz synchronizować aktualizacje z upstream `https://github.com/diegosouzapw/ShiguangGateway`, postępuj według poniższego przepływu.
...
- Przeglądasz inną aplikację (np. `oroute` zamiast `shiguang-gateway`)
