...
**Orbit** — unified AI proxy/router. One endpoint, 329 provider catalog entries, auto-fallback when an upstream route is available.
...
Orbit has three related but distinct temporary-failure mechanisms. Keep their
...
repo** instead of polluting the main Orbit tree.
...
- **Data directory**: `DATA_DIR` env var, defaults to `~/.orbit/`
...
Orbit has **~48 quality-gate scripts** (`scripts/check/` + `scripts/quality/`) wired
