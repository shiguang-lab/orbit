...
title: "CLI इंटीग्रेशन — किसी भी कोडिंग CLI को Orbit पर पॉइंट करें"
version: 3.8.50
...
Orbit एक परिवार के `setup-*` कमांड्स के साथ आता है जो एक कोडिंग
CLI (Codex, Claude Code, OpenCode, Cline, …) को Orbit को उसके बैकएंड के रूप में उपयोग करने के लिए कॉन्फ़िगर करता है — ताकि
उपकरण **एक** एंडपॉइंट से बात करे और Orbit सही प्रदाता की ओर रूट करता है
ऑटो-फॉलबैक के साथ। प्रत्येक कमांड एक चल रहे
Orbit (स्थानीय या दूरस्थ) से **लाइव** मॉडल कैटलॉग पढ़ता है और **आपके**
...
एक सामान्य लॉन्चर भी है — `orbit run <target>` — जो
...
`open-code`, `qwen-code`, `gemini-cli`) से प्राप्त किया जाता है, और `orbit completion` वही मैनिफेस्ट-व्युत्पन्न लक्ष्य शब्द प्रदान करता है। पुराने प्रति-उपकरण लॉन्चर —
`orbit launch` (Claude Code) और `orbit launch-codex` (Codex) — उपलब्ध रहते हैं।
...
- [दूरस्थ मोड](./REMOTE-MODE.md) — अपने लैपटॉप से एक दूरस्थ Orbit (VPS / Tailnet) चलाएं
...
हर कमांड **सक्रिय संदर्भ** का सम्मान करता है (जिसे `orbit connect` के साथ सेट किया गया है, देखें
...
| -------------------------- | ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------- |
| `orbit setup-codex`    | OpenAI Codex CLI             | `~/.codex/<name>.config.toml` — एक संगत पाठ मॉडल के लिए एक प्रोफ़ाइल (`codex --profile <name>`)                                                                       | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | दोनों               |
| `orbit setup-claude`   | Claude Code                  | `~/.claude/profiles/<name>/settings.json` — एक मेल खाने वाले मॉडल के लिए एक प्रोफ़ाइल (`CLAUDE_CONFIG_DIR`)                                                           | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | दोनों               |
| `orbit setup-opencode` | OpenCode (openai-compatible) | `~/.config/opencode/opencode.json` — `orbit` प्रदाता के साथ हर कैटलॉग मॉडल (`opencode -m orbit/<model>`)                                                      | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | दोनों               |
| `orbit setup-cline`    | Cline                        | `~/.cline/data/{globalState,secrets}.json` (CLI मोड) + VS Code एक्सटेंशन सेटिंग्स प्रिंट करता है                                                                      | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | दोनों               |
| `orbit setup-kilo`     | Kilo Code                    | `~/.local/share/kilo/auth.json` (CLI) + यदि मौजूद हो तो VS Code `settings.json` में `kilocode.*` को मर्ज करता है                                                      | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | दोनों               |
| `orbit setup-continue` | Continue / `cn` CLI          | `~/.continue/config.yaml` — `provider: openai` मॉडल, कुंजी के माध्यम से `${{ secrets.ORBIT_API_KEY }}`                                                            | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | दोनों               |
| `orbit setup-cursor`   | Cursor                       | कुछ नहीं — ऐप में चरणों को प्रिंट करता है (Cursor कॉन्फ़िगरेशन अपारदर्शी SQLite है)                                                                                   | `--remote` `--api-key` `--only` `--port`                                                                                                   | दोनों               |
| `orbit setup-roo`      | Roo Code                     | `~/.orbit/roo-settings.json` (आयात दस्तावेज़) + यदि एक VS Code `settings.json` मौजूद है तो `roo-cline.autoImportSettingsPath` सेट करता है                         | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | दोनों               |
| `orbit setup-crush`    | Crush                        | `~/.config/crush/crush.json` — `openai-compat` प्रदाता, कुंजी के माध्यम से `$ORBIT_API_KEY`                                                                       | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | दोनों               |
| `orbit setup-goose`    | Goose                        | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + पर्यावरण नुस्खा प्रिंट करता है                                                         | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | दोनों               |
| `orbit setup-aider`    | Aider                        | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + पर्यावरण नुस्खा प्रिंट करता है                                                                       | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | दोनों               |
| `orbit setup-qwen`     | Qwen Code                    | `~/.qwen/settings.json` — V4 `modelProviders.openai` ऐरे + `ORBIT_API_KEY` `~/.qwen/.env` में                                                                     | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | दोनों               |
| `orbit run <target>`   | रनटाइम लॉन्च (सामान्य)       | कुछ नहीं — सही वातावरण और तर्कों के साथ `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` को स्पॉन करता है; Qwen और Gemini अस्थायी अलग घर का उपयोग करते हैं | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | दोनों               |
| `orbit launch`         | Claude Code                  | कुछ नहीं — `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` को इंजेक्ट करके `claude` को स्पॉन करता है                                                                      | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | दोनों               |
| `orbit launch-codex`   | OpenAI Codex CLI             | कुछ नहीं — `-c` फ्लैग्स के माध्यम से `orbit` प्रदाता को इंजेक्ट करके `codex` को स्पॉन करता है                                                                     | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | दोनों               |

फ्लैग्स पर नोट्स (कमांड स्रोत में सत्यापित):

- `--remote <url>` — एक दूरस्थ Orbit से कैटलॉग लाता है (यह `--port`
  और सक्रिय संदर्भ को ओवरराइड करता है)। `--api-key <key>` उस
  सर्वर के लिए क्रेडेंशियल प्रदान करता है (डिफ़ॉल्ट रूप से `ORBIT_API_KEY` पर्यावरण चर, या सक्रिय संदर्भ के टोकन पर)।
...
- `--model <id>` पर `orbit run` मैनिफेस्ट के प्रति-लक्ष्य वायरिंग का पालन करता है
  (`bin/cli/cli-manifest.mjs`): **aider** को `--model openai/<id>` प्राप्त होता है और
  **opencode** को `--model orbit/<id>` (प्रिफिक्स केवल तब जोड़ा जाता है जब आईडी
  ...
  **codex** को `-c model_providers.orbit.*` तर्कों के माध्यम से। **Qwen एकमात्र रन
  लक्ष्य है जिसे `--model` की सख्त आवश्यकता है** — `orbit run qwen` इसके बिना `2` के साथ
  स्पष्ट त्रुटि के साथ समाप्त होता है।
- `--port <port>` — स्थानीय Orbit पोर्ट (डिफ़ॉल्ट `20128`, जब `--remote`
  सेट होता है तो अनदेखा किया जाता है)। सभी `setup-*` और दोनों लॉन्चरों पर मौजूद है।
- `orbit run` निकासी कोड: बच्चे CLI का अपना निकासी कोड वर्बटिम प्रकट होता है;
...
> एक समृद्ध प्लगइन इंटीग्रेशन भी है — `orbit setup opencode` — जो
...
जब Orbit `localhost:20128` पर चल रहा हो, तो बस अपने टूल के लिए सेटअप कमांड चलाएँ। कैटलॉग स्थानीय सर्वर से लाया जाता है।
...
export ORBIT_API_KEY=sk-...  # {env:ORBIT_API_KEY} के माध्यम से संदर्भित, कभी भी डिस्क पर नहीं
...
```bash
orbit launch                 # Claude Code → स्थानीय Orbit
orbit launch-codex           # Codex CLI → स्थानीय Orbit
...
orbit run claude -- --print-system-prompt "इस अंतर की समीक्षा करें"
```
...
किसी भी सेटअप कमांड को `--remote` + `--api-key` के साथ एक दूरस्थ Orbit पर इंगित करें। कैटलॉग दूरस्थ से लाया जाता है; कॉन्फ़िगरेशन आपके स्थानीय मशीन पर लिखा जाता है।
...
orbit setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m orbit/glm/glm-5.2 "..."   # पहले ORBIT_API_KEY निर्यात करें
...
```bash
orbit connect 192.168.0.15        # एक स्कोप्ड टोकन बनाता है, संदर्भ को संग्रहीत करता है
orbit setup-codex                 # ← अब दूरस्थ कैटलॉग का उपयोग करता है
orbit setup-opencode              # ← वही
orbit launch                      # ← Claude Code दूरस्थ के खिलाफ
```
...
Orbit OpenAI सतह को `/v1` पर, एंथ्रोपिक सतह को रूट पर, और एक मूल जेमिनी सतह को `/v1beta` पर उजागर करता है। प्रत्येक एकीकरण उस रूप में वायर्ड है जिसकी इसकी टूल अपेक्षा करती है (कमांड स्रोत में सत्यापित):
...
| `setup-codex`, `launch-codex` (`model_providers.orbit.base_url`)       | `/v1` के साथ     | हाँ                                             |
...
जब आप `orbit update` के साथ अपडेट करते हैं (पुष्टि करने के बाद, या `--apply` के साथ),
Orbit `--include=optional` के साथ इंस्टॉल चलाता है:

```bash
npm install -g orbit@latest --include=optional
```

यह **नहीं** है एक ध्वज जिसे आप `orbit update` को पास करते हैं — यह हमेशा
...
# [DRY RUN] चलाएगा: npm install -g orbit@latest --include=optional
```

अन्य `orbit update` ध्वज (स्रोत में सत्यापित): `--check` (यदि पुराना है तो
एक्ज़िट 1), `--apply` (बिना संकेत के इंस्टॉल करें), `--changelog`, `--no-backup`,
`--yes`।

---

## Google Gemini CLI के माध्यम से `orbit run gemini`
...
(और `:streamGenerateContent?alt=sse`) इसके खिलाफ जारी करता है — बिल्कुल Orbit का
स्थानीय Gemini सतह (`/v1beta`)। `orbit run gemini` इसे स्वचालित रूप से कनेक्ट करता है:

- `GOOGLE_GEMINI_BASE_URL` → सक्रिय Orbit बेस URL (रूट, कोई `/v1` नहीं);
- `GEMINI_API_KEY` → हल किया गया Orbit क्रेडेंशियल (विकल्प/पर्यावरण/संदर्भ);
- एक **अस्थायी अलग `GEMINI_CLI_HOME`** जिसका `.gemini/settings.json`
  ...
  (कोड सहायता) कभी भी Orbit-निर्देशित लॉन्च को ओवरराइड न करे — निकासी के बाद हटा दिया जाता है;
...
`tests/unit/cli/run-execution.test.ts`)। एक वास्तविक Orbit सर्वर के खिलाफ वास्तविक
...
वैकल्पिक: `ORBIT_SMOKE_TARGETS="codex,opencode,qwen"` स्वेप को सीमित करता है;
`ORBIT_SMOKE_TIMEOUT_MS` 120 सेकंड प्रति-लक्ष्य टाइमआउट को ओवरराइड करता है।
...
- [Codex CLI कॉन्फ़िगरेशन](./CODEX-CLI-CONFIGURATION.md) — एक बार का `[model_providers.orbit]` बेस सेटअप
