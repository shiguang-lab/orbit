...
- before the active account is exhausted, Orbit generates a compact structured summary
- after authentication selects a different account for the same session, Orbit injects
...
If the active provider is enabled in `handoffProviders`, Orbit generates a structured
...
Orbit prepends the stored handoff as a system message. Injection happens only after the
...
At injection time, Orbit converts that payload into a `<context_handoff>` system
