...
title: "CLI Mchanganyiko — elekeza CLI ya uandishi kwenye Orbit"
version: 3.8.50
...
Orbit inatoa familia ya amri `setup-*` ambazo zinaweka mchanganyiko wa uandishi
CLI (Codex, Claude Code, OpenCode, Cline, …) kutumia Orbit kama backend yake — hivyo
chombo kinawasiliana na **nukta** moja na Orbit inaelekeza kwa mtoa huduma sahihi kwa
kuanguka kiotomatiki. Kila amri inasoma **katalogi** ya mfano wa moja kwa moja kutoka kwa
Orbit inayofanya kazi (ya ndani au ya mbali) na kuandika faili la usanidi la chombo
...
Pia kuna mchezaji wa jumla — `orbit run <target>` — ambaye anazalisha
...
`open-code`, `qwen-code`, `gemini-cli`), na `orbit completion` inatoa
maneno ya malengo yanayotokana na orodha hiyo. Mchezaji wa zamani wa kila chombo —
`orbit launch` (Claude Code) na `orbit launch-codex` (Codex) — bado
zinapatikana.
...
- [Hali ya Mbali](./REMOTE-MODE.md) — endesha Orbit ya mbali (VPS / Tailnet) kutoka kwa kompyuta yako
...
Kila amri heshimu **muktadha wa sasa** (iliyowekwa na `orbit connect`, ona
...
| -------------------------- | ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------- |
| `orbit setup-codex`    | OpenAI Codex CLI              | `~/.codex/<name>.config.toml` — wasifu mmoja kwa kila mfano wa maandiko unaofaa (`codex --profile <name>`)                                               | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | Zote                    |
| `orbit setup-claude`   | Claude Code                   | `~/.claude/profiles/<name>/settings.json` — wasifu mmoja kwa kila mfano uliofanana (`CLAUDE_CONFIG_DIR`)                                                 | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | Zote                    |
| `orbit setup-opencode` | OpenCode (inayofaa na openai) | `~/.config/opencode/opencode.json` — mtoa huduma wa `orbit` na kila mfano wa katalogi (`opencode -m orbit/<model>`)                              | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | Zote                    |
| `orbit setup-cline`    | Cline                         | `~/.cline/data/{globalState,secrets}.json` (hali ya CLI) + inachapisha mipangilio ya nyongeza ya VS Code                                                 | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | Zote                    |
| `orbit setup-kilo`     | Kilo Code                     | `~/.local/share/kilo/auth.json` (CLI) + inachanganya `kilocode.*` katika `settings.json` ya VS Code ikiwa inapatikana                                    | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | Zote                    |
| `orbit setup-continue` | Continue / `cn` CLI           | `~/.continue/config.yaml` — `provider: openai` mifano, funguo kupitia `${{ secrets.ORBIT_API_KEY }}`                                                 | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Zote                    |
| `orbit setup-cursor`   | Cursor                        | Hakuna — inachapisha hatua za ndani ya programu (mipangilio ya Cursor ni SQLite isiyoonekana)                                                            | `--remote` `--api-key` `--only` `--port`                                                                                                   | Zote                    |
| `orbit setup-roo`      | Roo Code                      | `~/.orbit/roo-settings.json` (nyaraka ya kuagiza) + inaweka `roo-cline.autoImportSettingsPath` ikiwa `settings.json` ya VS Code inapatikana          | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | Zote                    |
| `orbit setup-crush`    | Crush                         | `~/.config/crush/crush.json` — mtoa huduma wa `openai-compat`, funguo kupitia `$ORBIT_API_KEY`                                                       | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Zote                    |
| `orbit setup-goose`    | Goose                         | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + inachapisha mapishi ya mazingira                                          | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Zote                    |
| `orbit setup-aider`    | Aider                         | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + inachapisha mapishi ya mazingira                                                        | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Zote                    |
| `orbit setup-qwen`     | Qwen Code                     | `~/.qwen/settings.json` — V4 `modelProviders.openai` orodha + `ORBIT_API_KEY` katika `~/.qwen/.env`                                                  | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | Zote                    |
| `orbit run <target>`   | Uzinduzi wa wakati (jumla)    | Hakuna — anazalisha `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` na mazingira na hoja sahihi; Qwen na Gemini hutumia nyumbani iliyotengwa | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | Zote                    |
| `orbit launch`         | Claude Code                   | Hakuna — anazalisha `claude` na `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` ikingizwa                                                                    | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | Zote                    |
| `orbit launch-codex`   | OpenAI Codex CLI              | Hakuna — anazalisha `codex` na mtoa huduma wa `orbit` ikingizwa kupitia bendera `-c`                                                                 | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | Zote                    |

Maelezo kuhusu bendera (yamehakikishwa katika chanzo cha amri):

- `--remote <url>` — pata katalogi kutoka kwa Orbit ya mbali (inabatilisha `--port`
  na muktadha wa sasa). `--api-key <key>` inatoa akidi kwa ajili ya
  seva hiyo (inatumika kama chaguo la `ORBIT_API_KEY` env var, au token ya muktadha wa sasa).
...
- `--model <id>` kwenye `orbit run` inafuata uunganisho wa orodha ya malengo
  (`bin/cli/cli-manifest.mjs`): **aider** inapata `--model openai/<id>` na
  **opencode** `--model orbit/<id>` (kiambatisho kinajumuishwa tu wakati id
  ...
  **codex** kupitia `-c model_providers.orbit.*` hoja. **Qwen ndiyo lengo pekee la kuendesha
  ambalo linahitaji kwa nguvu `--model`** — `orbit run qwen` bila hiyo inatoka
  `2` na makosa wazi.
- `--port <port>` — bandari ya ndani ya Orbit (chaguo la msingi `20128`, ignored when `--remote`
  is set). Ipo kwenye kila `setup-*` na mchezaji wote wawili.
- Nambari za kutoka za `orbit run`: nambari ya kutoka ya CLI ya mtoto inasambazwa
...
> Pia kuna mchanganyiko wa nyongeza wenye utajiri zaidi — `orbit setup opencode` — ambayo
...
Ikiwa Orbit inafanya kazi kwenye `localhost:20128`, endesha tu amri ya usanidi kwa zana yako. Katalogi inapatikana kutoka kwa seva ya ndani.
...
export ORBIT_API_KEY=sk-...  # inarejelea kupitia {env:ORBIT_API_KEY}, kamwe sio kwenye diski
...
```bash
orbit launch                 # Claude Code → Orbit ya ndani
orbit launch-codex           # Codex CLI → Orbit ya ndani
...
Elekeza amri yoyote ya usanidi kwenye Orbit ya mbali kwa `--remote` + `--api-key`. Katalogi inapatikana kutoka kwa mbali; usanidi unandikwa kwenye mashine yako ya ndani.
...
orbit setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m orbit/glm/glm-5.2 "..."   # export ORBIT_API_KEY kwanza
...
```bash
orbit connect 192.168.0.15        # inaunda token iliyo na upeo, inahifadhi muktadha
orbit setup-codex                 # ← sasa inatumia katalogi ya mbali
orbit setup-opencode              # ← sawa
orbit launch                      # ← Claude Code dhidi ya mbali
```
...
Orbit inatoa uso wa OpenAI kwenye `/v1`, uso wa Anthropic kwenye mzizi,
...
| `setup-codex`, `launch-codex` (`model_providers.orbit.base_url`)       | pamoja na `/v1`            | Ndio                                              |
...
Unaposasisha kwa kutumia `orbit update` (baada ya kuthibitisha, au kwa `--apply`),
Orbit inatekeleza usakinishaji kwa kutumia `--include=optional` iliyojumuishwa:

```bash
npm install -g orbit@latest --include=optional
```

Hii **si** bendera unayoipatia `orbit update` — inatumika kila wakati na
...
# [DRY RUN] Ingefanya: npm install -g orbit@latest --include=optional
```

Bendera nyingine za `orbit update` (zilizothibitishwa kwenye chanzo): `--check` (ondoka 1 ikiwa
imepitwa na wakati), `--apply` (sakinisha bila kuomba), `--changelog`, `--no-backup`,
`--yes`.

---

## Google Gemini CLI kupitia `orbit run gemini`
...
Gemini wa Orbit (`/v1beta`). `orbit run gemini` inafanya hivyo kiotomatiki:

- `GOOGLE_GEMINI_BASE_URL` → URL ya msingi ya Orbit inayotumika (mizizi, hakuna `/v1`);
- `GEMINI_API_KEY` → akidi ya Orbit iliyotatuliwa (chaguo/env/muktadha);
- **nyumba ya muda ya `GEMINI_CLI_HOME`** ambayo `.gemini/settings.json`
  ...
  hakitabadilisha uzinduzi unaoelekezwa na Orbit — inatolewa baada ya kutoka;
...
server HALISI ya Orbit, kuna vifaa vya kujiunga katika
...
Hiari: `ORBIT_SMOKE_TARGETS="codex,opencode,qwen"` inapunguza safu;
`ORBIT_SMOKE_TIMEOUT_MS` inabadilisha muda wa sekunde 120 kwa kila lengo.
...
- [Usanidi wa Codex CLI](./CODEX-CLI-CONFIGURATION.md) — usanidi wa msingi wa mara moja `[model_providers.orbit]`
