...
Orbit integrerer med tre kategorier af CLI værktøjer fordelt på tre dedikerede dashboard sider:

| Side            | Rute                    | Koncept                                                                       | Antal       |
| --------------- | ----------------------- | ----------------------------------------------------------------------------- | ----------- |
| **CLI Kode's**  | `/dashboard/cli-code`   | Kodningsværktøjer, du peger på Orbit (Klient → CLI → Orbit → Udbyder) | 26          |
| **CLI Agenter** | `/dashboard/cli-agents` | Autonome agenter, du peger på Orbit (samme flow, bredere omfang)          | 8           |
| **ACP Agenter** | `/dashboard/acp-agents` | CLIs, som Orbit genererer som backend via stdio/ACP (omvendt flow)        | se register |
...
Claude / Codex / OpenCode / Cline / KiloCode / Continue / Hermes Agent / Goose / ...
    ...
    http://YOUR_SERVER:20128/v1
           │
           ▼  (Orbit ruter til den rigtige udbyder)
    Anthropic / OpenAI / Gemini / DeepSeek / Groq / Mistral / ...

ACP Agenter (omvendt genereringsflow):
    Klientanmodning → Orbit → genererer CLI via stdio/ACP → svar
```
...
Du behøver ikke at skrive hver værktøjs konfiguration i hånden. Orbit leverer en `setup-*`
kommando pr. understøttet CLI, der læser den **live** modelkatalog fra en kørende
Orbit (lokal eller fjern) og skriver værktøjets egen konfiguration på din maskine:
...
fjern Orbit), `--dry-run` (forhåndsvisning uden at skrive), og `--port`. Værktøjer
...
per-værktøj launchers `orbit launch` (Claude Code) og `orbit launch-codex`
(Codex) forbliver tilgængelige. Gemini CLI er kun til lancering: det er et `orbit run`
...
En `setup-*` kommando udført inde i Orbit containeren skriver ind i
containerens egen hjemmemappe, som ingen værts-CLI læser og som forsvinder med
containeren. Orbit opdager det og afslutter med `2` med instruktioner i stedet for
at skrive. To understøttede måder fremad — installer CLI på værten og
`orbit connect` til containeren, eller bind-mount konfigurationsmapperne og sæt
`CLI_CONFIG_HOME` (den compose `host` profil). Hver `setup-*` kommando, plus
`orbit configure` og `orbit config set`, accepterer
`--allow-container-write`, når konfiguration af containerens egne CLIs er det, du
faktisk mente; `ORBIT_ALLOW_CONTAINER_CONFIG_WRITE=true` gør det samme for
serveren. Se
[Docker Guide → Konfigurering af værts-CLI værktøjer](../guides/DOCKER_GUIDE.md#configuring-host-cli-tools-when-orbit-runs-in-docker).
...
kilo, continue) — en `hostSetupCommand` (f.eks. `orbit setup-opencode`) der skal køres
...
| **Konfigurerbar** | Understøttet af `orbit configure <cli>` (opsætningsopskrift eksisterer)       | `bin/cli/cli-manifest.mjs` (`configure: true`)                    |
| **Lancerbar**     | Understøttet af `orbit run <target>` (env/args injektion defineret)           | `bin/cli/cli-manifest.mjs` (`run: true`)                          |
...
Denne side (omdøbt fra `/dashboard/agents`) viser CLIs, som Orbit kan **spawne** som backend eksekveringsmotorer via stdio/ACP protokol. Katalogen vedligeholdes separat i `src/lib/acp/registry.ts` og er **ikke** den samme som `CLI_TOOLS`.
...
forbliver i miljøvariablen `ORBIT_API_KEY` (`env_key`), aldrig
inde i filen:
...
> Brug `opencode run "your prompt" --model orbit/claude-sonnet-4-5-thinking --variant high`
...
Eller brug Orbit-dashboardet → **CLI Tools → Cline → Anvend konfiguration**.
...
Eller brug Orbit-dashboardet → **CLI Tools → KiloCode → Anvend konfiguration**.
...
Brug dette, når VS Code Insiders er konfigureret til brugerdefinerede endpoint-modeller, og du ønsker, at Orbit skal fungere uden et brugerdefineret headerfelt.
...
**Eksempel ved brug af den tokeniserede Orbit-alias:**
...
- Erstat `sk-your-orbit-key` med en API-nøgle oprettet i Orbit.
...
# CLI'en bruger sin egen autentificering — Orbit er ikke nødvendig som backend for Kiro CLI selv.
# Brug kiro-cli sammen med Orbit til andre værktøjer.
kiro-cli status
```

For **Kiro IDE** desktopapp, brug MITM-endpointet, der er eksponeret af Orbit
...
Den `orbit` binære fil giver kommandoer til serverlivscyklus, opsætning, diagnostik og leverandørstyring. Indgangspunkt: `bin/orbit.mjs`.

```bash
orbit                              # Start server (standard port 20128)
orbit setup                        # Interaktiv opsætningsguide
orbit doctor                       # Tjek konfiguration, DB, porte, runtime
orbit providers list               # Konfigurerede leverandørforbindelser
orbit providers test-all           # Test hver aktiv forbindelse
orbit reset-password               # Nulstil adminadgangskode
orbit logs                         # Stream anmodningslogs
orbit health                       # Detaljeret sundhed (afbrydere, cache, hukommelse)
orbit --version                    # Udskriv version
...
```bash
orbit setup                        # Interaktiv opsætningsguide
orbit setup --non-interactive      # CI/automatiseringsmode (læser miljøvariabler + flags)
orbit setup --password '<value>'   # Indstil adminadgangskode direkte
...
| Var                 | Formål                                                               |
| ------------------- | -------------------------------------------------------------------- |
| `ORBIT_API_KEY` | Leverandør API-nøgle (bundet til `--api-key` via Commander `.env()`) |
| `DATA_DIR`          | Overskriv Orbit data katalog                                     |

Alle andre ikke-interaktive input gives som flags, ikke miljøvariabler:
`--password`, `--provider`, `--provider-name`, `--provider-base-url`, `--default-model`
(se `orbit setup` mulighederne ovenfor).

### Diagnostik

```bash
orbit doctor                       # Tjek konfiguration, DB, porte, runtime, hukommelse, livlighed
orbit doctor --json                # Maskinlæsbart JSON
orbit doctor --no-liveness         # Spring HTTP sundhedsprobe over
orbit doctor --host 0.0.0.0        # Overskriv livlighedsvært
orbit doctor --liveness-url <url>  # Fuldt sundhedsendepunkt URL-overskrivning
```
...
orbit providers available --search openai       # Filtrer katalog efter id/navn/alias/kategori
orbit providers available --category api-key    # Filtrer efter kategori (api-key, oauth, gratis, ...)
orbit providers available --json                # Maskinlæsbart JSON

orbit providers list                            # Konfigurerede leverandørforbindelser
orbit providers list --json

orbit providers test <id|name>                  # Test én konfigureret forbindelse
orbit providers test-all                        # Test hver aktiv forbindelse
orbit providers validate                        # Lokalt strukturel validering
orbit providers add <provider> --credential-env PROVIDER_KEY
orbit providers import ./providers.json --dry-run --json
orbit providers auth <provider>                 # Eksisterende OAuth-flow
...
redigeret tilstedeværelse/form. `providers available` læser Orbit kataloget;
...
```bash
orbit reset-password                # Nulstil adminadgangskode (også: orbit-reset-password)
orbit reset-encrypted-columns       # Vis advarsel + tørkørsel for nulstilling af krypterede legitimationsoplysninger
orbit reset-encrypted-columns --force  # Faktisk nulstil krypterede legitimationsoplysninger i SQLite
```

### Eksport af legitimationsoplysninger (⚠ håndter med omhu)

```bash
orbit auth export                                 # Vis advarsel + bekræftelsesport — ingen DB-adgang
orbit auth export --force                          # Eksporter ALLE forbindelsers DEKRYPPERET legitimationsoplysninger til stdout som JSON
orbit auth export --force --id <id>                 # Eksporter kun den matchende forbindelse
orbit auth export --force --format env               # Udsend ORBIT_<PROVIDER>_<FIELD>=<value> linjer
orbit auth export --force --out creds.json           # Skriv til en fil (oprettet med 0600 tilladelser)
```
...
Disse antager en kørende Orbit server, medmindre andet er angivet:

```bash
orbit status                       # Omfattende runtime status
orbit logs                         # Stream anmodningslogs (--json, --search, --follow)
orbit config show                  # Vis nuværende konfiguration

orbit provider list                # Liste over tilgængelige leverandører (alias af providers list)
orbit provider add                 # Registrer Orbit som en leverandør på et værktøj
orbit keys add | list | remove     # Administrer API-nøgler
orbit models [provider]            # Liste over modeller (--json, --search)
orbit combo list | switch | create | delete

orbit backup                       # Snapshot konfiguration + DB
orbit restore                      # Gendan fra et tidligere snapshot

orbit health                       # Detaljeret sundhed (afbrydere, cache, hukommelse)
orbit quota                        # Leverandør kvote brug
orbit cache                        # Cache status
orbit cache clear                  # Ryd semantiske + signatur caches

orbit mcp status | restart         # MCP server status / genstart
orbit a2a status | card            # A2A server status / agentkort

orbit tunnel list | create | stop  # Administrer tunneler (cloudflare/tailscale/ngrok)
orbit env show | get <k> | set <k> <v>  # Inspicer / indstil miljøvariabler (midlertidige)

orbit test                         # Leverandør tilslutning røgtest
orbit update                       # Tjek for opdateringer
orbit completion                   # Generer shell completion
```
...
Klar-til-at-indsætte eksempler med en tokeniseret Orbit URL:
...
| Fejl                                              | Årsag                     | Løsning                                            |
| ------------------------------------------------- | ------------------------- | -------------------------------------------------- |
| `Connection refused`                              | Orbit kører ikke      | `orbit serve`                                  |
