...
title: "CLI Integrace — nasměrujte jakýkoli kódovací CLI na Orbit"
version: 3.8.50
...
Orbit dodává rodinu příkazů `setup-*`, které konfiguruje kódovací
CLI (Codex, Claude Code, OpenCode, Cline, …) pro použití Orbit jako svého backendu — takže
nástroj komunikuje s **jedním** koncovým bodem a Orbit směruje k správnému poskytovateli s
automatickým zálohováním. Každý příkaz čte **živý** katalog modelů z běžícího
Orbit (místního nebo vzdáleného) a zapisuje vlastní konfigurační soubor nástroje na **vašem**
...
K dispozici je také generický spouštěč — `orbit run <target>` — který spouští
...
`open-code`, `qwen-code`, `gemini-cli`), a `orbit completion` nabízí
stejné cílové výrazy odvozené z manifestu. Dědictví per-tool spouštěče —
`orbit launch` (Claude Code) a `orbit launch-codex` (Codex) — zůstávají
k dispozici.
...
- [Vzdálený režim](./REMOTE-MODE.md) — ovládejte vzdálený Orbit (VPS / Tailnet) ze svého laptopu
...
Každý příkaz ctí **aktivní kontext** (nastavený pomocí `orbit connect`, viz
...
| -------------------------- | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------- |
| `orbit setup-codex`    | OpenAI Codex CLI               | `~/.codex/<name>.config.toml` — jeden profil pro každý kompatibilní textový model (`codex --profile <name>`)                                                      | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | Oba                 |
| `orbit setup-claude`   | Claude Code                    | `~/.claude/profiles/<name>/settings.json` — jeden profil pro každý odpovídající model (`CLAUDE_CONFIG_DIR`)                                                       | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | Oba                 |
| `orbit setup-opencode` | OpenCode (openai-kompatibilní) | `~/.config/opencode/opencode.json` — `orbit` poskytovatel se všemi modely katalogu (`opencode -m orbit/<model>`)                                          | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | Oba                 |
| `orbit setup-cline`    | Cline                          | `~/.cline/data/{globalState,secrets}.json` (CLI režim) + tiskne nastavení rozšíření VS Code                                                                       | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | Oba                 |
| `orbit setup-kilo`     | Kilo Code                      | `~/.local/share/kilo/auth.json` (CLI) + slučuje `kilocode.*` do `settings.json` VS Code, pokud je přítomno                                                        | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | Oba                 |
| `orbit setup-continue` | Continue / `cn` CLI            | `~/.continue/config.yaml` — `provider: openai` modely, klíč přes `${{ secrets.ORBIT_API_KEY }}`                                                               | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Oba                 |
| `orbit setup-cursor`   | Cursor                         | Nic — tiskne kroky v aplikaci (konfigurace Cursor je neprůhledná SQLite)                                                                                          | `--remote` `--api-key` `--only` `--port`                                                                                                   | Oba                 |
| `orbit setup-roo`      | Roo Code                       | `~/.orbit/roo-settings.json` (import doc) + nastaví `roo-cline.autoImportSettingsPath`, pokud existuje `settings.json` VS Code                                | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | Oba                 |
| `orbit setup-crush`    | Crush                          | `~/.config/crush/crush.json` — `openai-compat` poskytovatel, klíč přes `$ORBIT_API_KEY`                                                                       | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Oba                 |
| `orbit setup-goose`    | Goose                          | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + tiskne recept pro prostředí                                                        | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Oba                 |
| `orbit setup-aider`    | Aider                          | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + tiskne recept pro prostředí                                                                      | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Oba                 |
| `orbit setup-qwen`     | Qwen Code                      | `~/.qwen/settings.json` — V4 `modelProviders.openai` pole + `ORBIT_API_KEY` v `~/.qwen/.env`                                                                  | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | Oba                 |
| `orbit run <target>`   | Runtime launch (generic)       | Nic — spouští `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` s odpovídajícím prostředím a argumenty; Qwen a Gemini používají dočasný izolovaný domov | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | Oba                 |
| `orbit launch`         | Claude Code                    | Nic — spouští `claude` s `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` injektovaným                                                                                 | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | Oba                 |
| `orbit launch-codex`   | OpenAI Codex CLI               | Nic — spouští `codex` s poskytovatelem `orbit` injektovaným pomocí `-c` příznaků                                                                              | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | Oba                 |

Poznámky k příznakům (ověřeno ve zdroji příkazů):

- `--remote <url>` — získává katalog z vzdáleného Orbit (přepisuje `--port`
  a aktivní kontext). `--api-key <key>` dodává pověření pro tento
  server (výchozí hodnota je proměnná prostředí `ORBIT_API_KEY`, nebo token aktivního kontextu).
...
- `--model <id>` na `orbit run` následuje propojení per-target manifestu
  (`bin/cli/cli-manifest.mjs`): **aider** přijímá `--model openai/<id>` a
  **opencode** `--model orbit/<id>` (prefix je přidán pouze tehdy, když id
  jej již nenese); **qwen** a **gemini** přijímají id doslovně;
  **claude** jej získává přes `ANTHROPIC_MODEL`, **goose** přes `GOOSE_MODEL`, a
  **codex** přes `-c model_providers.orbit.*` argumenty. **Qwen je jediným cílem běhu,
  který tvrdě vyžaduje `--model`** — `orbit run qwen` bez něj končí
  `2` s explicitní chybou.
- `--port <port>` — místní port Orbit (výchozí `20128`, ignorováno, když je nastaveno `--remote`).
  Přítomno na všech `setup-*` a obou spouštěčích.
- Kódy ukončení `orbit run`: vlastní kód ukončení podřízeného CLI je propagován
...
> K dispozici je také bohatší pluginová integrace — `orbit setup opencode` — která
...
S Orbit běžícím na `localhost:20128`, stačí spustit příkaz pro nastavení vašeho
...
export ORBIT_API_KEY=sk-...  # odkazováno přes {env:ORBIT_API_KEY}, nikdy na disku
...
```bash
orbit launch                 # Claude Code → místní Orbit
orbit launch-codex           # Codex CLI → místní Orbit
orbit launch-codex --profile glm52
orbit run claude --model openai/gpt-5.4
orbit run codex --model openai/gpt-5.4 --dry-run --json
orbit run aider --model glm/glm-5.2 -- --message "odpověď OK"
orbit run goose --model glm/glm-5.2
orbit run opencode --model glm/glm-5.2 -- run "odpověď OK"
orbit run qwen --model glm/glm-5.2 -- -p "odpověď OK"
orbit run gemini --model glm/glm-5.2 -- --skip-trust -p "odpověď OK"

# Explicitní cesta příkazu: předat cokoliv, co přijde po --
orbit run claude -- --print-system-prompt "zkontrolujte tento rozdíl"
```
...
Nasměrujte jakýkoli příkaz pro nastavení na vzdálený Orbit s `--remote` + `--api-key`. Katalog je načten ze vzdáleného serveru; konfigurace je zapsána na vašem místním počítači.
...
orbit setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m orbit/glm/glm-5.2 "..."   # nejprve exportujte ORBIT_API_KEY
...
```bash
orbit connect 192.168.0.15        # vytvoří token s omezeným rozsahem, uloží kontext
orbit setup-codex                 # ← nyní používá vzdálený katalog
orbit setup-opencode              # ← stejné
orbit launch                      # ← Claude Code proti vzdálenému
```
...
Orbit vystavuje OpenAI rozhraní na `/v1`, Anthropic rozhraní na kořenové úrovni,
...
| `setup-codex`, `launch-codex` (`model_providers.orbit.base_url`)       | s `/v1`              | Ano                                         |
...
Když aktualizujete pomocí `orbit update` (po potvrzení nebo s `--apply`),
Orbit spouští instalaci s `--include=optional` zabudovaným:

```bash
npm install -g orbit@latest --include=optional
```

To **není** příznak, který předáváte `orbit update` — je vždy aplikován
...
# [DRY RUN] By běžel: npm install -g orbit@latest --include=optional
```

Další příznaky `orbit update` (ověřeno ve zdrojovém kódu): `--check` (ukončí s 1, pokud
je zastaralý), `--apply` (nainstaluje bez výzvy), `--changelog`, `--no-backup`,
`--yes`.

---

## Google Gemini CLI přes `orbit run gemini`
...
Gemini rozhraní Orbit (`/v1beta`). `orbit run gemini` to automaticky
propojuje:

- `GOOGLE_GEMINI_BASE_URL` → aktivní základní URL Orbit (kořen, žádné `/v1`);
- `GEMINI_API_KEY` → vyřešené pověření Orbit (volba/env/context);
- **dočasný izolovaný `GEMINI_CLI_HOME`**, jehož `.gemini/settings.json`
  ...
  nikdy nepřepíše spuštění řízené Orbit — odstraněno po ukončení;
...
serveru Orbit existuje opt-in rámec na
...
Volitelně: `ORBIT_SMOKE_TARGETS="codex,opencode,qwen"` omezuje testování;
`ORBIT_SMOKE_TIMEOUT_MS` přepisuje časový limit 120s na cíl.

## Viz také

- [Konfigurace Claude Code](./CLAUDE-CODE-CONFIGURATION.md) — hlubší průvodce Claude Code
- [Konfigurace Codex CLI](./CODEX-CLI-CONFIGURATION.md) — jednorázové základní nastavení `[model_providers.orbit]`
