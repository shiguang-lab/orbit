...
<img src="../../screenshots/MainShiguangGateway.png" alt="ShiguangGateway 控制台" width="820"/>
...
**约 1.53B 可统计的循环免费 Token / 月** — 计入一次性注册奖励后，首月约 **~2.15B**。另有未公布 Token 上限但受速率、并发、账户、地区、KYC 与服务条款限制的访问，单独列示而不计入标题数字。([统计方法 →](../../reference/FREE_TIERS.md#tldr--how-much-free-inference-does-shiguang-gateway-actually-aggregate))
...
**疑难解答、服务商攻略、路线图与支持 → [Discord](https://discord.gg/U47eFqAXCn) · [Telegram](https://t.me/shiguang-gatewayOficial) · WhatsApp [🌍 全球](https://chat.whatsapp.com/JI7cDQ1GyaiDHhVBpLxf8b?mode=gi_t) / [🇧🇷 巴西](https://chat.whatsapp.com/LTSpdFhXTxjH4R6CCNiKWz)**
...
[**🚀 快速开始**](#-quick-start) • [**🎯 Combo**](#-combos--the-flagship) • [**🌐 服务商**](#-329-ai-providers--155-freeno-auth) • [**🔌 CLI 与 MCP**](#-full-cli--a2a--mcp) • [**🗜️ 压缩**](#%EF%B8%8F-save-1595-tokens--automatically) • [**🌍 官网**](https://shiguang-gateway.online)

[💥 我们的承诺](#-the-promise) • [🤔 为什么选择 ShiguangGateway](#-why-shiguang-gateway) • [🏆 核心优势](#-what-sets-shiguang-gateway-apart) • [🤖 兼容的编程工具](#-compatible-clis--coding-agents) • [🖥️ 运行平台](#%EF%B8%8F-where-shiguang-gateway-runs--anywhere) • [🔒 隐私优先](#-private--local-first) • [🎬 实机演示](#-shiguang-gateway-in-action) • [📚 探索更多](#-explore-more) • [📧 支持](#-support--community)
...
> 手动凑各家免费额度有多痛苦 — 数十套 SDK、数十个速率限制，根本搞不清到底还剩多少。ShiguangGateway 当前公开 **155 个标记为免费／免验证的目录项**；其中严格量化的预算覆盖 **43 个服务商池 / 522 个模型预算项**，并在控制台实时展示 (`/dashboard/free-tiers`)。
>
...
> 一个端点。**329 个服务商目录项。** ShiguangGateway 尝试选择最便宜且符合条件的可用路由。
...
# 🤔 为什么选择 ShiguangGateway？

</div>

> 告别在十个控制台之间疲于奔命、处理失效的 API 密钥和天降账单的日子。

| ❌ 日常痛点                                       | ✅ ShiguangGateway 如何解决                                       |
| ------------------------------------------------- | ----------------------------------------------------------- |
...
> **Combo** 是 ShiguangGateway **自动**路由的模型接力链路。配额耗尽、服务商宕机或成本飙升 — Combo 自动滑向下一个模型，无声无息。**正是它让 ShiguangGateway 坚不可摧。** 🛡️

### ⚡ 零配置 — 只需设为 `auto`

无需预先配置 Combo。将模型 ID 设为 `auto`（或其变体），ShiguangGateway 会基于你已连接的服务商实时评分，自动构建虚拟 Combo：
...
# 🏆 ShiguangGateway 何以脱颖而出

</div>

| 功能                           | ShiguangGateway                                                        | 其他路由方案 |
| ------------------------------ | ---------------------------------------------------------------- | ------------ |
...
<sub>📊 与 LiteLLM、OpenRouter、Portkey 的详细对比 → [`docs/comparison/SHIGUANG_GATEWAY_VS_ALTERNATIVES.md`](../../comparison/SHIGUANG_GATEWAY_VS_ALTERNATIVES.md)</sub>
...
- **🤖 一键 CLI/Agent 配置** — 专用 `setup-*` 命令为各编程工具一键配置 ShiguangGateway 路由（Claude Code、Codex、Cline、Continue、Cursor、Roo Code、Kilo Code、Crush、Goose、Qwen Code、Aider、OpenCode）；`shiguang-gateway launch` / `shiguang-gateway launch-codex` 为零配置启动器。→ [CLI 集成](../../guides/CLI-INTEGRATIONS.md)
- **🛰️ 远程模式** — 通过授权范围 Token 从任意机器操控远程 ShiguangGateway（`shiguang-gateway connect` / `shiguang-gateway contexts` / `shiguang-gateway tokens`）；另附 `shiguang-gateway login antigravity` 辅助命令，在你的本机运行 Google "native/desktop" OAuth 后将凭证 blob 粘贴至远程/VPS 安装实例（因远程环境无法接收 loopback 回调）。→ [远程模式](../../guides/REMOTE-MODE.md)
...
- **🗜️ 可插拔压缩体系** — **9 大可组合引擎**的异步流水线，含 Compression Studios、LLMLingua-2 ONNX 引擎和启发式/SLM 双层 **Ultra**、RTK、委托式 Anthropic 上下文编辑、**输出风格**（输出轴调控：简洁文章 / 少代码 / 简洁文言）、**自适应上下文预算旋钮**（仅推进到刚好适应上下文窗口的程度）、每请求 `x-shiguang-gateway-compression` 控制、可选离线评估套件、控制台一键 **Headroom** 代理生命周期管理（支持 Docker 边车）、合成**压缩演练场**（Play 通道 + A/B 对比，附 USD 上限保真度判定）、可选**每步保真度门控**（在有损引擎降低 Prompt 质量前将其拦截）、**Best-of-N 候选编码器**（GCF vs TOON — 取更短者，Studio 中附 A/B 字节/Token 对照表）、**CCR 范围/grep/统计检索**（直接拉取储存块的精确字节/行切片或摘要而无需全量展开），以及统一面板含命名配置文件 + 活动配置文件选择器。→ [压缩](../../compression/COMPRESSION_ENGINES.md)
- **🕵️ 透明 MITM 解密（TPROXY）** — 捕获并翻译忽略代理环境变量的 CLI 流量，含每 SNI 证书颁发机构和信任存储安装器。→ [MITM/TPROXY](../../security/MITM-TPROXY-DECRYPT.md)
- **💸 全方位成本遥测** — 每个端点上的 `X-ShiguangGateway-*` 成本/用量响应头（含媒体端点）、非 Token 成本引擎、缓存命中 `X-ShiguangGateway-Cost-Saved` 响应头，以及每密钥美元消费配额。→ [API 参考](../../reference/API_REFERENCE.md)
- **🧠 完全可控的记忆系统** — 可选 int8 向量量化（Qdrant + sqlite-vec）、默认关闭记忆、每请求 `x-shiguang-gateway-no-memory` 响应头。→ [记忆系统](../../frameworks/MEMORY.md)
...
- **⚡ 本地性能与基础设施** — 一键本地 Redis 启动器（`shiguang-gateway redis up`，含控制台 Redis 面板）、一键 **Cloudflare Workers** 和 **Deno Deploy** 中继部署器（接入代理池），以及可选 Bifrost Go 边车将最热中继路径卸载至 Go 侧（`BIFROST_BASE_URL`，超时自动回退 TypeScript 路径）— 现支持中继后端选择器（`SHIGUANG_GATEWAY_RELAY_BACKEND=ts|bifrost|auto`），`/v1/relay` 端点保持对外稳定接口的同时内部自动择取最快后端。→ [环境配置](../../reference/ENVIRONMENT.md)
...
<sub>📖 16+ 款工具的逐项配置指南 → [`docs/reference/CLI-TOOLS.md`](../../reference/CLI-TOOLS.md) · 🧩 OpenCode 插件 → [`@orbit/opencode-provider`](https://www.npmjs.com/package/@orbit/opencode-provider)</sub>
...
| 平台                     | 安装方式                                 | 亮点                                              |
| ------------------------ | ---------------------------------------- | ------------------------------------------------- |
| 📦 **npm（全局）**       | `npm install -g shiguang-gateway`               | 一行命令，任意 OS                                 |
| 🐳 **Docker**            | `docker run … diegosouzapw/shiguang-gateway`    | 多架构 **AMD64 + ARM64**                          |
| 🖥️ **桌面（Electron）**  | `npm run electron:build`                 | 原生窗口 + 系统托盘 — **Windows / macOS / Linux** |
| 💪 **ARM**               | 原生 `arm64`                             | 树莓派、ARM 服务器、Apple Silicon                 |
| 📱 **Android（Termux）** | `pkg install nodejs && npx -y shiguang-gateway` | **在手机上** 7×24 运行，无需 Root                 |
| 📲 **PWA**               | "添加到主屏幕"                           | 全屏、离线、可从浏览器安装                        |
| 🧩 **OpenCode 插件**     | `@orbit/opencode-provider`           | 原生 OpenCode 集成                                |
...
> 你的密钥、你的机器、你的数据。ShiguangGateway 是**本地代理** — 绝不会向外回传。

- 🏠 **100% 运行在本地硬件上** — npm、Docker、桌面端或你的手机。请求链路中不存在任何 ShiguangGateway 云端节点。
...
> ShiguangGateway 不只是一台服务器 — 它是拥有 **60+ 命令**的**全功能命令行驾驶舱**，外加开放的代理协议，让 AI 代理**自主**操控 ShiguangGateway。

### ⌨️ 真正的 CLI（不止 `start`）

```bash
shiguang-gateway               # 启动网关 + 控制台（端口 20128）
shiguang-gateway chat          # 交互式 TUI 聊天客户端（斜杠命令：/model /combo /skill /memory）
shiguang-gateway setup         # 引导式首次设置向导
shiguang-gateway doctor        # 诊断服务商、端口、原生依赖
```

### 🛰️ 远程模式 — CLI 在本地，ShiguangGateway 在远端的 VPS

ShiguangGateway 跑在服务器上？用**同一套 CLI** 从笔记本远程操控。登录一次，绑定授权范围 Token；后续所有命令自动指向远端。

```bash
shiguang-gateway connect 192.168.0.15            # 密码 → 范围 Token，保存为上下文
shiguang-gateway models list                     # ← 在远端服务器上执行
shiguang-gateway configure codex                 # ← 选择远端模型，写入本地 Codex 配置文件
shiguang-gateway tokens create --name ci --scope read   # 为其他机器签发更窄范围的 Token
shiguang-gateway contexts use default            # ← 切回本机服务器
```
...
### 🤝 接入 AI 代理 — 让代理自主操控 ShiguangGateway

通过 **MCP** 或 **A2A** 协议暴露 ShiguangGateway，任何智能代理都能获得网关的完整控制权 — 路由、服务商、Combo、缓存、压缩、记忆 — 全自主运行。

| 协议                | 端点                                            | 用途                                                 |
| ------------------- | ----------------------------------------------- | ---------------------------------------------------- |
| 🧰 **MCP（stdio）** | `shiguang-gateway --mcp`                               | 接入 Claude Desktop、Cursor 等各种 MCP 客户端        |
...
```bash
# 通过 MCP 将 ShiguangGateway 完整工具集赋予 Claude Code：
...
> **Token 够用就好，何必铺张浪费？** 每个请求**透明地**通过 ShiguangGateway 压缩流水线 — 客户端无需任何改动。现已升级为 **9 大可组合引擎**的级联体系，按 Combo 自由排列组合 — 凝聚了 [RTK](https://github.com/rtk-ai/rtk)、[Caveman](https://github.com/JuliusBrussee/caveman)（⭐ 51K+）、[LLMLingua-2](https://github.com/microsoft/LLMLingua) 和 [Troglodita](https://github.com/leninejunior/troglodita)（PT-BR）的技术精华。
...
```
Client (10,000 tok) ──▶ ShiguangGateway Compression (9 engines) ──▶ Provider (~1,080 tok, 节省高达 95%)
```
...
- **🎛️ 压缩决策的优先链路** _（从高到低）_ — 逐请求 `x-shiguang-gateway-compression` 头 › Combo 覆写 › 活动命名配置 › 自适应/自动触发 › 面板默认 › 关闭。最终采用的压缩方案会通过 `X-ShiguangGateway-Compression: <mode>; source=<source>` 响应头回显。
...
你应该能看到已连接模型的列表。🎉 至此大功告成 — 开始编码，ShiguangGateway 自动路由、自动容灾。

如果你的客户端无法发送自定义请求头，ShiguangGateway 也提供 Token 化兼容别名：
...
> 🎬 **制作了关于 ShiguangGateway 的视频？** 通过链接创建 [issue](https://github.com/diegosouzapw/ShiguangGateway/issues/new) 或 [discussion](https://github.com/diegosouzapw/ShiguangGateway/discussions) — 我们将在本节予以展示。
...
> 💡 控制台上的"费用"是**节省追踪器**，而非账单 — ShiguangGateway 从不向你收费。显示"$290 总费用"意味着你使用免费模型**省下了 $290**。
...
🇷🇺 🇨🇳 🇮🇷 🇨🇺 🇹🇷 身处受限地区？ShiguangGateway 的**三级代理体系**（全局 / 按服务商 / 按连接）代理 API 请求、OAuth 流程、连通性测试、Token 刷新和模型同步。
...
📖 [MCP 服务器](../../inference/mcp-server/README.md) · [A2A 服务器](../../src/lib/a2a/README.md) · [容灾指南](../../architecture/RESILIENCE_GUIDE.md) · [功能画廊](../../guides/FEATURES.md)
...
| `DATA_DIR`        | `~/.shiguang-gateway` | 数据库与配置存储路径         |

**ShiguangGateway 会向我收费吗？** 不会 — 它是运行在你本机的免费开源软件。你只直接向付费服务商付款。ShiguangGateway 不含任何计费系统。
...
- 🌍 **官网**：[shiguang-gateway.online](https://shiguang-gateway.online)
- 🐙 **GitHub**：[github.com/diegosouzapw/ShiguangGateway](https://github.com/diegosouzapw/ShiguangGateway)
- 🐛 **Issues**：[报告 Bug](https://github.com/diegosouzapw/ShiguangGateway/issues)（请附上 `npm run system-info` 的输出结果）
...
- **官网**：[shiguang-gateway.online](https://shiguang-gateway.online)
- **npm 包**：[npmjs.com/package/shiguang-gateway](https://www.npmjs.com/package/shiguang-gateway)
...
| [远程模式](../../guides/REMOTE-MODE.md)                       | 通过授权范围 Token 从笔记本 CLI 操控远端 ShiguangGateway（VPS）                |
| [Claude Code 配置](../../guides/CLAUDE-CODE-CONFIGURATION.md) | 使用 `launch` + 按模型配置文件将 Claude Code 指向 ShiguangGateway（本地/远程） |
...
| [Termux 指南](../../guides/TERMUX_GUIDE.md)         | 通过 Termux 在 Android 上运行 ShiguangGateway                |
...
| [MCP 服务器](../../inference/mcp-server/README.md) | 107 个 MCP 工具、IDE 配置、Python/TS/Go 客户端 |
...
> ShiguangGateway 由充满热情的开源社区共同塑造。以下同仁做出了卓越贡献，直接影响着项目的质量、稳定性与影响力。**衷心感谢。**
...
> 🙏 这些贡献者的功能、Bug 修复和基础设施改进，是 ShiguangGateway 可靠且功能丰富的**核心支柱**。每一个 Pull Request、每一个测试用例、每一个 i18n 翻译文件都意义重大。开源正是由他们这样的人建造的。
...
ShiguangGateway 是站在巨人肩膀上的作品。它始于 **[9router](https://github.com/decolua/9router)** 的一个 Fork 以及 Go 项目 **[CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI)** 的 TypeScript 移植 — 自此，以下每个子系统均受惠于先行者的开源成果。每一个项目都在 ShiguangGateway 中留下了具体印记。这是我们对所有项目的由衷感谢。🙏

> ⭐ 星标数为 2026 年 6 月数据 — 请给这些项目点颗星。

### 🧬 渊源与网关

| 项目                                                                            | ⭐    | 对 ShiguangGateway 的启发                                                                       |
| ------------------------------------------------------------------------------- | ----- | ----------------------------------------------------------------------------------------- |
...
ShiguangGateway 是免费开源项目，在公开环境中持续构建与维护。如果它帮你节省了时间或金钱，请考虑以以下方式支持开发：

- ⭐ **为本仓库加颗 Star** — 这确确实实能帮我们提升可见度
- 💖 **[GitHub Sponsors](https://github.com/sponsors/diegosouzapw)** — 资助持续维护和新服务商接入
- 🐛 **在 [Discussions](https://github.com/diegosouzapw/ShiguangGateway/discussions) 中反馈 Bug 和分享意见**
...
**[⬆ 返回顶部](#-shiguang-gateway)** · 用 ❤️ 为开源 AI 社区构建。
