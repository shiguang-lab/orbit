...
| `HEADROOM_URL`                                  | `http://localhost:8787` | `apps/control-api/src/headroom/runtime/detect.ts`                                        | Headroom Token 节省器代理 URL。Dashboard 生命周期（`api/headroom/*`）默认在 loopback 上启动本地 `headroom-ai` CLI；覆盖以指向外部 Docker sidecar 代理。                                                                                                                                                                                                                                                         |
