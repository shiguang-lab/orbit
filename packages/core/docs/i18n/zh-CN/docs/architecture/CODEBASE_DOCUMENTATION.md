...
默认 HTTP 端口：**`20128`**（API 和仪表盘共享同一进程）。数据目录由 `DATA_DIR` 环境变量指定，默认为 `~/.orbit/`。
...
├── bin/                  CLI 入口点（orbit、reset-password）
...
| `responses.ts`、`orbitResponseMeta.ts` | 响应格式辅助工具              |
