...
> همچنین یک ادغام پلاگین غنی‌تر وجود دارد — `shiguang-gateway setup opencode` — که `@orbit/opencode-plugin` را نصب می‌کند. این‌ها دستورات متفاوتی هستند؛ جدول بالا `setup-opencode` را مستند می‌کند.
