...
> Referensi lengkap untuk setiap variabel lingkungan yang dikenali oleh Orbit.
...
Orbit menggunakan **SQLite** (melalui `better-sqlite3`) untuk semua persistensi data. Variabel-variabel ini mengontrol lokasi data, enkripsi, dan siklus hidup data.

| Variable                         | Default              | Source File                                     | Deskripsi                                                                                                              |
| -------------------------------- | -------------------- | ----------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `DATA_DIR`                       | `~/.orbit/`      | `src/lib/db/core.ts`                            | Direktori utama untuk DB SQLite, cadangan, dan file data. Override untuk volume Docker atau path khusus.               |
...
| `ORBIT_CRYPT_KEY`            | _(unset)_            | `src/lib/db/encryption.ts`                      | **Alias legacy** untuk `STORAGE_ENCRYPTION_KEY`. Diterima sebagai fallback ketika variabel utama tidak ada.            |
| `ORBIT_API_KEY_BASE64`       | _(unset)_            | `src/lib/db/encryption.ts`                      | **Alias legacy** (bentuk yang dikodekan Base64) diterima sebagai fallback. Didekode secara otomatis sebelum digunakan. |

### Skenario

| Skenario                    | Konfigurasi                                                                                  |
| --------------------------- | -------------------------------------------------------------------------------------------- |
| **Pengembangan lokal**      | Biarkan semua nilai default. DB berada di `~/.orbit/orbit.db`.                       |
...
| **CI/Testing**              | `DATA_DIR=/tmp/orbit-test` — bersifat sementara, tidak perlu enkripsi.                   |
...
| `ORBIT_PORT`      | _(unset)_    | `src/lib/runtime/ports.ts` | Mengambil prioritas di atas `PORT` saat berjalan di dalam Electron atau wrapper lainnya. |
...
Orbit menyediakan pertahanan dua lapis: pemindaian injeksi di sisi permintaan dan penghapusan PII di sisi respons.
...
> [!IMPORTANT]
> Saat melakukan deployment di balik reverse proxy (nginx, Caddy), `NEXT_PUBLIC_BASE_URL` **harus** diatur ke URL publik Anda (misalnya, `https://orbit.example.com`). Tanpa ini, callback OAuth akan gagal karena redirect_uri tidak akan cocok.
...
Mengontrol bagaimana Orbit menemukan dan menjalankan sidecar CLI (Claude Code, Codex, dll.).
...
| `CLI_ALLOW_CONFIG_WRITES` | `false`    | `src/shared/services/cliRuntime.ts` | Izinkan Orbit menulis file konfigurasi CLI (penyegaran token, data sesi).    |
...
```bash
# Mount biner host ke kontainer dan beri tahu Orbit lokasinya:
...
| Variable                                | Default          | Source File                                 | Deskripsi                                                                                                                         |
| --------------------------------------- | ---------------- | ------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `ORBIT_BASE_URL`                    | deteksi otomatis | `open-sse/mcp-server/server.ts`             | URL eksplisit agar alat MCP/A2A dapat menjangkau Orbit. Menimpa deteksi otomatis localhost.                                   |
| `ORBIT_API_KEY`                     | _(unset)_        | MCP/A2A modules                             | Kunci API untuk panggilan alat MCP internal dan skill A2A.                                                                        |
| `ORBIT_API_KEY_ID`                  | _(unset)_        | `open-sse/mcp-server/audit.ts`              | ID kunci untuk atribusi log audit MCP.                                                                                            |
| `ROUTER_API_KEY`                        | _(unset)_        | Legacy                                      | Alias legacy untuk `ORBIT_API_KEY`.                                                                                           |
| `ORBIT_MCP_ENFORCE_SCOPES`          | `false`          | `open-sse/mcp-server/server.ts`             | Terapkan kontrol akses berbasis scope pada panggilan alat MCP.                                                                    |
| `ORBIT_MCP_SCOPES`                  | _(all)_          | `open-sse/mcp-server/server.ts`             | Scope yang dipisahkan koma: `admin`, `combos`, `health`, `models`, `routing`, `budget`, `metrics`, `pricing`, `memory`, `skills`. |
...
| `ORBIT_DISABLE_BACKGROUND_SERVICES` | `false`          | `src/instrumentation-node.ts`               | Nonaktifkan semua layanan latar belakang (sinkronisasi, harga, pembaruan model). Berguna untuk CI/pengujian.                      |
| `ORBIT_BOOTSTRAPPED`                | `false`          | `src/app/(dashboard)/dashboard/page.tsx`    | Diatur ke `true` oleh skrip bootstrap setelah pengaturan awal. Mengontrol visibilitas wizard pengaturan.                          |
| `ORBIT_ALLOW_BODY_PROJECT_OVERRIDE` | `0`              | `open-sse/executors/antigravity.ts`         | Escape hatch: izinkan body permintaan untuk menimpa field proyek Antigravity.                                                     |

### Jembatan CLI OAuth (Internal)

| Variable            | Default          | Source File                     | Deskripsi                                            |
| ------------------- | ---------------- | ------------------------------- | ---------------------------------------------------- |
| `ORBIT_SERVER`  | deteksi otomatis | `src/lib/oauth/config/index.ts` | URL server untuk jembatan autentikasi CLI↔Orbit. |
| `ORBIT_TOKEN`   | _(unset)_        | `src/lib/oauth/config/index.ts` | Token autentikasi untuk jembatan CLI.                |
| `ORBIT_USER_ID` | `cli`            | `src/lib/oauth/config/index.ts` | ID pengguna untuk sesi jembatan CLI.                 |
| `SERVER_URL`        | _(unset)_        | `src/lib/oauth/config/index.ts` | Alias legacy untuk `ORBIT_SERVER`.               |
| `CLI_TOKEN`         | _(unset)_        | `src/lib/oauth/config/index.ts` | Alias legacy untuk `ORBIT_TOKEN`.                |
| `CLI_USER_ID`       | _(unset)_        | `src/lib/oauth/config/index.ts` | Alias legacy untuk `ORBIT_USER_ID`.              |
...
| `ORBIT_QODER_WORKSPACE`       | Qoder                   | Alias untuk `QODER_CLI_WORKSPACE`.                                             |
...
Saat diaktifkan, Orbit mengatur ulang urutan header HTTP dan field body JSON agar cocok dengan tanda tangan persis dari alat CLI resmi. Hal ini mengurangi risiko pemblokiran akun sambil mempertahankan IP proxy Anda.
...
| Variable                   | Default              | Deskripsi                                                                                                                 |
| -------------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `ORBIT_MEMORY_MB`      | `512`                | Batas heap V8 saat runtime. Docker standalone dan `orbit serve` menggunakannya untuk mengatur `--max-old-space-size`. |
...
| `NEXT_PUBLIC_ORBIT_E2E_MODE` | _(unset)_ | E2E test harness                          | Atur ke `true` untuk mengaktifkan mode pengujian E2E (autentikasi santai, test hook). |
...
| `INSTANCE_NAME=orbit`                             | Ada di template docs/env lama tetapi tidak digunakan saat runtime. Mungkin kembali dalam fitur multi-instansi di masa depan. |
