...
(Codex) tetap tersedia. CLI Gemini hanya untuk peluncuran: itu adalah target `orbit run`
...
kontainer. Orbit mendeteksi itu dan keluar dengan kode `2` dengan instruksi daripada
...
`--allow-container-write` ketika mengonfigurasi CLI kontainer itu sendiri adalah apa yang sebenarnya Anda maksud; `ORBIT_ALLOW_CONTAINER_CONFIG_WRITE=true` melakukan hal yang sama untuk
server. Lihat
...
Halaman ini (yang diubah namanya dari `/dashboard/agents`) menunjukkan CLI yang dapat **dibuat** oleh Orbit sebagai mesin eksekusi backend melalui protokol stdio/ACP. Katalog ini dikelola secara terpisah di `src/lib/acp/registry.ts` dan **tidak** sama dengan `CLI_TOOLS`.
...
```bash
orbit doctor                       # Periksa konfigurasi, DB, port, runtime, memori, kelangsungan
orbit doctor --json                # JSON yang dapat dibaca mesin
orbit doctor --no-liveness         # Lewati probe kesehatan HTTP
orbit doctor --host 0.0.0.0        # Ganti host kelangsungan
...
orbit providers test-all                        # Uji setiap koneksi aktif
orbit providers validate                        # Validasi struktural hanya lokal
...
orbit auth export --force                          # Ekspor SEMUA kredensial DECRYPTED koneksi ke stdout sebagai JSON
