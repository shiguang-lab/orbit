...
5. Direktori `open-sse/` diterbitkan sebagai `@orbit/inference` **paket ruang kerja npm**. Kode sumber mengimpornya melalui `@orbit/inference/...` (diselesaikan oleh Next.js `transpilePackages`). Jalur file dalam dokumen ini masih menggunakan nama direktori `open-sse/` untuk konsistensi.
