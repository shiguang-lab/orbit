...
alat tersebut berbicara ke **satu** endpoint dan ShiguangGateway mengarahkan ke penyedia yang tepat dengan
fallback otomatis. Setiap perintah membaca katalog model **langsung** dari ShiguangGateway yang berjalan
...
| `shiguang-gateway setup-cursor`   | Cursor                              | Tidak ada — mencetak langkah-langkah dalam aplikasi (konfigurasi Cursor tidak transparan SQLite)                                                                                          | `--remote` `--api-key` `--only` `--port`                                                                                                   | Keduanya        |
...
- `--model <id>` pada `shiguang-gateway run` mengikuti pengkabelan per-target dari manifest
...
```bash
shiguang-gateway connect 192.168.0.15        # membuat token terikat, menyimpan konteks
...
ShiguangGateway mengekspos permukaan OpenAI di `/v1`, permukaan Anthropic di root, dan permukaan Gemini asli di `/v1beta`. Setiap integrasi terhubung ke bentuk yang diharapkan alatnya (diverifikasi dalam sumber perintah):
...
Gemini native ShiguangGateway (`/v1beta`). `shiguang-gateway run gemini` menghubungkan itu secara otomatis:
...
- **sebuah `GEMINI_CLI_HOME` yang terisolasi sementara** yang `.gemini/settings.json`
  ...
  tidak pernah menggantikan peluncuran yang diarahkan oleh ShiguangGateway — dihapus setelah keluar;
...
ShiguangGateway yang REAL, terdapat harness opt-in di
...
- [Konfigurasi Codex CLI](./CODEX-CLI-CONFIGURATION.md) — pengaturan dasar `[model_providers.shiguang-gateway]` sekali saja
