...
5. O diretório `open-sse/` é publicado como o pacote de **workspace npm** `@orbit/inference`. O código-fonte o importa via `@orbit/inference/...` (resolvido pelo Next.js `transpilePackages`). Os caminhos de arquivo neste documento ainda usam o nome do diretório `open-sse/` para consistência.
