...
- before the active account is exhausted, ShiguangGateway generates a compact structured summary
- after authentication selects a different account for the same session, ShiguangGateway injects
...
If the active provider is enabled in `handoffProviders`, ShiguangGateway generates a structured
...
ShiguangGateway prepends the stored handoff as a system message. Injection happens only after the
...
At injection time, ShiguangGateway converts that payload into a `<context_handoff>` system
