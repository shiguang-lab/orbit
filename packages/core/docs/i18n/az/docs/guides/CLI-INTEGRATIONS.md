...
> Həmçinin daha zəngin bir plugin inteqrasiyası var — `shiguang-gateway setup opencode` — bu, `@orbit/opencode-plugin`-i quraşdırır. Onlar fərqli əmrlərdir; yuxarıdakı cədvəl `setup-opencode`-i sənədləşdirir.
