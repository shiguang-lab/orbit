...
alat tersebut berkomunikasi dengan **satu** endpoint dan Orbit mengarahkan ke penyedia yang tepat dengan
fallback otomatis. Setiap perintah membaca katalog model **langsung** dari Orbit yang sedang berjalan
...
| `orbit setup-cursor`   | Cursor                              | Tidak ada — mencetak langkah-langkah dalam aplikasi (konfigurasi Cursor tidak terlihat)                                                                                      | `--remote` `--api-key` `--only` `--port`                                                                                                   | Keduanya        |
...
| `orbit run <target>`   | Peluncuran runtime (generik)        | Tidak ada — memunculkan `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` dengan env dan argumen yang tepat; Qwen dan Gemini menggunakan home terisolasi sementara | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | Keduanya        |
...
- `--model <id>` pada `orbit run` mengikuti pengkabelan per-target manifest
...
```bash
orbit connect 192.168.0.15        # menciptakan token terikat, menyimpan konteks
...
Orbit mengekspos permukaan OpenAI di `/v1`, permukaan Anthropic di root, dan permukaan Gemini asli di `/v1beta`. Setiap integrasi terhubung ke bentuk yang diharapkan alatnya (diverifikasi di sumber perintah):
...
(dan `:streamGenerateContent?alt=sse`) terhadapnya — persis seperti permukaan Gemini native Orbit (`/v1beta`). `orbit run gemini` menghubungkan itu secara otomatis:
...
- **`GEMINI_CLI_HOME` sementara yang terisolasi** yang `.gemini/settings.json`
  ...
  tidak pernah menimpa peluncuran yang diarahkan oleh Orbit — dihapus setelah keluar;
...
Orbit yang REAL, ada harness opt-in di
...
- [Konfigurasi CLI Codex](./CODEX-CLI-CONFIGURATION.md) — pengaturan dasar `[model_providers.orbit]` sekali saja
