...
title: "CLI Інтеграції — налаштуйте будь-який CLI для кодування на Orbit"
version: 3.8.50
...
Orbit постачається з набором команд `setup-*`, які налаштовують CLI для кодування (Codex, Claude Code, OpenCode, Cline тощо) для використання Orbit як свого бекенду — таким чином, інструмент спілкується з **одним** кінцевим пунктом, а Orbit маршрутизує до правильного постачальника з автоматичним резервуванням. Кожна команда читає **активний** каталог моделей з працюючого Orbit (локального або віддаленого) і записує конфігураційний файл інструмента на **вашому** комп'ютері. API-ключ посилається на змінну середовища, де це підтримується інструментом. Команди, які зберігають локальний файл середовища інструмента, зазначені нижче.

Також є загальний запускник — `orbit run <target>` — який запускає `claude`, `codex`, `aider`, `goose`, `opencode`, `qwen` або `gemini` з правильним середовищем, без запису будь-якої конфігурації. Цілі та їхні псевдоніми беруться з канонічного маніфесту `bin/cli/cli-manifest.mjs`
(`claude-code|cc|anthropic`, `codex-cli|openai-codex|openai`, `goose-cli`,
`open-code`, `qwen-code`, `gemini-cli`), а `orbit completion` пропонує
ті ж слова-цілі, отримані з маніфесту. Спадкові запускники для кожного інструмента —
`orbit launch` (Claude Code) та `orbit launch-codex` (Codex) — залишаються
доступними.
...
- [Віддалений режим](./REMOTE-MODE.md) — керуйте віддаленим Orbit (VPS / Tailnet) з вашого ноутбука
...
Кожна команда поважає **активний контекст** (встановлений за допомогою `orbit connect`, див.
...
| -------------------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------- |
| `orbit setup-codex`    | OpenAI Codex CLI                      | `~/.codex/<name>.config.toml` — один профіль для кожної сумісної текстової моделі (`codex --profile <name>`)                                                                               | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | Обидва                      |
| `orbit setup-claude`   | Claude Code                           | `~/.claude/profiles/<name>/settings.json` — один профіль для кожної відповідної моделі (`CLAUDE_CONFIG_DIR`)                                                                               | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | Обидва                      |
| `orbit setup-opencode` | OpenCode (сумісний з openai)          | `~/.config/opencode/opencode.json` — постачальник `orbit` з кожною моделлю каталогу (`opencode -m orbit/<model>`)                                                                  | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | Обидва                      |
| `orbit setup-cline`    | Cline                                 | `~/.cline/data/{globalState,secrets}.json` (CLI режим) + виводить налаштування розширення VS Code                                                                                          | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | Обидва                      |
| `orbit setup-kilo`     | Kilo Code                             | `~/.local/share/kilo/auth.json` (CLI) + об'єднує `kilocode.*` у `settings.json` VS Code, якщо він присутній                                                                                | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | Обидва                      |
| `orbit setup-continue` | Continue / `cn` CLI                   | `~/.continue/config.yaml` — моделі `provider: openai`, ключ через `${{ secrets.ORBIT_API_KEY }}`                                                                                       | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Обидва                      |
| `orbit setup-cursor`   | Cursor                                | Нічого — виводить кроки в додатку (конфігурація Cursor є непрозорою SQLite)                                                                                                                | `--remote` `--api-key` `--only` `--port`                                                                                                   | Обидва                      |
| `orbit setup-roo`      | Roo Code                              | `~/.orbit/roo-settings.json` (імпортний документ) + встановлює `roo-cline.autoImportSettingsPath`, якщо існує `settings.json` VS Code                                                  | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | Обидва                      |
| `orbit setup-crush`    | Crush                                 | `~/.config/crush/crush.json` — постачальник `openai-compat`, ключ через `$ORBIT_API_KEY`                                                                                               | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Обидва                      |
| `orbit setup-goose`    | Goose                                 | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + виводить рецепт середовища                                                                                  | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Обидва                      |
| `orbit setup-aider`    | Aider                                 | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + виводить рецепт середовища                                                                                                | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Обидва                      |
| `orbit setup-qwen`     | Qwen Code                             | `~/.qwen/settings.json` — масив V4 `modelProviders.openai` + `ORBIT_API_KEY` у `~/.qwen/.env`                                                                                          | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | Обидва                      |
| `orbit run <target>`   | Запуск в режимі виконання (загальний) | Нічого — запускає `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` з правильним середовищем і аргументами; Qwen і Gemini використовують тимчасовий ізольований домашній каталог | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | Обидва                      |
| `orbit launch`         | Claude Code                           | Нічого — запускає `claude` з `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` інжектованими                                                                                                     | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | Обидва                      |
| `orbit launch-codex`   | OpenAI Codex CLI                      | Нічого — запускає `codex` з постачальником `orbit`, інжектованим через `-c` прапори                                                                                                    | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | Обидва                      |

Примітки щодо прапорів (перевірено в джерелі команди):

- `--remote <url>` — отримати каталог з віддаленого Orbit (перезаписує `--port`
  і активний контекст). `--api-key <key>` постачає облікові дані для цього
  сервера (за замовчуванням використовує змінну середовища `ORBIT_API_KEY` або токен активного контексту).
...
- `--model <id>` на `orbit run` слідує за підключенням маніфесту для кожної цілі
  (`bin/cli/cli-manifest.mjs`): **aider** отримує `--model openai/<id>` і
  **opencode** `--model orbit/<id>` (префікс додається лише тоді, коли id
  ...
  **codex** через `-c model_providers.orbit.*` аргументи. **Qwen є єдиною ціллю запуску,
  яка жорстко вимагає `--model`** — `orbit run qwen` без нього завершується
  з кодом `2` з явною помилкою.
- `--port <port>` — локальний порт Orbit (за замовчуванням `20128`, ігнорується, коли встановлено `--remote`).
  Присутній у всіх командах `setup-*` і обох запускниках.
- Код виходу `orbit run`: код виходу дочірнього CLI передається
...
> Існує також більш багатша інтеграція плагіна — `orbit setup opencode` — яка
> встановлює `@orbit/opencode-plugin`. Це різні команди; таблиця
...
З Orbit, що працює на `localhost:20128`, просто виконайте команду налаштування для вашого інструменту. Каталог отримується з локального сервера.
...
export ORBIT_API_KEY=sk-...  # посилається через {env:ORBIT_API_KEY}, ніколи не на диску
...
```bash
orbit launch                 # Claude Code → локальний Orbit
orbit launch-codex           # Codex CLI → локальний Orbit
...
Вкажіть будь-яку команду налаштування на віддалений Orbit з `--remote` + `--api-key`. Каталог отримується з віддаленого сервера; конфігурація записується на вашому локальному комп'ютері.
...
orbit setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m orbit/glm/glm-5.2 "..."   # спочатку експортуйте ORBIT_API_KEY
...
```bash
orbit connect 192.168.0.15        # створює токен з обмеженим доступом, зберігає контекст
orbit setup-codex                 # ← тепер використовує віддалений каталог
orbit setup-opencode              # ← те ж саме
orbit launch                      # ← Claude Code проти віддаленого
```
...
Orbit надає поверхню OpenAI за адресою `/v1`, поверхню Anthropic на кореневому рівні, і рідну поверхню Gemini за адресою `/v1beta`. Кожна інтеграція підключена до форми, яку очікує її інструмент (перевірено в джерелі команди):
...
| `setup-codex`, `launch-codex` (`model_providers.orbit.base_url`)       | з `/v1`               | Так                                       |
...
Коли ви оновлюєте за допомогою `orbit update` (після підтвердження або з `--apply`),
Orbit виконує установку з `--include=optional`, вбудованим у команду:

```bash
npm install -g orbit@latest --include=optional
```

Це **не** прапорець, який ви передаєте до `orbit update` — він завжди застосовується
...
# [DRY RUN] Виконало б: npm install -g orbit@latest --include=optional
```

Інші прапорці `orbit update` (перевірені в коді): `--check` (вихід 1, якщо
застаріло), `--apply` (встановити без запиту), `--changelog`, `--no-backup`,
`--yes`.

---

## Google Gemini CLI через `orbit run gemini`
...
Gemini поверхня Orbit (`/v1beta`). `orbit run gemini` автоматично підключає це:

- `GOOGLE_GEMINI_BASE_URL` → активна базова URL-адреса Orbit (корінь, без `/v1`);
- `GEMINI_API_KEY` → розв'язаний обліковий запис Orbit (опція/середовище/контекст);
- **тимчасовий ізольований `GEMINI_CLI_HOME`**, чий `.gemini/settings.json`
  ...
  ніколи не перекриває запуск, спрямований Orbit — видаляється після виходу;
...
сервера Orbit, існує опційний хардвер у
...
Опційно: `ORBIT_SMOKE_TARGETS="codex,opencode,qwen"` обмежує тестування;
`ORBIT_SMOKE_TIMEOUT_MS` перевизначає тайм-аут 120с на ціль.

## Дивіться також

- [Конфігурація Claude Code](./CLAUDE-CODE-CONFIGURATION.md) — поглиблений посібник з Claude Code
- [Конфігурація Codex CLI](./CODEX-CLI-CONFIGURATION.md) — одноразова базова налаштування `[model_providers.orbit]`
