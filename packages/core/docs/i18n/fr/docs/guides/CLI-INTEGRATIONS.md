...
> Il existe également une intégration de plugin plus riche — `shiguang-gateway setup opencode` — qui installe `@orbit/opencode-plugin`. Ce sont des commandes différentes ; le tableau ci-dessus documente `setup-opencode`.
