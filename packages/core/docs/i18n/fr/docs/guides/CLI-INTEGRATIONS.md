...
title: "Intégrations CLI — dirigez n'importe quel CLI de codage vers Orbit"
version: 3.8.50
...
Orbit propose une famille de commandes `setup-*` qui configurent un CLI de codage (Codex, Claude Code, OpenCode, Cline, …) pour utiliser Orbit comme son backend — ainsi l'outil communique avec **un** point de terminaison et Orbit redirige vers le bon fournisseur avec un retour automatique. Chaque commande lit le catalogue de modèles **en direct** d'un Orbit en cours d'exécution (local ou distant) et écrit le fichier de configuration de l'outil sur **votre** machine. La clé API est référencée par une variable d'environnement chaque fois que l'outil le supporte. Les commandes qui persistent un fichier d'environnement local à l'outil sont notées ci-dessous.

Il existe également un lanceur générique — `orbit run <target>` — qui lance `claude`, `codex`, `aider`, `goose`, `opencode`, `qwen` ou `gemini` avec le bon environnement injecté, sans écrire de configuration du tout. Les cibles et leurs alias proviennent du manifeste canonique `bin/cli/cli-manifest.mjs` (`claude-code|cc|anthropic`, `codex-cli|openai-codex|openai`, `goose-cli`, `open-code`, `qwen-code`, `gemini-cli`), et `orbit completion` propose les mêmes mots cibles dérivés du manifeste. Les lanceurs par outil hérités — `orbit launch` (Claude Code) et `orbit launch-codex` (Codex) — restent disponibles.
...
- [Mode Distant](./REMOTE-MODE.md) — pilotez un Orbit distant (VPS / Tailnet) depuis votre ordinateur portable
...
Chaque commande respecte le **contexte actif** (défini avec `orbit connect`, voir [Mode Distant](./REMOTE-MODE.md)) ou les drapeaux explicites `--remote <url> --api-key <key>`. "Local vs distant" ci-dessous signifie : sans drapeaux, cela cible `http://localhost:20128` ; avec `--remote` (ou un contexte distant actif), cela récupère le catalogue depuis ce serveur et écrit la configuration localement.

| Commande                   | Outil                             | Ce qu'elle écrit                                                                                                                                                                   | Drapeaux clés                                                                                                                              | Local vs distant |
| -------------------------- | --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ---------------- |
| `orbit setup-codex`    | OpenAI Codex CLI                  | `~/.codex/<name>.config.toml` — un profil par modèle de texte compatible (`codex --profile <name>`)                                                                                | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | Les deux         |
| `orbit setup-claude`   | Claude Code                       | `~/.claude/profiles/<name>/settings.json` — un profil par modèle correspondant (`CLAUDE_CONFIG_DIR`)                                                                               | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | Les deux         |
| `orbit setup-opencode` | OpenCode (compatible openai)      | `~/.config/opencode/opencode.json` — fournisseur `orbit` avec chaque modèle du catalogue (`opencode -m orbit/<model>`)                                                     | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | Les deux         |
| `orbit setup-cline`    | Cline                             | `~/.cline/data/{globalState,secrets}.json` (mode CLI) + imprime les paramètres de l'extension VS Code                                                                              | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | Les deux         |
| `orbit setup-kilo`     | Kilo Code                         | `~/.local/share/kilo/auth.json` (CLI) + fusionne `kilocode.*` dans `settings.json` de VS Code si présent                                                                           | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | Les deux         |
| `orbit setup-continue` | Continue / `cn` CLI               | `~/.continue/config.yaml` — modèles `provider: openai`, clé via `${{ secrets.ORBIT_API_KEY }}`                                                                                 | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Les deux         |
| `orbit setup-cursor`   | Cursor                            | Rien — imprime les étapes dans l'application (la configuration de Cursor est opaque SQLite)                                                                                        | `--remote` `--api-key` `--only` `--port`                                                                                                   | Les deux         |
| `orbit setup-roo`      | Roo Code                          | `~/.orbit/roo-settings.json` (doc d'importation) + définit `roo-cline.autoImportSettingsPath` si un `settings.json` de VS Code existe                                          | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | Les deux         |
| `orbit setup-crush`    | Crush                             | `~/.config/crush/crush.json` — fournisseur `openai-compat`, clé via `$ORBIT_API_KEY`                                                                                           | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Les deux         |
| `orbit setup-goose`    | Goose                             | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + imprime la recette d'environnement                                                                  | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Les deux         |
| `orbit setup-aider`    | Aider                             | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + imprime la recette d'environnement                                                                                | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Les deux         |
| `orbit setup-qwen`     | Qwen Code                         | `~/.qwen/settings.json` — tableau `modelProviders.openai` V4 + `ORBIT_API_KEY` dans `~/.qwen/.env`                                                                             | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | Les deux         |
| `orbit run <target>`   | Lancement d'exécution (générique) | Rien — lance `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` avec le bon environnement et les bons arguments ; Qwen et Gemini utilisent un répertoire temporaire isolé | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | Les deux         |
| `orbit launch`         | Claude Code                       | Rien — lance `claude` avec `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` injectés                                                                                                    | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | Les deux         |
| `orbit launch-codex`   | OpenAI Codex CLI                  | Rien — lance `codex` avec le fournisseur `orbit` injecté via des drapeaux `-c`                                                                                                 | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | Les deux         |

Notes sur les drapeaux (vérifiés dans la source de la commande) :

- `--remote <url>` — récupère le catalogue depuis un Orbit distant (remplace `--port` et le contexte actif). `--api-key <key>` fournit l'information d'identification pour ce serveur (par défaut à la variable d'environnement `ORBIT_API_KEY`, ou le jeton du contexte actif).
...
- `--model <id>` sur `orbit run` suit le câblage par cible du manifeste (`bin/cli/cli-manifest.mjs`) : **aider** reçoit `--model openai/<id>` et **opencode** `--model orbit/<id>` (le préfixe est ajouté uniquement lorsque l'id ne le porte pas déjà) ; **qwen** et **gemini** reçoivent l'id tel quel ; **claude** l'obtient via `ANTHROPIC_MODEL`, **goose** via `GOOSE_MODEL`, et **codex** via des arguments `-c model_providers.orbit.*`. **Qwen est la seule cible d'exécution qui nécessite absolument `--model`** — `orbit run qwen` sans cela sort `2` avec une erreur explicite.
- `--port <port>` — port local d'Orbit (par défaut `20128`, ignoré lorsque `--remote` est défini). Présent sur toutes les commandes `setup-*` et les deux lanceurs.
- Codes de sortie de `orbit run` : le code de sortie du CLI enfant est propagé tel quel ; `2` = arguments invalides (cible non prise en charge, `--model` requis manquant, garde de conteneur) ; `127` = le binaire cible n'est pas dans `PATH` ; `130`/`143`/`129` lorsque le lancement est terminé par `SIGINT`/`SIGTERM`/`SIGHUP` ; `1` = autre échec de lancement d'exécution.
...
> Il existe également une intégration de plugin plus riche — `orbit setup opencode` — qui installe `@orbit/opencode-plugin`. Ce sont des commandes différentes ; le tableau ci-dessus documente `setup-opencode`.

---

## Utilisation locale

Avec Orbit en cours d'exécution sur `localhost:20128`, il suffit d'exécuter la commande de configuration pour votre outil. Le catalogue est récupéré depuis le serveur local.
...
export ORBIT_API_KEY=sk-...  # référencé via {env:ORBIT_API_KEY}, jamais sur disque
...
orbit run aider --model glm/glm-5.2 -- --message "réponse OK"
orbit run goose --model glm/glm-5.2
orbit run opencode --model glm/glm-5.2 -- run "réponse OK"
orbit run qwen --model glm/glm-5.2 -- -p "réponse OK"
orbit run gemini --model glm/glm-5.2 -- --skip-trust -p "réponse OK"

# Chemin de commande explicite : passez tout ce qui vient après --
orbit run claude -- --print-system-prompt "révisez cette différence"
```
...
Pointez toute commande de configuration vers un Orbit distant avec `--remote` + `--api-key`. Le catalogue est récupéré depuis le distant ; la configuration est écrite sur votre machine locale.
...
orbit setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m orbit/glm/glm-5.2 "..."   # exportez d'abord ORBIT_API_KEY
...
```bash
orbit connect 192.168.0.15        # génère un jeton de portée, stocke le contexte
orbit setup-codex                 # ← utilise maintenant le catalogue distant
orbit setup-opencode              # ← même chose
orbit launch                      # ← Claude Code contre le distant
```
...
Orbit expose la surface OpenAI à `/v1`, la surface Anthropic à la racine, et une surface Gemini native à `/v1beta`. Chaque intégration est câblée à la forme que son outil attend (vérifiée dans la source de la commande) :
...
| `setup-codex`, `launch-codex` (`model_providers.orbit.base_url`)       | avec `/v1`         | Oui                                         |
...
Lorsque vous mettez à jour avec `orbit update` (après confirmation, ou avec `--apply`), Orbit exécute l'installation avec `--include=optional` intégré :

```bash
npm install -g orbit@latest --include=optional
```

Ce n'est **pas** un drapeau que vous passez à `orbit update` — il est toujours appliqué par le
...
Autres drapeaux `orbit update` (vérifiés dans la source) : `--check` (sortie 1 si
...
native Gemini d'Orbit (`/v1beta`). `orbit run gemini` le connecte automatiquement :

- `GOOGLE_GEMINI_BASE_URL` → l'URL de base active d'Orbit (racine, pas de `/v1`) ;
- `GEMINI_API_KEY` → les informations d'identification résolues d'Orbit (option/env/contexte) ;
- un **`GEMINI_CLI_HOME` isolé temporaire** dont le `.gemini/settings.json`
  ...
  ne remplace jamais le lancement dirigé par Orbit — supprimé après la sortie ;
...
`tests/unit/cli/run-execution.test.ts`). Pour valider les binaires RÉELS contre un serveur Orbit RÉEL,
...
Optionnel : `ORBIT_SMOKE_TARGETS="codex,opencode,qwen"` restreint le balayage ;
`ORBIT_SMOKE_TIMEOUT_MS` remplace le délai d'attente de 120s par cible.
...
- [Configuration de Codex CLI](./CODEX-CLI-CONFIGURATION.md) — la configuration de base `[model_providers.orbit]` à effectuer une seule fois
