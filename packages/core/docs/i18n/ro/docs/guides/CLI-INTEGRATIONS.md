...
title: "Integrări CLI — direcționează orice CLI de codare către Orbit"
version: 3.8.50
...
Orbit oferă o familie de comenzi `setup-*` care configurează un CLI de codare
(Codex, Claude Code, OpenCode, Cline, …) pentru a folosi Orbit ca backend — astfel
încât instrumentul comunică cu **un** endpoint și Orbit redirecționează către furnizorul corect cu
fallback automat. Fiecare comandă citește catalogul de modele **live** de la un Orbit
...
Există, de asemenea, un launcher generic — `orbit run <target>` — care lansează
...
`open-code`, `qwen-code`, `gemini-cli`), iar `orbit completion` oferă
aceleași cuvinte țintă derivate din manifest. Launcherele legate de fiecare instrument —
`orbit launch` (Claude Code) și `orbit launch-codex` (Codex) — rămân
disponibile.
...
- [Modul Remote](./REMOTE-MODE.md) — controlează un Orbit la distanță (VPS / Tailnet) de pe laptopul tău
...
Fiecare comandă respectă **contextul activ** (setat cu `orbit connect`, vezi
...
| -------------------------- | ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | --------------- |
| `orbit setup-codex`    | OpenAI Codex CLI             | `~/.codex/<name>.config.toml` — un profil pentru fiecare model de text compatibil (`codex --profile <name>`)                                                   | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | Ambele          |
| `orbit setup-claude`   | Claude Code                  | `~/.claude/profiles/<name>/settings.json` — un profil pentru fiecare model potrivit (`CLAUDE_CONFIG_DIR`)                                                      | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | Ambele          |
| `orbit setup-opencode` | OpenCode (compatibil openai) | `~/.config/opencode/opencode.json` — furnizor `orbit` cu fiecare model din catalog (`opencode -m orbit/<model>`)                                       | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | Ambele          |
| `orbit setup-cline`    | Cline                        | `~/.cline/data/{globalState,secrets}.json` (mod CLI) + imprimă setările extensiei VS Code                                                                      | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | Ambele          |
| `orbit setup-kilo`     | Kilo Code                    | `~/.local/share/kilo/auth.json` (CLI) + îmbină `kilocode.*` în `settings.json` al VS Code, dacă este prezent                                                   | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | Ambele          |
| `orbit setup-continue` | Continue / `cn` CLI          | `~/.continue/config.yaml` — modele `provider: openai`, cheie prin `${{ secrets.ORBIT_API_KEY }}`                                                           | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Ambele          |
| `orbit setup-cursor`   | Cursor                       | Nimic — imprimă pașii în aplicație (configurația Cursor este opacă SQLite)                                                                                     | `--remote` `--api-key` `--only` `--port`                                                                                                   | Ambele          |
| `orbit setup-roo`      | Roo Code                     | `~/.orbit/roo-settings.json` (document de import) + setează `roo-cline.autoImportSettingsPath` dacă există un `settings.json` al VS Code                   | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | Ambele          |
| `orbit setup-crush`    | Crush                        | `~/.config/crush/crush.json` — furnizor `openai-compat`, cheie prin `$ORBIT_API_KEY`                                                                       | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Ambele          |
| `orbit setup-goose`    | Goose                        | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + imprimă rețeta de mediu                                                         | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Ambele          |
| `orbit setup-aider`    | Aider                        | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + imprimă rețeta de mediu                                                                       | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Ambele          |
| `orbit setup-qwen`     | Qwen Code                    | `~/.qwen/settings.json` — array `V4 modelProviders.openai` + `ORBIT_API_KEY` în `~/.qwen/.env`                                                             | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | Ambele          |
| `orbit run <target>`   | Lansare runtime (generic)    | Nimic — lansează `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` cu mediul și argumentele corecte; Qwen și Gemini folosesc un home izolat temporar | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | Ambele          |
| `orbit launch`         | Claude Code                  | Nimic — lansează `claude` cu `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` injectat                                                                              | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | Ambele          |
| `orbit launch-codex`   | OpenAI Codex CLI             | Nimic — lansează `codex` cu furnizorul `orbit` injectat prin flag-uri `-c`                                                                                 | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | Ambele          |

Note despre flag-uri (verificate în sursa comenzii):

- `--remote <url>` — preia catalogul de la un Orbit la distanță (suprascrie `--port`
  și contextul activ). `--api-key <key>` furnizează acreditivul pentru acel
  server (se default-ează la variabila de mediu `ORBIT_API_KEY`, sau token-ul contextului activ).
...
- `--model <id>` pe `orbit run` urmează conectarea per-țintă din manifest
  (`bin/cli/cli-manifest.mjs`): **aider** primește `--model openai/<id>` și
  **opencode** `--model orbit/<id>` (prefixul este adăugat doar când id-ul
  nu îl poartă deja); **qwen** și **gemini** primesc id-ul exact; **claude** îl primește prin `ANTHROPIC_MODEL`, **goose** prin `GOOSE_MODEL`, și
  **codex** prin argumente `-c model_providers.orbit.*`. **Qwen este singura țintă de execuție
  care necesită în mod strict `--model`** — `orbit run qwen` fără el iese
  `2` cu o eroare explicită.
- `--port <port>` — portul local Orbit (default `20128`, ignorat când `--remote`
  este setat). Prezent pe toate comenzile `setup-*` și pe ambele launchere.
- Codurile de ieșire ale `orbit run`: codul de ieșire al CLI-ului copil este propagat
...
> Există de asemenea o integrare mai bogată a plugin-ului — `orbit setup opencode` — care
...
Cu Orbit rulând pe `localhost:20128`, pur și simplu rulează comanda de configurare pentru instrumentul tău. Catalogul este obținut de la serverul local.
...
export ORBIT_API_KEY=sk-...  # referit prin {env:ORBIT_API_KEY}, niciodată pe disc
...
orbit run claude -- --print-system-prompt "revizuiește acest diff"
```
...
Indică orice comandă de configurare către un Orbit la distanță cu `--remote` + `--api-key`. Catalogul este obținut de la distanță; configurația este scrisă pe mașina ta locală.
...
orbit setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m orbit/glm/glm-5.2 "..."   # exportă mai întâi ORBIT_API_KEY
...
```bash
orbit connect 192.168.0.15        # generează un token scoperit, stochează contextul
orbit setup-codex                 # ← acum folosește catalogul la distanță
orbit setup-opencode              # ← același lucru
orbit launch                      # ← Claude Code împotriva distanței
```
...
Orbit expune suprafața OpenAI la `/v1`, suprafața Anthropic la rădăcină, și o suprafață nativă Gemini la `/v1beta`. Fiecare integrare este conectată la forma pe care instrumentul său o așteaptă (verificat în sursa comenzii):
...
| `setup-codex`, `launch-codex` (`model_providers.orbit.base_url`)       | cu `/v1`          | Da                                         |
...
Când actualizezi cu `orbit update` (după confirmare sau cu `--apply`),
Orbit rulează instalarea cu `--include=optional` inclus:

```bash
npm install -g orbit@latest --include=optional
```

Aceasta **nu** este o opțiune pe care o transmiți la `orbit update` — este întotdeauna aplicată de
...
# [DRY RUN] Ar rula: npm install -g orbit@latest --include=optional
```

Alte opțiuni `orbit update` (verificate în sursă): `--check` (iese cu 1 dacă
este depășit), `--apply` (instalează fără a solicita), `--changelog`, `--no-backup`,
`--yes`.

---

## Google Gemini CLI prin `orbit run gemini`
...
Gemini a Orbit (`/v1beta`). `orbit run gemini` conectează asta automat:

- `GOOGLE_GEMINI_BASE_URL` → URL-ul de bază activ Orbit (rădăcină, fără `/v1`);
- `GEMINI_API_KEY` → acreditivul rezolvat Orbit (opțiune/env/context);
- un **`GEMINI_CLI_HOME`** temporar izolat al cărui `.gemini/settings.json`
  ...
  să nu suprascrie lansarea dirijată de Orbit — eliminată după ieșire;
...
Orbit, există un cadru opțional la
...
Opțional: `ORBIT_SMOKE_TARGETS="codex,opencode,qwen"` restricționează verificarea;
`ORBIT_SMOKE_TIMEOUT_MS` suprascrie timeout-ul de 120s pe țintă.
...
- [Configurarea Codex CLI](./CODEX-CLI-CONFIGURATION.md) — configurarea de bază `[model_providers.orbit]` unică
