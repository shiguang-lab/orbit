# ShiguangGateway MCP Server Documentation (Português (Portugal))
...
| Tool                            | Description                              |
| :------------------------------ | :--------------------------------------- |
| `shiguang-gateway_get_health`          | Gateway health, circuit breakers, uptime |
| `shiguang-gateway_list_combos`         | All configured combos with models        |
| `shiguang-gateway_get_combo_metrics`   | Performance metrics for a specific combo |
| `shiguang-gateway_switch_combo`        | Switch active combo by ID/name           |
| `shiguang-gateway_check_quota`         | Quota status per provider or all         |
| `shiguang-gateway_route_request`       | Send a chat completion through ShiguangGateway |
| `shiguang-gateway_cost_report`         | Cost analytics for a time period         |
| `shiguang-gateway_list_models_catalog` | Full model catalog with capabilities     |

## Advanced Tools (8)

| Tool                               | Description                                                 |
| :--------------------------------- | :---------------------------------------------------------- |
| `shiguang-gateway_simulate_route`         | Dry-run routing simulation with fallback tree               |
| `shiguang-gateway_set_budget_guard`       | Session budget with degrade/block/alert actions             |
| `shiguang-gateway_set_resilience_profile` | Apply conservative/balanced/aggressive preset               |
| `shiguang-gateway_test_combo`             | Live-test all models in a combo via a real upstream request |
| `shiguang-gateway_get_provider_metrics`   | Detailed metrics for one provider                           |
| `shiguang-gateway_best_combo_for_task`    | Task-fitness recommendation with alternatives               |
| `shiguang-gateway_explain_route`          | Explain a past routing decision                             |
| `shiguang-gateway_get_session_snapshot`   | Full session state: costs, tokens, errors                   |
