# Orbit MCP Server Documentation (Português (Portugal))
...
| Tool                            | Description                              |
| :------------------------------ | :--------------------------------------- |
| `orbit_get_health`          | Gateway health, circuit breakers, uptime |
| `orbit_list_combos`         | All configured combos with models        |
| `orbit_get_combo_metrics`   | Performance metrics for a specific combo |
| `orbit_switch_combo`        | Switch active combo by ID/name           |
| `orbit_check_quota`         | Quota status per provider or all         |
| `orbit_route_request`       | Send a chat completion through Orbit |
| `orbit_cost_report`         | Cost analytics for a time period         |
| `orbit_list_models_catalog` | Full model catalog with capabilities     |

## Advanced Tools (8)

| Tool                               | Description                                                 |
| :--------------------------------- | :---------------------------------------------------------- |
| `orbit_simulate_route`         | Dry-run routing simulation with fallback tree               |
| `orbit_set_budget_guard`       | Session budget with degrade/block/alert actions             |
| `orbit_set_resilience_profile` | Apply conservative/balanced/aggressive preset               |
| `orbit_test_combo`             | Live-test all models in a combo via a real upstream request |
| `orbit_get_provider_metrics`   | Detailed metrics for one provider                           |
| `orbit_best_combo_for_task`    | Task-fitness recommendation with alternatives               |
| `orbit_explain_route`          | Explain a past routing decision                             |
| `orbit_get_session_snapshot`   | Full session state: costs, tokens, errors                   |
