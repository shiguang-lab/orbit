...
afterEach(() => {
  delete process.env.ORBIT_INTERNAL_SERVICE_TOKEN;
  delete process.env.ORBIT_INTERNAL_SERVICE_TOKEN_FILE;
});

test("requires a configured matching token", () => {
  assert.equal(isInternalServiceRequest(new Request("http://internal/")), false);
  process.env.ORBIT_INTERNAL_SERVICE_TOKEN = "shared-secret";
  ...
  process.env.ORBIT_INTERNAL_SERVICE_TOKEN = "a";
...
test("loads a token file and separately requires loopback locality", async () => {
  ...
  try {
    ...
    process.env.ORBIT_INTERNAL_SERVICE_TOKEN_FILE = tokenFile;
    const headers = { [INTERNAL_SERVICE_AUTH_HEADER]: "file-secret", "x-orbit-peer-locality": "loopback" };
    assert.equal(isTrustedLoopbackInternalServiceRequest(new Request("http://internal/", { headers })), true);
    headers["x-orbit-peer-locality"] = "private";
