 ...
 * `extra.authInfo` (ShiguangGateway authenticates with API keys, not OAuth client ids —
 ...
 *
 * On stdio transport (`shiguangGateway --mcp`) there is no HTTP context, so we fall
 * back to the SHIGUANG_GATEWAY_API_KEY / ROUTER_API_KEY environment variable.
 *
 ...
 *
 * Falls back to SHIGUANG_GATEWAY_API_KEY / ROUTER_API_KEY env vars for stdio transport
...
/**
 * Resolve the principal id from the SHIGUANG_GATEWAY_API_KEY (or ROUTER_API_KEY) env var.
...
async function resolvePrincipalFromEnv(): Promise<string | undefined> {
  const rawKey = process.env.SHIGUANG_GATEWAY_API_KEY || process.env.ROUTER_API_KEY;
  if (!rawKey) return undefined;
  try {
