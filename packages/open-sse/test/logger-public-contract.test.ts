...
import * as loggerRuntime from "../utils/logger.js";
...
test("logger public runtime and declaration expose the same exact keys", () => {
  ...
  const declaration = fs.readFileSync(path.join(packageRoot, "public/logger.d.ts"), "utf8");
...
test("workspace consumers use the real log object instead of ghost namespace methods", () => {
  for (const relativePath of [
    ...
    assert.match(source, /import \{ log \} from "@shiguang-gateway\/open-sse\/utils\/logger"/);
