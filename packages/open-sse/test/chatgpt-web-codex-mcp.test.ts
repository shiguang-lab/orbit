...
} from "@shiguang-gateway/open-sse/executors/chatgpt-web-codex/mcp-command";
import {
  buildChatGptWebCodexMcpCommand,
  resolveChatGptWebCodexMcpCommandEntry,
} from "../executors/chatgpt-web-codex/tunnelClient.ts";

const commandModuleUrl = new URL("../executors/chatgpt-web-codex/mcpCommand.ts", import.meta.url).href;
...
test("resolves the package-owned MCP implementation without using cwd", () => {
  ...
  assert.match(entry, /packages\/open-sse\/vendor\/codex-chatgpt-web\/.*\/mcp-server\.ts$/);
});

test("builds a source command with the tsx loader and absolute package-owned entry", () => {
  const tunnelModuleUrl = new URL("../executors/chatgpt-web-codex/tunnelClient.ts", import.meta.url).href;
