...
test("runtime port modules install explicitly and idempotently", async () => {
  const { providerRuntimePorts } = await import(
    "@shiguang-gateway/core-domain/runtime/provider-ports"
  );
  assert.throws(() => providerRuntimePorts.parseModel("openai/gpt-4o"), /before runtime registration/);

  const { installRuntimePorts } = await import("../services/dbRuntimeHooks.ts");
  assert.throws(() => providerRuntimePorts.parseModel("openai/gpt-4o"), /before runtime registration/);
  installRuntimePorts();
  installRuntimePorts();
...
test("runtime settings port does not register at import time", async () => {
  const { applyProviderModelAliases } = (await import(
    "@shiguang-gateway/core-domain/runtime/provider-settings-port"
  )) as unknown as {
  ...
  const { installRuntimeSettingsPort } = await import("../services/runtimeSettingsHooks.ts");
  assert.throws(() => applyProviderModelAliases({}), /before registration/);
...
test("application owners call installers and app modules have no side-effect hook imports", () => {
  ...
  for (const relativePath of settingsOwners) {
    const source = fs.readFileSync(path.join(repoRoot, relativePath), "utf8");
    assert.match(source, /\binstallRuntimeSettingsPort\b[\s\S]*?from ["']@shiguang-gateway\/open-sse\/services\/runtime-settings-hooks["']/, relativePath);
  ...
  assert.doesNotMatch(
    controlRuntime,
    /@shiguang-gateway\/open-sse|\binstallRuntimeSettingsPort\b|\binstallResilienceRuntimeSettingsPort\b|\bhydrateRequestRuntime\b/,
  );
  assert.doesNotMatch(
    fs.readFileSync(path.join(repoRoot, "apps/control-api/src/bootstrap.ts"), "utf8"),
    /@shiguang-gateway\/open-sse|\binstallRuntimePorts\b|\binstallMemoryRuntimePort\b/,
  );
  ...
  for (const relativePath of [
    ...
    assert.doesNotMatch(
      ...
      /import\s*["']@shiguang-gateway\/open-sse\/services\/(?:dbRuntimeHooks|runtime-settings-hooks)["']/,
      relativePath,
    );
  }
});
