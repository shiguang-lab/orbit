...
import { registerQuotaSaturationRuntime } from "@shiguang-gateway/core-domain/quota/saturation-port";
import {
  updateAccountBuckets,
  type ClaudeUsageResult,
} from "@shiguang-gateway/core-domain/quota/account-buckets";
import type {
  QuotaUnit,
  QuotaWindow,
...
import { convertUsageToQuotaInfo } from "./genericQuotaFetcher.js";

const log = logger("quota:saturation");
...
async function fetchCodexSaturation(
  ...
  const quota = await fetchCodexQuota(connectionId, connection);
  if (!quota) return 0;
...
async function defaultAnthropicDeps(): Promise<AnthropicSaturationDeps> {
  return {
    loadConnection: (connectionId) =>
      getCachedProviderConnectionById(connectionId) as Promise<Record<
        string,
        unknown
      > | null>,
    fetchUsage: (conn) =>
      getUsageForProvider(conn as Parameters<typeof getUsageForProvider>[0]),
  };
}
...
async function fetchAnthropicSaturation(
  ...
  try {
    ...
    log.warn("anthropic oauth/usage saturation failed — falling back to rate-limit headers", {
...
async function defaultGenericUsageFetch(
  ...
  const conn = { id: connectionId, provider } as Parameters<typeof getUsageForProvider>[0];
  return getUsageForProvider(conn);
}
