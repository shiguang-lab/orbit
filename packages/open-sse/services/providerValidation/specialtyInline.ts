...
import { validateQoderCliPat } from "../../services/qoderCli.ts";
import { KiroService } from "@shiguang-gateway/open-sse/oauth/services/kiro";
import { resolveNvidiaValidationModel } from "@shiguang-gateway/core-domain/runtime/nvidia-validation-model";
...
export async function validateAuggieProvider() {
  const { checkAuggieCliVersion } = await import("../../executors/auggie.ts");
...
export async function validateCursorApiProvider({ apiKey }: { apiKey?: string }) {
  const { exchangeCursorApiKey, CursorApiKeyExchangeError, isCursorApiKey } =
    await import("../../services/cursorApiKeyAuth.ts");
...
export async function validateVertexProvider({ apiKey }: any) {
  try {
    const { parseSAFromApiKey, getAccessToken, isExpressApiKey } =
      await import("../../executors/vertex.ts");
