...
test("control API does not claim ownership of whole-gateway lifecycle", () => {
  const controller = read("apps/control/src/gateway/gateway.controller.ts");
  const service = read("apps/control/src/gateway/gateway.service.ts");
...
test("public API contracts keep the retired lifecycle routes absent", () => {
  ...
  const adminSource = readSourceTree("apps/console/src");
...
test("external CLI owns all split-service stop and restart operations", () => {
  ...
  assert.match(lifecycle, /SPLIT_SERVICE_NAMES\s*=\s*\["gateway",\s*"control",\s*"realtime",\s*"worker"\]/);
