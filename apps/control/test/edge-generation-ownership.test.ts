...
test("control-generated chat requests cross the edge HTTP ownership boundary", () => {
  for (const relativePath of [
    "apps/control/src/chaos/runtime/executor.ts",
    "apps/control/src/evals/evals.service.ts",
    "apps/control/src/issue-agent/issue-agent.service.ts",
  ]) {
...
test("model probes rely on edge-owned rate limiting only", () => {
  const source = read("apps/control/src/models/model-test.runner.ts");
