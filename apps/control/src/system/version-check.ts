...
export { normalizeVersion, isNewer } from "./version-compare.js";
...
async function readStreamChunks(
  ...
  try {
    while (true) {
      if (signal?.aborted) throw new Error("Version metadata request aborted");
...
export async function resolveLatestVersionCached(opts?: {
  ...
  const cached = latestVersionCache;
  if (!opts?.bypassCache && cached && cached.expiresAt > now()) {
    return cached.value;
  }
