...
const enabled = () => new Set(["1", "true", "yes", "on"]).has((process.env.ORBIT_ISSUE_AGENT_ENABLED ?? "").toLowerCase());
...
export class IssueAgentController {
  ...
  post(@Req() req: FastifyRequest, @Res() reply: FastifyReply) {
    return this.routes.dispatch(req, reply, async (request) => {
      ...
      if (!enabled()) return Response.json({ error: "Issue Agent execution is disabled", enabled: false, requiredEnv: "ORBIT_ISSUE_AGENT_ENABLED=true" }, { status: 403 });
      try {
        ...
        return Response.json({ ...run, runner: "orbit-chat-completions", auditPath: audit.path, completion: completion.body }, { status: completion.status });
      } catch (error) {
