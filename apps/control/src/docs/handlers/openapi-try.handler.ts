...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    const { method, path: targetPath, headers, body } = parsed.data; const base = (process.env.SHIGUANG_GATEWAY_BASE_URL || process.env.EDGE_GATEWAY_INTERNAL_URL || new URL(request.url).origin).replace(/\/+$/, ""); const url = new URL(targetPath, base); if (url.origin !== new URL(base).origin) return Response.json({ error: "Path must be same-origin" }, { status: 400 });
