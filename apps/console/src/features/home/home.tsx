...
import { getConnectionHealth } from "@/features/providers/connection-health";
...
const useStyles = createStyles(({ token }) => ({
  ...
  emptyState: { width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center" },
  emptyImage: { height: "88px !important" },
  topologyControls: {
  ...
  recentContent: { height: 420, minHeight: 420 },
  recentList: { height: "100%", minHeight: 0 },
...
function providerState(connection: ProviderConnection, _metric?: Metric, activity?: Activity, activeProviders?: Set<string>, healthByProvider?: Map<string, ProviderHealth>): ProviderState {
  const id = connection.provider.toLowerCase();
  if (activeProviders?.has(id)) return "active";
  const health = healthByProvider?.get(id) ?? "idle";
  if (activity?.errorProvider?.toLowerCase() === id && health !== "active") return "error";
  if (health === "error") return "error";
  if (activity?.lastProvider?.toLowerCase() === id) return "recent";
  return health;
}
...
function ProviderTopologyGraph({ providers, metrics, activity, activeProviders, healthByProvider, catalog, providerNodes, navigate, controlsClassName }: { providers: ProviderConnection[]; metrics: Record<string, Metric>; activity?: Activity; activeProviders: Set<string>; healthByProvider: Map<string, ProviderHealth>; catalog: Map<string, ProviderCatalogEntry>; providerNodes: Map<string, ProviderNode>; navigate: (path: string) => void; controlsClassName: string }) {
  const { nodes: layoutNodes, edges } = useMemo(() => {
    const sorted = [...providers].sort((a, b) => {
      const rank = (connection: ProviderConnection) => {
        if (activeProviders.has(connection.provider.toLowerCase())) return 0;
        const state = providerState(connection, metrics[connection.provider], activity, activeProviders, healthByProvider);
    ...
    return { nodes, edges };
  }, [providers, metrics, activity, activeProviders, healthByProvider, catalog, providerNodes]);
...
export default function HomePage() {
  ...
  const { t } = useI18n();
  const [dismissed, setDismissed] = useState<string[]>(() => { try { return typeof window === "undefined" ? [] : JSON.parse(window.localStorage.getItem("orbit-home-banners-dismissed") ?? "[]") as string[]; } catch { return []; } });
  ...
  const healthByProvider = useMemo(() => {
    const summary = new Map<string, { connected: boolean; errors: boolean }>();
    for (const connection of connections) {
      if (connection.isActive === false) continue;
      const providerId = connection.provider.toLowerCase();
      const current = summary.get(providerId) ?? { connected: false, errors: false };
      const health = getConnectionHealth(connection);
      if (health === "connected") current.connected = true;
      if (health === "error") current.errors = true;
      summary.set(providerId, current);
    }
    return new Map(Array.from(summary, ([providerId, health]) => [providerId, health.connected ? "active" : health.errors ? "error" : "idle"] as const));
  }, [connections]);
