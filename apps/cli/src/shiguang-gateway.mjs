 ...
 *
 * All other commands are routed through Commander (src/cli/program.mjs).
 */
...
if (process.argv.includes("reset-encrypted-columns")) {
  const { runResetEncryptedColumns } = await import(
    pathToFileURL(join(ROOT, "src", "cli", "commands", "reset-encrypted-columns.mjs")).href
  );
...
if (process.argv.includes("reset-password")) {
  ...
  await import(pathToFileURL(join(ROOT, "src", "reset-password.mjs")).href);
  await new Promise(() => {});
}

try {
  const { createProgram } = await import(
    pathToFileURL(join(ROOT, "src", "cli", "program.mjs")).href
  );
