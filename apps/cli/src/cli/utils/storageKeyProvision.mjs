 ...
 *
 * Purely informational invocations must NOT create `~/.orbit/.env` or write
 * a key — they never touch encrypted storage. Generating a 32-byte key and a
 * `.env` file just to print `orbit --version` (or `--help`) is a surprising
 ...
 *
 * Returns TRUE for everything else, INCLUDING a bare `orbit` (no args) — the
...
export function shouldProvisionStorageKey(argv) {
  ...
  // Bare `orbit` runs the default `serve` command → must provision.
