/**
 * orbit setup-goose — configure Goose (block/goose) for Orbit.
 *
...
export async function runSetupGooseCommand(opts = {}) {
  ...
  const guard = await guardHostConfigTarget(configPath, {
    toolLabel: "Goose",
    hostCommand: "orbit setup-goose",
    allowContainerWrite: Boolean(opts.allowContainerWrite ?? opts["allow-container-write"]),
    dryRun,
  });
  if (guard !== 0) return guard;

  printHeading("Orbit → Goose (openai-compatible)");
...
export function registerSetupGoose(program) {
  program
    .command("setup-goose")
    .description(
      "Configure Goose for Orbit: write ~/.config/goose/config.yaml + print the env recipe"
    )
