...
/**
 * `orbit login antigravity` — local OAuth helper for remote installs.
 *
 ...
 *
 * It talks ONLY to Google (no Orbit server needed locally), so it works even
 * if the remote VPS is firewalled from the user's machine.
 *
 * Push mode: when an active remote context exists (`orbit connect <host>`), the
...
/** Read the active CLI context (baseUrl + scoped token) written by `orbit connect`. */
...
export async function runAntigravityLogin(opts = {}, deps = {}) {
  ...
  print(
    "\n" +
      ...
      'Orbit dashboard: Providers → Antigravity → Connect → "Paste credentials".\n' +
...
export function registerLogin(program) {
  const login = program
    .command("login")
    .description("Local OAuth helpers for remote Orbit installs (run on your own machine)");
