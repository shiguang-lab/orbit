...
/**
 * `shiguangGateway login antigravity` — local OAuth helper for remote installs.
 *
 ...
 *
 * It talks ONLY to Google (no ShiguangGateway server needed locally), so it works even
 * if the remote VPS is firewalled from the user's machine.
 *
 * Push mode: when an active remote context exists (`shiguangGateway connect <host>`), the
...
function defaultStartServer(preferredPort) {
  return new Promise((resolve, reject) => {
    ...
    const server = createServer((req, res) => {
      ...
      res.end(
        "<!doctype html><meta charset=utf-8><title>ShiguangGateway</title>" +
...
/** Read the active CLI context (baseUrl + scoped token) written by `shiguangGateway connect`. */
...
async function loadDeps() {
  const { getProvider } = await import("@orbit/inference/oauth/providers");
  const antigravity = getProvider("antigravity");
  const { encodeCredentialBlob } = await import("@orbit/auth/credential-blob");
...
export async function runAntigravityLogin(opts = {}, deps = {}) {
  ...
  print(
    "\n" +
      ...
      'ShiguangGateway dashboard: Providers → Antigravity → Connect → "Paste credentials".\n' +
...
export function registerLogin(program) {
  const login = program
    .command("login")
    .description("Local OAuth helpers for remote ShiguangGateway installs (run on your own machine)");
