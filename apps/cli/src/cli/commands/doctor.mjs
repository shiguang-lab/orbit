...
async function checkNodeRuntime() {
  try {
    const { getNodeRuntimeSupport } = await import("../../nodeRuntimeSupport.mjs");
...
async function checkNativeBinary(rootDir = CLI_APP_ROOT) {
  ...
  try {
    const { isNativeBinaryCompatible } = await import("../runtime/nativeBinaryCompat.mjs");
...
export async function collectDoctorChecks(context = {}, options = {}) {
  const rootDir = context.rootDir || CLI_APP_ROOT;
