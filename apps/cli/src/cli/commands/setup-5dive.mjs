/**
 * orbit setup-5dive — point a 5dive agent fleet at Orbit.
 *
 ...
 * there is nothing for `orbit run` to launch — this is a configure-only
 * target.
 *
 ...
 *   --base-url      Orbit's Anthropic surface, ROOT url with no /v1.
...
export function validateFivediveBaseUrl(rawUrl) {
  ...
  return {
    ...
    reason:
      ...
      `API key would travel in plaintext. Serve Orbit over https:// and pass ` +
      `--remote https://${host}...`,
  };
}
...
export async function runSetup5diveCommand(opts = {}) {
  ...
  // Orbit model provider to filter on, and it reaches setup recipes
  ...
  printHeading("Orbit -> 5dive (claude BYO endpoint)");
  ...
  if (!apiKey) {
    printError("An Orbit API key is required. Pass --api-key, or set ORBIT_API_KEY.");
    return 2;
  }
...
export function registerSetup5dive(program) {
  program
    .command("setup-5dive")
    .description(
      "Point a 5dive agent fleet's claude seats at Orbit (writes a 5dive auth profile)"
    )
    .option("--port <port>", "Local Orbit port (ignored when --remote is set)", "8787")
    .option("--remote <url>", "Remote Orbit URL, e.g. https://orbit.example.com")
    .option("--context <name>", "Named local/remote context")
    .option("--api-key <key>", "Orbit API key (defaults to the active context/env)")
    .option("--model <id>", "Orbit model or combo id the agents should use")
