...
/**
 * `orbit configure <cli>` — interactive provider+model picker that writes a
 * local CLI config pointed at the ACTIVE Orbit context (local or remote).
 *
 * The model catalog comes from the active context's GET /v1/models, so when you
 * are in remote mode (`orbit connect ...`) you pick from the remote server's
 ...
 * `ORBIT_API_KEY`, but the older setup modules receive plain options and
...
export function resolveConfigureTargetOptions(opts = {}) {
  const resolved = { ...opts };
  const ambientKey = process.env.ORBIT_API_KEY || "";
...
function preferenceContextName(opts = {}) {
  if (opts.context || process.env.ORBIT_CONTEXT) {
    return String(opts.context || process.env.ORBIT_CONTEXT);
  }
  try {
...
function buildCodexProfile(modelId, ctx) {
  const lines = [
    ...
    `# ${modelId} — generated by 'orbit configure codex'`,
    `model                          = "${modelId}"`,
...
async function configureCodex(modelId, ctxWindow, opts) {
  ...
  const guard = await guardHostConfigTarget(codexHome, {
    toolLabel: "Codex",
    hostCommand: "orbit configure codex",
  ...
  printInfo("Prereq: ~/.codex/config.toml must define the [model_providers.orbit] block");
