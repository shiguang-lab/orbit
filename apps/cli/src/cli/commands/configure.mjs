...
/**
 * `shiguangGateway configure <cli>` — interactive provider+model picker that writes a
 * local CLI config pointed at the ACTIVE ShiguangGateway context (local or remote).
 *
 * The model catalog comes from the active context's GET /v1/models, so when you
 * are in remote mode (`shiguangGateway connect ...`) you pick from the remote server's
 ...
 * `SHIGUANG_GATEWAY_API_KEY`, but the older setup modules receive plain options and
...
export function resolveConfigureTargetOptions(opts = {}) {
  const resolved = { ...opts };
  const ambientKey = process.env.SHIGUANG_GATEWAY_API_KEY || "";
  ...
  if (!explicitRemote) {
    const localDefault = `http://localhost:${opts.port || process.env.PORT || "8787"}`;
...
function preferenceContextName(opts = {}) {
  if (opts.context || process.env.SHIGUANG_GATEWAY_CONTEXT) {
    return String(opts.context || process.env.SHIGUANG_GATEWAY_CONTEXT);
  }
  try {
...
function buildCodexProfile(modelId, ctx) {
  const lines = [
    ...
    `# ${modelId} — generated by 'shiguangGateway configure codex'`,
    `model                          = "${modelId}"`,
...
async function configureCodex(modelId, ctxWindow, opts) {
  ...
  const guard = await guardHostConfigTarget(codexHome, {
    toolLabel: "Codex",
    hostCommand: "shiguangGateway configure codex",
  ...
  printInfo("Prereq: ~/.codex/config.toml must define the [model_providers.shiguangGateway] block");
...
export function registerConfigure(program) {
  program
    ...
    .option("--api-key <key>", "ShiguangGateway API key (defaults to the active context/env)")
