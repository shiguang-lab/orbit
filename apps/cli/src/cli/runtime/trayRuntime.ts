...
export interface SystrayInstance {
  onClick(handler: (action: { seq_id: number }) => void | Promise<void>): void;
  sendAction(action: unknown): void;
  kill(exitProcess?: boolean): void;
}

export interface SystrayConstructor {
...
export async function loadSystray(): Promise<SystrayConstructor | null> {
  ensureRuntimeDir();
  if (!isInstalled()) {
    try {
      installSystray();
