...
export function isControlPlanePath(path) {
  ...
  return !path.startsWith("/api/v1/") && !path.startsWith("/api/internal/tunnels/");
}
...
function getDefaultControlBaseUrl() {
  ...
  const host = process.env.CONTROL_API_HOST === "0.0.0.0"
    ? "127.0.0.1"
    : (process.env.CONTROL_API_HOST || "localhost");
  const port = process.env.CONTROL_API_PORT || "8788";
  return `http://${host}:${port}`;
}

export function resolveApiUrl(path, opts = {}) {
...
export async function apiFetch(path, opts = {}) {
  ...
  const url = resolveApiUrl(path, opts);
