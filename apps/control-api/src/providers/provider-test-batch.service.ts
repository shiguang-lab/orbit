...
import { getProviderConnections } from "../../../../packages/core-domain/src/lib/db/provider-connections.ts";
import { providersBatchTestSchema } from "../../../../packages/core-domain/src/shared/validation/schemas/provider.ts";
import { isValidationFailure, validateBody } from "../../../../packages/core-domain/src/shared/validation/helpers.ts";
import { sanitizeErrorMessage } from "@shiguang-gateway/open-sse/utils/error";

@Injectable()
export class ProviderTestBatchService {
  async test(rawBody: unknown) {
    const { getProviderConnectionFamilyIds, AI_PROVIDERS, NOAUTH_PROVIDERS, OAUTH_PROVIDERS, APIKEY_PROVIDERS, LOCAL_PROVIDERS, UPSTREAM_PROXY_PROVIDERS, WEB_COOKIE_PROVIDERS, SEARCH_PROVIDERS, AUDIO_ONLY_PROVIDERS, CLOUD_AGENT_PROVIDERS, IDE_PROVIDER_IDS, OPENAI_COMPATIBLE_PREFIX, ANTHROPIC_COMPATIBLE_PREFIX } = await import("../../../../packages/core-domain/src/shared/constants/providers.ts");
    const { testSingleConnection } = await import("../../../../packages/core-domain/src/app/api/providers/[id]/test/route.ts");
    ...
    let selected = all;
    if (mode === "selected") selected = all.filter((c) => new Set(connectionIds || []).has(c.id));
    else if (mode === "provider" && providerId) {
      const ids = new Set(getProviderConnectionFamilyIds(providerId));
      selected = all.filter((c) => ids.has(c.provider));
    } else if (["oauth", "no-auth", "apikey", "web-cookie", "search", "audio", "local", "upstream-proxy", "cloud-agent"].includes(mode)) {
      selected = all.filter((c) => group(c.provider) === mode);
    } else if (mode === "free") selected = all.filter((c) => AI_PROVIDERS[c.provider]?.hasFree === true);
    else if (mode === "ide") selected = all.filter((c) => IDE_PROVIDER_IDS.has(c.provider));
    else if (mode === "compatible") selected = all.filter((c) => compatible(c.provider));
          ...
          batch.map(async (c) => {
            try {
              ...
              return {
                ...
                authType: c.authType || group(c.provider),
                valid: d.valid,
