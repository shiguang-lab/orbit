...
import { isNamedOpenAIStyleProvider } from "./discovery/providerSets.js";
...
export async function getProviderModels(
  ...
  try {
    ...
    const buildDiscoveryErrorFallbackResponse = (
      ...
      const status = getSafeOutboundFetchErrorStatus(error) as number | null;
    ...
    if (provider === "glm" || provider === "glm-cn" || provider === "glmt") {
      ...
      try {
        for (const target of discoveryTargets) {
          const targetResponse = await safeOutboundFetch(target.url, {
            ...SAFE_OUTBOUND_FETCH_PRESETS.modelsDiscovery,
            guard: getProviderOutboundGuard(),
            proxyConfig: proxy,
            method: "GET",
          ...
          if (targetResponse.ok) break;
          if (targetResponse.status === 401 || targetResponse.status === 403) break;
        }
      } catch (error) {
        const fallback = buildDiscoveryErrorFallbackResponse(error);
        if (fallback) return fallback;
    ...
    while (pageUrl && pageCount < MAX_PAGES) {
      ...
      if (provider === "alibaba" || provider === "alibaba-cn") {
        const { parseAlibabaModelStudioModelsForConnection } =
          await import("./discovery/providerModelsConfig.js");
