...
export async function testSingleConnection(connectionId: string, validationModelId?: string) {
  ...
  if ((runtime as any)?.diagnosis) {
    ...
    // Imported compatible nodes can carry an API key while older snapshots
    // incorrectly label auth_type as oauth. Prefer the credential actually
    // present so these connections reach the real compatible-node validator.
    (isOpenAICompatibleProvider(provider) && typeof connection.apiKey === "string" &&
      connection.apiKey.length > 0 && !connection.accessToken)) {
