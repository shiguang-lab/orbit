...
export class ObsidianWebdavController {
  ...
  private async authorize(request: FastifyRequest, reply: FastifyReply) { if (await isAuthenticated(request.raw as unknown as Request)) return true; reply.status(401).send({ error: { message: "Authentication required" } }); return false; }
