...
export class ObsidianSettingsController {
  ...
  private async authorize(request: FastifyRequest, reply: FastifyReply) { if (await isAuthenticated(toWebRequest(request))) return true; reply.status(401).send({ error: "Unauthorized" }); return false; }
