...
function findOrCreateHead(doc: Document): Element {
  for (const child of doc.childNodes) {
    if (isElement(child) && child.tagName === "html") {
      ...
      (head as Element & { parentNode: ParentNode }).parentNode = child;
      return head;
    }
  }
  ...
  const head = makeElement("head");
  head.childNodes = [];
  (head as Element & { parentNode: ParentNode }).parentNode = htmlEl;
  htmlEl.childNodes = [head as ChildNode];
  (htmlEl as Element & { parentNode: ParentNode }).parentNode = doc;
  doc.childNodes.push(htmlEl as ChildNode);
  return head;
}
...
function injectBase(head: Element, baseHref: string): void {
  ...
  head.childNodes.unshift(baseEl as ChildNode);
}
