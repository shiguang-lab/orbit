 ...
 * only read and written by the control-api AgentBridgeModule; keeping its
 * bootstrap beside that module prevents the shared migration package from
 * exposing an app-private schema to every runtime.
 */
export function ensureAgentBridgeSchema(): void {
  getDbInstance().exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_agent_bridge_mappings_agent
      ON agent_bridge_mappings(agent_id);
  `);
}
