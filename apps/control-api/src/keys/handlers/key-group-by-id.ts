...
export async function GET(_request: Request, { params }: RouteParams) {
  try { const { id } = await params; const group = getKeyGroupWithPermissions(id); if (!group) return json({ error: "Group not found" }, { status: 404 }); return json({ group, members: getGroupMembers(id) }); }
...
export async function DELETE(_request: Request, { params }: RouteParams) {
  try { const { id } = await params; if (!deleteKeyGroup(id)) return json({ error: "Group not found" }, { status: 404 }); return json({ success: true }); }
