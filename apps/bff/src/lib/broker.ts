 ...
 *  - BFF 的 /api/auth/session 返回 broker session(本地即"已登录"，身份来自线上)
 *  - 业务路由注入 broker 的 identityToken 作为 x-sg-identity
 *
 * 前提(auth-service 部署配置)：
 *  - LOCAL_BROKER_POLICIES 需含 omniroute 产品条目(publicOrigin 指向本地 BFF origin)
 *  - 提供真实 shiguang 账号(SG_BROKER_USERNAME / SG_BROKER_PASSWORD)
 *
 * broker 不可用时返回不可用状态，由前端显示明确错误；本地默认模式使用
 * SG_DEV_IDENTITY，不依赖 broker。
 */
const REFRESH_SKEW_MS = 15_000;
const PRODUCT_ID = "omniroute";
...
export class LocalAuthBroker {
  ...
  async identity(): Promise<string | null> {
    ...
    try {
      ...
      console.warn("[bff:broker] identity unavailable:", (err as Error).message);
      return null;
    }
  }
