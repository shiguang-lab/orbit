/**
 * 鉴权中间件：复刻 Orbit src/lib/api/requireManagementAuth.ts + isDashboardSessionAuthenticated。
 *
 * 迁移策略：
 *  - JWT cookie 校验：直接用 jose 在本 BFF 内实现(与原逻辑一致：auth_token JWT + JWT_SECRET)。
 *  - API key / CLI token / 引擎侧校验：通过 EngineAuthAdapter 注入(指向 vendor/orbit 的
