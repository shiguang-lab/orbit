...
// The NAS Orbit version used by local development does not expose the static
...
export default function HomePage() {
  ...
  const { t } = useI18n();
  const [dismissed, setDismissed] = useState<string[]>(() => { try { return typeof window === "undefined" ? [] : JSON.parse(window.localStorage.getItem("omniroute-home-banners-dismissed") ?? "[]") as string[]; } catch { return []; } });
