...
export type { WarmupResult, WarmupFailureKind } from "./warmup/core.js";

function extractResolvedProxyConfig(resolvedProxy: unknown): unknown {
  if (
    resolvedProxy &&
...
async function executeWarmup(): Promise<void> {
  ...
  const enabledMap = (
    (settings as unknown as { claudeWarmup?: { connections?: Record<string, boolean> } })
      .claudeWarmup
  )?.connections;
...
function classifyForWarmup(conn: {
  ...
  if (
    orgType &&
    ["claude_pro", "claude_max", "claude_team", "claude_enterprise"].includes(orgType)
  ) {
    return "subscription";
  }
