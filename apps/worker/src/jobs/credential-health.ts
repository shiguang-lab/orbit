...
async function testConnection(
  ...
  try {
    const validation = await validateProviderApiKey({
      provider,
      apiKey: connection.apiKey || connection.accessToken || undefined,
      providerSpecificData: connection.providerSpecificData || undefined,
    });
    const result = validation.unsupported
      ? {
          ...validation,
          valid: false,
          skipped: true,
          diagnosis: { code: "unsupported", type: "unsupported", source: "provider-validation" },
        }
      : validation;
  ...
  try {
    ...
    for (const batch of batches) {
      await Promise.allSettled(
        batch.map((conn) => testConnection(conn, getConnIntervalMs(conn)))
      );
    }
  } finally {
