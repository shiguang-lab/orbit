...
async function buildStore(): Promise<CircuitBreakerStore> {
  ...
  if (redisUrl) {
    try {
      ...
      const RedisCtor = (mod.default ?? mod) as unknown as RedisCtor;
      ...
      await redis.connect();
      await redis.ping();
      const { RedisCircuitBreakerStore } = await import("./redisCircuitBreakerStore.js");
  ...
  const { SqliteCircuitBreakerStore } = await import("./sqliteCircuitBreakerStore.js");
