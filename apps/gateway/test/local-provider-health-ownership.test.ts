...
test("edge exclusively owns local provider polling lifecycle", () => {
  const edgeService = read("apps/gateway/src/runtime-control/local-provider-health.service.ts");
  const edgeModule = read("apps/gateway/src/runtime-control/runtime-control.module.ts");
  const edgeCommands = read("apps/gateway/src/runtime-control/runtime-control.service.ts");
  const controlMonitoring = read("apps/control/src/monitoring/monitoring-health.service.ts");

  assert.equal(existsSync(resolve(repoRoot, "apps/control/src/monitoring/local-provider-health.service.ts")), false);
