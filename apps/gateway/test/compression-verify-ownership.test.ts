...
test("compression fidelity execution is owned by edge behind a narrow command", () => {
  ...
  const control = read("apps/control/src/compression/compression-verify.legacy.ts");
  const edge = read("apps/gateway/src/runtime-control/compression-verify.ts");
  ...
  assert.equal(
    existsSync(resolve(repoRoot, "apps/control/src/compression/judge-model-client.ts")),
    false,
  );
});
