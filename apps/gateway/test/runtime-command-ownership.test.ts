...
test("edge owns runtime state and control uses the authenticated command client", () => {
  const controller = read("apps/gateway/src/runtime-control/runtime-control.controller.ts");
  const service = read("apps/gateway/src/runtime-control/runtime-control.service.ts");
  const routes = read("apps/gateway/src/routes/owned-routes.manifest.ts");
  const client = read("apps/control/src/edge-runtime/client.ts");
  ...
  assert.doesNotMatch(read("apps/control/src/proxies/proxies.service.ts"), /\bclearDispatcherCache\b/);
  const proxySettings = read("apps/control/src/settings/proxy/proxy-settings.service.ts");
  assert.doesNotMatch(proxySettings, /\bclearDispatcherCache\b/);
  assert.match(proxySettings, /command: ["']runtime-cache\.invalidate["']/);
  const cacheService = read("apps/control/src/cache/cache.service.ts");
  ...
  assert.doesNotMatch(read("apps/control/src/settings/handlers/root.handler.ts"), /\bclearCliproxyapiUrlCache\b/);
  for (const path of [
  ...
  const autoProjection = read("apps/gateway/src/runtime-control/auto-combo-projection.ts");
...
test("control persists request settings before asking the edge process to apply them", () => {
  const persistence = read("apps/control/src/settings/runtime-settings-persistence.ts");
  const settingsDb = read("packages/core/src/lib/db/settings.ts");
  const edgeRuntime = read("apps/gateway/src/runtime-control/runtime-control.service.ts");
