...
import { GATEWAY_ENTITIES, TABLE_OWNERSHIP } from "@orbit/contracts/db-schema";
...
test("memory decay is a versioned edge runtime command", () => {
  ...
  assert.match(read("apps/gateway/src/runtime-control/runtime-control.service.ts"), /case "memory\.decay"/);
});

test("memories have one declared writer and worker reaches it over the authenticated command", () => {
  assert.equal(GATEWAY_ENTITIES.memories.owner, "gateway");
  assert.equal(TABLE_OWNERSHIP.find(({ table }) => table === "memories")?.owner, "gateway");

  const registry = read("apps/worker/src/jobs/registry.ts");
  const worker = read("apps/worker/src/jobs/memory-decay.ts");
  const mcpTools = read("packages/inference/src/mcp-server/tools/memoryTools.ts");
  const manifest = JSON.parse(read("packages/core/package.json")) as { exports: Record<string, unknown> };
  ...
  assert.doesNotMatch(worker, /core\/(?:edge\/memory-decay|worker\/typed-memory-decay)/);
