# ShiguangGateway Web/API 重构迁移规范

本规范适用于后续所有页面、菜单单元和功能模块的重构迁移。目标是将 Web 与 HTTP 服务直接实现于本仓库，同时保持既有数据来源、业务逻辑、功能和用户行为不变。
...
禁止使用硬编码、Mock 数据或前端临时计算替代原有真实数据源。原页面使用配置驱动时，新页面也必须通过本地 API 接入同一配置来源。
...
- Provider 风险提示只在标题右侧显示单个问号图标，完整风险说明通过 Tooltip 展示；风险文案和变体必须来自 catalog/API，不得在卡片上堆叠“注意”文字标签。
...
- 前端和 API 的请求/响应类型应同步维护；新增接口时优先补充 `packages/contracts` 中的共享契约。
- 原接口存在部分失败或批处理结果时，API 必须保留这些业务行为，不能简化为单一成功/失败。
...
- 新页面应复用 asset-hub/website 已采用的 `@shiguang2/components` 版本和 API，不得在 `@orbit/ui` 或业务目录重复实现同名滚动条。
...
- API 服务内部代码结构、类型安全和错误封装方式的改善；
- 明确修复原 UI 的小型可用性问题，并在迁移记录中说明。

禁止的差异：

- 缺少原页面接口、字段、状态、弹窗、操作或刷新机制；
- 用静态数组、占位数据或前端假状态代替 API 数据；
- 因新 UI 实现方便而修改业务规则；
...
- 侧栏可见性、排序、隐藏项和 selectedKeys 必须由 API 返回的设置与原菜单配置共同决定；
...
| 页面动作 | 原接口 | API 路由 | 方法/参数 | 响应投影 | 领域服务方法 | 鉴权/脱敏 |
|---|---|---|---|---|---|---|
...
3. 业务 handler 由所属 app 直接装配；`http` 只提供 Fastify transport 基础设施与兼容调度协议，不负责加载领域引擎；
...
8. 路由注册、错误处理、requestId、CSRF 和鉴权必须走 API 的统一插件链。

标准链路必须可追踪：

```text
用户操作 → React Query/表单 → Admin /api → Fastify API
→ authz/CSRF/requestId → 领域服务 → 数据库/Provider
→ 投影/脱敏 → 页面状态更新
```
...
1. **静态验证**：检查路由、菜单、API 注册和所有按钮是否有处理函数；
2. **类型验证**：`pnpm --filter <app> typecheck`；
3. **API 验证**：使用 Fastify `inject` 或接口测试覆盖成功、参数错误、未找到、无领域服务、部分失败、脱敏和权限拒绝；
...
类型判断必须来自 API 返回的真实 catalog/node 数据；新增类型时先补矩阵和接口，再补页面，避免所有详情页回退成同一表单。
...
限制：不得使用 Mock 或静态业务数据，不得绕过本地 API，不得依赖外部官方服务，
