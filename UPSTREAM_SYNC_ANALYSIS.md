...
| 347 | `5d07bf32f` | feat(catalog): add feature flag to disable thinking level variants in catalog (#11971) | `feat` | ✅ 已吸收并保留现有增强：新增 `ORBIT_DISABLE_THINKING_LEVEL_VARIANTS` 在生成阶段禁止 synced thinking variants；公共响应聚合改为按请求通过 `effort_variants` 参数或 `X-Orbit-Effort-Variants` 请求头选择 | 已迁移 |
