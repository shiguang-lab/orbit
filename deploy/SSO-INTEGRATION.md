...
- ✅ control `/api/auth/session`：终结 X-SG-Identity，认可管理员角色或配置的产品 entitlement，返回统一会话
...
- [ ] 新 Admin nginx、control、gateway、realtime 接入受控 Docker 网络（Admin 静态资源由独立 Admin nginx 提供，control 不托管静态资源）
- [ ] control/gateway 生产环境变量：
  ```bash
...
| `EDGE_GATEWAY_PORT` | `8787` | gateway 监听端口 |
| `CONTROL_API_PORT` | `8788` | control 监听端口 |

## 本地 Web/服务数据链路

本地开发可以启动 console（5173）与本地 API（8787）；生产使用拆分后的 gateway（8787）、
control（8788）、realtime（8790）和 worker。Provider、首页统计、设置及其他业务接口
直接读取本项目自己的 `DATA_DIR`；不存在 NAS 代理或旧 live server：

```text
浏览器 → control（管理 API）→ /app/data/storage.sqlite
模型客户端 → gateway（模型 API）→ /app/data/storage.sqlite
```
