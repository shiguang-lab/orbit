# Orbit 接入 shiguang SSO —— auth-service 配置变更

> auth-service 是 shiguang 统一会话/OIDC/JWKS 服务。
> Orbit 接入无需改 auth-service 代码；替换现有部署时可以继续使用其产品身份与授权。
...
改名而新增 `orbit:access`，也无需为此重启 auth-service。上线仍须以真实会话
确认原授权存在，并验证登录回跳和受保护 API；配置文件本身不是线上成功证据。
以下新增产品配置仅适用于主动采用独立 `orbit` 身份的部署。
...
### DEFAULT_ENTITLEMENTS(追加 `orbit:access`)

```bash
DEFAULT_ENTITLEMENTS=superagents:access,huiguang:access,platform:access,asset-hub:access,orbit:access
```
...
# 2. 在 policies 数组追加 orbit 条目(保留 asset-hub/opc 等已有条目)
LOCAL_BROKER_POLICIES=[{"productId":"asset-hub","audience":"asset-hub-api","requiredEntitlements":["asset-hub:access"]},{"productId":"orbit","audience":"orbit-api","requiredEntitlements":["orbit:access"]}]
...
- `LOCAL_BROKER_POLICIES` 里**必须含 orbit** 条目，否则返回 `invalid_product`。
